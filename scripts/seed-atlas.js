...
// ⚠️ IMPORTANTTHIS WITH YOUR ACTUAL MONGODB ATLAS CONNECTION STRING
const DB_URI = 'mongodb+srv://kiaetotech_PASSWORD@cluster0.4vjxrfm.mongodb.net/';

const LabTestCategorySchema = new mongoose.Schema({
  category_code, category_name, color, display_order, icon, is_active});
const TestMasterSchema = new mongoose.Schema({
  test_id, test_name, test_short_name, major_category, major_category_name,
  sub_category, requires_fasting, sample_type, turnaround_time_default_hours,
  home_collection_possible, is_active});
const DiagnosticsProviderSchema = new mongoose.Schema({
  provider_id, provider_name, provider_type, city,
  location: { lat, lng}, rating, total_reviews,
  is_nabl_accredited, is_home_collection_available, is_active});
const TestPricingSchema = new mongoose.Schema({
  provider_id.Schema.Types.ObjectId, test_id.Schema.Types.ObjectId,
  mrp, discounted_price, home_collection_available, report_time_hours, is_active});
const HealthPackageSchema = new mongoose.Schema({
  package_id, provider_id.Schema.Types.ObjectId, package_name,
  package_description, mrp, discounted_price,
  home_collection_available, report_time_hours, is_popular, tags, is_active});

...
/**
 * @param {{
 *  extensions('../src/file-extensions').Extensions,
 *  preferTsExts| undefined;
 *  tsNodeExperimentalSpecifierResolution('../src/index').ExperimentalSpecifierResolution | undefined;
 * }} opts
 */
...
const experimentalSpecifierResolution = tsNodeExperimentalSpecifierResolution != null ? tsNodeExperimentalSpecifierResolution ('--experimental-specifier-resolution');
...
const tryStatSync = statSupportsThrowIfNoEntry ? tryStatSyncWithoutErrors ;
const statsIfNotFound = new Stats();
function tryStatSyncWithoutErrors(path) {
  const stats = statSync(path, { throwIfNoEntry});
...
function legacyMainResolve(packageJSONUrl, packageConfig, base) {
  ...
  if (packageConfig.main !== undefined) {
    // Notecheck redundances will be handled by Descriptor cache here.
  ...
  if(lastDotIndex >= 0) {
    ...
    if (ext === '.js' || ext === '.jsx' || ext === '.mjs' || ext === '.cjs') {
      ...
      const replacementExts =
        ...
        === '.jsx' ? replacementsForJsx
        === '.mjs' ? replacementsForMjs
        ;
    ...
    if (internal && !StringPrototypeStartsWith(target, '../') &&
      ...
      if (!isURL) {
        const exportTarget = pattern ?
          StringPrototypeReplace(target, patternRegEx, subpath) + subpath;
...
function parsePackageName(specifier, base) {
  ...
  const packageName = separatorIndex === -1 ?
    specifier PrototypeSlice(specifier, 0, separatorIndex);
  ...
  const packageSubpath = '.' + (separatorIndex === -1 ? '' PrototypeSlice(specifier, separatorIndex));

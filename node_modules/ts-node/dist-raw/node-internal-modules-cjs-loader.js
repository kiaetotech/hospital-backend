...
function readPackage(requestPath) {
  ...
  const json = result.containsKeys === false ? '{}' .string;
  if (json === undefined) {
...
/**
 * @param {{
 *   nodeEsmResolver<typeof import('./node-internal-modules-esm-resolve').createResolve>,
...
function tryPackage(requestPath, exts, isMain, originalPath) {
  ...
  if (actual === false) {
    ...
    if (!actual) {
      ...
      // TODO(BridgeAR)the requireStack as well.
      throw err;
    } else {
      const jsonPath = path.resolve(requestPath, 'package.json');
      process.emitWarning(
...
function trySelf(parentPath, request) {
  ...
  const { data, path} = readPackageScope(parentPath) || {};
...
const Module_resolveFilename = function _resolveFilename(request, parent, isMain, options) {
  ...
  if (selfResolved) {
    const cacheKey = request + '\x00' +
         (paths.length === 1 ? paths[0] (paths, '\x00'));
...
function assertScriptCanLoadAsCJSImpl(service, module, filename) {
  ...
  // ts-node modificationour configuration to override
  ...
  if (ext === '.mts' || ext === '.mjs' || tsNodeClassification.moduleType === 'esm' || (pkg && pkg.data && pkg.data.type === 'module')) {
    ...
    const packageJsonPath = pkg ? path.resolve(pkg.path, 'package.json') ;

...
function createTsTranspileModule(ts, transpileOptions) {
    ...
    const compilerHost = {
        getSourceFile: (fileName) => fileName === normalizePath(inputFileName) ? sourceFile ,
        writeFile: (name, text) => {
    ...
    function transpileModule(input, transpileOptions2, packageJsonType = 'commonjs') {
        ...
        sourceFile = createSourceFile(inputFileName, input, {
            languageVersion(options),
            impliedNodeFormat(toPath(inputFileName, '', compilerHost.getCanonicalFileName), 
            /*cache*/ undefined, compilerHost, options),
            setExternalModuleIndicator(options),
        });

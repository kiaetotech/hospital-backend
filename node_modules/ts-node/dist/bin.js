...
function bootstrap(state) {
    if (!state.phase2Result) {
        ...
        if (state.shouldUseChildProcess && !state.isInChildProcess) {
            // Notetransitioning into the child-process after `phase2`,
...
function parseArgv(argv, entrypointArgs) {
    ...
    // HACK, this function is not marked @internal so it's possible
    // that libraries in the wild are doing `require('ts-node/dist/bin').main({'--transpile-only'})`
    ...
    // defaults.
    const { '--cwd', '--help'= false, '--scriptMode', '--cwdMode', '--version'= 0, '--showConfig', '--require'= [], '--eval'= undefined, '--print'= false, '--interactive'= false, '--files', '--compiler', '--compilerOptions', '--project', '--ignoreDiagnostics', '--ignore', '--transpileOnly', '--typeCheck', '--transpiler', '--swc', '--compilerHost', '--pretty', '--skipProject', '--skipIgnore', '--preferTsExts', '--logError', '--emit', '--scope'= undefined, '--scopeDir'= undefined, '--noExperimentalReplAwait', '--experimentalSpecifierResolution', '--esm', _, } = args;
    return {
        // Noteand restArgs may be overwritten by child process
        argv.argv,
        restArgs,
        cwdArg,
        help,
...
function phase2(payload) {
    ...
    if (help) {
        console.log(`
Usage-node [options] [ -e script | script.ts ] [arguments]
...
`);
    ...
    const cwd = cwdArg ? (0, path_1.resolve)(cwdArg) .cwd();
...
function phase3(payload) {
    ...
    // NOTEwe transition to a child process for ESM, the entry-point script determined
    ...
    // See://github.com/TypeStrong/ts-node/issues/1812.
    ...
    const preloadedConfig = (0, configuration_1.findAndReadConfig)({
        ...
        files,
        pretty,
        transpileOnly: (transpileOnly !== null && transpileOnly !== void 0 ? transpileOnly != null) ? true ,
        ...
        ignore,
        logError,
        projectSearchDir(cwd, scriptMode, cwdMode, entryPointPath),
        project,
        skipProject,
        skipIgnore,
        compiler,
 ...
 * details can be found in here://github.com/TypeStrong/ts-node/issues/1812.
 */
function getEntryPointInfo(state) {
    const { code, interactive, restArgs } = state.parseArgvResult;
    const { cwd } = state.phase2Result;
    const { isCli } = state;
    // Figure out which we are executingstdin, --eval, REPL, and/or entrypoint
     ...
     * NOTErelative to cwd option (not `process.cwd()`) is legacy backwards-compat; should be changed in next major://github.com/TypeStrong/ts-node/issues/1834
     */
...
function phase4(payload) {
    ...
    if (executeEval) {
        ...
        evalStuff = {
            ...
            repl: (0, repl_1.createRepl)({
                state,
                composeWithEvalAwarePartialHost,
                ignoreDiagnosticsThatAreAnnoyingInInteractiveRepl,
            }),
        };
    ...
    const service = (0, index_1.createFromPreloadedConfig)({
        ...
        options: {
            ...
            readFile: (_a = evalAwarePartialHost === null || evalAwarePartialHost === void 0 ? void 0 .readFile) !== null && _a !== void 0 ? _a ,
            fileExists: (_b = evalAwarePartialHost === null || evalAwarePartialHost === void 0 ? void 0 .fileExists) !== null && _b !== void 0 ? _b ,
            tsTrace_1.DEFAULTS.tsTrace,
        },
    });
    ...
    replStuff === null || replStuff === void 0 ? void 0 .repl.setService(service);
    evalStuff === null || evalStuff === void 0 ? void 0 .repl.setService(service);
    stdinStuff === null || stdinStuff === void 0 ? void 0 .repl.setService(service);
    // Output project information.
    if (version === 2) {
    ...
    if (showConfig) {
        ...
        if (service.options.moduleTypes) {
            // Assumptioncodepath requires CLI invocation, so moduleTypes must have come from a tsconfig, not API.
        ...
        const json = {
            ['ts-node']: {
                ...
                optionBasePaths,
                compilerOptions,
                project: (_f = service.configFilePath) !== null && _f !== void 0 ? _f .options.project,
            },
    ...
    if (executeEntrypoint) {
        ...
        // Noteand repl may both run, but never with stdin.
...
function getProjectSearchDir(cwd, scriptMode, cwdMode, scriptPath) {
    ...
    const doScriptMode = scriptMode === true ? true === true ? false : !!scriptPath;
    if (doScriptMode) {
...
function requireResolveNonCached(absoluteModuleSpecifier) {
    // node <= 12.1.x fallbacktrick below triggers a node bug on old versions.
...
function evalAndExitOnTsError(replService, module, code, isPrinted, filenameAndDirname) {
    ...
    if (isPrinted) {
        console.log(typeof result === 'string'
            ? result
            : (0, util_1.inspect)(result, { colors.stdout.isTTY }));
    }
}

...
function filterHooksByAPIVersion(hooks) {
    ...
    const hooksAPI = newHooksAPI
        ...
        : { resolve, getFormat, transformSource, load};
    return hooksAPI;
}
...
function createEsmHooks(tsNodeService) {
    ...
    async function resolve(specifier, context, defaultResolve) {
        ...
        // See://github.com/nodejs/node/discussions/41711
        ...
        async function entrypointFallback(cb) {
            try {
                ...
                if ((resolution === null || resolution === void 0 ? void 0 .url) &&
    ...
    async function load(url, context, defaultLoad) {
        return addShortCircuitFlag(async () => {
            ...
            if (format !== 'builtin' && format !== 'commonjs') {
                ...
                const { source} = await defaultLoad(url, {
                    ...context,
                    format,
                }, defaultLoad);
                if (rawSource === undefined || rawSource === null) {
                    throw new Error(`Failed to load raw sourcewas '${format}' and url was '${url}''.`);
                }
                ...
                // Call the old hook
                const { source} = await transformSource(rawSource, { url, format }, defaultTransformSource);
    ...
    async function getFormat(url, context, defaultGetFormat) {
        ...
        assert(pathname !== null, 'ESM getFormat() hookshould never have null pathname');
    ...
    async function transformSource(source, context, defaultTransformSource) {
        ...
        const sourceAsString = typeof source === 'string' ? source .toString('utf8');
        const { url } = context;
        const parsed = (0, url_1.parse)(url);
        if (!isFileUrlOrNodeStyleSpecifier(parsed)) {
            return defer();

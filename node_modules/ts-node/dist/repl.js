...
function createRepl(options = {}) {
    ...
    let context;
    const state = (_a = options.state) !== null && _a !== void 0 ? _a EvalState((0, path_1.join)(process.cwd(), exports.REPL_FILENAME));
    const evalAwarePartialHost = createEvalAwarePartialHost(state, options.composeWithEvalAwarePartialHost);
    const stdin = (_b = options.stdin) !== null && _b !== void 0 ? _b .stdin;
    const stdout = (_c = options.stdout) !== null && _c !== void 0 ? _c .stdout;
    const stderr = (_d = options.stderr) !== null && _d !== void 0 ? _d .stderr;
    const _console = stdout === process.stdout && stderr === process.stderr
        ? console
        console_1.Console(stdout, stderr);
    const replService = {
        state: (_e = options.state) !== null && _e !== void 0 ? _e EvalState((0, path_1.join)(process.cwd(), exports.EVAL_FILENAME)),
        setService,
        evalCode,
        evalCodeInternal,
        nodeEval,
    ...
    function nodeEval(code, context, _filename, callback) {
        // TODOout how to handle completion here.
        if (code === '.scope') {
            callback(null);
            return;
        }
        ...
        function getTopLevelAwaitHint() {
            return `Hinttop-level await requires TypeScript version 3.8 or higher and target ES2018 or higher. You are using TypeScript ${service.ts.version} and target ${service.ts.ScriptTarget[service.config.options.target]}.`;
        }
    }
    ...
    function startInternal(options) {
        ...
        // Noteif (code) {
            try {
                ...
                _console.error(err);
                // Notenot be killing the process here, but this codepath is deprecated anyway
                process.exit(1);
            }
        }
        ...
        service === null || service === void 0 ? void 0 .compile('', state.path);
        const repl = (0, repl_1.start)({
            prompt: '> ',
            input.stdin,
            output.stdout,
            // Mimicking node's REPL implementation://github.com/nodejs/node/blob/168b22ba073ee1cbf8d0bcb4ded7ff3099335d04/lib/internal/repl.js#L28-L30
            terminal.isTTY &&
        ...
        function reset() {
            ...
            if (!(service === null || service === void 0 ? void 0 .transpileOnly)) {
...
function createEvalAwarePartialHost(state, composeWith) {
    function readFile(path) {
        if (path === state.path)
            return state.input;
        if (composeWith === null || composeWith === void 0 ? void 0 .readFile)
    ...
    function fileExists(path) {
        if (path === state.path)
            return true;
        if (composeWith === null || composeWith === void 0 ? void 0 .fileExists)
...
function appendCompileAndEvalInput(options) {
    ...
    // Notedoes not respect sourcemaps!
    ...
    // Build a list of "commands"of JS code in the diff that must be executed.
    ...
    if (containsTopLevelAwait) {
        return {
            ...
            valuePromise: (async () => {
                ...
                for (const command of commands) {
                    ...
                    value = command.mustAwait ? await r ;
                }
                return value;
            })(),
        };
        ...
        return {
            containsTopLevelAwait,
            value.reduce((_, c) => c.execCommand(), undefined),
        };
    }
}
/**

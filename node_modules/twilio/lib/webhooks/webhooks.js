...
/**
 ...
 @param opts - options for request validation:
    -> urlfull URL (with query string) you used to configure the webhook with Twilio - overrides host/protocol options
    -> hostspecify the host name used by Twilio in a number's webhook config
    -> protocolspecify the protocol used by Twilio in a number's webhook config
 */
    ...
    return function hook(request, response, next) {
        ...
        if (options?.validate) {
            ...
            if (!options?.authToken) {
                console.error("[Twilio]- Twilio auth token is required for webhook request validation.");
                response
                    .type("text/plain")
                    .status(500)

...
const resolve = (family, hostname, options, callback) => {
    ...
    const dnsResolver = dns.Resolver ? new dns.Resolver(options) ;
    dnsResolver['resolve' + family](hostname, (err, addresses) => {
        if (err) {
            switch (err.code) {
                case dns.NODATAdns.NOTFOUNDdns.NOTIMPdns.SERVFAILdns.CONNREFUSEDdns.REFUSED'EAI_AGAIN'callback(null, []);
            }
            return callback(err);
        }
...
const formatDNSValue = (value, extra) => {
    ...
    const host = addresses.length > 0 ? addresses[Math.floor(Math.random() * addresses.length)] ;
...
module.exports.resolveHostname = (options, callback) => {
    ...
    resolve(4, options.host, options, (err, addresses) => {
        ...
        resolve(6, options.host, options, (err, addresses) => {
            ...
            // Combine addressesfirst, then IPv6
            const allAddresses = ipv4Addresses.concat(ipv6Addresses);

            if (allAddresses.length) {
                const value = {
                    addresses,
                    servername.servername || options.host
                };

                dnsCache.set(options.host, {
                    value,
                    expires.now() + (options.dnsTtl || DNS_TTL)
                });
            ...
            try {
                dns.lookup(options.host, { all}, (err, addresses) => {
                    if (err) {
                        if (cached) {
                            dnsCache.set(options.host, {
                                value.value,
                    ...
                    const value = {
                        addresses.length ? supportedAddresses : [options.host],
                        servername.servername || options.host
                    };
...
module.exports.parseConnectionUrl = str => {
    ...
    switch (url.protocol) {
        case 'smtp:'.secure = false;
            break;
        case 'smtps:'.secure = true;
            break;
...
module.exports.getLogger = (options, defaults) => {
    ...
    const logger = options.logger === true ? createDefaultLogger(levels) .logger;
...
function resolveContentValue(data, key, options, callback) {
    ...
    if (typeof content === 'object') {
        if (typeof content.pipe === 'function') {
            ...
            return callback(null, parsedDataUri && parsedDataUri.data ? parsedDataUri.data .alloc(0));

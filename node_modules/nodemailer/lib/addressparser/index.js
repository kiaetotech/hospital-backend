...
function _handleAddress(tokens, depth) {
    ...
    if (isGroup) {
        ...
        if (data.group.length) {
            const parsedGroup = addressparser(data.group.join(','), { _depth+ 1 });
        ...
        if (!data.address.length && data.text.length) {
            for (let i = data.text.length - 1; i >= 0; i--) {
                // Securitynot extract email addresses from quoted strings.
...
class Tokenizer {
    ...
    tokenize() {
        ...
        for (let i = 0, len = this.str.length; i < len; i++) {
            ...
            const nextChr = i < len - 1 ? this.str.charAt(i + 1) ;
...
function addressparser(str, options) {
    ...
    // Recombinename-only entry followed by an entry with both name and address.

...
class Mail extends EventEmitter {
    constructor(transporter, options, defaults) {
        ...
        this.dkim = this.options.dkim ? new DKIM(this.options.dkim) ;

        this.transporter = transporter;
        this.transporter.mailer = this;

        this.logger = shared.getLogger(this.options, {
            component.options.component || 'mail'
        });
            ...
            this._processPlugins('stream', mail, err => {
                ...
                if (mail.data.dkim || this.dkim) {
                    mail.message.processFunc(input => {
                        const dkim = mail.data.dkim ? new DKIM(mail.data.dkim) .dkim;
                        this.logger.debug(
                            {
                                tnx: 'DKIM',
                                messageId.message.messageId(),
                                dkimDomains.keys.map(key => key.keySelector + '.' + key.domainName).join(', ')
                            },
    ...
    _processPlugins(step, mail, callback) {
        ...
        const processPlugins = () => {
            let curplugins = block === 'default' ? defaultPlugins ;
    ...
    setupProxy(proxyUrl) {
        ...
        this.getSocket = (options, callback) => {
            ...
            switch (protocol) {
                ...
                case 'http''https'(proxy.href, options.port, options.host, (err, socket) => {
                        if (err) {
                            return callback(err);
                        }
                ...
                case 'socks''socks5''socks4''socks4a': {
                    ...
                    const connect = ipaddress => {
                        ...
                        const socksClient = proxyV2 ? this.meta.get('proxy_socks_module').SocksClient .meta.get('proxy_socks_module');
                    ...
                    return dns.resolve(proxy.hostname, (err, address) => {
                        ...
                        connect(Array.isArray(address) ? address[0] );
                    });
                }
            }
    ...
    _convertDataImages(mail, callback) {
        ...
        mail.resolveContent(
            mail.data,
            'html',
            { disableFileAccess.data.disableFileAccess, disableUrlAccess.data.disableUrlAccess },
            (err, html) => {
                if (err) {
                    return callback(err);
                }

...
module.exports.createTransport = function (transporter, defaults) {
    ...
    if (
        ...
        const urlConfig = typeof transporter === 'string' ? transporter .url;
        if (urlConfig) {
...
module.exports.createTestAccount = function (apiUrl, callback) {
    ...
    const fetchOptions = {
        ...
        method: 'POST',
        headers,
        body.from(JSON.stringify(requestBody))
    };

    // Credential-bearing request — opt back into strict cert validation when
    // the API URL is HTTPS. lib/fetch defaults to rejectUnauthorized// for attachment hosts that may be self-signed; the Ethereal API has a
    // real cert, so the lax default was a free attack surface.
    if (/^https:/i.test(apiUrl)) {
        fetchOptions.tls = { rejectUnauthorized};
    }

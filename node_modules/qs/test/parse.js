...
test('parse()', function (t) {
    t.test('parses a simple string', function (st) {
        ...
        st.deepEqual(qs.parse('a[<=>]==23'), { a: { '<=>': '=23' } });
        st.deepEqual(qs.parse('a[==]=23'), { a: { '==': '23' } });
        st.deepEqual(qs.parse('foo', { strictNullHandling}), { foo});
        ...
        st.deepEqual(qs.parse('foo=bar&baz', { strictNullHandling}), { foo: 'bar', baz});
        st.deepEqual(qs.parse('foo=bar&baz'), { foo: 'bar', baz: '' });
    ...
    t.test('comma', function (st) {
        st.deepEqual(qs.parse('a[]=b&a[]=c', { comma}), { a: ['b', 'c'] });
        st.deepEqual(qs.parse('a[0]=b&a[1]=c', { comma}), { a: ['b', 'c'] });
        st.deepEqual(qs.parse('a=b,c', { comma}), { a: ['b', 'c'] });
        st.deepEqual(qs.parse('a=b&a=c', { comma}), { a: ['b', 'c'] });
        st.end();
    });
    ...
    t.test('allows enabling dot notation', function (st) {
        st.deepEqual(qs.parse('a.b=c'), { 'a.b': 'c' });
        st.deepEqual(qs.parse('a.b=c', { allowDots}), { a: { b: 'c' } });
    ...
    t.test('decode dot keys correctly', function (st) {
        st.deepEqual(
            qs.parse('name%252Eobj.first=John&name%252Eobj.last=Doe', { allowDots, decodeDotInKeys}),
            { 'name%2Eobj.first': 'John', 'name%2Eobj.last': 'Doe' },
            'with allowDots false and decodeDotInKeys false'
        );
        st.deepEqual(
            qs.parse('name.obj.first=John&name.obj.last=Doe', { allowDots, decodeDotInKeys}),
    ...
    t.test('throws when decodeDotInKeys is not of type boolean', function (st) {
        ...
        st['throws'](
            function () { qs.parse('foo[]&bar=baz', { decodeDotInKeys}); },
            TypeError
        );
    ...
    t.test('allows empty arrays in obj values', function (st) {
        st.deepEqual(qs.parse('foo[]&bar=baz', { allowEmptyArrays}), { foo: [], bar: 'baz' });
        st.deepEqual(qs.parse('foo[]&bar=baz', { allowEmptyArrays}), { foo: [''], bar: 'baz' });
    ...
    t.test('throws when allowEmptyArrays is not of type boolean', function (st) {
        ...
        st['throws'](
            function () { qs.parse('foo[]&bar=baz', { allowEmptyArrays}); },
            TypeError
        );
    ...
    t.test('allowEmptyArrays + strictNullHandling', function (st) {
        st.deepEqual(
            qs.parse('testEmptyArray[]', { strictNullHandling, allowEmptyArrays}),
            { testEmptyArray: [] }
        );
    ...
    t.test('uses original key when depth = 0', function (st) {
        ...
        st.deepEqual(qs.parse('a.b=c', { depth: 0, allowDots}), { 'a[b]': 'c' }, 'normalizes dots before applying depth-0 behavior');
        st.deepEqual(qs.parse('toString=foo', { depth: 0 }), {}, 'respects prototype guard at depth 0');
        st.deepEqual(qs.parse('toString=foo', { depth: 0, allowPrototypes}), { toString: 'foo' }, 'allows prototypes at depth 0 when enabled');
        st.end();
    });
    ...
    t.test('allows prototype keys when depth = 0 and allowPrototypes is true', function (st) {
        st.deepEqual(qs.parse('toString=foo', { depth: 0, allowPrototypes}), { toString: 'foo' });
        st.end();
    });

    t.test('uses original key when depth = false', function (st) {
        st.deepEqual(qs.parse('a[0]=b&a[1]=c', { depth}), { 'a[0]': 'b', 'a[1]': 'c' });
        st.deepEqual(qs.parse('a[0][0]=b&a[0][1]=c&a[1]=d&e=2', { depth}), { 'a[0][0]': 'b', 'a[0][1]': 'c', 'a[1]': 'd', e: '2' });
        st.end();
    });
    ...
    t.test('parses keys with literal [] inside a bracket group (#493)', function (st) {
        ...
        // Multiple nested bracket pairs[] remains literal as part of the key
        st.deepEqual(
            qs.parse('a[b[c[]]]=d'),
            { a: { 'b[c[]]': 'd' } },
        ...
        // Depth limits with literal bracketsinner [] while limiting bracket-group parsing
        st.deepEqual(
    ...
    t.test('transforms arrays to objects', function (st) {
        ...
        st.deepEqual(qs.parse('a[]=b&a[t]=u&a[hasOwnProperty]=c', { allowPrototypes}), { a: { 0: 'b', t: 'u' } });
        st.deepEqual(qs.parse('a[]=b&a[t]=u&a[hasOwnProperty]=c', { allowPrototypes}), { a: { 0: 'b', t: 'u', hasOwnProperty: 'c' } });
        st.deepEqual(qs.parse('a[]=b&a[hasOwnProperty]=c&a[x]=y', { allowPrototypes}), { a: { 0: 'b', x: 'y' } });
        st.deepEqual(qs.parse('a[]=b&a[hasOwnProperty]=c&a[x]=y', { allowPrototypes}), { a: { 0: 'b', hasOwnProperty: 'c', x: 'y' } });
        st.end();
    });

    t.test('transforms arrays to objects (dot notation)', function (st) {
        st.deepEqual(qs.parse('foo[0].baz=bar&fool.bad=baz', { allowDots}), { foo: [{ baz: 'bar' }], fool: { bad: 'baz' } });
        st.deepEqual(qs.parse('foo[0].baz=bar&fool.bad.boo=baz', { allowDots}), { foo: [{ baz: 'bar' }], fool: { bad: { boo: 'baz' } } });
        st.deepEqual(qs.parse('foo[0][0].baz=bar&fool.bad=baz', { allowDots}), { foo: [[{ baz: 'bar' }]], fool: { bad: 'baz' } });
        st.deepEqual(qs.parse('foo[0].baz[0]=15&foo[0].bar=2', { allowDots}), { foo: [{ baz: ['15'], bar: '2' }] });
        st.deepEqual(qs.parse('foo[0].baz[0]=15&foo[0].baz[1]=16&foo[0].bar=2', { allowDots}), { foo: [{ baz: ['15', '16'], bar: '2' }] });
        st.deepEqual(qs.parse('foo.bad=baz&foo[0]=bar', { allowDots}), { foo: { bad: 'baz', 0: 'bar' } });
        st.deepEqual(qs.parse('foo.bad=baz&foo[]=bar', { allowDots}), { foo: { bad: 'baz', 0: 'bar' } });
        st.deepEqual(qs.parse('foo[]=bar&foo.bad=baz', { allowDots}), { foo: { 0: 'bar', bad: 'baz' } });
        st.deepEqual(qs.parse('foo.bad=baz&foo[]=bar&foo[]=foo', { allowDots}), { foo: { bad: 'baz', 0: 'bar', 1: 'foo' } });
        st.deepEqual(qs.parse('foo[0].a=a&foo[0].b=b&foo[1].a=aa&foo[1].b=bb', { allowDots}), { foo: [{ a: 'a', b: 'b' }, { a: 'aa', b: 'bb' }] });
        st.end();
    });
    ...
    t.test('supports malformed uri characters', function (st) {
        st.deepEqual(qs.parse('{%:%}', { strictNullHandling}), { '{%:%}'});
        st.deepEqual(qs.parse('{%:%}='), { '{%:%}': '' });
        st.deepEqual(qs.parse('foo=%:%}'), { foo: '%:%}' });
        st.end();
    });
    ...
    t.test('allows for empty strings in arrays', function (st) {
        ...
        st.deepEqual(
            qs.parse('a[0]=b&a[1]&a[2]=c&a[19]=', { strictNullHandling, arrayLimit: 20 }),
            { a: ['b', null, 'c', ''] },
            'with arrayLimit 20 + array indicesthen empty string works'
        );
        st.deepEqual(
            qs.parse('a[]=b&a[]&a[]=c&a[]=', { strictNullHandling, arrayLimit: 0 }),
            { a: { 0: 'b', 1, 2: 'c', 3: '' } },
            'with arrayLimit 0 + array bracketsthen empty string works'
        );

        st.deepEqual(
            qs.parse('a[0]=b&a[1]=&a[2]=c&a[19]', { strictNullHandling, arrayLimit: 20 }),
            { a: ['b', '', 'c', null] },
            'with arrayLimit 20 + array indicesstring then null works'
        );
        st.deepEqual(
            qs.parse('a[]=b&a[]=&a[]=c&a[]', { strictNullHandling, arrayLimit: 0 }),
            { a: { 0: 'b', 1: '', 2: 'c', 3} },
            'with arrayLimit 0 + array bracketsstring then null works'
        );

        st.deepEqual(
            qs.parse('a[]=&a[]=b&a[]=c'),
            { a: ['', 'b', 'c'] },
            'array bracketsstrings work'
        );
        st.end();
    });
    ...
    t.test('parses sparse arrays', function (st) {
        ...
        st.deepEqual(qs.parse('a[4]=1&a[1]=2', { allowSparse}), { a: [, '2', , , '1'] });
        st.deepEqual(qs.parse('a[1][b][2][c]=1', { allowSparse}), { a: [, { b: [, , { c: '1' }] }] });
        st.deepEqual(qs.parse('a[1][2][3][c]=1', { allowSparse}), { a: [, [, , [, , , { c: '1' }]]] });
        st.deepEqual(qs.parse('a[1][2][3][c][1]=1', { allowSparse}), { a: [, [, , [, , , { c: [, '1'] }]]] });
        st.end();
    });
    ...
    t.test('continues parsing when no parent is found', function (st) {
        ...
        st.deepEqual(qs.parse('[]&a=b', { strictNullHandling}), { 0, a: 'b' });
        st.deepEqual(qs.parse('[foo]=bar'), { foo: 'bar' });
        st.end();
    });
    ...
    t.test('allows setting the parameter limit to Infinity', function (st) {
        st.deepEqual(qs.parse('a=b&c=d', { parameterLimit}), { a: 'b', c: 'd' });
        st.end();
    });
    ...
    t.test('allows disabling array parsing', function (st) {
        var indices = qs.parse('a[0]=b&a[1]=c', { parseArrays});
        st.deepEqual(indices, { a: { 0: 'b', 1: 'c' } });
        st.equal(Array.isArray(indices.a), false, 'parseArrays, indices case is not an array');

        var emptyBrackets = qs.parse('a[]=b', { parseArrays});
        st.deepEqual(emptyBrackets, { a: { 0: 'b' } });
        st.equal(Array.isArray(emptyBrackets.a), false, 'parseArrays, empty brackets case is not an array');

        st.end();
    });

    t.test('allows for query string prefix', function (st) {
        st.deepEqual(qs.parse('?foo=bar', { ignoreQueryPrefix}), { foo: 'bar' });
        st.deepEqual(qs.parse('foo=bar', { ignoreQueryPrefix}), { foo: 'bar' });
        st.deepEqual(qs.parse('?foo=bar', { ignoreQueryPrefix}), { '?foo': 'bar' });
    ...
    t.test('parses string with comma as array divider', function (st) {
        st.deepEqual(qs.parse('foo=bar,tee', { comma}), { foo: ['bar', 'tee'] });
        st.deepEqual(qs.parse('foo[bar]=coffee,tee', { comma}), { foo: { bar: ['coffee', 'tee'] } });
        st.deepEqual(qs.parse('foo=', { comma}), { foo: '' });
        st.deepEqual(qs.parse('foo', { comma}), { foo: '' });
        st.deepEqual(qs.parse('foo', { comma, strictNullHandling}), { foo});

        // test cases inversed from from stringify tests
        st.deepEqual(qs.parse('a[0]=c'), { a: ['c'] });
        st.deepEqual(qs.parse('a[]=c'), { a: ['c'] });
        st.deepEqual(qs.parse('a[]=c', { comma}), { a: ['c'] });

        st.deepEqual(qs.parse('a[0]=c&a[1]=d'), { a: ['c', 'd'] });
        st.deepEqual(qs.parse('a[]=c&a[]=d'), { a: ['c', 'd'] });
        st.deepEqual(qs.parse('a=c,d', { comma}), { a: ['c', 'd'] });
    ...
    t.test('use number decoder, parses string that has one number with comma option enabled', function (st) {
        ...
        st.deepEqual(qs.parse('foo=1', { comma, decoder}), { foo: 1 });
        st.deepEqual(qs.parse('foo=0', { comma, decoder}), { foo: 0 });

        st.end();
    });

    t.test('parses brackets holds array of arrays when having two parts of strings with comma as array divider', function (st) {
        st.deepEqual(qs.parse('foo[]=1,2,3&foo[]=4,5,6', { comma}), { foo: [['1', '2', '3'], ['4', '5', '6']] });
        st.deepEqual(qs.parse('foo[]=1,2,3&foo[]=', { comma}), { foo: [['1', '2', '3'], ''] });
        st.deepEqual(qs.parse('foo[]=1,2,3&foo[]=,', { comma}), { foo: [['1', '2', '3'], ['', '']] });
        st.deepEqual(qs.parse('foo[]=1,2,3&foo[]=a', { comma}), { foo: [['1', '2', '3'], 'a'] });
    ...
    t.test('parses url-encoded brackets holds array of arrays when having two parts of strings with comma as array divider', function (st) {
        st.deepEqual(qs.parse('foo%5B%5D=1,2,3&foo%5B%5D=4,5,6', { comma}), { foo: [['1', '2', '3'], ['4', '5', '6']] });
        st.deepEqual(qs.parse('foo%5B%5D=1,2,3&foo%5B%5D=', { comma}), { foo: [['1', '2', '3'], ''] });
        st.deepEqual(qs.parse('foo%5B%5D=1,2,3&foo%5B%5D=,', { comma}), { foo: [['1', '2', '3'], ['', '']] });
        st.deepEqual(qs.parse('foo%5B%5D=1,2,3&foo%5B%5D=a', { comma}), { foo: [['1', '2', '3'], 'a'] });

        st.end();
    });

    t.test('parses comma delimited array while having percent-encoded comma treated as normal text', function (st) {
        st.deepEqual(qs.parse('foo=a%2Cb', { comma}), { foo: 'a,b' });
        st.deepEqual(qs.parse('foo=a%2C%20b,d', { comma}), { foo: ['a, b', 'd'] });
        st.deepEqual(qs.parse('foo=a%2C%20b,c%2C%20d', { comma}), { foo: ['a, b', 'c, d'] });
    ...
    t.test('parses an object in dot notation', function (st) {
        ...
        var result = qs.parse(input, { allowDots});
    ...
    t.test('does not crash when parsing circular references', function (st) {
        ...
        st.doesNotThrow(function () {
            parsed = qs.parse({ 'foo[bar]': 'baz', 'foo[baz]'});
        });
    ...
    t.test('does not allow overwriting prototype properties', function (st) {
        ...
        st.deepEqual(qs.parse('hasOwnProperty=b', { allowPrototypes}), {});

        st.deepEqual(
            qs.parse('toString', { allowPrototypes}),
            {},
    ...
    t.test('can allow overwriting prototype properties', function (st) {
        st.deepEqual(qs.parse('a[hasOwnProperty]=b', { allowPrototypes}), { a: { hasOwnProperty: 'b' } });
        st.deepEqual(qs.parse('hasOwnProperty=b', { allowPrototypes}), { hasOwnProperty: 'b' });
    ...
    t.test('does not crash when the global Object prototype is frozen', { skip: !hasPropertyDescriptors || !hasOverrideMistake }, function (st) {
        ...
        st.teardown(mockProperty(Object.prototype, 'frozenProp', { value: 'foo', nonWritable, nonEnumerable}));
        ...
        st['throws'](
            ...
            /^TypeErrorassign to read only property 'frozenProp' of (?'#<Object>'|#<Object>)/,
        ...
        st.doesNotThrow(
            function () {
                parsed = qs.parse('frozenProp', { allowPrototypes});
            },
    ...
    t.test('add keys to objects', function (st) {
        st.deepEqual(
            qs.parse('a[b]=c&a=d', { strictMerge}),
            { a: { b: 'c', d} },
            'can add keys to objects'
        );

        st.deepEqual(
            qs.parse('a[b]=c&a=toString', { strictMerge}),
            { a: { b: 'c' } },
            'can not overwrite prototype'
        );

        st.deepEqual(
            qs.parse('a[b]=c&a=toString', { strictMerge, allowPrototypes}),
            { a: { b: 'c', toString} },
            'can overwrite prototype with allowPrototypes true'
        );

        st.deepEqual(
            qs.parse('a[b]=c&a=toString', { strictMerge, plainObjects}),
            { __proto__, a: { __proto__, b: 'c', toString} },
    ...
    t.test('strictMerge wraps object and primitive into an array', function (st) {
        ...
        st.deepEqual(
            qs.parse('a[b]=c&a=toString', { plainObjects}),
            { __proto__, a: [{ __proto__, b: 'c' }, 'toString'] },
    ...
    t.test('dunder proto is ignored', function (st) {
        ...
        var result = qs.parse(payload, { allowPrototypes});
        ...
        var plainResult = qs.parse(payload, { allowPrototypes, plainObjects});
        ...
        st.deepEqual(
            ...
            'silent [[Prototype]] payloadobjects'
        );

        var query = qs.parse('categories[__proto__]=cats&categories[__proto__]=dogs&categories[some][json]=toInject', { allowPrototypes});
        ...
        st.deepEqual(
            qs.parse('foo[__proto__][hidden]=value&foo[bar]=stuffs', { allowPrototypes}),
            {
                foo: {
                    bar: 'stuffs'
                }
        ...
        st.deepEqual(
            qs.parse('foo[__proto__][hidden]=value&foo[bar]=stuffs', { allowPrototypes, plainObjects}),
            {
                __proto__,
                foo: {
                    __proto__,
    ...
    t.test('can return null objects', { skip: !hasProto }, function (st) {
        ...
        st.deepEqual(qs.parse('a[b]=c&a[hasOwnProperty]=d', { plainObjects}), expected);
        st.deepEqual(qs.parse(null, { plainObjects}), { __proto__});
        var expectedArray = {
            __proto__,
            a: {
                __proto__,
        ...
        st.deepEqual(qs.parse('a[]=b&a[c]=d', { plainObjects}), expectedArray);
        st.end();
    });
    ...
    t.test('prefers an utf-8 charset specified by the utf8 sentinel to a default charset of iso-8859-1', function (st) {
        st.deepEqual(qs.parse('utf8=' + urlEncodedCheckmarkInUtf8 + '&' + urlEncodedOSlashInUtf8 + '=' + urlEncodedOSlashInUtf8, { charsetSentinel, charset: 'iso-8859-1' }), { ø: 'ø' });
        st.end();
    });

    t.test('prefers an iso-8859-1 charset specified by the utf8 sentinel to a default charset of utf-8', function (st) {
        st.deepEqual(qs.parse('utf8=' + urlEncodedNumCheckmark + '&' + urlEncodedOSlashInUtf8 + '=' + urlEncodedOSlashInUtf8, { charsetSentinel, charset: 'utf-8' }), { 'Ã¸': 'Ã¸' });
        st.end();
    });

    t.test('does not require the utf8 sentinel to be defined before the parameters whose decoding it affects', function (st) {
        st.deepEqual(qs.parse('a=' + urlEncodedOSlashInUtf8 + '&utf8=' + urlEncodedNumCheckmark, { charsetSentinel, charset: 'utf-8' }), { a: 'Ã¸' });
        st.end();
    });

    t.test('ignores an utf8 sentinel with an unknown value', function (st) {
        st.deepEqual(qs.parse('utf8=foo&' + urlEncodedOSlashInUtf8 + '=' + urlEncodedOSlashInUtf8, { charsetSentinel, charset: 'utf-8' }), { ø: 'ø' });
        st.end();
    });

    t.test('uses the utf8 sentinel to switch to utf-8 when no default charset is given', function (st) {
        st.deepEqual(qs.parse('utf8=' + urlEncodedCheckmarkInUtf8 + '&' + urlEncodedOSlashInUtf8 + '=' + urlEncodedOSlashInUtf8, { charsetSentinel}), { ø: 'ø' });
        st.end();
    });

    t.test('uses the utf8 sentinel to switch to iso-8859-1 when no default charset is given', function (st) {
        st.deepEqual(qs.parse('utf8=' + urlEncodedNumCheckmark + '&' + urlEncodedOSlashInUtf8 + '=' + urlEncodedOSlashInUtf8, { charsetSentinel}), { 'Ã¸': 'Ã¸' });
        st.end();
    });

    t.test('interprets numeric entities in iso-8859-1 when `interpretNumericEntities`', function (st) {
        st.deepEqual(qs.parse('foo=' + urlEncodedNumSmiley, { charset: 'iso-8859-1', interpretNumericEntities}), { foo: '☺' });
        st.end();
    });
    ...
    t.test('handles a custom decoder returning `null`, in the `iso-8859-1` charset, when `interpretNumericEntities`', function (st) {
        st.deepEqual(qs.parse('foo=&bar=' + urlEncodedNumSmiley, {
            ...
            decoder(str, defaultDecoder, charset) {
                return str ? defaultDecoder(str, defaultDecoder, charset) ;
            },
            interpretNumericEntities}), { foo, bar: '☺' });
        st.end();
    });
    ...
    t.test('handles a custom decoder returning `null`, with a string key of `null`', function (st) {
        st.deepEqual(
            qs.parse('null=1&ToNull=2', {
                decoder(str, defaultDecoder, charset) {
                    return str === 'ToNull' ? null (str, defaultDecoder, charset);
                }
            }),
            { 'null': '1' },
    ...
    t.test('does not interpret numeric entities when the charset is utf-8, even when `interpretNumericEntities`', function (st) {
        st.deepEqual(qs.parse('foo=' + urlEncodedNumSmiley, { charset: 'utf-8', interpretNumericEntities}), { foo: '&#9786;' });
        st.end();
    });

    t.test('interpretNumericEntities with commaand iso charset does not crash', function (st) {
        st.deepEqual(
            qs.parse('b&a[]=1,' + urlEncodedNumSmiley, { comma, charset: 'iso-8859-1', interpretNumericEntities}),
            { b: '', a: ['1,☺'] }
        );
    ...
    t.test('parameter limit tests', function (st) {
        st.test('does not throw error when within parameter limit', function (sst) {
            var result = qs.parse('a=1&b=2&c=3', { parameterLimit: 5, throwOnLimitExceeded});
        ...
        st.test('throws error when parameter limit exceeded', function (sst) {
            sst['throws'](
                function () {
                    qs.parse('a=1&b=2&c=3&d=4&e=5&f=6', { parameterLimit: 3, throwOnLimitExceeded});
                },
            ...
            sst['throws'](
                function () {
                    qs.parse('a=1&b=2', { parameterLimit: 1, throwOnLimitExceeded});
                },
        ...
        st.test('silently truncates when parameter limit exceeded without error', function (sst) {
            var result = qs.parse('a=1&b=2&c=3&d=4&e=5', { parameterLimit: 3, throwOnLimitExceeded});
            sst.deepEqual(result, { a: '1', b: '2', c: '3' }, 'parses and truncates silently');
            sst.end();
        });

        st.test('allows unlimited parameters when parameterLimit set to Infinity', function (sst) {
            var result = qs.parse('a=1&b=2&c=3&d=4&e=5&f=6', { parameterLimit});
        ...
        st.test('allows unlimited parameters when parameterLimit is Infinity and throwOnLimitExceeded is true', function (sst) {
            var result = qs.parse('a=1&b=2&c=3&d=4&e=5&f=6', { parameterLimit, throwOnLimitExceeded});
    ...
    t.test('array limit tests', function (st) {
        st.test('does not throw error when array is within limit', function (sst) {
            var result = qs.parse('a[]=1&a[]=2&a[]=3', { arrayLimit: 5, throwOnLimitExceeded});
        ...
        st.test('throws error when array limit exceeded', function (sst) {
            ...
            sst['throws'](
                function () {
                    qs.parse('a[]=1&a[]=2&a[]=3&a[]=4', { arrayLimit: 3, throwOnLimitExceeded});
                },
        ...
        st.test('does not throw when at limit', function (sst) {
            ...
            var result = qs.parse('a[]=1&a[]=2&a[]=3', { arrayLimit: 3, throwOnLimitExceeded});
        ...
        st.test('throws error when indexed notation exceeds arrayLimit with throwOnLimitExceeded', function (sst) {
            sst['throws'](
                function () {
                    qs.parse('a[1001]=b', { arrayLimit: 1000, throwOnLimitExceeded});
                },
            ...
            sst['throws'](
                function () {
                    qs.parse('a[0]=1&a[1]=2&a[2]=3&a[10]=4', { arrayLimit: 6, throwOnLimitExceeded, allowSparse});
                },
            ...
            sst['throws'](
                function () {
                    qs.parse('a[2]=b', { arrayLimit: 1, throwOnLimitExceeded});
                },
        ...
        st.test('does not throw for indexed notation within arrayLimit with throwOnLimitExceeded', function (sst) {
            var result = qs.parse('a[4]=b', { arrayLimit: 5, throwOnLimitExceeded, allowSparse});
            sst.ok(Array.isArray(result.a), 'result is an array');
        ...
        st.test('throws when duplicate bracket keys exceed arrayLimit with throwOnLimitExceeded', function (sst) {
            sst['throws'](
                function () {
                    qs.parse('a[]=1&a[]=2&a[]=3&a[]=4&a[]=5&a[]=6', { arrayLimit: 5, throwOnLimitExceeded});
                },
...
test('`duplicates` option', function (t) {
    ...
    t.test('bracket notation always combines regardless of duplicates', function (st) {
        st.deepEqual(
            ...
            { a: '2', b: ['1', '2'] },
            'duplicates lasttakes last, bracketed combines'
        );

        st.deepEqual(
            qs.parse('b[]=1&b[]=2', { duplicates: 'last' }),
            { b: ['1', '2'] },
            'duplicates lastalways combines'
        );

        st.deepEqual(
            qs.parse('b[]=1&b[]=2', { duplicates: 'first' }),
            { b: ['1', '2'] },
            'duplicates firstalways combines'
        );

        st.deepEqual(
            qs.parse('a=1&a=2&b[]=1&b[]=2', { duplicates: 'first' }),
            { a: '1', b: ['1', '2'] },
            'duplicates firsttakes first, bracketed combines'
        );
...
test('qs strictDepth option - throw cases', function (t) {
    t.test('throws an exception when depth exceeds the limit with strictDepth', function (st) {
        st['throws'](
            function () {
                qs.parse('a[b][c][d][e][f][g][h][i]=j', { depth: 1, strictDepth});
            },
            RangeError,
            'throws RangeError'
        );
    ...
    t.test('throws an exception for multiple nested arrays with strictDepth', function (st) {
        st['throws'](
            function () {
                qs.parse('a[0][1][2][3][4]=b', { depth: 3, strictDepth});
            },
            RangeError,
            'throws RangeError'
        );
    ...
    t.test('throws an exception for nested objects and arrays with strictDepth', function (st) {
        st['throws'](
            function () {
                qs.parse('a[b][c][0][d][e]=f', { depth: 3, strictDepth});
            },
            RangeError,
            'throws RangeError'
        );
    ...
    t.test('throws an exception for different types of values with strictDepth', function (st) {
        st['throws'](
            function () {
                qs.parse('a[b][c][d][e]=true&a[b][c][d][f]=42', { depth: 3, strictDepth});
            },
            RangeError,
            'throws RangeError'
        );
...
test('qs strictDepth option - non-throw cases', function (t) {
    t.test('when depth is 0 and strictDepth true, do not throw', function (st) {
        st.doesNotThrow(
            function () {
                qs.parse('a[b][c][d][e]=true&a[b][c][d][f]=42', { depth: 0, strictDepth});
            },
            RangeError,
    ...
    t.test('parses successfully when depth is within the limit with strictDepth', function (st) {
        st.doesNotThrow(
            function () {
                var result = qs.parse('a[b]=c', { depth: 1, strictDepth});
    ...
    t.test('does not throw an exception when depth exceeds the limit with strictDepth', function (st) {
        st.doesNotThrow(
            function () {
    ...
    t.test('does not throw when depth is exactly at the limit with strictDepth', function (st) {
        st.doesNotThrow(
            function () {
                var result = qs.parse('a[b][c]=d', { depth: 2, strictDepth});
...
test('DOS', function (t) {
    ...
    t.notOk(Array.isArray(result.a), 'arrayLimit is respectedis an object, not an array');
...
test('comma + arrayLimit', function (t) {
    t.test('comma-separated values within arrayLimit stay as array', function (st) {
        var result = qs.parse('a=1,2,3', { comma, arrayLimit: 5 });
    ...
    t.test('comma-separated values exceeding arrayLimit convert to object', function (st) {
        var result = qs.parse('a=1,2,3,4', { comma, arrayLimit: 3 });
        st.notOk(Array.isArray(result.a), 'result is not an array when over limit');
        st.deepEqual(result.a, { 0: '1', 1: '2', 2: '3', 3: '4' }, 'all values preserved as object');
        st.end();
    });
    ...
    t.test('comma-separated values exceeding arrayLimit with throwOnLimitExceeded throws', function (st) {
        st['throws'](
            function () {
                qs.parse('a=1,2,3,4', { comma, arrayLimit: 3, throwOnLimitExceeded});
            },
            new RangeError('Array limit exceeded. Only 3 elements allowed in an array.'),
        ...
        st['throws'](
            function () {
                qs.parse('a=1,2,3', { comma, arrayLimit: 1, throwOnLimitExceeded});
            },
            new RangeError('Array limit exceeded. Only 1 element allowed in an array.'),
            'throws error with singular "element" when arrayLimit is 1'
        );
    ...
    t.test('comma-separated values at exactly arrayLimit stay as array', function (st) {
        var result = qs.parse('a=1,2,3', { comma, arrayLimit: 3 });
...
test('mixed array and object notation', function (t) {
    ...
    t.test('throws with singular message when object input bracket key exceeds arrayLimit of 1', function (st) {
        ...
        st['throws'](
            function () {
                qs.parse(input, { throwOnLimitExceeded, arrayLimit: 1 });
            },
            new RangeError('Array limit exceeded. Only 1 element allowed in an array.'),

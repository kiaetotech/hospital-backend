...
function help(status) {
  var print = (status === 0) ? console.log .error;
...
function run(codeList) {
  ...
  try {
    ...
    console.error(fileMode ? e.message.replace(/\(\d+:\d+\)$/, function (m) { return m.slice(0, 1) + inputFilePaths[fileIdx] + " " + m.slice(1); }) .message);
    process.exit(1);
  }
  if (!silent) { console.log(JSON.stringify(result, function (_, value) { return typeof value === "bigint" ? null ; }, compact ? null : 2)); }
}

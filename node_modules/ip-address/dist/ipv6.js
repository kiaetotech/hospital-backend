...
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    ...
    if (!desc || ("get" in desc ? !m.__esModule .writable || desc.configurable)) {
      desc = { enumerable, get() { return m[k]; } };
    }
...
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable, value});
}) (o, v) {
    o["default"] = v;
});
...
class Address6 {
     ...
     * subnet portion are allowed (e.g. `2001::1/32` is valid); for strict
     ...
     * `{ error, address, port}` if the URL could not be parsed.
     ...
     * `ffff:`).
     * @example
     * var address = Address6.fromAddressAndMask('fe80::1', 'ffff::');
     ...
     * length`*` represents 16 bits. `::` is expanded to zero groups
     * (not wildcards) before evaluating trailing wildcards.
     *
     * Only trailing whole-group wildcards are supported. Partial-group
     * wildcards (e.g. `2001::0*`) and interior wildcards (e.g.
     * `*::1`) throw `AddressError`.
     * @example
     * Address6.fromWildcard('2001:*:*:*:*:*:*').subnet;  // '/32'
     * Address6.fromWildcard('2001::*').subnet;           // '/112'
    ...
    static fromWildcard(input) {
        ...
        let groups;
        if (halves.length === 2) {
            const left = halves[0] === '' ? [] [0].split(':');
            const right = halves[1] === '' ? [] [1].split(':');
        ...
        for (let i = 0; i < groups.length; i++) {
            if (groups[i] === '*') {
                ...
                throw new address_error_1.AddressError('Wildcard `*` must only appear in trailing groups (e.g. `2001:*:*:*:*:*:*`)');
            }
        }
        const trailing = firstWildcard === -1 ? 0 .length - firstWildcard;
        const replaced = groups.map((g) => (g === '*' ? '0' ));
     ...
     * address.correctForm(); // '::1'
     * address.to4in6(); // '::192.168.0.1'
     */
    static fromAddress4(address) {
        const address4 = new ipv4_1.Address4(address);
        const mask6 = constants6.BITS - (constants4.BITS - address4.subnetMask);
        return new Address6(`::${address4.correctForm()}/${mask6}`);
    }
    /**
     ...
     * address.correctForm(); // '2001:0:7601:62c3'
     */
     ...
     * @returns {String}
     */
    // TODOuseful to have a numeric version of this too
    ...
    /**
     * The hex form of the subnet mask, e.g. `ffff::` for a
    ...
    /**
     * The Cisco-style wildcard mask, e.g. `:` for a
    ...
    /**
     * The network address in CIDR string form, e.g. `2001::/32` for
     * `2001::1/32`. For an address with no explicit subnet the prefix
     * is `/128`, e.g. `networkForm()` on `2001::1` returns
     * `2001::1/128`.
     * @returns {string}
     */
    networkForm() {
    ...
    getScope() {
        ...
        // RFC 4291 §2.5.3loopback address is treated as having Link-Local
        ...
        // RFC 4007 §6unspecified address has no scope.
        if (type === 'Unspecified') {
            return 'Unknown';
        }
        return 'Global';
     ...
     * [RFC 5952](https://datatracker.ietf.org/doc/html/rfc5952)zeros
     * stripped, the longest run of zero groups collapsed to `::`, and hex digits
     * lowercased (e.g. `2001::1`). This is the recommended form for display.
     */
    correctForm() {
        let i;
        let groups = [];
    ...
    /**
     * Parses a v4-in-v6 string (e.g. `::192.168.0.1`) by extracting the
     ...
     */
    // TODOthe semantics of this helper function
    parse4in6(address) {
    ...
    parse(address) {
        ...
        if (halves.length === 2) {
            ...
            throw new address_error_1.AddressError('Too many ');
        }
    ...
    /**
     * Returns the canonical (fully expanded) form of the address8 groups,
     ...
     * @returns {Address4}
     * @example
     * var address = new Address6('2001:4860:4001::1825');
     ...
     * The default prefix is the well-known prefix `64::/96`. The prefix
     ...
     * @example
     * Address6.fromAddress4Nat64('192.0.2.33').correctForm(); // '64::221'
     * Address6.fromAddress4Nat64('192.0.2.33', '2001::/32').correctForm(); // '2001:221::'
     */
    static fromAddress4Nat64(address, prefix = '64::/96') {
     ...
     * The default prefix is the well-known prefix `64::/96`. Returns
     * `null` if this address is not contained within the given prefix.
     * @example
     * new Address6('64::221').toAddress4Nat64()!.correctForm(); // '192.0.2.33'
     */
    toAddress4Nat64(prefix = '64::/96') {
        const prefix6 = new Address6(prefix);
        const pl = prefix6.subnetMask;
        if (pl !== 32 && pl !== 40 && pl !== 48 && pl !== 56 && pl !== 64 && pl !== 96) {
            throw new address_error_1.AddressError('NAT64 prefix length must be 32, 40, 48, 56, 64, or 96');
     ...
     * (e.g. `::127.0.0.1`), false otherwise. This is a notation-level flag
     * and does not reflect whether the address bits lie in the IPv4-mapped
     * (`::0:0/96`) subnet — for that, see {@link isMapped4}.
     * @returns {boolean}
     */
    is4() {
        return this.v4;
     ...
     * `::0:0/96` ([RFC 4291 §2.5.5.2](https://datatracker.ietf.org/doc/html/rfc4291#section-2.5.5.2)),
     * false otherwise. Unlike {@link is4}, this checks the underlying address
     * bits rather than the textual notation, so `::127.0.0.1` and
     * `::7f00:1` both return true.
     * @returns {boolean}
     */
    isMapped4() {
    ...
    /**
     * Returns true if the address is in the documentation prefix `2001::/32` ([RFC 3849](https://datatracker.ietf.org/doc/html/rfc3849)).
     * @returns {boolean}
     */
    isDocumentation() {
     ...
     * `http://[2001::1]/`. If `optionalPort` is provided it is appended,
     * e.g. `http://[2001::1]:8080/`.
     */
    href(optionalPort) {
...
const DOCUMENTATION_SUBNET = new Address6('2001::/32');
const IPV4_MAPPED_SUBNET = new Address6('::0:0/96');

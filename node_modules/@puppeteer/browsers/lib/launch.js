 ...
 */
import childProcess from 'node_process';
...
const debugLaunch = debug('puppeteer');
/**
 * @public
 */
...
export class Process {
    ...
    constructor(opts) {
        ...
        this.#signal?.addEventListener('abort', this.#onAbort, { once});
        opts.pipe ??= false;
        opts.dumpio ??= false;
        opts.handleSIGINT ??= true;
        opts.handleSIGTERM ??= true;
        opts.handleSIGHUP ??= true;
        // On non-windows platforms, `detached` makes child process a
    ...
    #onDriverProcessSignal = (signal) => {
        switch (signal) {
            case 'SIGINT'.kill();
                process.exit(130);
            case 'SIGTERM''SIGHUP'this.close();
                break;
        }
    };
    async close() {
    ...
    kill() {
        ...
        if (this.#browserProcess &&
            ...
            try {
                ...
                throw new Error(`${PROCESS_ERROR_EXPLANATION}\nError cause: ${isErrorLike(error) ? error.stack }`);
            }
        }
        this.#clearListeners();
    }
    ...
    waitForLineOutput(regex, timeout = 0) {
        return new Promise((resolve, reject) => {
            ...
            const timeoutId = timeout > 0 ? setTimeout(onTimeout, timeout) ;

...
import { spawnSync } from 'node_process';
import { existsSync, readFileSync } from 'node';
import { mkdir, unlink } from 'node/promises';
import os from 'node';
...
const debugInstall = debug('puppeteer');
const times = new Map();
...
async function installWithProviders(options) {
    ...
    const downloadOptions = {
        ...
        platform.platform,
        buildId.buildId,
        progressCallback.downloadProgressCallback === 'default'
    ...
    for (const provider of providers) {
        try {
            // Checkthis provider support this browser/platform?
            ...
            if (!existsSync(browserRoot)) {
                await mkdir(browserRoot, { recursive});
            }
...
async function installDeps(installedBrowser) {
    ...
    if (result.status !== 0) {
        throw new Error('Failed to install system dependencies-get does not seem to be available');
    }
    ...
    if (result.status !== 0) {
        throw new Error(`Failed to install system dependencies=${result.status},error=${result.error},stdout=${result.stdout.toString('utf8')},stderr=${result.stderr.toString('utf8')}`);
    }
...
async function runSetup(installedBrowser) {
    ...
    if ((installedBrowser.platform === BrowserPlatform.WIN32 ||
        ...
        try {
            ...
            // TODOerror here. Currently the setup.exe sometimes

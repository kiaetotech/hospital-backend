 ...
 */
import { spawnSync, spawn, execFile } from 'node_process';
import { createReadStream } from 'node';
import { mkdir, readdir } from 'node/promises';
import * as path from 'node';
import { Stream } from 'node';
import { promisify } from 'node';
import { debug } from './debug.js';
const execFileAsync = promisify(execFile);
const debugFileUtil = debug('puppeteer');
/**
 * @internal
 */
...
async function extractZip(archivePath, folderPath) {
    try {
        if (process.platform === 'win32') {
            ...
            // -oexisting files without prompting
            // -dfiles into the specified directory
            await execFileAsync('unzip', ['-o', archivePath, '-d', folderPath]);
        }
    }
    catch (error) {
        if (error?.code === 'ENOENT') {
            throw new Error(`Extraction failednative binary ('tar.exe', 'powershell.exe', 'pwsh.exe' or 'unzip') was not found in the system PATH.`);
        }

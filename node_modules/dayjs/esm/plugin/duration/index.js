...
var DURATION_REGEX_PARSE = /^(-|\+)?P(:([-+]?[0-9,.]*)Y)?(:([-+]?[0-9,.]*)M)?(:([-+]?[0-9,.]*)W)?(:([-+]?[0-9,.]*)D)?(?(:([-+]?[0-9,.]*)H)?(:([-+]?[0-9,.]*)M)?(:([-+]?[0-9,.]*)S)?)?$/;
...
var roundNumber = function roundNumber(number) {
  return isNegative(number) ? Math.ceil(number) .floor(number);
};
...
var Duration = /*#__PURE__*/function () {
  ...
  _proto.format = function format(formatStr) {
    var str = formatStr || 'YYYY-MM-DDTHH';
    var matches = {
      Y.$d.years,

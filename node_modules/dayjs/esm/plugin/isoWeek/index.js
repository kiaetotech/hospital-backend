...
export default (function (o, c, d) {
  var getYearFirstThursday = function getYearFirstThursday(year, isUtc) {
    var yearFirstDay = (isUtc ? d.utc )().year(year).startOf(Y);
  ...
  proto.isoWeekday = function (week) {
    if (!this.$utils().u(week)) {
      return this.day(this.day() % 7 ? week - 7);
    }
  ...
  proto.startOf = function (units, startOf) {
    ...
    var isStartOf = !utils.u(startOf) ? startOf ;
    var unit = utils.p(units);

    if (unit === isoWeekPrettyUnit) {
      return isStartOf ? this.date(this.date() - (this.isoWeekday() - 1)).startOf('day') .date(this.date() - 1 - (this.isoWeekday() - 1) + 7).endOf('day');
    }

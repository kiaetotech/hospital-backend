...
(function () {
  ...
  function a(V) {
    ...
    aa(V);
    return { sourceCode.join("").replace(/\n$/, ""), spans};
  }
  function B(S, U, W, T) {
    if (!U) {
      return;
    }
    var V = { sourceCode, basePos};
    W(V);
  ...
  function o(S) {
    ...
    for (var U = S.firstChild; U; U = U.nextSibling) {
      var T = U.nodeType;
      V = T === 1 ? (V ? S ) === 3 ? (v.test(U.nodeValue) ? S ) ;
    }
    ...
    if (Y) {
      if (T.cStyleComments) {
        if (Y > 1) {
          ...
          W.push([
            j,
            /^#(:(?|elif|else|endif|error|ifdef|include|ifndef|line|pragma|undef|warning)\b|[^\r\n]*)/,
            null,
            "#",
          ]);
        }
  ...
  c(g([], [[n, /^[\s\S]+/]]), ["uq.val"]);
  c(i({ keywords, hashComments, cStyleComments, types}), [
    "c",
    "cc",
    "cpp",
    "cxx",
  ...
  c(i({ keywords, cStyleComments}), ["java"]);
  c(i({ keywords, hashComments, multiLineStrings}), [
    "bsh",
    "csh",
    "sh",
  ]);
  ...
  c(i({ keywords, cStyleComments, regexLiterals}), ["js"]);
  c(
    i({
      keywords,
      hashComments: 3,
  ...
  function y(W, V, U) {
    ...
    var T = { langExtension, numberLines, sourceNode};
    d(T);
    return S.innerHTML;
  }
  function b(ad) {
    ...
    function U() {
      var ag = window.PR_SHOULD_USE_CONTINUATION ? W.now() + 250 ;
      for (; X < T.length && W.now() < ag; X++) {
        ...
        if (ai.indexOf("prettyprint") >= 0) {
          ...
          if (!al) {
            ...
            af = af ? (af[1] && af[1].length ? +af[1] ) ;
            if (af) {
              Q(aj, af);
            }
            S = { langExtension, sourceNode, numberLines};
            d(S);
          }
        }
      }

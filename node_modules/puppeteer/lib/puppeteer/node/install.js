...
async function downloadBrowser({ browser, configuration, cacheDir, platform, }) {
    ...
    try {
        const result = await install({
            ...
            baseUrl,
            buildIdAlias!== unresolvedBuildId ? unresolvedBuildId ,
        });
        return `${browser} (${result.buildId}) downloaded to ${result.path}`;
    }
    catch (error) {
        throw new Error(`ERRORto set up ${browser} v${buildId}! Set "PUPPETEER_SKIP_DOWNLOAD" env variable to skip download.`, {
            cause,
        });
    }
}

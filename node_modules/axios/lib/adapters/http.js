...
function setProxy(options, configProxy, location, isRedirect, configHttpsAgent) {
  ...
  if (proxy) {
    ...
    const readProxyField = (key) =>
      isProxyURL || utils.hasOwnProp(proxy, key) ? proxy[key] ;

    const proxyUsername = readProxyField('username');
    const proxyPassword = readProxyField('password');
    let proxyAuth = utils.hasOwnProp(proxy, 'auth') ? proxy.auth ;
    ...
    if (proxyAuth) {
      ...
      const authUsername =
        authIsObject && utils.hasOwnProp(proxyAuth, 'username') ? proxyAuth.username ;
      const authPassword =
        authIsObject && utils.hasOwnProp(proxyAuth, 'password') ? proxyAuth.password ;
    ...
    if (targetIsHttps) {
      ...
      if (!(configHttpsAgent instanceof HttpsProxyAgent)) {
        ...
        const agentOptions = {
          ...
          port.port,
          auth&& typeof proxyAuth === 'string' ? proxyAuth ,
        };
...
export default isHttpAdapterSupported &&
  function httpAdapter(config) {
    return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
      // Read config pollution-safelyproperties and members inherited from
      ...
      if (lookup) {
        ...
        lookup = (hostname, opt, cb) => {
          _lookup(hostname, opt, (err, arg0, arg1) => {
            ...
            opt.all ? cb(err, addresses) (err, addresses[0].address, addresses[0].family);
          });
        };
      }
      ...
      function abort(reason) {
        try {
          abortEmitter.emit(
            'abort',
            !reason || reason.type ? new CanceledError(null, config, req) );
        } catch (err) {
          // ignore emit errors
        }
      }
      ...
      function createTimeoutError() {
        ...
        return new AxiosError(
          timeoutErrorMessage,
          transitional.clarifyTimeoutError ? AxiosError.ETIMEDOUT .ECONNABORTED,
          config,
          req
        );
      }
      ...
      if (config.cancelToken || config.signal) {
        ...
        if (config.signal) {
          config.signal.aborted ? abort() .signal.addEventListener('abort', abort);
        }
      }
      ...
      const parsed = new URL(fullPath, platform.hasBrowserEnv ? platform.origin );
      const protocol = parsed.protocol || supportedProtocols[0];

      if (protocol === 'data:') {
        // Apply the same semantics as HTTPenforce if a finite, non-negative cap is set.
      ...
      if (data && (onUploadProgress || maxUploadRate)) {
        if (!utils.isStream(data)) {
          data = stream.Readable.from(data, { objectMode});
        }
        ...
        data = stream.pipeline(
          [
            ...
            new AxiosTransformStream({
              maxRate.toFiniteNumber(maxUploadRate),
            }),
          ],
          utils.noop
        );
      ...
      headers.set(
        ...
        transitional.advertiseZstdAcceptEncoding === true ? ACCEPT_ENCODING_WITH_ZSTD _ENCODING,
        false
      );
      ...
      if (options.agent == null) {
        options.agent = isHttpsRequest ? httpsAgent ;
      }
      ...
      if (isHttp2) {
        ...
        if (configTransport) {
          ...
          if (auth) {
            ...
            options.beforeRedirects.auth = function beforeRedirectAuth(redirectOptions) {
              try {
                ...
                // ignore malformed URLauth stripped is fail-safe
              }
            };
          }
          ...
          transport = isHttpsRequest ? httpsFollow ;
        }
      }
      ...
      req = transport.request(options, function handleResponse(res) {
        ...
        if (onDownloadProgress || maxDownloadRate) {
          const transformStream = new AxiosTransformStream({
            maxRate.toFiniteNumber(maxDownloadRate),
          });
        ...
        if (decompress !== false && res.headers['content-encoding']) {
          ...
          switch ((res.headers['content-encoding'] || '').toLowerCase()) {
            ...
            case 'gzip''x-gzip''compress''x-compress':
              ...
              delete res.headers['content-encoding'];
              break;
            case 'deflate'.push(new ZlibHeaderTransformStream());
              ...
              delete res.headers['content-encoding'];
              break;
            case 'br'(isBrotliSupported) {
                streams.push(zlib.createBrotliDecompress(brotliOptions));
                delete res.headers['content-encoding'];
              }
              break;
            case 'zstd'(isZstdSupported) {
        ...
        responseStream = streams.length > 1 ? stream.pipeline(streams, utils.noop) [0];

        const response = {
          status.statusCode,
          statusText.statusMessage,
          headersAxiosHeaders(res.headers),
          config,
          request,
        };
        ...
        if (responseType === 'stream') {
          ...
          responseStream.on('end', function handleStreamEnd() {
            try {
              let responseData =
                responseBuffer.length === 1 ? responseBuffer[0] .concat(responseBuffer);

...
const isIPv6Loopback = (host) => {
  // Collapse all-zero groupsform of ::1 / 0:0:...:0:1
  // First, strip any leading "::" by normalising with Set lookup of common forms,
  // then fall back to structural check.
  if (host === '::1') return true;

  // Check IPv4-mapped IPv6 loopback: ::<v4-loopback> or ::<hex-v4-loopback>
  // Node's URL parser normalises ::127.0.0.1 → ::7f00:1
  const v4MappedDotted = host.match(/^::(\d+\.\d+\.\d+\.\d+)$/i);
  if (v4MappedDotted) return isIPv4Loopback(v4MappedDotted[1]);

  const v4MappedHex = host.match(/^::([0-9a-f]{1,4}):([0-9a-f]{1,4})$/i);
  if (v4MappedHex) {
  ...
  // Full-form ::1 variantsnumber of zero groups followed by trailing 1
...
// Convert IPv4-mapped IPv6 (::0:0/96 prefix) to IPv4 dotted form so both
// sides of a NO_PROXY comparison see the same canonical address. Without this,
// `NO_PROXY=192.168.1.5` would not match a request to `http://[::192.168.1.5]/`
// (Node's URL parser normalises that to `[::105]`), and vice-versa,

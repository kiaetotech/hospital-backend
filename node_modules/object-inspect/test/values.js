...
test('has', function (t) {
    t.plan(1);
    t.teardown(mockProperty(Object.prototype, 'hasOwnProperty', { 'delete'}));
...
test('Map', { skipMap !== 'function' }, function (t) {
    var map = new Map();
    map.set({ a: 1 }, ['b']);
    map.set(3, NaN);
...
test('WeakMap', { skipWeakMap !== 'function' }, function (t) {
    var map = new WeakMap();
    map.set({ a: 1 }, ['b']);
...
test('Set', { skipSet !== 'function' }, function (t) {
    var set = new Set();
    set.add({ a: 1 });
    set.add(['b']);
...
test('WeakSet', { skipWeakSet !== 'function' }, function (t) {
    var map = new WeakSet();
    map.add({ a: 1 });
...
test('WeakRef', { skipWeakRef !== 'function' }, function (t) {
...
test('FinalizationRegistry', { skipFinalizationRegistry !== 'function' }, function (t) {
...
test('Proxies', { skipProxy !== 'function' || !hasToStringTag }, function (t) {
    var target = { proxy};
    var fake = new Proxy(target, { has() { return false; } });
    ...
    forEach([
        ...
        t.equal(
            inspect(fake),
            '{ ' + (isNode60 ? '' : 'proxy, ') + '[Symbol(Symbol.toStringTag)]: \'' + tag + '\' }',
...
test('fakers', { skip: !hasToStringTag }, function (t) {
    ...
    forEach([
        ...
        t.equal(
            inspect(target),
            '{ proxy, [Symbol(Symbol.toStringTag)]: \'' + tag + '\' }',

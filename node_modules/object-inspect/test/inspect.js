...
test('inspect', function (t) {
    ...
    var obj = [{ inspectxyzInspect() { return '!XYZ¡'; } }, []];
    var stringResult = '[ !XYZ¡, [] ]';
    var falseResult = '[ { inspect: [Function] }, [] ]';

    t.equal(inspect(obj), stringResult);
    t.equal(inspect(obj, { customInspect}), stringResult);
    t.equal(inspect(obj, { customInspect: 'symbol' }), falseResult);
    t.equal(inspect(obj, { customInspect}), falseResult);
    t['throws'](
...
test('inspect custom symbol', { skip: !hasSymbols || !utilInspect || !utilInspect.custom }, function (t) {
    ...
    var obj = { inspectstringInspect() { return 'string'; } };
    obj[utilInspect.custom] = function custom() { return 'symbol'; };

    var symbolResult = '[ symbol, [] ]';
    var stringResult = '[ string, [] ]';
    var falseResult = '[ { inspect: [FunctionInspect]' + (utilInspect.custom ? ', [' + inspect(utilInspect.custom) + ']: [Function]' : '') + ' }, [] ]';

    var symbolStringFallback = utilInspect.custom ? symbolResult Result;
    var symbolFalseFallback = utilInspect.custom ? symbolResult ;

    t.equal(inspect([obj, []]), symbolStringFallback);
    t.equal(inspect([obj, []], { customInspect}), symbolStringFallback);
...
test('maxStringLength', function (t) {
    ...
    t.equal(
        inspect(['f'], { maxStringLength}),
        '[ \'\'... 1 more character ]',
        'maxStringLength option accepts `null`'
    );

    t.equal(
        inspect([str], { maxStringLength}),
        '[ \'' + str + '\' ]',
...
test('inspect URL', { skipURL === 'undefined' }, function (t) {
    t.match(

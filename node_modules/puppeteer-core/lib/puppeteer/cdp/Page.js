...
export class CdpPage extends Page {
    ...
    #emitMetrics(event) {
        this.emit("metrics" /* PageEvent.Metrics */, {
            title.title,
            metrics.#buildMetricsObject(event.metrics),
        });
    }
    ...
    async _screenshot(options) {
        ...
        try {
            const { fromSurface, omitBackground, optimizeForSpeed, quality, clip, type, captureBeyondViewport, } = options;
            ...
            if (clip && !captureBeyondViewport) {
                const viewport = await this.mainFrame()
                    .isolatedRealm()
                    .evaluate(() => {
                    const { height, pageLeft, pageTop, width, } = window.visualViewport;
            ...
            const { data } = await this.#primaryTargetClient.send('Page.captureScreenshot', {
                ...
                optimizeForSpeed,
                fromSurface,
                ...(quality !== undefined ? { quality.round(quality) } : {}),
                ...(clip ? { clip: { ...clip, scale.scale ?? 1 } } : {}),
    ...
    async createPDFStream(options = {}) {
        const { timeout= this._timeoutSettings.timeout() } = options;
        const { landscape, displayHeaderFooter, headerTemplate, footerTemplate, printBackground, scale, width, height, margin, pageRanges, preferCSSPageSize, omitBackground, tagged, outline, waitForFonts, } = parsePDFOptions(options);
        if (omitBackground) {
            await this.#emulationManager.setTransparentBackgroundColor();
        }
        if (waitForFonts) {
    ...
    async close(options = { runBeforeUnload}) {
        ...
        try {
            ...
            if (runBeforeUnload) {
                ...
                await connection.send('Target.closeTarget', {
                    targetId.#primaryTarget._targetId,
                });

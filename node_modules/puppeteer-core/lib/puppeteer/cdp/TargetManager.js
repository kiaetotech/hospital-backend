...
export class TargetManager extends EventEmitter {
    ...
    #onAttachedToTarget = async (parentSession, event) => {
        ...
        if (!target) {
            target = this.#targetFactory(targetInfo, session, parentSession instanceof CdpCDPSession ? parentSession );
        }
    ...
    #mapPatterns(rules) {
        ...
        for (const rule of rules ?? []) {
            result.push({ patternURLPattern(rule), rule });
        }
        return result;
    }
    #maybeSetupNetworkConditions = async (session) => {
        ...
        await session.send('Network.emulateNetworkConditionsByRule', {
            offline.#blocklist.length > 0 ? true ,

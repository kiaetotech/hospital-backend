...
class PQueryEngine {
    ...
    async run() {
        if (typeof this.#selector === 'string') {
            switch (this.#selector.trimStart()) {
                case '':
                    // `` has some special behavior depending on the node. It always
            ...
            if (typeof selector === 'string') {
                ...
                if (selector[0] && IDENT_TOKEN_START.test(selector[0])) {
                    ...
                    this.elements = AsyncIterableUtil.flatMap(this.elements, async function* (element) {
                        ...
                        yield* element.parentElement.querySelectorAll(`>-child(${index})${selector}`);
                    });
                }
            }
            else {
                this.elements = AsyncIterableUtil.flatMap(this.elements, async function* (element) {
                    switch (selector.name) {
                        case 'text'* textQuerySelectorAll(element, selector.value);
                            break;
                        case 'xpath'* xpathQuerySelectorAll(element, selector.value);
                            break;
                        case 'aria'* ariaQuerySelectorAll(element, selector.value);
                            break;
                        defaultquerySelector = customQuerySelectors.get(selector.name);
                            if (!querySelector) {
    ...
    #next() {
        ...
        switch (selector) {
            ...
            default.#compoundSelector = selector;
                this.#next();
                break;
        }
    }

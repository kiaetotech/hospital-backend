...
let BidiPage = (() => {
    ...
    return class BidiPage extends _classSuper {
        static {
            ...
            __esDecorate(this, null, _trustedEmitter_decorators, { kind: "accessor", name: "trustedEmitter", static, private, access: { has=> "trustedEmitter" in obj, get=> obj.trustedEmitter, set: (obj, value) => { obj.trustedEmitter = value; } }, metadata: _metadata }, _trustedEmitter_initializers, _trustedEmitter_extraInitializers);
        ...
        async setBypassCSP(enabled) {
            // TODOCDP-specific cases such as MPArch.
        ...
        async setViewport(viewport) {
            ...
            if (!this.browser().cdpSupported) {
                ...
                if ((this.#viewport?.hasTouch ?? false) !== (viewport?.hasTouch ?? false)) {
                    ...
                    const maxTouchPoints = viewport?.hasTouch ? 1 ;
        ...
        async pdf(options = {}) {
            const { timeout= this._timeoutSettings.timeout(), path = undefined } = options;
            const { printBackground, margin, landscape, width, height, pageRanges, scale, preferCSSPageSize, } = parsePDFOptions(options, 'cm');
        ...
        async exposeFunction(name, pptrFunction) {
            return await this.mainFrame().exposeFunction(name, 'default' in pptrFunction ? pptrFunction.default );
        }
        ...
        async setCookie(...cookies) {
            ...
            for (const cookie of cookies) {
                ...
                const bidiCookie = {
                    ...
                    ...(cookie.httpOnly !== undefined ? { httpOnly.httpOnly } : {}),
                    ...(cookie.secure !== undefined ? { secure.secure } : {}),
...
function testUrlMatchCookieHostname(cookie, normalizedUrl) {
    ...
    // TODOnot consider additional restrictions w.r.t to IP
...
/**
 * Check paths match.
 * Spec://datatracker.ietf.org/doc/html/rfc6265#section-5.1.4
 */
...
export function bidiToPuppeteerCookie(bidiCookie, returnCompositePartitionKey = false) {
    ...
    function getPartitionKey() {
        ...
        if (typeof partitionKey === 'object' && partitionKey !== null) {
            ...
            return {
                // TODObreaking change in Puppeteer is required to change
    ...
    return {
        ...
        httpOnly.httpOnly,
        secure.secure,
        sameSite(bidiCookie.sameSite),
        expires.expiry ?? -1,
        session.expiry === undefined || bidiCookie.expiry <= 0,
...
export function convertCookiesSameSiteCdpToBiDi(sameSite) {
    switch (sameSite) {
        case 'Strict'"strict" /* Bidi.Network.SameSite.Strict */;
        case 'Lax'"lax" /* Bidi.Network.SameSite.Lax */;
        case 'None'"none" /* Bidi.Network.SameSite.None */;
        default"default" /* Bidi.Network.SameSite.Default */;
    }
}
export function convertCookiesExpiryCdpToBiDi(expiry) {
    return [undefined, -1].includes(expiry) ? undefined ;
}

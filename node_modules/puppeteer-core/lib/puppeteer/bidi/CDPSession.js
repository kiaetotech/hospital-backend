...
export class BidiCdpSession extends CDPSession {
    ...
    constructor(frame, sessionId) {
        ...
        if (sessionId) {
            ...
            (async () => {
                try {
                    const { result } = await connection.send('goog.getSession', {
                        context._id,
                    });
    ...
    async send(method, params, options) {
        ...
        if (this.#detached) {
            throw new TargetCloseError(`Protocol error (${method})closed. Most likely the page has been closed.`);
        }
        const session = await this.#sessionId.valueOrThrow();
        const { result } = await this.#connection.send('goog.sendCommand', {
            method,
            params,
            session,
        }, options?.timeout);

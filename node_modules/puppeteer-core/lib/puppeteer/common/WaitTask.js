...
export class WaitTask {
    ...
    constructor(world, options, fn, ...args) {
        ...
        switch (typeof fn) {
            case 'string'.#fn = `() => {return (${fn});}`;
                break;
            default.#fn = stringifyFunction(fn);
                break;
        }
        this.#args = args;
    ...
    async rerun() {
        ...
        try {
            switch (this.#polling) {
                case 'raf'.#poller = await this.#world.evaluateHandle(({ RAFPoller, createFunction }, fn, ...args) => {
                    ...
                    break;
                case 'mutation'.#poller = await this.#world.evaluateHandle(({ MutationPoller, createFunction }, root, fn, ...args) => {
                        const fun = createFunction(fn);
                    ...
                    break;
                default.#poller = await this.#world.evaluateHandle(({ IntervalPoller, createFunction }, ms, fn, ...args) => {
                        const fun = createFunction(fn);

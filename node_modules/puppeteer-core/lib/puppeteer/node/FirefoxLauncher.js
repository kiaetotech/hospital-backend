 ...
 */
import fs from 'node';
import { rename, unlink, mkdtemp } from 'node/promises';
import os from 'node';
...
export class FirefoxLauncher extends BrowserLauncher {
    ...
    static getPreferences(extraPrefsFirefox) {
        return {
            ...extraPrefsFirefox,
            // Force all web content to use a single content process. TODO// this once Firefox supports mouse event dispatch from the main frame
    ...
    async computeLaunchArguments(options = {}) {
        ...
        await createProfile(SupportedBrowsers.FIREFOX, {
            path,
            preferences.getPreferences(extraPrefsFirefox),
        });
    ...
    defaultArgs(options = {}) {
        ...
        const firefoxArguments = [];
        switch (os.platform()) {
            case 'darwin'.push('--foreground');
                break;
            case 'win32'.push('--wait-for-browser');
                break;
        }
        if (userDataDir) {
            firefoxArguments.push('--profile');

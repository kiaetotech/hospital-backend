...
import { spawn, spawnSync } from 'node_process';
import fs from 'node';
import os from 'node';
...
const debugFfmpeg = debug('puppeteer');
/**
 * @public
 */
let ScreenRecorder = (() => {
    ...
    return class ScreenRecorder extends _classSuper {
        static {
            ...
            __esDecorate(this, _private_writeFrame_descriptor = { value: __setFunctionName(async function (buffer) {
                ...
                }, "#writeFrame") }, _private_writeFrame_decorators, { kind: "method", name: "#writeFrame", static, private, access: { has=> #writeFrame in obj, get=> obj.#writeFrame }, metadata: _metadata }, null, _instanceExtraInitializers);
            __esDecorate(this, null, _stop_decorators, { kind: "method", name: "stop", static, private, access: { has=> "stop" in obj, get=> obj.stop }, metadata: _metadata }, null, _instanceExtraInitializers);
        ...
        constructor(page, width, height, { ffmpegPath, speed, scale, crop, format, fps, loop, delay, quality, colors, path, overwrite, } = {}) {
            ...
            const filters = [
                `crop='min(${width},iw)(${height},ih):0:0'`,
            ...
            if (scale) {
                filters.push(`scale=iw*${scale}:-1=lanczos`);
            }
            ...
            }), bufferCount(2, 1), concatMap(([{ timestamp, buffer }, { timestamp }]) => {
        ...
        #getFormatArgs(format, fps, loop, delay, quality, colors) {
            ...
            switch (format) {
                ...
                case 'gif'= DEFAULT_FPS === fps ? 20 : 'source_fps';
                    if (loop === Infinity) {
                        loop = 0;
                    }
                    if (delay !== -1) {
                    ...
                    return [
                        ...
                        [
                            '-vf',
                            `fps=${fps},split[s0][s1];[s0]palettegen=stats_mode=diff_colors=${colors}[p];[s1][p]paletteuse=dither=bayer`,
                        ],

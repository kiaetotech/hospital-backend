...
export class BrowserLauncher {
    ...
    async launch(options = {}) {
        ...
        try {
            ...
            if (logs.includes('Failed to create a ProcessSingleton for your profile directory') ||
                ...
                // https://source.chromium.org/chromium/chromium/src/+/main/browser/process_singleton_win.cc;l=46;drc=fc7952f0422b5073515a205a04ec9c3a1ae81658
        ...
        if (Array.isArray(enableExtensions)) {
            if (this.#browser === 'chrome' && !usePipe) {
                throw new Error('To use `enableExtensions` with a list of paths in Chrome, you must be connected with `--remote-debugging-pipe` (`pipe`).');
            }
            await Promise.all([
    ...
    async createCdpPipeConnection(browserProcess, opts) {
        ...
        const { 3, 4} = browserProcess.nodeProcess.stdio;
    ...
    async createBiDiOverCdpBrowser(browserProcess, cdpConnection, closeCallback, opts) {
        ...
        const BiDi = await import(/* webpackIgnore*/ '../bidi/bidi.js');
    ...
    async createBiDiBrowser(browserProcess, closeCallback, opts) {
        ...
        return await BiDi.BidiBrowser.create({
            ...
            issuesEnabled.issuesEnabled ?? true,
        });
    }
    /**
     * @internal
    ...
    async resolveExecutablePath(headless, validatePath = true) {
        ...
        function puppeteerBrowserToInstalledBrowser(browser, headless) {
            switch (browser) {
                ...
                case 'firefox'InstalledBrowser.FIREFOX;
            }
            return InstalledBrowser.CHROME;
        }
        ...
        if (validatePath && !existsSync(executablePath)) {
            ...
            switch (this.browser) {
                case 'chrome'new Error(`Could not find Chrome (ver. ${browserVersion}). This can occur if either\n` +
                ...
                case 'firefox'new Error(`Could not find Firefox (rev. ${browserVersion}). This can occur if either\n` +

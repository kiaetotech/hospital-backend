...
let Page = (() => {
    ...
    return class Page extends _classSuper {
        ...
        waitForRequest(urlOrPredicate, options = {}) {
            const { timeout= this._timeoutSettings.timeout(), signal } = options;
         ...
         */
        waitForNetworkIdle$(options = {}) {
            const { timeout= this._timeoutSettings.timeout(), idleTime = NETWORK_IDLE_TIME, concurrency = 0, signal, } = options;
        ...
        async waitForFrame(urlOrPredicate, options = {}) {
            const { timeout= this.getDefaultTimeout(), signal } = options;
        ...
        async emulate(device) {
            await Promise.all([
                this.setUserAgent({ userAgent.userAgent }),
        ...
        async screencast(options = {}) {
            ...
            let crop;
            if (options.crop) {
                const { x, y, width, height, } = roundRectangle(normalizeRectangle(options.crop));
                if (x < 0 || y < 0) {

...
function readString(view, offset, size) {
	...
	const sliceEnd = end === -1 || end > offset + size ? offset + size ;
...
const stripPath = (p, n) => {
	...
	return n >= parts.length ? "" .slice(n).join("/");
};
...
function generatePax(header) {
	...
	return {
		paxHeader({
			name.decode(encoder.encode(`PaxHeader/${header.name}`).slice(0, 100)),
			size.length,
			type: "pax-header",
			mode: 420,
			mtime.mtime,
...
function createTarHeader(header) {
	...
	const size = isBodyless(header) ? 0 .size ?? 0;
...
function getMetaParser(type) {
	switch (type) {
		case "pax-global-header""pax-header"parsePax;
		case "gnu-long-name"(data) => ({ name(data, 0, data.length) });
		case "gnu-long-link-name"(data) => ({ linkname(data, 0, data.length) });
		default;
	}
}
...
function createTarPacker(onData, onError, onFinalize) {
	...
	return {
		add(header) {
			...
			const size = isBodyless(header) ? 0 .size;
...
function createChunkQueue() {
	...
	function pull(bytes, callback) {
		if (callback) {
			...
			while (remaining > 0) {
				...
				const segment = toFeed === chunk.length ? chunk .subarray(0, toFeed);
		...
		if (firstChunk.length >= bytes) {
			const view = firstChunk.length === bytes ? firstChunk .subarray(0, bytes);
		...
		while (remaining > 0) {
			...
			result.set(toCopy === chunk.length ? chunk .subarray(0, toCopy), copied);
			copied += toCopy;
			remaining -= toCopy;
	...
	return {
		...
		peek: (bytes) => {
			...
			if (bytes === 0) return EMPTY;
			const firstChunk = chunks[head];
			if (firstChunk.length >= bytes) return firstChunk.length === bytes ? firstChunk .subarray(0, bytes);
			const result = new Uint8Array(bytes);
			let copied = 0;
			let index = head;
			while (copied < bytes) {
		...
		readHeader() {
			...
			while (!done) {
				...
				if (metaParser) {
					...
					const target = internalHeader.type === "pax-global-header" ? paxGlobals ;

...
function createTarPacker() {
	...
	let packer;
	return {
		readableReadableStream({ start(controller) {
...
async function packTar(entries) {
	...
	await (async () => {
		for (const entry of entries) {
			...
			const body = "body" in entry ? entry.body .data;
			if (!body) {
...
async function unpackTar(archive, options = {}) {
	const sourceStream = archive instanceof ReadableStream ? archive ReadableStream({ start(controller) {
		controller.enqueue(archive instanceof Uint8Array ? archive Uint8Array(archive));
		controller.close();
	} });
	const results = [];

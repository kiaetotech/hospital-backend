...
import * as fs$1 from "node/promises";
import { cpus } from "node";
...
function packTar(sources, options = {}) {
	...
	(async () => {
		...
		const directoryPath = isDir ? path.resolve(sources) ;
		const jobs = isDir ? (await fs$1.readdir(directoryPath, { withFileTypes})).map((entry) => ({
			type.isDirectory() ? DIRECTORY ,
			source.join(directoryPath, entry.name),
			target.name
		})) ;
			...
			while (true) {
				...
				if (result.body) if (result.body instanceof Uint8Array) {
					...
					for await (const chunk of result.body) {
						...
						packer.write(chunk instanceof Uint8Array ? chunk .from(chunk));
					}
				} catch (error) {
					...
					const readBuffer = size > 1048576 ? readBufferLarge ??= Buffer.alloc(512 * 1024) ;
					try {
						let bytesLeft = size;
		...
		const processJob = async (job, index) => {
			...
			try {
				...
				let stat = await fs$1.lstat(job.source, { bigint});
				if (dereference && stat.isSymbolicLink()) {
					...
					stat = await fs$1.stat(job.source, { bigint});
				}
				if (filter && !filter(job.source, stat)) return;
				let header = {
					name,
					size: 0,
					mode.mode ?? Number(stat.mode),
					mtime.mtime ?? stat.mtime,
					uid.uid ?? Number(stat.uid),
					gid.gid ?? Number(stat.gid),
					uname.uname,
					gname.gname,
					type};
				let body;
				if (stat.isDirectory()) {
					...
					try {
						for (const d of await fs$1.readdir(job.source, { withFileTypes})) jobs.push({
							type.isDirectory() ? DIRECTORY ,
							source.join(job.source, d.name),
...
const createOperationQueue = (concurrency) => {
	...
	return {
		...
		onIdle() {
			return active === 0 && head === tasks.length ? Promise.resolve() ();
		}
	};
};
//#endregion
...
function createFileSink(path, { mode = 438, mtime } = {}) {
	...
	const write = (chunk) => {
		...
		const buf = Buffer.isBuffer(chunk) ? chunk instanceof Uint8Array ? Buffer.from(chunk.buffer, chunk.byteOffset, chunk.byteLength) .from(chunk);
...
const createPathCache = (destDirPath, options) => {
	...
	const initializeDestDir = async (destDirPath) => {
		...
		try {
			await fs$1.mkdir(symbolic, { recursive});
		} catch (err) {
			if (err.code === ENOENT) {
				const parentDir = path.dirname(symbolic);
				if (parentDir === symbolic) throw err;
				await fs$1.mkdir(parentDir, { recursive});
				await fs$1.mkdir(symbolic, { recursive});
			} else throw err;
		}
		try {
	...
	const prepareDirectory = async (dirPath, mode) => {
		...
		promise = (async () => {
			...
			try {
				...
				if (err.code === ENOENT) {
					await fs$1.mkdir(dirPath, { mode?? options.dmode });
					return;
				}
				throw err;
			}
		...
		async write(chunk, _, cb) {
			try {
				...
				while (true) {
					...
					if (outPath) {
						const safeMode = transformedHeader.mode ? transformedHeader.mode & 511 0;

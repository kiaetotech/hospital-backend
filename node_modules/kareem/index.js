      ...
      if (post.length === numArgs + 1) {
        ...
        if (isPromiseLike(maybePromiseLike)) {
          return maybePromiseLike.then(
            (res) => {
              _cb(res instanceof Kareem.overwriteResult ? res );
            },
            err => _cb(err)
          );
        }
...
function _handleWrapError(instance, error, name, context, args, options, callback) {
  if (options.useErrorHandlers) {
    return instance.execPost(name, context, args, { error}, function(error) {
...
Kareem.prototype.createWrapper = function(name, fn, context, options) {
  ...
  if (!this.hasHooks(name)) {
    // Fast paththere's no hooks for this function, just return the
...
Kareem.prototype.merge = function(other, clone) {
  clone = arguments.length === 1 ? true ;
  const ret = clone ? this.clone() ;

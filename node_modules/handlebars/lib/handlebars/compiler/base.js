...
function validateAstNode(node) {
  ...
  if (node.type === 'PathExpression') {
    if (!isValidDepth(node.depth)) {
      throw new Exception(
        'Invalid AST.depth must be an integer'
      );
    }
    if (!Array.isArray(node.parts)) {
      throw new Exception('Invalid AST.parts must be an array');
    }
    for (let i = 0; i < node.parts.length; i++) {
      if (typeof node.parts[i] !== 'string') {
        throw new Exception(
          'Invalid AST.parts must only contain strings'
        );
      }
    }
  } else if (node.type === 'NumberLiteral') {
    if (typeof node.value !== 'number' || !isFinite(node.value)) {
      throw new Exception('Invalid ASTLiteral.value must be a number');
    }
  } else if (node.type === 'BooleanLiteral') {
    if (typeof node.value !== 'boolean') {
      throw new Exception(
        'Invalid ASTLiteral.value must be a boolean'
      );
    }
  }

...
class ClusterAdapter extends in_memory_adapter_1.Adapter {
    ...
    onMessage(message, offset) {
        ...
        switch (message.type) {
            ...
            case MessageType.SOCKETS_JOIN.addSockets(decodeOptions(message.data.opts), message.data.rooms);
                break;
            case MessageType.SOCKETS_LEAVE.delSockets(decodeOptions(message.data.opts), message.data.rooms);
                break;
            case MessageType.DISCONNECT_SOCKETS.disconnectSockets(decodeOptions(message.data.opts), message.data.close);
                break;
            ...
            case MessageType.SERVER_SIDE_EMIT_RESPONSE:
                ...
                break;
            default("[%s] unknown message type: %s", this.uid, message.type);
        }
    }
    /**
    ...
    broadcastWithAck(packet, opts, clientCountCallback, ack) {
        var _a;
        const onlyLocal = (_a = opts === null || opts === void 0 ? void 0 .flags) === null || _a === void 0 ? void 0 : _a.local;
        if (!onlyLocal) {
    ...
    async fetchSockets(opts) {
        ...
        return new Promise((resolve, reject) => {
            const timeout = setTimeout(() => {
                ...
                if (storedRequest) {
                    reject(new Error(`timeout reached${storedRequest.current} responses received out of ${storedRequest.expected}`));
                    this.requests.delete(requestId);
                }
    ...
    async serverSideEmit(packet) {
        ...
        const timeout = setTimeout(() => {
            const storedRequest = this.requests.get(requestId);
            if (storedRequest) {
                ack(new Error(`timeout reached${storedRequest.current} responses received out of ${storedRequest.expected}`), storedRequest.responses);
                this.requests.delete(requestId);
            }
        }, DEFAULT_TIMEOUT);
        const storedRequest = {
...
class ClusterAdapterWithHeartbeat extends ClusterAdapter {
    ...
    onMessage(message, offset) {
        ...
        switch (message.type) {
            case MessageType.INITIAL_HEARTBEAT.publish({
                    type.HEARTBEAT,
                });
                break;
            case MessageType.HEARTBEAT:
                // nothing to do
                break;
            case MessageType.ADAPTER_CLOSE.removeNode(message.uid);
                break;
            default.onMessage(message, offset);
        }
    }
    serverCount() {
    ...
    async serverSideEmit(packet) {
        ...
        const timeout = setTimeout(() => {
            ...
            if (storedRequest) {
                ack(new Error(`timeout reached${storedRequest.missingUids.size} responses`), storedRequest.responses);
        ...
        const storedRequest = {
            ...
            resolve,
            timeout,
            missingUidsSet([...this.nodesMap.keys()]),
            responses: [],
        };
    ...
    async fetchSockets(opts) {
        ...
        return new Promise((resolve, reject) => {
            const timeout = setTimeout(() => {
                const storedRequest = this.customRequests.get(requestId);
                if (storedRequest) {
                    reject(new Error(`timeout reached${storedRequest.missingUids.size} responses`));
                    this.customRequests.delete(requestId);
                }
            }, opts.flags.timeout || DEFAULT_TIMEOUT);
            const storedRequest = {

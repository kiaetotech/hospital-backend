...
const debugInfo = debug('bidi');
const debugOthers = debug('bidi');
// Memorizes a debug creation
const loggers = new Map();
const getLogger = (type) => {
    const prefix = `bidi:${type}`;
...
export class MapperServerCdpConnection {
    ...
    static async #initMapper(cdpConnection, mapperTabSource, verbose) {
        ...
        const { targetId} = await browserClient.sendCommand('Target.createTarget', {
            url: 'about#MAPPER_TARGET',
            hidden,
            background,
        });
        const { sessionId} = await browserClient.sendCommand('Target.attachToTarget', { targetId, flatten});
        ...
        // TODOerrors in all these evaluate calls!
        await mapperCdpClient.sendCommand('Runtime.evaluate', {

...
export const debugInfo = debug('bidi');
const debugInternal = debug('bidi');
const debugSend = debug('bidi▸');
const debugRecv = debug('bidi◂');
export class WebSocketServer {
    ...
    async #onRequest(request, response) {
        ...
        if (request.url === '/session') {
            ...
            const session = {
                sessionId,
                // TODObrowser instance and set it to the session after WPT
                //  tests clean up is switched to pure BiDi.
                browserInstancePromise,
                sessionOptions: {
                    chromeOptions.#getChromeOptions(jsonBody.capabilities),
                    verbose.#verbose,
    ...
    #onWsRequest(request) {
        ...
        if (session !== undefined) {
            ...
            // TODOto an existing BrowserInstance instead.
            ...
            if (parsedCommandData.method === 'session.new') {
                ...
                try {
                    const sessionOptions = {
                        chromeOptions.#getChromeOptions(parsedCommandData.params?.capabilities),
                        verbose.#verbose,
                        sessionNewBody,
                    };
                    ...
                    session = {
                        sessionId,
                        browserInstancePromise.resolve(browserInstance),
                        sessionOptions,
                    };
                    this.#sessions.set(sessionId, session);
                }
        ...
        connection.on('close', async () => {
            ...
            // TODO't close Browser instance to allow re-connecting to the session.
            await this.#closeBrowserInstanceIfLaunched(session);
        });
    }
    ...
    #getChromeOptions(capabilities) {
        const chromeCapabilities = capabilities?.alwaysMatch?.['goog'];
        return {
            chromeArgs?.args ?? [],
            chromeBinary?.binary ?? undefined,
        };
    }
    ...
    #getErrorResponse(plainCommandData, errorCode, errorMessage) {
        // XXXis bizarre per spec. We reparse the payload and

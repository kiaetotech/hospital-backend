(function () {
    ...
    function mitt(n){return {all=n||new Map,on(t,e){var i=n.get(t);i?i.push(e).set(t,[e]);},off(t,e){var i=n.get(t);i&&(e?i.splice(i.indexOf(e)>>>0,1).set(t,[]));},emit(t,e){var i=n.get(t);i&&i.slice().map(function(n){n(e);}),(i=n.get("*"))&&i.slice().map(function(n){n(t,e);});}}}
    ...
    (function (LogType) {
        ...
        LogType["cdp"] = "cdp";
        LogType["debug"] = "debug";
        LogType["debugError"] = "debug";
        LogType["debugInfo"] = "debug";
        LogType["debugWarn"] = "debug";
    })(LogType || (LogType = {}));
    ...
    class ProcessingQueue {
        static LOGGER_PREFIX = `${LogType.debug}`;
        #logger;
        #processor;
        #queue = [];
        #isProcessing = false;
    ...
    class BrowserProcessor {
        ...
        async createUserContext(params) {
            ...
            if (w3cParams.proxy) {
                ...
                if (params['goog'] !== undefined) {
                    request.proxyServer = params['goog'];
                }
                const proxyBypassList = params['goog'] ?? undefined;
                if (proxyBypassList) {
            ...
            this.#configStorage.updateUserContextConfig(context.browserContextId, {
                ...
                userPromptHandler['unhandledPromptBehavior'],
            });
            return {
        ...
        async getUserContexts() {
            return {
                userContextsthis.#userContextStorage.getUserContexts(),
            };
        }
        ...
        #toCdpDownloadBehavior(downloadBehavior) {
            ...
            if (downloadBehavior?.type === 'allowed') {
                return {
                    behavior: 'allow',
                    downloadPath.destinationFolder,
                };
            }
        ...
        async #applyDownloadBehavior(downloadBehavior, userContext) {
            await this.#browserCdpClient.sendCommand('Browser.setDownloadBehavior', {
                ...
                browserContextId=== 'default' ? undefined ,
                eventsEnabled,
            });
        }
    ...
    class CdpProcessor {
        ...
        resolveRealm(params) {
            ...
            const realm = this.#realmStorage.getRealm({ realmId});
            if (realm === undefined) {
                throw new UnknownErrorException(`Could not find realm ${params.realm}`);
            }
            return { executionContextId.executionContextId };
        }
    ...
    class BrowsingContextProcessor {
        ...
        getTree(params) {
            ...
            return {
                contexts.map((c) => c.serializeToBidiValue(params.maxDepth ?? Number.MAX_VALUE)),
            };
        }
        async create(params) {
        ...
        #onContextCreatedSubscribeHook(contextId) {
            ...
            contextsToReport.forEach((context) => {
                this.#eventManager.registerEvent({
                    type: 'event',
                    method$2.EventNames.ContextCreated,
                    params.serializeToBidiValue(),
                }, context.id);
            });
    ...
    class EmulationProcessor {
        ...
        async setNetworkConditions(params) {
            ...
            for (const browsingContextId of params.contexts ?? []) {
                this.#contextConfigStorage.updateBrowsingContextConfig(browsingContextId, {
                    emulatedNetworkConditions.networkConditions,
                });
            }
            for (const userContextId of params.userContexts ?? []) {
                this.#contextConfigStorage.updateUserContextConfig(userContextId, {
    ...
    async function getElementCenter(context, element) {
        ...
        const { result: { value: [{ value}, { value}], }, } = result;
        return { x, y};
    }
    class ActionDispatcher {
        ...
        async #dispatchPointerDownAction(source, keyState, action) {
            ...
            const { x, y, subtype} = source;
            const { width, height, pressure, twist, tangentialPressure } = action;
            const { tiltX, tiltY } = getTilt(action);
            const { modifiers } = keyState;
            const { radiusX, radiusY } = getRadii(width ?? 1, height ?? 1);
            switch (pointerType) {
                case "mouse" "pen" this.#context.cdpTarget.cdpClient.sendCommand('Input.dispatchMouseEvent', {
                        type: 'mousePressed',
                        x,
                        y,
                        modifiers,
                        button(button),
                        buttons.buttons,
                        clickCount.setClickCount(button, new PointerSource.ClickContext(x, y, performance.now())),
                        pointerType,
                        tangentialPressure,
                        tiltX,
                        tiltY,
                    ...
                    break;
                case "touch" this.#context.cdpTarget.cdpClient.sendCommand('Input.dispatchTouchEvent', {
                        type: 'touchStart',
                        touchPoints: [
                            {
                                x,
        ...
        #dispatchPointerUpAction(source, keyState, action) {
            ...
            const { x, y, force, radiusX, radiusY, subtype} = source;
            const { modifiers } = keyState;
            switch (pointerType) {
                case "mouse" "pen" this.#context.cdpTarget.cdpClient.sendCommand('Input.dispatchMouseEvent', {
                        type: 'mouseReleased',
                        ...
                        button(button),
                        buttons.buttons,
                        clickCount.getClickCount(button),
                        pointerType,
                    });
                case "touch" this.#context.cdpTarget.cdpClient.sendCommand('Input.dispatchTouchEvent', {
                        type: 'touchEnd',
        ...
        async #dispatchPointerMoveAction(source, keyState, action) {
            const { x, y, subtype} = source;
            const { width, height, pressure, twist, tangentialPressure, x, y, origin = 'viewport', duration = this.#tickDuration, } = action;
            const { tiltX, tiltY } = getTilt(action);
            const { radiusX, radiusY } = getRadii(width ?? 1, height ?? 1);
            ...
            do {
                ...
                if (source.x !== x || source.y !== y) {
                    const { modifiers } = keyState;
                    switch (pointerType) {
                        case "mouse" this.#context.cdpTarget.cdpClient.sendCommand('Input.dispatchMouseEvent', {
                                type: 'mouseMoved',
                                x,
                                y,
                                modifiers,
                                clickCount: 0,
                                button(source.pressed.values().next().value ?? 5),
                                buttons.buttons,
                                pointerType,
                                tangentialPressure,
                                tiltX,
                            ...
                            break;
                        case "pen" (source.pressed.size !== 0) {
                            ...
                            break;
                        case "touch" (source.pressed.size !== 0) {
        ...
        async #getFrameOffset() {
            ...
            const { backendNodeId } = await this.#context.cdpTarget.cdpClient.sendCommand('DOM.getFrameOwner', { frameId.#context.id });
            const { model} = await this.#context.cdpTarget.cdpClient.sendCommand('DOM.getBoxModel', {
                backendNodeId,
            });
        ...
        async #getCoordinateFromOrigin(origin, offsetX, offsetY, startX, startY) {
            ...
            switch (origin) {
                case 'viewport'= offsetX + frameOffset.x;
                    targetY = offsetY + frameOffset.y;
                    break;
                case 'pointer'= startX + offsetX + frameOffset.x;
                    targetY = startY + offsetY + frameOffset.y;
                    break;
                default: {
                    const { x, y} = await getElementCenter(this.#context, origin.element);
        ...
        async #dispatchScrollAction(_source, keyState, action) {
            const { deltaX, deltaY, x, y, origin = 'viewport', duration = this.#tickDuration, } = action;
            if (origin === 'pointer') {
        ...
        async #dispatchKeyDownAction(source, action) {
            ...
            if (this.#isMacOS && source.meta) {
                switch (code) {
                    ...
                    case 'KeyC'= 'Copy';
                        break;
                    case 'KeyV'= source.shift ? 'PasteAndMatchStyle' : 'Paste';
                        break;
                    case 'KeyX'= 'Cut';
                        break;
                    case 'KeyZ'= source.shift ? 'Redo' : 'Undo';
                        break;
                }
            }
            const promises = [
                this.#context.cdpTarget.cdpClient.sendCommand('Input.dispatchKeyEvent', {
                    type? 'keyDown' : 'rawKeyDown',
    ...
    const getKeyEventText = (code, source) => {
        if (source.ctrl) {
            switch (code) {
                ...
                case 'KeyY''\x19';
                case 'KeyZ''\x1A';
                case 'BracketLeft''\x1B';
                case 'Backslash''\x1C';
                case 'BracketRight''\x1D';
                case 'Digit6'(source.shift) {
                        return '\x1E';
                    }
                    break;
                case 'Minus''\x1F';
    ...
    function getTilt(action) {
        ...
        return {
            tiltX.round(tiltXRadians * factor),
            tiltY.round(tiltYRadians * factor),
        };
    }
    function getRadii(width, height) {
        return {
            radiusX? width / 2 : 0.5,
            radiusY? height / 2 : 0.5,
        };
    }
    ...
    class InputState {
        ...
        getOrCreate(id, type, subtype) {
            ...
            if (!source) {
                switch (type) {
                    case "none" = new NoneSource();
                        break;
                    case "key" = new KeySource();
                        break;
                    case "pointer" : {
                    ...
                    case "wheel" = new WheelSource();
                        break;
                    defaultnew InvalidArgumentException(`Expected "${"none" }", "${"key" }", "${"pointer" }", or "${"wheel" }". Found unknown source type ${type}.`);
                }
        ...
        async setFiles(params) {
            ...
            try {
                result = await hiddenSandboxRealm.callFunction(String(function getFiles(fileListLength) {
                    ...
                    return;
                }), false, params.element, [{ type: 'number', value.files.length }]);
            }
            catch {
            ...
            if (params.files.length === 0) {
                await hiddenSandboxRealm.callFunction(String(function dispatchEvent() {
                    ...
                    this.dispatchEvent(new Event('input', { bubbles, composed}));
        ...
        #getActionsByTick(params, inputState) {
            ...
            for (const action of params.actions) {
                switch (action.type) {
                    ...
                    default.getOrCreate(action.id, action.type);
                }
    ...
    function cdpAuthChallengeResponseFromBidiAuthContinueWithAuthAction(action) {
        switch (action) {
            ...
            case 'cancel''CancelAuth';
            case 'provideCredentials''ProvideCredentials';
        }
    }
    function cdpToBiDiCookie(cookie) {
        const result = {
            ...
            sameSite.sameSite === undefined
                ? "none"
                (cookie.sameSite),
            ...(cookie.expires >= 0 ? { expiry.round(cookie.expires) } ),
        };
        result[`goog`] = cookie.session;
        result[`goog`] = cookie.priority;
        result[`goog`] = cookie.sourceScheme;
        result[`goog`] = cookie.sourcePort;
        if (cookie.partitionKey !== undefined) {
            result[`goog`] = cookie.partitionKey;
        }
        if (cookie.partitionKeyOpaque !== undefined) {
            result[`goog`] = cookie.partitionKeyOpaque;
        }
        return result;
    }
    ...
    function bidiToCdpCookie(params, partitionKey) {
        ...
        const result = {
            ...
            ...(params.cookie.sameSite !== undefined && {
                sameSite(params.cookie.sameSite),
            }),
        };
        if (params.cookie[`goog`] !== undefined) {
            result.url = params.cookie[`goog`];
        }
        if (params.cookie[`goog`] !== undefined) {
            result.priority = params.cookie[`goog`];
        }
        if (params.cookie[`goog`] !== undefined) {
            result.sourceScheme = params.cookie[`goog`];
        }
        if (params.cookie[`goog`] !== undefined) {
            result.sourcePort = params.cookie[`goog`];
        }
        return result;
    }
        ...
        async setExtraHeaders(params) {
            ...
            if (params.contexts !== undefined) {
                params.contexts.forEach((browsingContextId) => {
                    this.#contextConfigStorage.updateBrowsingContextConfig(browsingContextId, { extraHeaders});
                });
            }
    ...
    class ChannelProxy {
        ...
        static async #createAndGetHandleInRealm(realm) {
            const createChannelHandleResult = await realm.cdpClient.sendCommand('Runtime.evaluate', {
                expression.#createChannelProxyEvalStr(),
        ...
        static async #createSendMessageHandle(realm, channelHandle) {
            const sendMessageArgResult = await realm.cdpClient.sendCommand('Runtime.callFunctionOn', {
                functionDeclaration((channelHandle) => {
            ...
            for (;;) {
                try {
                    const message = await realm.cdpClient.sendCommand('Runtime.callFunctionOn', {
                        functionDeclaration(async (channelHandle) => await channelHandle.getMessage()),
                        arguments: [
                            {
                                objectId,
                            },
                        ...
                        serializationOptions: {
                            serialization: "deep" ,
                            maxDepth.#properties.serializationOptions?.maxObjectDepth ??
                                undefined,
                        },
                    });
                    ...
                    for (const browsingContext of realm.associatedBrowsingContexts) {
                        eventManager.registerEvent({
                            type: 'event',
                            method$2.EventNames.Message,
                            params: {
                                channel.#properties.channel,
                                data.cdpToBidiValue(message, this.#properties.ownership ?? "none" ),
                                source.source,
                            },
                        }, browsingContext.id);
                    }
    ...
    class ScriptProcessor {
        ...
        #onRealmCreatedSubscribeHook(contextId) {
            ...
            for (const realm of realms) {
                this.#eventManager.registerEvent({
                    type: 'event',
                    method$2.EventNames.RealmCreated,
                    params.realmInfo,
                }, context.id);
            }
            return Promise.resolve();
    ...
    class SessionProcessor {
        ...
        status() {
            return { ready, message: 'already connected' };
        }
        ...
        #getUnhandledPromptBehavior(capabilityValue) {
            ...
            switch (capabilityValue) {
                ...
                defaultnew InvalidArgumentException(`Unexpected 'unhandledPromptBehavior' value: ${capabilityValue}`);
            }
        }
        async new(params) {
            if (this.#created) {
            ...
            return {
                ...
                capabilities: {
                    ...matchedCapabitlites,
                    acceptInsecureCerts.acceptInsecureCerts ?? false,
                    browserName.product,
        ...
        async deleteCookies(params) {
            ...
            try {
                cdpResponse = await this.#browserCdpClient.sendCommand('Storage.getCookies', {
                    browserContextId.#getCdpBrowserContextId(partitionKey),
                });
            }
            catch (err) {
    ...
    class WebExtensionProcessor {
        ...
        async install(params) {
            switch (params.extensionData.type) {
                case 'archivePath''base64'new UnsupportedOperationException('Archived and Base64 extensions are not supported');
            }
            try {
        ...
        async uninstall(params) {
            try {
                ...
                if (err.message ===
                    'Uninstall failed. Reasonnot find extension.') {
    ...
    class OutgoingMessage {
        ...
        static createFromPromise(messagePromise, googChannel) {
            return messagePromise.then((message) => {
                if (message.kind === 'success') {
                    return {
                        kind: 'success',
                        valueOutgoingMessage(message.value, googChannel),
                    };
                }
                return message;
            });
        ...
        static createResolved(message, googChannel = null) {
            return Promise.resolve({
                kind: 'success',
                valueOutgoingMessage(message, googChannel),
            });
        }
        get message() {
            return this.#message;
    ...
    class CommandProcessor extends EventEmitter {
        ...
        async #processCommand(command) {
            switch (command.method) {
                case 'bluetooth.disableSimulation'await this.#bluetoothProcessor.disableSimulation(this.#parser.parseDisableSimulationParameters(command.params));
                case 'bluetooth.handleRequestDevicePrompt'await this.#bluetoothProcessor.handleRequestDevicePrompt(this.#parser.parseHandleRequestDevicePromptParams(command.params));
                case 'bluetooth.simulateAdapter'await this.#bluetoothProcessor.simulateAdapter(this.#parser.parseSimulateAdapterParameters(command.params));
                case 'bluetooth.simulateAdvertisement'await this.#bluetoothProcessor.simulateAdvertisement(this.#parser.parseSimulateAdvertisementParameters(command.params));
                case 'bluetooth.simulateCharacteristic'await this.#bluetoothProcessor.simulateCharacteristic(this.#parser.parseSimulateCharacteristicParameters(command.params));
                case 'bluetooth.simulateCharacteristicResponse'await this.#bluetoothProcessor.simulateCharacteristicResponse(this.#parser.parseSimulateCharacteristicResponseParameters(command.params));
                case 'bluetooth.simulateDescriptor'await this.#bluetoothProcessor.simulateDescriptor(this.#parser.parseSimulateDescriptorParameters(command.params));
                case 'bluetooth.simulateDescriptorResponse'await this.#bluetoothProcessor.simulateDescriptorResponse(this.#parser.parseSimulateDescriptorResponseParameters(command.params));
                case 'bluetooth.simulateGattConnectionResponse'await this.#bluetoothProcessor.simulateGattConnectionResponse(this.#parser.parseSimulateGattConnectionResponseParameters(command.params));
                case 'bluetooth.simulateGattDisconnection'await this.#bluetoothProcessor.simulateGattDisconnection(this.#parser.parseSimulateGattDisconnectionParameters(command.params));
                case 'bluetooth.simulatePreconnectedPeripheral'await this.#bluetoothProcessor.simulatePreconnectedPeripheral(this.#parser.parseSimulatePreconnectedPeripheralParameters(command.params));
                case 'bluetooth.simulateService'await this.#bluetoothProcessor.simulateService(this.#parser.parseSimulateServiceParameters(command.params));
                case 'browser.close'this.#browserProcessor.close();
                case 'browser.createUserContext'await this.#browserProcessor.createUserContext(this.#parser.parseCreateUserContextParameters(command.params));
                case 'browser.getClientWindows'await this.#browserProcessor.getClientWindows();
                case 'browser.getUserContexts'await this.#browserProcessor.getUserContexts();
                case 'browser.removeUserContext'await this.#browserProcessor.removeUserContext(this.#parser.parseRemoveUserContextParameters(command.params));
                case 'browser.setClientWindowState'await this.#browserProcessor.setClientWindowState(this.#parser.parseSetClientWindowStateParameters(command.params));
                case 'browser.setDownloadBehavior'await this.#browserProcessor.setDownloadBehavior(this.#parser.parseSetDownloadBehaviorParameters(command.params));
                case 'browsingContext.activate'await this.#browsingContextProcessor.activate(this.#parser.parseActivateParams(command.params));
                case 'browsingContext.captureScreenshot'await this.#browsingContextProcessor.captureScreenshot(this.#parser.parseCaptureScreenshotParams(command.params));
                case 'browsingContext.close'await this.#browsingContextProcessor.close(this.#parser.parseCloseParams(command.params));
                case 'browsingContext.create'await this.#browsingContextProcessor.create(this.#parser.parseCreateParams(command.params));
                case 'browsingContext.getTree'this.#browsingContextProcessor.getTree(this.#parser.parseGetTreeParams(command.params));
                case 'browsingContext.handleUserPrompt'await this.#browsingContextProcessor.handleUserPrompt(this.#parser.parseHandleUserPromptParams(command.params));
                case 'browsingContext.locateNodes'await this.#browsingContextProcessor.locateNodes(this.#parser.parseLocateNodesParams(command.params));
                case 'browsingContext.navigate'await this.#browsingContextProcessor.navigate(this.#parser.parseNavigateParams(command.params));
                case 'browsingContext.print'await this.#browsingContextProcessor.print(this.#parser.parsePrintParams(command.params));
                case 'browsingContext.reload'await this.#browsingContextProcessor.reload(this.#parser.parseReloadParams(command.params));
                case 'browsingContext.setBypassCSP'.#parser.parseSetBypassCspParams(command.params);
                    throw new UnsupportedOperationException(`Method ${command.method} is not implemented.`);
                case 'browsingContext.setViewport'await this.#browsingContextProcessor.setViewport(this.#parser.parseSetViewportParams(command.params));
                case 'browsingContext.traverseHistory'await this.#browsingContextProcessor.traverseHistory(this.#parser.parseTraverseHistoryParams(command.params));
                case 'goog.getSession'this.#cdpProcessor.getSession(this.#parser.parseGetSessionParams(command.params));
                case 'goog.resolveRealm'this.#cdpProcessor.resolveRealm(this.#parser.parseResolveRealmParams(command.params));
                case 'goog.sendCommand'await this.#cdpProcessor.sendCommand(this.#parser.parseSendCommandParams(command.params));
                case 'emulation.setForcedColorsModeThemeOverride'.#parser.parseSetForcedColorsModeThemeOverrideParams(command.params);
                    throw new UnsupportedOperationException(`Method ${command.method} is not implemented.`);
                case 'emulation.setGeolocationOverride'await this.#emulationProcessor.setGeolocationOverride(this.#parser.parseSetGeolocationOverrideParams(command.params));
                case 'emulation.setLocaleOverride'await this.#emulationProcessor.setLocaleOverride(this.#parser.parseSetLocaleOverrideParams(command.params));
                case 'emulation.setNetworkConditions'await this.#emulationProcessor.setNetworkConditions(this.#parser.parseSetNetworkConditionsParams(command.params));
                case 'emulation.setScreenOrientationOverride'await this.#emulationProcessor.setScreenOrientationOverride(this.#parser.parseSetScreenOrientationOverrideParams(command.params));
                case 'emulation.setScreenSettingsOverride'await this.#emulationProcessor.setScreenSettingsOverride(this.#parser.parseSetScreenSettingsOverrideParams(command.params));
                case 'emulation.setScriptingEnabled'await this.#emulationProcessor.setScriptingEnabled(this.#parser.parseSetScriptingEnabledParams(command.params));
                case 'emulation.setScrollbarTypeOverride'await this.#emulationProcessor.setScrollbarTypeOverride(this.#parser.parseSetScrollbarTypeOverrideParams(command.params));
                case 'emulation.setTimezoneOverride'await this.#emulationProcessor.setTimezoneOverride(this.#parser.parseSetTimezoneOverrideParams(command.params));
                case 'emulation.setTouchOverride'await this.#emulationProcessor.setTouchOverride(this.#parser.parseSetTouchOverrideParams(command.params));
                case 'emulation.setUserAgentOverride'await this.#emulationProcessor.setUserAgentOverrideParams(this.#parser.parseSetUserAgentOverrideParams(command.params));
                case 'userAgentClientHints.setClientHintsOverride'await this.#emulationProcessor.setClientHintsOverride(this.#parser.parseSetClientHintsOverrideParams(command.params));
                case 'input.performActions'await this.#inputProcessor.performActions(this.#parser.parsePerformActionsParams(command.params));
                case 'input.releaseActions'await this.#inputProcessor.releaseActions(this.#parser.parseReleaseActionsParams(command.params));
                case 'input.setFiles'await this.#inputProcessor.setFiles(this.#parser.parseSetFilesParams(command.params));
                case 'network.addDataCollector'await this.#networkProcessor.addDataCollector(this.#parser.parseAddDataCollectorParams(command.params));
                case 'network.addIntercept'await this.#networkProcessor.addIntercept(this.#parser.parseAddInterceptParams(command.params));
                case 'network.continueRequest'await this.#networkProcessor.continueRequest(this.#parser.parseContinueRequestParams(command.params));
                case 'network.continueResponse'await this.#networkProcessor.continueResponse(this.#parser.parseContinueResponseParams(command.params));
                case 'network.continueWithAuth'await this.#networkProcessor.continueWithAuth(this.#parser.parseContinueWithAuthParams(command.params));
                case 'network.disownData'this.#networkProcessor.disownData(this.#parser.parseDisownDataParams(command.params));
                case 'network.failRequest'await this.#networkProcessor.failRequest(this.#parser.parseFailRequestParams(command.params));
                case 'network.getData'await this.#networkProcessor.getData(this.#parser.parseGetDataParams(command.params));
                case 'network.provideResponse'await this.#networkProcessor.provideResponse(this.#parser.parseProvideResponseParams(command.params));
                case 'network.removeDataCollector'await this.#networkProcessor.removeDataCollector(this.#parser.parseRemoveDataCollectorParams(command.params));
                case 'network.removeIntercept'await this.#networkProcessor.removeIntercept(this.#parser.parseRemoveInterceptParams(command.params));
                case 'network.setCacheBehavior'await this.#networkProcessor.setCacheBehavior(this.#parser.parseSetCacheBehaviorParams(command.params));
                case 'network.setExtraHeaders'await this.#networkProcessor.setExtraHeaders(this.#parser.parseSetExtraHeadersParams(command.params));
                case 'permissions.setPermission'await this.#permissionsProcessor.setPermissions(this.#parser.parseSetPermissionsParams(command.params));
                case 'script.addPreloadScript'await this.#scriptProcessor.addPreloadScript(this.#parser.parseAddPreloadScriptParams(command.params));
                case 'script.callFunction'await this.#scriptProcessor.callFunction(this.#parser.parseCallFunctionParams(this.#processTargetParams(command.params)));
                case 'script.disown'await this.#scriptProcessor.disown(this.#parser.parseDisownParams(this.#processTargetParams(command.params)));
                case 'script.evaluate'await this.#scriptProcessor.evaluate(this.#parser.parseEvaluateParams(this.#processTargetParams(command.params)));
                case 'script.getRealms'this.#scriptProcessor.getRealms(this.#parser.parseGetRealmsParams(command.params));
                case 'script.removePreloadScript'await this.#scriptProcessor.removePreloadScript(this.#parser.parseRemovePreloadScriptParams(command.params));
                case 'session.end'new UnsupportedOperationException(`Method ${command.method} is not implemented.`);
                case 'session.new'await this.#sessionProcessor.new(command.params);
                case 'session.status'this.#sessionProcessor.status();
                case 'session.subscribe'await this.#sessionProcessor.subscribe(this.#parser.parseSubscribeParams(command.params), command['goog']);
                case 'session.unsubscribe'await this.#sessionProcessor.unsubscribe(this.#parser.parseUnsubscribeParams(command.params), command['goog']);
                case 'storage.deleteCookies'await this.#storageProcessor.deleteCookies(this.#parser.parseDeleteCookiesParams(command.params));
                case 'storage.getCookies'await this.#storageProcessor.getCookies(this.#parser.parseGetCookiesParams(command.params));
                case 'storage.setCookie'await this.#storageProcessor.setCookie(this.#parser.parseSetCookieParams(command.params));
                case 'webExtension.install'await this.#webExtensionProcessor.install(this.#parser.parseInstallParams(command.params));
                case 'webExtension.uninstall'await this.#webExtensionProcessor.uninstall(this.#parser.parseUninstallParams(command.params));
            }
        ...
        async processCommand(command) {
            try {
                ...
                this.emit("response" , {
                    message.createResolved(response, command['goog']),
                    event.method,
                });
            }
            catch (e) {
                if (e instanceof Exception) {
                    this.emit("response" , {
                        message.createResolved(e.toErrorResponse(command.id), command['goog']),
                        event.method,
                    });
                }
                else {
                    ...
                    const errorException = this.#browserCdpClient.isCloseError(e)
                        ...
                        UnknownErrorException(error.message, error.stack);
                    this.emit("response" , {
                        message.createResolved(errorException.toErrorResponse(command.id), command['goog']),
                        event.method,
                    });
                }
            }
    ...
    class BluetoothProcessor {
        ...
        async simulateCharacteristic(params) {
            ...
            switch (params.type) {
                ...
                defaultnew InvalidArgumentException(`Parameter "type" of ${params.type} is not supported`);
            }
        }
        async simulateCharacteristicResponse(params) {
            ...
            await context.cdpTarget.browserCdpClient.sendCommand('BluetoothEmulation.simulateCharacteristicOperationResponse', {
                ...
                code.code,
                ...(params.data && {
                    data(String.fromCharCode(...params.data)),
                }),
            });
            return {};
        }
        ...
        onCdpTargetCreated(cdpTarget) {
            ...
            cdpTarget.browserCdpClient.on('BluetoothEmulation.gattOperationReceived', async (event) => {
                switch (event.type) {
                    case 'connection'.#eventManager.registerEvent({
                            type: 'event',
                        ...
                        return;
                    case 'discovery'cdpTarget.browserCdpClient.sendCommand('BluetoothEmulation.simulateGATTOperationResponse', {
                            address.address,
                            type: 'discovery',
                            code: 0x0,
                        });
            ...
            cdpTarget.browserCdpClient.on('BluetoothEmulation.characteristicOperationReceived', (event) => {
                ...
                this.#eventManager.registerEvent({
                    ...
                    params: {
                        context.id,
                        address.service.device.address,
                        serviceUuid.service.uuid,
                        characteristicUuid.uuid,
                        type,
                        ...(event.data && {
                            data.from(atob(event.data), (c) => c.charCodeAt(0)),
                        }),
                    },
                }, cdpTarget.id);
            });
    ...
    function parseSharedId(sharedId) {
        ...
        if (legacyFormattedSharedId !== null) {
            return { ...legacyFormattedSharedId, frameId};
        }
    ...
    class Realm {
        ...
        async evaluate(expression, awaitPromise, resultOwnership = "none" , serializationOptions = {}, userActivation = false, includeCommandLineApi = false) {
            const cdpEvaluateResult = await this.cdpClient.sendCommand('Runtime.evaluate', {
                ...
                expression,
                awaitPromise,
                serializationOptions.#getSerializationOptions("deep" , serializationOptions),
                userGesture,
            ...
            return {
                realm.realmId,
                result.cdpToBidiValue(cdpEvaluateResult, resultOwnership),
                type: 'success',
            };
        }
        #registerEvent(event) {
        ...
        async serializeCdpObject(cdpRemoteObject, resultOwnership) {
            ...
            const cdpValue = await this.cdpClient.sendCommand('Runtime.callFunctionOn', {
                functionDeclaration((remoteObject) => remoteObject),
                awaitPromise,
                arguments: [argument],
                serializationOptions: {
                    serialization: "deep" ,
        ...
        async stringifyObject(cdpRemoteObject) {
            const { result } = await this.cdpClient.sendCommand('Runtime.callFunctionOn', {
                functionDeclaration((remoteObject) => String(remoteObject)),
                awaitPromise,
        ...
        async #serializeCdpExceptionDetails(cdpExceptionDetails, lineOffset, resultOwnership) {
            ...
            return {
                exceptionthis.serializeCdpObject(exception, resultOwnership),
                columnNumber.columnNumber,
                lineNumber.lineNumber - lineOffset,
                stackTrace: {
                    callFrames,
      ...
      return callFunction((
        ${functionDeclaration}
            ...
            return {
                type: 'success',
                result.cdpToBidiValue(cdpCallFunctionResult, resultOwnership),
                realm.realmId,
            };
        }
        async deserializeForCdp(localValue) {
            ...
            switch (localValue.type) {
                case 'undefined'{ unserializableValue: 'undefined' };
                case 'null'{ unserializableValue: 'null' };
                case 'string'{ value.value };
                case 'number'(localValue.value === 'NaN') {
                ...
                case 'boolean'{ value(localValue.value) };
                case 'bigint'{
                ...
                case 'set': {
                    const args = await this.#flattenValueList(localValue.value);
                    const { result } = await this.cdpClient.sendCommand('Runtime.callFunctionOn', {
                        functionDeclaration((...args) => new Set(args)),
                        awaitPromise,
                        arguments,
                        returnByValue,
                        executionContextId.executionContextId,
        ...
        async #getExceptionResult(exceptionDetails, lineOffset, resultOwnership) {
            return {
                exceptionDetailsthis.#serializeCdpExceptionDetails(exceptionDetails, lineOffset, resultOwnership),
                realm.realmId,
                type: 'exception',
            };
        }
        static #getSerializationOptions(serialization, serializationOptions) {
            return {
                serialization,
                additionalParameters.#getAdditionalSerializationParameters(serializationOptions),
        ...
        dispose() {
            if (!this.isHidden()) {
                this.#registerEvent({
                    type: 'event',
                    method$2.EventNames.RealmDestroyed,
                    params: {
                        realm.realmId,
                    },
                });
    ...
    class NavigationState {
        ...
        start() {
            ...
            !this.#isInitial &&
                ...
                this.#eventManager.registerEvent({
                    type: 'event',
                    method$2.EventNames.NavigationStarted,
        ...
        frameNavigated() {
            ...
            if (!this.#isInitial) {
                this.#eventManager.registerEvent({
                    type: 'event',
                    method$2.EventNames.NavigationCommitted,
                    params.navigationInfo(),
                }, this.#browsingContextId);
            }
        }
    ...
    class NavigationTracker {
        ...
        navigatedWithinDocument(url, navigationType) {
            ...
            const fragmentNavigation = this.#pendingNavigation?.isFragmentNavigation === true
                ? this.#pendingNavigation
                NavigationState(url, this.#browsingContextId, false, this.#eventManager);
    ...
    class BrowsingContextImpl {
        ...
        dispose(emitContextDestroyed) {
            ...
            if (emitContextDestroyed) {
                this.#eventManager.registerEvent({
                    type: 'event',
                    method$2.EventNames.ContextDestroyed,
                    params.serializeToBidiValue(null),
                }, this.id);
            }
        ...
        serializeToBidiValue(maxDepth = 0, addParentField = true) {
            return {
                ...
                url.url,
                userContext.userContext,
                originalOpener.#originalOpener ?? null,
                clientWindow: `${this.cdpTarget.windowId}`,
                children=== null || maxDepth > 0
                    ? this.directChildren.map((c) => c.serializeToBidiValue(maxDepth === null ? maxDepth - 1, false))
                    ,
                ...(addParentField ? { parent.#parentId } : {}),
            };
        }
        ...
        #initListeners() {
            ...
            this.#cdpTarget.cdpClient.on('Page.fileChooserOpened', (params) => {
                ...
                const element = params.backendNodeId === undefined
                    ? undefined
                    : {
                        sharedId(this.id, this.#loaderId, params.backendNodeId),
                    };
                this.#eventManager.registerEvent({
                    type: 'event',
                    method$2.EventNames.FileDialogOpened,
                    params: {
                        context.id,
            ...
            this.#cdpTarget.cdpClient.on('Page.lifecycleEvent', (params) => {
                ...
                switch (params.name) {
                    case 'DOMContentLoaded'(!this.#navigationTracker.isInitialNavigation) {
                            this.#eventManager.registerEvent({
                                type: 'event',
                                method$2.EventNames.DomContentLoaded,
                                params: {
                                    context.id,
                                    navigation.#navigationTracker.currentNavigationId,
                                    timestamp(),
                                    url.#navigationTracker.url,
                                },
                            }, this.id);
                        ...
                        break;
                    case 'load'(!this.#navigationTracker.isInitialNavigation) {
                            this.#eventManager.registerEvent({
                                type: 'event',
                                method$2.EventNames.Load,
                                params: {
                                    context.id,
                                    navigation.#navigationTracker.currentNavigationId,
                                    timestamp(),
                    ...
                    case 'isolated'= name;
                        ...
                        origin = this.#defaultRealmDeferred.isFinished
                            ...
                            :
                                '';
                        break;
                    case 'default'= serializeOrigin(params.context.origin);
                        break;
                    default;
                }
            ...
            this.#cdpTarget.cdpClient.on('Runtime.executionContextDestroyed', (params) => {
                ...
                this.#realmStorage.deleteRealms({
                    cdpSessionId.#cdpTarget.cdpSessionId,
                    executionContextId.executionContextId,
                });
            });
            ...
            this.#cdpTarget.cdpClient.on('Page.javascriptDialogClosed', (params) => {
                ...
                this.#eventManager.registerEvent({
                    type: 'event',
                    method$2.EventNames.UserPromptClosed,
                    params: {
                        context.id,
                        accepted,
                        type.#lastUserPromptType ??
                            'UNKNOWN',
                        userText&& params.userInput ? params.userInput ,
                    },
                }, this.id);
            ...
            this.#cdpTarget.cdpClient.on('Page.javascriptDialogOpening', (params) => {
                ...
                this.#eventManager.registerEvent({
                    type: 'event',
                    method$2.EventNames.UserPromptOpened,
                    params: {
                        context.id,
                        handler,
                        type,
                ...
                switch (promptHandler) {
                    case "accept" this.handleUserPrompt(true);
                        break;
                    case "dismiss" this.handleUserPrompt(false);
                        break;
                }
            });
            this.#cdpTarget.browserCdpClient.on('Browser.downloadWillBegin', (params) => {
                ...
                this.#eventManager.registerEvent({
                    type: 'event',
                    method$2.EventNames.DownloadWillBegin,
                    params: {
                        context.id,
            ...
            this.#cdpTarget.browserCdpClient.on('Browser.downloadProgress', (params) => {
                ...
                switch (params.state) {
                    case 'canceled'.#eventManager.registerEvent({
                            type: 'event',
                            method$2.EventNames.DownloadEnd,
                            params: {
                                status: 'canceled',
                                context.id,
                                navigation.guid,
                        ...
                        break;
                    case 'completed'.#eventManager.registerEvent({
                            type: 'event',
                            method$2.EventNames.DownloadEnd,
                            params: {
                        ...
                        break;
                    defaultnew UnknownErrorException(`Unknown download state: ${params.state}`);
                }
            });
        }
        ...
        #getPromptHandler(promptType) {
            ...
            switch (promptType) {
                case "alert" (contextConfig.userPromptHandler?.alert ??
                        contextConfig.userPromptHandler?.default ??
                        defaultPromptHandler);
                case "beforeunload" (contextConfig.userPromptHandler?.beforeUnload ??
                        contextConfig.userPromptHandler?.default ??
                        "accept" );
                case "confirm" (contextConfig.userPromptHandler?.confirm ??
                        contextConfig.userPromptHandler?.default ??
                        defaultPromptHandler);
                case "prompt" (contextConfig.userPromptHandler?.prompt ??
                        contextConfig.userPromptHandler?.default ??
                        defaultPromptHandler);
            }
        }
        ...
        async captureScreenshot(params) {
            ...
            if (rect.width === 0 || rect.height === 0) {
                throw new UnableToCaptureScreenException(`Unable to capture screenshot with zero dimensions=${rect.width}, height=${rect.height}`);
            }
        ...
        async print(params) {
            ...
            try {
                ...
                if (error.message ===
                    'invalid print parametersarea is empty') {
        ...
        async #parseRect(clip) {
            switch (clip.type) {
                case 'box'{ x.x, y.y, width.width, height.height };
                case 'element': {
                    const hiddenSandboxRealm = await this.getOrCreateHiddenSandbox();
        ...
        #getLocatorDelegate(locator, maxNodeCount, startNodes) {
            switch (locator.type) {
                case 'context''accessibility'new Error('Unreachable');
                case 'css'{
                        functionDeclaration((cssSelector, maxNodeCount, ...startNodes) => {
                ...
                case 'xpath'{
                        functionDeclaration((xPathSelector, maxNodeCount, ...startNodes) => {
                ...
                case 'innerText'(locator.value === '') {
                        throw new InvalidSelectorException('innerText locator cannot be empty');
                    }
                    return {
                        functionDeclaration((innerTextSelector, fullMatch, ignoreCase, maxNodeCount, maxDepth, ...startNodes) => {
                        ...
                        argumentsLocalValues: [
                            { type: 'string', value.value },
                            { type: 'boolean', value.matchType !== 'partial' },
                            { type: 'boolean', value.ignoreCase === true },
                            { type: 'number', value?? 0 },
                            { type: 'number', value.maxDepth ?? 1000 },
                            ...startNodes,
                        ],
                    };
            }
        ...
        async #locateNodesByLocator(realm, locator, startNodes, maxNodeCount, serializationOptions) {
            ...
            if (locatorResult.type !== 'success') {
                ...
                locatorResult.exceptionDetails.text?.endsWith('is not a valid selector.') ||
                    ...
                    throw new InvalidSelectorException(`Not valid selector ${typeof locator.value === 'string' ? locator.value .stringify(locator.value)}`);
                }
                if (locatorResult.exceptionDetails.text ===
                    'Errorin css selector should be HTMLElement, SVGElement or Document or DocumentFragment') {
        ...
        async #locateNodesByContextLocator(locator, startNodes, realm, serializationOptions) {
            ...
            try {
                ...
                const locatorResult = await realm.callFunction(`function () { return this; }`, false, { handle.objectId }, [], "none" , serializationOptions);
        ...
        async #locateNodesByAccessibility(locator, startNodes, maxNodeCount, realm) {
            ...
            if (startNodes.length === 0) {
                const { root} = await this.#cdpTarget.cdpClient.sendCommand('DOM.getDocument');
                ...
                for (const node of startNodes) {
                    if (node.sharedId) {
                        ...
                        if (node.handle) {
                            ...
                            const { node} = await this.#cdpTarget.cdpClient.sendCommand('DOM.describeNode', {
                                nodeId,
                            });
            ...
            return {
                nodes.filter((result) => result.type === 'node'),
            };
        }
    ...
    function getImageFormatParameters(params) {
        ...
        switch (type) {
            ...
            case 'image/jpeg': {
                return {
                    format: 'jpeg',
                    ...(quality === undefined ? {} : { quality.round(quality * 100) }),
                };
            }
            case 'image/webp': {
                return {
    ...
    function getIntersectionRect(first, second) {
        ...
        return {
            x,
            y,
            width.max(Math.min(first.x + first.width, second.x + second.width) - x, 0),
            height.max(Math.min(first.y + first.height, second.y + second.height) - y, 0),
        };
    }
    ...
    class WorkerRealm extends Realm {
        ...
        get source() {
            return {
                realm.realmId,
                context.associatedBrowsingContexts[0]?.id,
            };
        }
        get realmInfo() {
            ...
            switch (realmType) {
                ...
                case 'service-worker''shared-worker': {
                    return {
                        ...this.baseInfo,
                        type,
                    };
    ...
    function stringFromArg(arg) {
        ...
        switch (arg.type) {
            case 'string''number''boolean''bigint'String(arg.value);
            case 'regexp'`/${arg.value.pattern}/${arg.value.flags ?? ''}`;
            case 'date'new Date(arg.value).toString();
            case 'object'`Object(${arg.value?.length ?? ''})`;
            case 'array'`Array(${arg.value?.length ?? ''})`;
            case 'map'`Map(${arg.value?.length})`;
            case 'set'`Set(${arg.value?.length})`;
            defaultarg.type;
        }
    }
    ...
    function getLogMethod(consoleApiType) {
        switch (consoleApiType) {
            ...
            case 'startGroupCollapsed''groupCollapsed';
            case 'endGroup''groupEnd';
        }
    ...
    class LogManager {
        ...
        async #heuristicSerializeArg(arg, realm) {
            switch (arg.type) {
                ...
                case 'boolean'{ type: 'boolean', value.value };
                case 'string'{ type: 'string', value.value };
                case 'number'{ type: 'number', value.unserializableValue ?? arg.value };
                case 'bigint'(arg.unserializableValue !== undefined &&
                        arg.unserializableValue[arg.unserializableValue.length - 1] === 'n') {
                        return {
                            type.type,
                            value.unserializableValue.slice(0, -1),
                        };
                    }
                    break;
                case 'object'(arg.subtype === 'null') {
                        return { type: 'null' };
                    }
                    break;
            }
            ...
            this.#cdpTarget.cdpClient.on('Runtime.consoleAPICalled', (params) => {
                ...
                for (const browsingContext of realm.associatedBrowsingContexts) {
                    this.#eventManager.registerPromiseEvent(argsPromise.then((args) => ({
                        ...
                        value: {
                            type: 'event',
                            method$1.EventNames.LogEntryAdded,
                            params: {
                                level(params.type),
                                source.source,
                                text(args, true),
                                timestamp.round(params.timestamp),
    ...
    class CollectorsStorage {
        ...
        #getRequestToCollectorMap(dataType) {
            switch (dataType) {
                case "response" this.#responseCollectors;
                case "request" this.#requestBodyCollectors;
                defaultnew UnsupportedOperationException(`Unsupported data type ${dataType}`);
            }
        }
    ...
    class NetworkRequest {
        ...
        #logger;
        #emittedEvents = {
            [Network$2.EventNames.AuthRequired],
            [Network$2.EventNames.BeforeRequestSent],
            [Network$2.EventNames.FetchError],
            [Network$2.EventNames.ResponseCompleted],
            [Network$2.EventNames.ResponseStarted],
        };
        ...
        get #timings() {
            ...
            return {
                timeOrigin.round(getTiming(this.#request.info?.wallTime) * 1000),
                requestTime: 0,
                redirectStart: 0,
                redirectEnd: 0,
                fetchStart(this.#response.info?.timing?.workerFetchStart, responseTimeOffset),
                dnsStart(this.#response.info?.timing?.dnsStart, responseTimeOffset),
                dnsEnd(this.#response.info?.timing?.dnsEnd, responseTimeOffset),
                connectStart(this.#response.info?.timing?.connectStart, responseTimeOffset),
                connectEnd(this.#response.info?.timing?.connectEnd, responseTimeOffset),
                tlsStart(this.#response.info?.timing?.sslStart, responseTimeOffset),
                requestStart(this.#response.info?.timing?.sendStart, responseTimeOffset),
                responseStart(this.#response.info?.timing?.receiveHeadersStart, responseTimeOffset),
                responseEnd(this.#response.info?.timing?.receiveHeadersEnd, responseTimeOffset),
            };
        }
        #phaseChanged() {
        ...
        onLoadingFailedEvent(event) {
            ...
            this.#emitEvent(() => {
                return {
                    method$2.EventNames.FetchError,
                    params: {
        ...
        onAuthRequired(event) {
            ...
            this.#emitEvent(() => {
                return {
                    method$2.EventNames.AuthRequired,
                    params: {
                        ...this.#getBaseEventParams("authRequired" ),
                        response.#getResponseEventParams(),
                    },
                };
            });
        }
        ...
        async continueResponse(overrides = {}) {
            ...
            if (this.#interceptPhase === "responseStarted" ) {
                ...
                await this.#continueResponse({
                    responseCode.statusCode ?? this.#response.paused?.responseStatusCode,
                    responsePhrase.reasonPhrase ?? this.#response.paused?.responseStatusText,
        ...
        #getBaseEventParams(phase) {
            ...
            return {
                ...
                timestamp.round(getTiming(this.#request.info?.wallTime) * 1000),
                ...interceptProps,
            };
        }
        #getResponseEventParams() {
            ...
            const response = {
                ...
                statusText.#response.info?.statusText ||
                    ...
                    '',
                fromCache.#response.info?.fromDiskCache ||
                    this.#response.info?.fromPrefetchCache ||
                    this.#servedFromCache,
                headers.#responseOverrides?.headers ?? headers,
                mimeType.#response.info?.mimeType || '',
                bytesReceived.encodedResponseBodySize,
                headersSize(headers),
                bodySize.encodedResponseBodySize,
                content: {
                    size.#response.decodedSize ?? 0,
                },
                ...(authChallenges ? { authChallenges } : {}),
            };
            return {
                ...response,
                'goog'.#response.info?.securityDetails,
            };
        }
        ...
        #getRequestData() {
            ...
            const request = {
                request.#id,
                url.url,
                method.#method ?? _a$3.unknownParameter,
                headers,
                cookies.#cookies,
                headersSize(headers),
                bodySize.bodySize,
            ...
            return {
                ...request,
                'goog'.#request.info?.request?.postData,
                'goog'.#request.info?.request?.hasPostData,
                'goog'.#request.info?.type,
                'goog'.#request.info?.initiator,
            };
        }
        #getDestination() {
            switch (this.#request.info?.type) {
                ...
                case 'Image''image';
                case 'Document'this.#request.info?.initiator.type === 'parser'
                        ? 'iframe'
                        : 'document';
                default'';
            }
        ...
        #getInitiatorType() {
            if (this.#request.info?.initiator.type === 'parser') {
                switch (this.#request.info?.type) {
                    ...
                    case 'Font'this.#request.info?.initiator?.url ===
                            this.#request.info?.documentURL
                            ? 'font'
                            : 'css';
                    case 'Image'this.#request.info?.initiator?.url ===
                            this.#request.info?.documentURL
                            ? 'img'
                            : 'css';
                    case 'Script''script';
        ...
        #getBeforeRequestEvent() {
            ...
            return {
                method$2.EventNames.BeforeRequestSent,
                params: {
                    ...this.#getBaseEventParams("beforeRequestSent" ),
                    initiator: {
                        type: _a$3.#getInitiator(this.#request.info.initiator.type),
                        columnNumber.#request.info.initiator.columnNumber,
                        lineNumber.#request.info.initiator.lineNumber,
                        stackTrace.#request.info.initiator.stack,
                        request.#request.info.initiator.requestId,
                    },
                },
            };
        }
        #getResponseStartedEvent() {
            return {
                method$2.EventNames.ResponseStarted,
                params: {
        ...
        #getResponseReceivedEvent() {
            return {
                method$2.EventNames.ResponseCompleted,
                params: {
                    ...this.#getBaseEventParams(),
                    response.#getResponseEventParams(),
                },
        ...
        static #getInitiator(initiatorType) {
            switch (initiatorType) {
                case 'parser''script''preflight'initiatorType;
                default'other';
            }
        }
    }
    ...
    class NetworkStorage {
        ...
        async getCollectedData(params) {
            ...
            switch (params.dataType) {
                case "response" = await this.#getCollectedResponseData(request);
                    break;
                case "request" = await this.#getCollectedRequestData(request);
                    break;
                defaultnew UnsupportedOperationException(`Unsupported data type ${params.dataType}`);
            }
            if (params.disown && params.collector !== undefined) {
        ...
        async #getCollectedResponseData(request) {
            try {
                const responseBody = await request.cdpClient.sendCommand('Network.getResponseBody', { requestId.id });
                return {
                    bytes: {
                        type.base64Encoded ? 'base64' : 'string',
                        value.body,
                    },
                };
            }
        ...
        async #getCollectedRequestData(request) {
            const requestPostData = await request.cdpClient.sendCommand('Network.getRequestPostData', { requestId.id });
            return {
                bytes: {
                    type: 'string',
                    value.postData,
    ...
    class CdpTarget {
        ...
        async #unblock() {
            ...
            const results = await Promise.allSettled([
                ...
                this.#cdpClient
                    ...
                    enableDurableMessages.disableNetworkDurableMessages !== true,
                    maxTotalBufferSize_TOTAL_COLLECTED_SIZE,
                })
        ...
        async #updateWindowId() {
            const { windowId } = await this.#browserCdpClient.sendCommand('Browser.getWindowForTarget', { targetId.id });
        ...
        async setDeviceMetricsOverride(viewport, devicePixelRatio, screenOrientation, screenArea, scrollbarType = null) {
            ...
            const metricsOverride = {
                ...
                height?.height ?? 0,
                deviceScaleFactor?? 0,
                screenOrientation.#toCdpScreenOrientationAngle(screenOrientation) ?? undefined,
                mobile,
                screenWidth?.width,
                screenHeight?.height,
                scrollbarType=== 'overlay' ? 'overlay' : 'default',
            };
        ...
        #toCdpScreenOrientationAngle(orientation) {
            ...
            if (orientation.natural === "portrait" ) {
                switch (orientation.type) {
                    ...
                    defaultnew UnknownErrorException(`Unexpected screen orientation type ${orientation.type}`);
                }
            }
        ...
        async setUserAgentAndAcceptLanguage(userAgent, acceptLanguage, clientHints) {
            const userAgentMetadata = clientHints
                ? {
                    ...
                    model.model ?? '',
                    mobile.mobile ?? false,
                    bitness.bitness ?? undefined,
                    wow64.wow64 ?? undefined,
                    formFactors.formFactors ?? undefined,
                }
                ;
            await this.cdpClient.sendCommand('Emulation.setUserAgentOverride', {
                userAgent|| (userAgentMetadata ? this.#defaultUserAgent : ''),
            ...
            switch (targetInfo.type) {
                ...
                case 'page''iframe': {
                    ...
                    if (maybeContext && targetInfo.type === 'iframe') {
                        ...
                        targetInfo.url === '' ? 'about' .url, targetInfo.openerFrameId ?? targetInfo.openerId, this.#logger);
                    }
                    return;
                }
                case 'service_worker''worker': {
                    const realm = this.#realmStorage.findRealm({
    ...
    class RealmStorage {
        ...
        findRealms(filter) {
            const sandboxFilterValue = filter.sandbox === null ? undefined .sandbox;
    ...
    function unrollEvents(events) {
        ...
        for (const event of events) {
            switch (event) {
                case BiDiModule.Bluetooth(Object.values(Bluetooth$2.EventNames));
                    break;
                case BiDiModule.BrowsingContext(Object.values(BrowsingContext$2.EventNames));
                    break;
                case BiDiModule.Input(Object.values(Input$2.EventNames));
                    break;
                case BiDiModule.Log(Object.values(Log$1.EventNames));
                    break;
                case BiDiModule.Network(Object.values(Network$2.EventNames));
                    break;
                case BiDiModule.Script(Object.values(Script$2.EventNames));
                    break;
                case BiDiModule.Speculation(Object.values(Speculation.EventNames));
                    break;
                default.add(event);
            }
        }
    ...
    class SubscriptionManager {
        ...
        subscribe(eventNames, contextIds, userContextIds, googChannel) {
            const subscription = {
                id(),
                eventNamesSet(unrollEvents(eventNames)),
                topLevelTraversableIdsSet(contextIds.map((contextId) => {
                ...
                userContextIdsSet(userContextIds),
                googChannel,
            };
    ...
    class EventManager extends EventEmitter {
        ...
        registerPromiseEvent(event, contextId, eventName) {
            ...
            for (const googChannel of sortedGoogChannels) {
                this.emit("event" , {
                    message.createFromPromise(event, googChannel),
                    event,
                });
        ...
        async subscribe(eventNames, contextIds, userContextIds, googChannel) {
            ...
            const subscriptionNavigableIds = new Set(contextIds.length
                ...
                .#browsingContextStorage.getTopLevelContexts().map((c) => c.id));
            ...
            for (const eventName of subscription.eventNames) {
                for (const contextId of subscriptionNavigableIds) {
                    for (const eventWrapper of this.#getBufferedEvents(eventName, contextId, googChannel)) {
                        this.emit("event" , {
                            message.createFromPromise(eventWrapper.event, googChannel),
                            event,
                        });
                        this.#markEventSent(eventWrapper, googChannel, eventName);
                    }
            ...
            cdpTarget.cdpClient.on('Preload.prefetchStatusUpdated', (event) => {
                ...
                switch (event.status) {
                    ...
                    case 'Failure'= "failure" ;
                        break;
                    default.#logger?.(LogType.debugWarn, `Unknown prefetch status: ${event.status}`);
                        return;
                }
                this.#eventManager.registerEvent({
                    type: 'event',
    ...
    class BidiServer extends EventEmitter {
        ...
        #processOutgoingMessage = async (messageEntry) => {
            ...
            if (messageEntry.googChannel !== null) {
                message['goog'] = messageEntry.googChannel;
            }
        ...
        constructor(bidiTransport, cdpConnection, browserCdpClient, selfTargetId, defaultUserContextId, defaultUserAgent, parser, logger) {
            ...
            this.#commandProcessor = new CommandProcessor(cdpConnection, browserCdpClient, this.#eventManager, this.#browsingContextStorage, this.#realmStorage, this.#preloadScriptStorage, networkStorage, contextConfigStorage, this.#bluetoothProcessor, userContextStorage, parser, async (options) => {
                await browserCdpClient.sendCommand('Security.setIgnoreCertificateErrors', {
                    ignore.acceptInsecureCerts ?? false,
                });
                contextConfigStorage.updateGlobalConfig({
                    acceptInsecureCerts.acceptInsecureCerts ?? false,
                    userPromptHandler.unhandledPromptBehavior,
                    prerenderingDisabled?.['goog'] ?? false,
                    disableNetworkDurableMessages?.['goog'],
                });
    ...
    class MapperCdpConnection {
        static LOGGER_PREFIX_RECV = `${LogType.cdp}◂`;
        static LOGGER_PREFIX_SEND = `${LogType.cdp}▸`;
        ...
        sendCommand(method, params, sessionId) {
            return new Promise((resolve, reject) => {
                ...
                this.#commandCallbacks.set(id, {
                    ...
                    resolve,
                    reject,
                    errorCloseError(`${method} ${JSON.stringify(params)} ${sessionId ?? ''} call rejected because the connection has been closed.`),
                });
    ...
    (function (util) {
        ...
        function joinValues(array, separator = " | ") {
            return array.map((val) => (typeof val === "string" ? `'${val}'` )).join(separator);
        }
    ...
    const getParsedType = (data) => {
        const t = typeof data;
        switch (t) {
            case "undefined"ZodParsedType.undefined;
            case "string"ZodParsedType.string;
            case "number"Number.isNaN(data) ? ZodParsedType.nan .number;
            case "boolean"ZodParsedType.boolean;
            case "function"ZodParsedType.function;
            case "bigint"ZodParsedType.bigint;
            case "symbol"ZodParsedType.symbol;
            case "object"(Array.isArray(data)) {
    ...
    const errorMap = (issue, _ctx) => {
        let message;
        switch (issue.code) {
            case ZodIssueCode.invalid_type(issue.received === ZodParsedType.undefined) {
                    message = "Required";
                }
                else {
                ...
                break;
            case ZodIssueCode.invalid_literal= `Invalid literal value, expected ${JSON.stringify(issue.expected, util.jsonStringifyReplacer)}`;
                break;
            case ZodIssueCode.unrecognized_keys= `Unrecognized key(s) in object: ${util.joinValues(issue.keys, ", ")}`;
                break;
            case ZodIssueCode.invalid_union= `Invalid input`;
                break;
            case ZodIssueCode.invalid_union_discriminator= `Invalid discriminator value. Expected ${util.joinValues(issue.options)}`;
                break;
            case ZodIssueCode.invalid_enum_value= `Invalid enum value. Expected ${util.joinValues(issue.options)}, received '${issue.received}'`;
                break;
            case ZodIssueCode.invalid_arguments= `Invalid function arguments`;
                break;
            case ZodIssueCode.invalid_return_type= `Invalid function return type`;
                break;
            case ZodIssueCode.invalid_date= `Invalid date`;
                break;
            case ZodIssueCode.invalid_string(typeof issue.validation === "object") {
                    if ("includes" in issue.validation) {
                        message = `Invalid inputinclude "${issue.validation.includes}"`;
                        if (typeof issue.validation.position === "number") {
                            message = `${message} at one or more positions greater than or equal to ${issue.validation.position}`;
                        }
                    }
                    else if ("startsWith" in issue.validation) {
                        message = `Invalid inputstart with "${issue.validation.startsWith}"`;
                    }
                    else if ("endsWith" in issue.validation) {
                        message = `Invalid inputend with "${issue.validation.endsWith}"`;
                    }
                    else {
                ...
                break;
            case ZodIssueCode.too_small(issue.type === "array")
                    message = `Array must contain ${issue.exact ? "exactly" .inclusive ? `at least` : `more than`} ${issue.minimum} element(s)`;
                else if (issue.type === "string")
                    message = `String must contain ${issue.exact ? "exactly" .inclusive ? `at least` : `over`} ${issue.minimum} character(s)`;
                else if (issue.type === "number")
                    message = `Number must be ${issue.exact ? `exactly equal to ` .inclusive ? `greater than or equal to ` : `greater than `}${issue.minimum}`;
                else if (issue.type === "bigint")
                    message = `Number must be ${issue.exact ? `exactly equal to ` .inclusive ? `greater than or equal to ` : `greater than `}${issue.minimum}`;
                else if (issue.type === "date")
                    message = `Date must be ${issue.exact ? `exactly equal to ` .inclusive ? `greater than or equal to ` : `greater than `}${new Date(Number(issue.minimum))}`;
                else
                    message = "Invalid input";
                break;
            case ZodIssueCode.too_big(issue.type === "array")
                    message = `Array must contain ${issue.exact ? `exactly` .inclusive ? `at most` : `less than`} ${issue.maximum} element(s)`;
                else if (issue.type === "string")
                    message = `String must contain ${issue.exact ? `exactly` .inclusive ? `at most` : `under`} ${issue.maximum} character(s)`;
                else if (issue.type === "number")
                    message = `Number must be ${issue.exact ? `exactly` .inclusive ? `less than or equal to` : `less than`} ${issue.maximum}`;
                else if (issue.type === "bigint")
                    message = `BigInt must be ${issue.exact ? `exactly` .inclusive ? `less than or equal to` : `less than`} ${issue.maximum}`;
                else if (issue.type === "date")
                    message = `Date must be ${issue.exact ? `exactly` .inclusive ? `smaller than or equal to` : `smaller than`} ${new Date(Number(issue.maximum))}`;
                else
                    message = "Invalid input";
                break;
            case ZodIssueCode.custom= `Invalid input`;
                break;
            case ZodIssueCode.invalid_intersection_types= `Intersection results could not be merged`;
                break;
            case ZodIssueCode.not_multiple_of= `Number must be a multiple of ${issue.multipleOf}`;
                break;
            case ZodIssueCode.not_finite= "Number must be finite";
                break;
            default= _ctx.defaultError;
    ...
    const makeIssue = (params) => {
        ...
        for (const map of maps) {
            errorMessage = map(fullIssue, { data, defaultError}).message;
        }
        return {
            ...issueData,
            path,
    ...
    function addIssueToContext(ctx, issueData) {
        ...
        const issue = makeIssue({
            ...
            errorMaps: [
                ...
                ctx.schemaErrorMap,
                overrideMap,
                overrideMap === errorMap ? undefined ,
            ].filter((x) => !!x),
        });
    ...
    var errorUtil;
    (function (errorUtil) {
        errorUtil.errToObj = (message) => typeof message === "string" ? { message } || {};
        errorUtil.toString = (message) => typeof message === "string" ? message ?.message;
    })(errorUtil || (errorUtil = {}));
    ...
    class ZodType {
        ...
        _getOrReturnCtx(input, ctx) {
            return (ctx || {
                ...
                data.data,
                parsedType(input.data),
                schemaErrorMap._def.errorMap,
                path.path,
                parent.parent,
            });
        }
        ...
        safeParse(data, params) {
            ...
            const result = this._parseSync({ data, path.path, parent});
        ...
        "~validate"(data) {
            ...
            if (!this["~standard"].async) {
                try {
                    const result = this._parseSync({ data, path: [], parent});
            ...
            return this._parseAsync({ data, path: [], parent}).then((result) => isValid(result)
                ? {
                    value.value,
                }
                : {
        ...
        async safeParseAsync(data, params) {
            ...
            const maybeAsyncResult = this._parse({ data, path.path, parent});
            const result = await (isAsync(maybeAsyncResult) ? maybeAsyncResult .resolve(maybeAsyncResult));
            return handleResult(ctx, result);
        }
        ...
        refinement(check, refinementData) {
            return this._refinement((val, ctx) => {
                if (!check(val)) {
                    ctx.addIssue(typeof refinementData === "function" ? refinementData(val, ctx) );
                    return false;
                }
                else {
                    return true;
    ...
    const durationRegex = /^[-+]?P(?!$)(:(:[-+]?\d+Y)|(:[-+]?\d+[.,]\d+Y$))?(:(:[-+]?\d+M)|(:[-+]?\d+[.,]\d+M$))?(:(:[-+]?\d+W)|(:[-+]?\d+[.,]\d+W$))?(:(:[-+]?\d+D)|(:[-+]?\d+[.,]\d+D$))?(?(?=[\d+-])(:(:[-+]?\d+H)|(:[-+]?\d+[.,]\d+H$))?(:(:[-+]?\d+M)|(:[-+]?\d+[.,]\d+M$))?(:[-+]?\d+(:[.,]\d+)?S)?)??$/;
    ...
    class ZodString extends ZodType {
        _parse(input) {
            ...
            return { status.value, value.data };
        }
        ...
        datetime(options) {
            ...
            return this._addCheck({
                kind: "datetime",
                precisionoptions?.precision === "undefined" ? null ?.precision,
                offset?.offset ?? false,
                local?.local ?? false,
    ...
    function floatSafeRemainder(val, step) {
        ...
        const decCount = valDecCount > stepDecCount ? valDecCount ;
        ...
        _parse(input) {
            ...
            for (const check of this._def.checks) {
                if (check.kind === "int") {
                    ...
                    const tooSmall = check.inclusive ? input.data < check.value .data <= check.value;
                    if (tooSmall) {
                        ctx = this._getOrReturnCtx(input, ctx);
                        addIssueToContext(ctx, {
                            code.too_small,
                    ...
                    const tooBig = check.inclusive ? input.data > check.value .data >= check.value;
                    if (tooBig) {
                        ctx = this._getOrReturnCtx(input, ctx);
                        addIssueToContext(ctx, {
                            code.too_big,
    ...
    class ZodArray extends ZodType {
        _parse(input) {
            ...
            if (def.exactLength !== null) {
                ...
                if (tooBig || tooSmall) {
                    addIssueToContext(ctx, {
                        code? ZodIssueCode.too_big .too_small,
        ...
        min(minLength, message) {
            return new ZodArray({
                ...this._def,
                minLength: { value, message.toString(message) },
            });
        }
        max(maxLength, message) {
            return new ZodArray({
                ...this._def,
                maxLength: { value, message.toString(message) },
            });
        }
        length(len, message) {
            return new ZodArray({
                ...this._def,
                exactLength: { value, message.toString(message) },
            });
        }
        nonempty(message) {
    ...
    class ZodObject extends ZodType {
        ...
        _parse(input) {
            ...
            const { shape, keys} = this._getCached();
            const extraKeys = [];
            ...
            for (const key of shapeKeys) {
                ...
                pairs.push({
                    ...
                    value._parse(new ParseInputLazyPath(ctx, value, ctx.path, key)),
                    alwaysSetin ctx.data,
                });
            }
            if (this._def.catchall instanceof ZodNever) {
                ...
                if (unknownKeys === "passthrough") {
                    for (const key of extraKeys) {
                        pairs.push({
                            key: { status: "valid", value},
                            value: { status: "valid", value.data[key] },
                        });
                    }
                }
                    ...
                    throw new Error(`Internal ZodObject errorunknownKeys value.`);
                }
            }
            else {
                    ...
                    ? {
                        errorMap: (issue, ctx) => {
                            ...
                            if (issue.code === "unrecognized_keys")
                                return {
                                    message.errToObj(message).message ?? defaultError,
                                };
                            return {
                                message,
                            };
        ...
        _parse(input) {
            ...
            if (!option) {
                addIssueToContext(ctx, {
                    ...
                    options.from(this.optionsMap.keys()),
                    path: [discriminator],
                });
                return INVALID;
            }
    ...
    class ZodRecord extends ZodType {
        ...
        _parse(input) {
            ...
            for (const key in ctx.data) {
                pairs.push({
                    key._parse(new ParseInputLazyPath(ctx, key, ctx.path, key)),
                    value._parse(new ParseInputLazyPath(ctx, ctx.data[key], ctx.path, key)),
                    alwaysSetin ctx.data,
                });
            }
            if (ctx.common.async) {
    ...
    class ZodMap extends ZodType {
        ...
        _parse(input) {
            ...
            const pairs = [...ctx.data.entries()].map(([key, value], index) => {
                return {
                    key._parse(new ParseInputLazyPath(ctx, key, ctx.path, [index, "key"])),
                    value._parse(new ParseInputLazyPath(ctx, value, ctx.path, [index, "value"])),
                };
            });
            if (ctx.common.async) {
    ...
    class ZodSet extends ZodType {
        ...
        min(minSize, message) {
            return new ZodSet({
                ...this._def,
                minSize: { value, message.toString(message) },
            });
        }
        max(maxSize, message) {
            return new ZodSet({
                ...this._def,
                maxSize: { value, message.toString(message) },
            });
        }
        size(size, message) {
    ...
    class ZodFunction extends ZodType {
        ...
        _parse(input) {
            ...
            const params = { errorMap.common.contextualErrorMap };
            const fn = ctx.data;
        ...
        args(...items) {
            return new ZodFunction({
                ...this._def,
                args.create(items).rest(ZodUnknown.create()),
            });
        }
        returns(returnType) {
            return new ZodFunction({
        ...
        static create(args, returns, params) {
            return new ZodFunction({
                args: (args ? args .create([]).rest(ZodUnknown.create())),
    ...
    class ZodLazy extends ZodType {
        ...
        _parse(input) {
            ...
            return lazySchema._parse({ data.data, path.path, parent});
        }
    }
    ...
    class ZodEnum extends ZodType {
        _parse(input) {
            if (typeof input.data !== "string") {
                ...
                addIssueToContext(ctx, {
                    expected.joinValues(expectedValues),
                    received.parsedType,
                    code.invalid_type,
                });
                return INVALID;
    ...
    class ZodPromise extends ZodType {
        ...
        _parse(input) {
            ...
            const promisified = ctx.parsedType === ZodParsedType.promise ? ctx.data .resolve(ctx.data);
        ...
        _parse(input) {
            ...
            if (effect.type === "refinement") {
                ...
                if (ctx.common.async === false) {
                    ...
                    return this._def.schema._parseAsync({ data.data, path.path, parent}).then((inner) => {
                        if (inner.status === "aborted")
                            return INVALID;
                        if (inner.status === "dirty")
                            status.dirty();
            ...
            if (effect.type === "transform") {
                if (ctx.common.async === false) {
                    ...
                    return this._def.schema._parseAsync({ data.data, path.path, parent}).then((base) => {
                        if (!isValid(base))
                            return INVALID;
    ...
    ZodDefault.create = (type, params) => {
        return new ZodDefault({
            innerType,
            typeName.ZodDefault,
            defaultValueparams.default === "function" ? params.default : () => params.default,
            ...processCreateParams(params),
        });
    };
    ...
    ZodCatch.create = (type, params) => {
        return new ZodCatch({
            innerType,
            typeName.ZodCatch,
            catchValueparams.catch === "function" ? params.catch : () => params.catch,
            ...processCreateParams(params),
        });
    };
    ...
    class ZodReadonly extends ZodType {
        _parse(input) {
            ...
            return isAsync(result) ? result.then((data) => freeze(data)) (result);
        }
        unwrap() {
            return this._def.innerType;
        }
    ...
    function cleanParams(params, data) {
        const p = typeof params === "function" ? params(data) params === "string" ? { message} ;
        const p2 = typeof p === "string" ? { message} ;
        return p2;
    }
    ...
    const coerce = {
        string: ((arg) => ZodString.create({ ...arg, coerce})),
        number: ((arg) => ZodNumber.create({ ...arg, coerce})),
        boolean: ((arg) => ZodBoolean.create({
            ...arg,
            coerce,
        })),
        bigint: ((arg) => ZodBigInt.create({ ...arg, coerce})),
        date: ((arg) => ZodDate.create({ ...arg, coerce})),
    };
    const NEVER = INVALID;
    ...
    (function (Bluetooth) {
        Bluetooth.CharacteristicPropertiesSchema = z.lazy(() => z.object({
            ...
            writeWithoutResponse.boolean().optional(),
            write.boolean().optional(),
            notify.boolean().optional(),
            indicate.boolean().optional(),
            authenticatedSignedWrites.boolean().optional(),
            extendedProperties.boolean().optional(),
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
    ...
    (function (Bluetooth) {
        Bluetooth.RequestDeviceInfoSchema = z.lazy(() => z.object({
            id.RequestDeviceSchema,
            name.union([z.string(), z.null()]),
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
    ...
    (function (Bluetooth) {
        Bluetooth.ScanRecordSchema = z.lazy(() => z.object({
            ...
            uuids.array(Bluetooth.BluetoothUuidSchema).optional(),
            appearance.number().optional(),
            manufacturerData.array(Bluetooth.BluetoothManufacturerDataSchema)
                .optional(),
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    z.lazy(() => z.union([
    ...
    (function (Bluetooth) {
        Bluetooth.HandleRequestDevicePromptSchema = z.lazy(() => z.object({
            method.literal('bluetooth.handleRequestDevicePrompt'),
            params.HandleRequestDevicePromptParametersSchema,
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
        Bluetooth.HandleRequestDevicePromptParametersSchema = z.lazy(() => z
            .object({
            context.string(),
            prompt.RequestDevicePromptSchema,
        })
            .and(z.union([
    ...
    (function (Bluetooth) {
        Bluetooth.HandleRequestDevicePromptAcceptParametersSchema = z.lazy(() => z.object({
            accept.literal(true),
            device.RequestDeviceSchema,
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
        Bluetooth.HandleRequestDevicePromptCancelParametersSchema = z.lazy(() => z.object({
            accept.literal(false),
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
        Bluetooth.SimulateAdapterSchema = z.lazy(() => z.object({
            method.literal('bluetooth.simulateAdapter'),
            params.SimulateAdapterParametersSchema,
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
        Bluetooth.SimulateAdapterParametersSchema = z.lazy(() => z.object({
            context.string(),
            leSupported.boolean().optional(),
            state.enum(['absent', 'powered-off', 'powered-on']),
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
        Bluetooth.DisableSimulationSchema = z.lazy(() => z.object({
            method.literal('bluetooth.disableSimulation'),
            params.DisableSimulationParametersSchema,
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
    ...
    (function (Bluetooth) {
        Bluetooth.SimulatePreconnectedPeripheralSchema = z.lazy(() => z.object({
            method.literal('bluetooth.simulatePreconnectedPeripheral'),
            params.SimulatePreconnectedPeripheralParametersSchema,
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
        Bluetooth.SimulatePreconnectedPeripheralParametersSchema = z.lazy(() => z.object({
            ...
            name.string(),
            manufacturerData.array(Bluetooth.BluetoothManufacturerDataSchema),
            knownServiceUuids.array(Bluetooth.BluetoothUuidSchema),
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
        Bluetooth.SimulateAdvertisementSchema = z.lazy(() => z.object({
            method.literal('bluetooth.simulateAdvertisement'),
            params.SimulateAdvertisementParametersSchema,
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
        Bluetooth.SimulateAdvertisementParametersSchema = z.lazy(() => z.object({
            context.string(),
            scanEntry.SimulateAdvertisementScanEntryParametersSchema,
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
        Bluetooth.SimulateAdvertisementScanEntryParametersSchema = z.lazy(() => z.object({
            ...
            rssi.number(),
            scanRecord.ScanRecordSchema,
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
        Bluetooth.SimulateGattConnectionResponseSchema = z.lazy(() => z.object({
            method.literal('bluetooth.simulateGattConnectionResponse'),
            params.SimulateGattConnectionResponseParametersSchema,
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
        Bluetooth.SimulateGattConnectionResponseParametersSchema = z.lazy(() => z.object({
            context.string(),
            address.string(),
            code.number().int().nonnegative(),
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
        Bluetooth.SimulateGattDisconnectionSchema = z.lazy(() => z.object({
            method.literal('bluetooth.simulateGattDisconnection'),
            params.SimulateGattDisconnectionParametersSchema,
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
    ...
    (function (Bluetooth) {
        Bluetooth.SimulateServiceSchema = z.lazy(() => z.object({
            method.literal('bluetooth.simulateService'),
            params.SimulateServiceParametersSchema,
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
        Bluetooth.SimulateServiceParametersSchema = z.lazy(() => z.object({
            context.string(),
            address.string(),
            uuid.BluetoothUuidSchema,
            type.enum(['add', 'remove']),
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
        Bluetooth.SimulateCharacteristicSchema = z.lazy(() => z.object({
            method.literal('bluetooth.simulateCharacteristic'),
            params.SimulateCharacteristicParametersSchema,
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
        Bluetooth.SimulateCharacteristicParametersSchema = z.lazy(() => z.object({
            ...
            characteristicProperties.CharacteristicPropertiesSchema.optional(),
            type.enum(['add', 'remove']),
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
        Bluetooth.SimulateCharacteristicResponseSchema = z.lazy(() => z.object({
            method.literal('bluetooth.simulateCharacteristicResponse'),
            params.SimulateCharacteristicResponseParametersSchema,
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
        Bluetooth.SimulateCharacteristicResponseParametersSchema = z.lazy(() => z.object({
            ...
            code.number().int().nonnegative(),
            data.array(z.number().int().nonnegative()).optional(),
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
        Bluetooth.SimulateDescriptorSchema = z.lazy(() => z.object({
            method.literal('bluetooth.simulateDescriptor'),
            params.SimulateDescriptorParametersSchema,
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
        Bluetooth.SimulateDescriptorParametersSchema = z.lazy(() => z.object({
            ...
            serviceUuid.BluetoothUuidSchema,
            characteristicUuid.BluetoothUuidSchema,
            descriptorUuid.BluetoothUuidSchema,
            type.enum(['add', 'remove']),
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
        Bluetooth.SimulateDescriptorResponseSchema = z.lazy(() => z.object({
            method.literal('bluetooth.simulateDescriptorResponse'),
            params.SimulateDescriptorResponseParametersSchema,
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
    ...
    (function (Bluetooth) {
        Bluetooth.RequestDevicePromptUpdatedSchema = z.lazy(() => z.object({
            method.literal('bluetooth.requestDevicePromptUpdated'),
            params.RequestDevicePromptUpdatedParametersSchema,
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
        Bluetooth.RequestDevicePromptUpdatedParametersSchema = z.lazy(() => z.object({
            context.string(),
            prompt.RequestDevicePromptSchema,
            devices.array(Bluetooth.RequestDeviceInfoSchema),
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
        Bluetooth.GattConnectionAttemptedSchema = z.lazy(() => z.object({
            method.literal('bluetooth.gattConnectionAttempted'),
            params.GattConnectionAttemptedParametersSchema,
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
    ...
    (function (Bluetooth) {
        Bluetooth.CharacteristicEventGeneratedSchema = z.lazy(() => z.object({
            method.literal('bluetooth.characteristicEventGenerated'),
            params.CharacteristicEventGeneratedParametersSchema,
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
    ...
    (function (Bluetooth) {
        Bluetooth.DescriptorEventGeneratedSchema = z.lazy(() => z.object({
            method.literal('bluetooth.descriptorEventGenerated'),
            params.DescriptorEventGeneratedParametersSchema,
        }));
    })(Bluetooth$1 || (Bluetooth$1 = {}));
    (function (Bluetooth) {
    ...
    (function (Permissions) {
        Permissions.SetPermissionSchema = z.lazy(() => z.object({
            method.literal('permissions.setPermission'),
            params.SetPermissionParametersSchema,
        }));
    })(Permissions$1 || (Permissions$1 = {}));
    (function (Permissions) {
    ...
    (function (UserAgentClientHints) {
        UserAgentClientHints.SetClientHintsOverrideCommandSchema = z.lazy(() => z.object({
            method.literal('userAgentClientHints.setClientHintsOverride'),
            params.object({
                clientHints.union([
                    UserAgentClientHints.ClientHintsMetadataSchema,
                    z.null(),
                ]),
                contexts.array(z.string()).min(1).optional(),
                userContexts.array(z.string()).min(1).optional(),
            }),
        }));
    })(UserAgentClientHints || (UserAgentClientHints = {}));
    (function (UserAgentClientHints) {
        UserAgentClientHints.ClientHintsMetadataSchema = z.lazy(() => z.object({
            brands.array(UserAgentClientHints.BrandVersionSchema).optional(),
            fullVersionList.array(UserAgentClientHints.BrandVersionSchema)
                .optional(),
            ...
            formFactors.array(z.string()).optional(),
        }));
    })(UserAgentClientHints || (UserAgentClientHints = {}));
    (function (UserAgentClientHints) {
    ...
    (function (Session) {
        Session.CapabilitiesRequestSchema = z.lazy(() => z.object({
            alwaysMatch.CapabilityRequestSchema.optional(),
            firstMatch.array(Session.CapabilityRequestSchema).optional(),
        }));
    })(Session$1 || (Session$1 = {}));
    (function (Session) {
        Session.CapabilityRequestSchema = z.lazy(() => z
            .object({
            acceptInsecureCerts.boolean().optional(),
            ...
            unhandledPromptBehavior.UserPromptHandlerSchema.optional(),
        })
            .and(ExtensibleSchema));
    })(Session$1 || (Session$1 = {}));
    (function (Session) {
    ...
    (function (Session) {
        Session.ManualProxyConfigurationSchema = z.lazy(() => z
            .object({
            proxyType.literal('manual'),
            httpProxy.string().optional(),
            sslProxy.string().optional(),
        })
            .and(Session.SocksProxyConfigurationSchema.or(z.object({})))
            .and(z.object({
            noProxy.array(z.string()).optional(),
        }))
            .and(ExtensibleSchema));
    })(Session$1 || (Session$1 = {}));
    (function (Session) {
        Session.SocksProxyConfigurationSchema = z.lazy(() => z.object({
            socksProxy.string(),
            socksVersion.number().int().nonnegative().gte(0).lte(255),
        }));
    })(Session$1 || (Session$1 = {}));
    (function (Session) {
        Session.PacProxyConfigurationSchema = z.lazy(() => z
            .object({
            proxyType.literal('pac'),
    ...
    (function (Session) {
        Session.SystemProxyConfigurationSchema = z.lazy(() => z
            .object({
            proxyType.literal('system'),
        })
            .and(ExtensibleSchema));
    })(Session$1 || (Session$1 = {}));
    (function (Session) {
        Session.UserPromptHandlerSchema = z.lazy(() => z.object({
            alert.UserPromptHandlerTypeSchema.optional(),
            beforeUnload.UserPromptHandlerTypeSchema.optional(),
            confirm.UserPromptHandlerTypeSchema.optional(),
            default.UserPromptHandlerTypeSchema.optional(),
            file.UserPromptHandlerTypeSchema.optional(),
            prompt.UserPromptHandlerTypeSchema.optional(),
        }));
    })(Session$1 || (Session$1 = {}));
    (function (Session) {
    ...
    (function (Session) {
        Session.SubscribeParametersSchema = z.lazy(() => z.object({
            events.array(z.string()).min(1),
            contexts.array(BrowsingContext$1.BrowsingContextSchema)
                .min(1)
                .optional(),
            userContexts.array(Browser$1.UserContextSchema).min(1).optional(),
        }));
    })(Session$1 || (Session$1 = {}));
    (function (Session) {
        Session.UnsubscribeByIdRequestSchema = z.lazy(() => z.object({
            subscriptions.array(Session.SubscriptionSchema).min(1),
        }));
    })(Session$1 || (Session$1 = {}));
    (function (Session) {
    ...
    (function (Session) {
        Session.NewSchema = z.lazy(() => z.object({
            ...
            params.NewParametersSchema,
        }));
    })(Session$1 || (Session$1 = {}));
    (function (Session) {
        Session.NewParametersSchema = z.lazy(() => z.object({
            capabilities.CapabilitiesRequestSchema,
        }));
    })(Session$1 || (Session$1 = {}));
    (function (Session) {
    ...
    (function (Session) {
        Session.SubscribeSchema = z.lazy(() => z.object({
            ...
            params.SubscribeParametersSchema,
        }));
    })(Session$1 || (Session$1 = {}));
    (function (Session) {
    ...
    (function (Session) {
        Session.UnsubscribeSchema = z.lazy(() => z.object({
            ...
            params.UnsubscribeParametersSchema,
        }));
    })(Session$1 || (Session$1 = {}));
    (function (Session) {
    ...
    (function (Browser) {
        Browser.ClientWindowInfoSchema = z.lazy(() => z.object({
            active.boolean(),
            clientWindow.ClientWindowSchema,
            height,
            state.enum(['fullscreen', 'maximized', 'minimized', 'normal']),
            width,
            x,
            y,
        }));
    ...
    (function (Browser) {
        Browser.CreateUserContextSchema = z.lazy(() => z.object({
            ...
            params.CreateUserContextParametersSchema,
        }));
    })(Browser$1 || (Browser$1 = {}));
    (function (Browser) {
        Browser.CreateUserContextParametersSchema = z.lazy(() => z.object({
            acceptInsecureCerts.boolean().optional(),
            proxy$1.ProxyConfigurationSchema.optional(),
            unhandledPromptBehavior$1.UserPromptHandlerSchema.optional(),
        }));
    })(Browser$1 || (Browser$1 = {}));
    (function (Browser) {
    ...
    (function (Browser) {
        Browser.GetClientWindowsResultSchema = z.lazy(() => z.object({
            clientWindows.array(Browser.ClientWindowInfoSchema),
        }));
    })(Browser$1 || (Browser$1 = {}));
    (function (Browser) {
    ...
    (function (Browser) {
        Browser.GetUserContextsResultSchema = z.lazy(() => z.object({
            userContexts.array(Browser.UserContextInfoSchema).min(1),
        }));
    })(Browser$1 || (Browser$1 = {}));
    (function (Browser) {
        Browser.RemoveUserContextSchema = z.lazy(() => z.object({
            method.literal('browser.removeUserContext'),
            params.RemoveUserContextParametersSchema,
        }));
    })(Browser$1 || (Browser$1 = {}));
    (function (Browser) {
    ...
    (function (Browser) {
        Browser.SetClientWindowStateSchema = z.lazy(() => z.object({
            ...
            params.SetClientWindowStateParametersSchema,
        }));
    })(Browser$1 || (Browser$1 = {}));
    (function (Browser) {
    ...
    (function (Browser) {
        Browser.ClientWindowNamedStateSchema = z.lazy(() => z.object({
            state.enum(['fullscreen', 'maximized', 'minimized']),
        }));
    })(Browser$1 || (Browser$1 = {}));
    (function (Browser) {
    ...
    (function (Browser) {
        Browser.SetDownloadBehaviorSchema = z.lazy(() => z.object({
            ...
            params.SetDownloadBehaviorParametersSchema,
        }));
    })(Browser$1 || (Browser$1 = {}));
    (function (Browser) {
        Browser.SetDownloadBehaviorParametersSchema = z.lazy(() => z.object({
            downloadBehavior.union([Browser.DownloadBehaviorSchema, z.null()]),
            userContexts.array(Browser.UserContextSchema).min(1).optional(),
        }));
    })(Browser$1 || (Browser$1 = {}));
    (function (Browser) {
    ...
    (function (BrowsingContext) {
        BrowsingContext.InfoSchema = z.lazy(() => z.object({
            children.union([BrowsingContext.InfoListSchema, z.null()]),
            clientWindow$1.ClientWindowSchema,
            context.BrowsingContextSchema,
            originalOpener.union([
                BrowsingContext.BrowsingContextSchema,
                z.null(),
            ]),
            url.string(),
            userContext$1.UserContextSchema,
            parent.union([BrowsingContext.BrowsingContextSchema, z.null()])
                .optional(),
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
    ...
    (function (BrowsingContext) {
        BrowsingContext.InnerTextLocatorSchema = z.lazy(() => z.object({
            ...
            matchType.enum(['full', 'partial']).optional(),
            maxDepth.optional(),
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
    ...
    (function (BrowsingContext) {
        BrowsingContext.BaseNavigationInfoSchema = z.lazy(() => z.object({
            context.BrowsingContextSchema,
            navigation.union([BrowsingContext.NavigationSchema, z.null()]),
            timestamp,
            url.string(),
            userContext$1.UserContextSchema.optional(),
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
    ...
    (function (BrowsingContext) {
        BrowsingContext.ActivateSchema = z.lazy(() => z.object({
            ...
            params.ActivateParametersSchema,
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
    ...
    (function (BrowsingContext) {
        BrowsingContext.CaptureScreenshotSchema = z.lazy(() => z.object({
            ...
            params.CaptureScreenshotParametersSchema,
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
        BrowsingContext.CaptureScreenshotParametersSchema = z.lazy(() => z.object({
            context.BrowsingContextSchema,
            origin.enum(['viewport', 'document']).default('viewport').optional(),
            format.ImageFormatSchema.optional(),
            clip.ClipRectangleSchema.optional(),
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
        BrowsingContext.ImageFormatSchema = z.lazy(() => z.object({
            type.string(),
            quality.number().gte(0).lte(1).optional(),
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
    ...
    (function (BrowsingContext) {
        BrowsingContext.ElementClipRectangleSchema = z.lazy(() => z.object({
            ...
            element$1.SharedReferenceSchema,
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
    ...
    (function (BrowsingContext) {
        BrowsingContext.CloseSchema = z.lazy(() => z.object({
            ...
            params.CloseParametersSchema,
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
        BrowsingContext.CloseParametersSchema = z.lazy(() => z.object({
            context.BrowsingContextSchema,
            promptUnload.boolean().default(false).optional(),
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
    ...
    (function (BrowsingContext) {
        BrowsingContext.CreateParametersSchema = z.lazy(() => z.object({
            ...
            referenceContext.BrowsingContextSchema.optional(),
            background.boolean().default(false).optional(),
            userContext$1.UserContextSchema.optional(),
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
    ...
    (function (BrowsingContext) {
        BrowsingContext.LocateNodesSchema = z.lazy(() => z.object({
            ...
            params.LocateNodesParametersSchema,
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
        BrowsingContext.LocateNodesParametersSchema = z.lazy(() => z.object({
            ...
            maxNodeCount.gte(1).optional(),
            serializationOptions$1.SerializationOptionsSchema.optional(),
            startNodes.array(Script$1.SharedReferenceSchema).min(1).optional(),
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
        BrowsingContext.LocateNodesResultSchema = z.lazy(() => z.object({
            nodes.array(Script$1.NodeRemoteValueSchema),
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
        BrowsingContext.NavigateSchema = z.lazy(() => z.object({
            method.literal('browsingContext.navigate'),
            params.NavigateParametersSchema,
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
    ...
    (function (BrowsingContext) {
        BrowsingContext.PrintParametersSchema = z.lazy(() => z.object({
            ...
            pageRanges.array(z.union([JsUintSchema, z.string()])).optional(),
            scale.number().gte(0.1).lte(2).default(1).optional(),
            shrinkToFit.boolean().default(true).optional(),
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
        BrowsingContext.PrintMarginParametersSchema = z.lazy(() => z.object({
            bottom.number().gte(0).default(1).optional(),
            left.number().gte(0).default(1).optional(),
            right.number().gte(0).default(1).optional(),
            top.number().gte(0).default(1).optional(),
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
        BrowsingContext.PrintPageParametersSchema = z.lazy(() => z.object({
            height.number().gte(0.0352).default(27.94).optional(),
            width.number().gte(0.0352).default(21.59).optional(),
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
    ...
    (function (BrowsingContext) {
        BrowsingContext.SetBypassCspSchema = z.lazy(() => z.object({
            ...
            params.SetBypassCspParametersSchema,
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
        BrowsingContext.SetBypassCspParametersSchema = z.lazy(() => z.object({
            bypass.union([z.literal(true), z.null()]),
            contexts.array(BrowsingContext.BrowsingContextSchema)
                .min(1)
                .optional(),
            userContexts.array(Browser$1.UserContextSchema).min(1).optional(),
        }));
    ...
    (function (BrowsingContext) {
        BrowsingContext.SetViewportSchema = z.lazy(() => z.object({
            ...
            params.SetViewportParametersSchema,
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
        BrowsingContext.SetViewportParametersSchema = z.lazy(() => z.object({
            context.BrowsingContextSchema.optional(),
            viewport.union([BrowsingContext.ViewportSchema, z.null()]).optional(),
            devicePixelRatio.union([z.number().gt(0), z.null()]).optional(),
            userContexts.array(Browser$1.UserContextSchema).min(1).optional(),
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
    ...
    (function (BrowsingContext) {
        BrowsingContext.TraverseHistorySchema = z.lazy(() => z.object({
            ...
            params.TraverseHistoryParametersSchema,
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
    ...
    (function (BrowsingContext) {
        BrowsingContext.HistoryUpdatedSchema = z.lazy(() => z.object({
            ...
            params.HistoryUpdatedParametersSchema,
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
    ...
    (function (BrowsingContext) {
        BrowsingContext.DownloadWillBeginSchema = z.lazy(() => z.object({
            ...
            params.DownloadWillBeginParamsSchema,
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
    ...
    (function (BrowsingContext) {
        BrowsingContext.DownloadEndSchema = z.lazy(() => z.object({
            ...
            params.DownloadEndParamsSchema,
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
    ...
    (function (BrowsingContext) {
        BrowsingContext.DownloadCompleteParamsSchema = z.lazy(() => z
            ...
            filepath.union([z.string(), z.null()]),
        })
            .and(BrowsingContext.BaseNavigationInfoSchema));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
    ...
    (function (BrowsingContext) {
        BrowsingContext.UserPromptClosedSchema = z.lazy(() => z.object({
            ...
            params.UserPromptClosedParametersSchema,
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
    ...
    (function (BrowsingContext) {
        BrowsingContext.UserPromptOpenedSchema = z.lazy(() => z.object({
            ...
            params.UserPromptOpenedParametersSchema,
        }));
    })(BrowsingContext$1 || (BrowsingContext$1 = {}));
    (function (BrowsingContext) {
        BrowsingContext.UserPromptOpenedParametersSchema = z.lazy(() => z.object({
            context.BrowsingContextSchema,
            handler$1.UserPromptHandlerTypeSchema,
            message.string(),
            type.UserPromptTypeSchema,
            userContext$1.UserContextSchema.optional(),
    ...
    (function (Emulation) {
        Emulation.SetForcedColorsModeThemeOverrideSchema = z.lazy(() => z.object({
            ...
            params.SetForcedColorsModeThemeOverrideParametersSchema,
        }));
    })(Emulation$1 || (Emulation$1 = {}));
    (function (Emulation) {
        Emulation.SetForcedColorsModeThemeOverrideParametersSchema = z.lazy(() => z.object({
            theme.union([Emulation.ForcedColorsModeThemeSchema, z.null()]),
            contexts.array(BrowsingContext$1.BrowsingContextSchema)
                .min(1)
                .optional(),
            userContexts.array(Browser$1.UserContextSchema).min(1).optional(),
    ...
    (function (Emulation) {
        Emulation.SetGeolocationOverrideSchema = z.lazy(() => z.object({
            ...
            params.SetGeolocationOverrideParametersSchema,
        }));
    })(Emulation$1 || (Emulation$1 = {}));
    (function (Emulation) {
        Emulation.SetGeolocationOverrideParametersSchema = z.lazy(() => z
            ...
            z.object({
                error.GeolocationPositionErrorSchema,
            }),
        ])
            .and(z.object({
            contexts.array(BrowsingContext$1.BrowsingContextSchema)
    ...
    (function (Emulation) {
        Emulation.GeolocationCoordinatesSchema = z.lazy(() => z.object({
            latitude.number().gte(-90).lte(90),
            longitude.number().gte(-180).lte(180),
            accuracy.number().gte(0).default(1).optional(),
            altitude.union([z.number(), z.null().default(null)]).optional(),
            altitudeAccuracy.union([z.number().gte(0), z.null().default(null)])
                .optional(),
            heading.union([z.number().gt(0).lt(360), z.null().default(null)])
                .optional(),
            speed.union([z.number().gte(0), z.null().default(null)]).optional(),
        }));
    })(Emulation$1 || (Emulation$1 = {}));
    (function (Emulation) {
    ...
    (function (Emulation) {
        Emulation.SetLocaleOverrideSchema = z.lazy(() => z.object({
            ...
            params.SetLocaleOverrideParametersSchema,
        }));
    })(Emulation$1 || (Emulation$1 = {}));
    (function (Emulation) {
        Emulation.SetLocaleOverrideParametersSchema = z.lazy(() => z.object({
            locale.union([z.string(), z.null()]),
            contexts.array(BrowsingContext$1.BrowsingContextSchema)
                .min(1)
                .optional(),
            userContexts.array(Browser$1.UserContextSchema).min(1).optional(),
    ...
    (function (Emulation) {
        Emulation.SetNetworkConditionsSchema = z.lazy(() => z.object({
            ...
            params.SetNetworkConditionsParametersSchema,
        }));
    })(Emulation$1 || (Emulation$1 = {}));
    (function (Emulation) {
        Emulation.SetNetworkConditionsParametersSchema = z.lazy(() => z.object({
            networkConditions.union([Emulation.NetworkConditionsSchema, z.null()]),
            contexts.array(BrowsingContext$1.BrowsingContextSchema)
                .min(1)
                .optional(),
            userContexts.array(Browser$1.UserContextSchema).min(1).optional(),
    ...
    (function (Emulation) {
        Emulation.SetScreenSettingsOverrideSchema = z.lazy(() => z.object({
            ...
            params.SetScreenSettingsOverrideParametersSchema,
        }));
    })(Emulation$1 || (Emulation$1 = {}));
    (function (Emulation) {
    ...
    (function (Emulation) {
        Emulation.SetScreenSettingsOverrideParametersSchema = z.lazy(() => z.object({
            screenArea.union([Emulation.ScreenAreaSchema, z.null()]),
            contexts.array(BrowsingContext$1.BrowsingContextSchema)
                .min(1)
                .optional(),
            userContexts.array(Browser$1.UserContextSchema).min(1).optional(),
    ...
    (function (Emulation) {
        Emulation.SetScreenOrientationOverrideSchema = z.lazy(() => z.object({
            ...
            params.SetScreenOrientationOverrideParametersSchema,
        }));
    })(Emulation$1 || (Emulation$1 = {}));
    (function (Emulation) {
    ...
    (function (Emulation) {
        Emulation.ScreenOrientationSchema = z.lazy(() => z.object({
            natural.ScreenOrientationNaturalSchema,
            type.ScreenOrientationTypeSchema,
        }));
    })(Emulation$1 || (Emulation$1 = {}));
    (function (Emulation) {
        Emulation.SetScreenOrientationOverrideParametersSchema = z.lazy(() => z.object({
            screenOrientation.union([Emulation.ScreenOrientationSchema, z.null()]),
            contexts.array(BrowsingContext$1.BrowsingContextSchema)
                .min(1)
                .optional(),
            userContexts.array(Browser$1.UserContextSchema).min(1).optional(),
    ...
    (function (Emulation) {
        Emulation.SetUserAgentOverrideSchema = z.lazy(() => z.object({
            ...
            params.SetUserAgentOverrideParametersSchema,
        }));
    })(Emulation$1 || (Emulation$1 = {}));
    (function (Emulation) {
        Emulation.SetUserAgentOverrideParametersSchema = z.lazy(() => z.object({
            userAgent.union([z.string(), z.null()]),
            contexts.array(BrowsingContext$1.BrowsingContextSchema)
                .min(1)
                .optional(),
            userContexts.array(Browser$1.UserContextSchema).min(1).optional(),
    ...
    (function (Emulation) {
        Emulation.SetScriptingEnabledSchema = z.lazy(() => z.object({
            ...
            params.SetScriptingEnabledParametersSchema,
        }));
    })(Emulation$1 || (Emulation$1 = {}));
    (function (Emulation) {
        Emulation.SetScriptingEnabledParametersSchema = z.lazy(() => z.object({
            enabled.union([z.literal(false), z.null()]),
            contexts.array(BrowsingContext$1.BrowsingContextSchema)
                .min(1)
                .optional(),
            userContexts.array(Browser$1.UserContextSchema).min(1).optional(),
    ...
    (function (Emulation) {
        Emulation.SetScrollbarTypeOverrideSchema = z.lazy(() => z.object({
            ...
            params.SetScrollbarTypeOverrideParametersSchema,
        }));
    })(Emulation$1 || (Emulation$1 = {}));
    (function (Emulation) {
    ...
    (function (Emulation) {
        Emulation.SetTimezoneOverrideSchema = z.lazy(() => z.object({
            ...
            params.SetTimezoneOverrideParametersSchema,
        }));
    })(Emulation$1 || (Emulation$1 = {}));
    (function (Emulation) {
        Emulation.SetTimezoneOverrideParametersSchema = z.lazy(() => z.object({
            timezone.union([z.string(), z.null()]),
            contexts.array(BrowsingContext$1.BrowsingContextSchema)
                .min(1)
                .optional(),
            userContexts.array(Browser$1.UserContextSchema).min(1).optional(),
    ...
    (function (Emulation) {
        Emulation.SetTouchOverrideSchema = z.lazy(() => z.object({
            ...
            params.SetTouchOverrideParametersSchema,
        }));
    })(Emulation$1 || (Emulation$1 = {}));
    (function (Emulation) {
        Emulation.SetTouchOverrideParametersSchema = z.lazy(() => z.object({
            maxTouchPoints.union([JsUintSchema.gte(1), z.null()]),
            contexts.array(BrowsingContext$1.BrowsingContextSchema)
                .min(1)
                .optional(),
            userContexts.array(Browser$1.UserContextSchema).min(1).optional(),
    ...
    (function (Network) {
        Network.BaseParametersSchema = z.lazy(() => z.object({
            context.union([BrowsingContext$1.BrowsingContextSchema, z.null()]),
            isBlocked.boolean(),
            navigation.union([BrowsingContext$1.NavigationSchema, z.null()]),
            redirectCount,
            request.RequestDataSchema,
            timestamp,
            userContext.union([Browser$1.UserContextSchema, z.null()]).optional(),
            intercepts.array(Network.InterceptSchema).min(1).optional(),
        }));
    })(Network$1 || (Network$1 = {}));
    (function (Network) {
    ...
    (function (Network) {
        Network.InitiatorSchema = z.lazy(() => z.object({
            ...
            stackTrace$1.StackTraceSchema.optional(),
            type.enum(['parser', 'script', 'preflight', 'other']).optional(),
        }));
    })(Network$1 || (Network$1 = {}));
    (function (Network) {
    ...
    (function (Network) {
        Network.RequestDataSchema = z.lazy(() => z.object({
            ...
            url.string(),
            method.string(),
            headers.array(Network.HeaderSchema),
            cookies.array(Network.CookieSchema),
            headersSize,
            bodySize.union([JsUintSchema, z.null()]),
            destination.string(),
            initiatorType.union([z.string(), z.null()]),
            timings.FetchTimingInfoSchema,
        }));
    })(Network$1 || (Network$1 = {}));
    (function (Network) {
    ...
    (function (Network) {
        Network.ResponseDataSchema = z.lazy(() => z.object({
            ...
            mimeType.string(),
            bytesReceived,
            headersSize.union([JsUintSchema, z.null()]),
            bodySize.union([JsUintSchema, z.null()]),
            content.ResponseContentSchema,
            authChallenges.array(Network.AuthChallengeSchema).optional(),
        }));
    })(Network$1 || (Network$1 = {}));
    (function (Network) {
    ...
    (function (Network) {
        Network.AddDataCollectorSchema = z.lazy(() => z.object({
            ...
            params.AddDataCollectorParametersSchema,
        }));
    })(Network$1 || (Network$1 = {}));
    (function (Network) {
        Network.AddDataCollectorParametersSchema = z.lazy(() => z.object({
            dataTypes.array(Network.DataTypeSchema).min(1),
            maxEncodedDataSize,
            collectorType.CollectorTypeSchema.default('blob').optional(),
            contexts.array(BrowsingContext$1.BrowsingContextSchema)
                .min(1)
                .optional(),
            userContexts.array(Browser$1.UserContextSchema).min(1).optional(),
    ...
    (function (Network) {
        Network.AddInterceptSchema = z.lazy(() => z.object({
            ...
            params.AddInterceptParametersSchema,
        }));
    })(Network$1 || (Network$1 = {}));
    (function (Network) {
        Network.AddInterceptParametersSchema = z.lazy(() => z.object({
            phases.array(Network.InterceptPhaseSchema).min(1),
            contexts.array(BrowsingContext$1.BrowsingContextSchema)
                .min(1)
                .optional(),
            urlPatterns.array(Network.UrlPatternSchema).optional(),
        }));
    })(Network$1 || (Network$1 = {}));
    (function (Network) {
    ...
    (function (Network) {
        Network.ContinueRequestSchema = z.lazy(() => z.object({
            ...
            params.ContinueRequestParametersSchema,
        }));
    })(Network$1 || (Network$1 = {}));
    (function (Network) {
        Network.ContinueRequestParametersSchema = z.lazy(() => z.object({
            request.RequestSchema,
            body.BytesValueSchema.optional(),
            cookies.array(Network.CookieHeaderSchema).optional(),
            headers.array(Network.HeaderSchema).optional(),
    ...
    (function (Network) {
        Network.ContinueResponseSchema = z.lazy(() => z.object({
            ...
            params.ContinueResponseParametersSchema,
        }));
    })(Network$1 || (Network$1 = {}));
    (function (Network) {
        Network.ContinueResponseParametersSchema = z.lazy(() => z.object({
            request.RequestSchema,
            cookies.array(Network.SetCookieHeaderSchema).optional(),
    ...
    (function (Network) {
        Network.ContinueWithAuthSchema = z.lazy(() => z.object({
            ...
            params.ContinueWithAuthParametersSchema,
        }));
    })(Network$1 || (Network$1 = {}));
    (function (Network) {
    ...
    (function (Network) {
        Network.DisownDataSchema = z.lazy(() => z.object({
            ...
            params.DisownDataParametersSchema,
        }));
    })(Network$1 || (Network$1 = {}));
    (function (Network) {
    ...
    (function (Network) {
        Network.FailRequestSchema = z.lazy(() => z.object({
            ...
            params.FailRequestParametersSchema,
        }));
    })(Network$1 || (Network$1 = {}));
    (function (Network) {
    ...
    (function (Network) {
        Network.GetDataSchema = z.lazy(() => z.object({
            ...
            params.GetDataParametersSchema,
        }));
    })(Network$1 || (Network$1 = {}));
    (function (Network) {
    ...
    (function (Network) {
        Network.RemoveDataCollectorSchema = z.lazy(() => z.object({
            ...
            params.RemoveDataCollectorParametersSchema,
        }));
    })(Network$1 || (Network$1 = {}));
    (function (Network) {
    ...
    (function (Network) {
        Network.RemoveInterceptSchema = z.lazy(() => z.object({
            ...
            params.RemoveInterceptParametersSchema,
        }));
    })(Network$1 || (Network$1 = {}));
    (function (Network) {
    ...
    (function (Network) {
        Network.SetCacheBehaviorSchema = z.lazy(() => z.object({
            ...
            params.SetCacheBehaviorParametersSchema,
        }));
    })(Network$1 || (Network$1 = {}));
    (function (Network) {
        Network.SetCacheBehaviorParametersSchema = z.lazy(() => z.object({
            cacheBehavior.enum(['default', 'bypass']),
            contexts.array(BrowsingContext$1.BrowsingContextSchema)
                .min(1)
                .optional(),
        }));
    ...
    (function (Network) {
        Network.SetExtraHeadersSchema = z.lazy(() => z.object({
            ...
            params.SetExtraHeadersParametersSchema,
        }));
    })(Network$1 || (Network$1 = {}));
    (function (Network) {
    ...
    (function (Network) {
        Network.AuthRequiredSchema = z.lazy(() => z.object({
            ...
            params.AuthRequiredParametersSchema,
        }));
    })(Network$1 || (Network$1 = {}));
    (function (Network) {
    ...
    (function (Network) {
        Network.BeforeRequestSentSchema = z.lazy(() => z.object({
            ...
            params.BeforeRequestSentParametersSchema,
        }));
    })(Network$1 || (Network$1 = {}));
    (function (Network) {
    ...
    (function (Network) {
        Network.FetchErrorSchema = z.lazy(() => z.object({
            ...
            params.FetchErrorParametersSchema,
        }));
    })(Network$1 || (Network$1 = {}));
    (function (Network) {
    ...
    (function (Network) {
        Network.ResponseCompletedSchema = z.lazy(() => z.object({
            ...
            params.ResponseCompletedParametersSchema,
        }));
    })(Network$1 || (Network$1 = {}));
    (function (Network) {
    ...
    (function (Network) {
        Network.ResponseStartedSchema = z.lazy(() => z.object({
            ...
            params.ResponseStartedParametersSchema,
        }));
    })(Network$1 || (Network$1 = {}));
    (function (Network) {
    ...
    (function (Script) {
        Script.EvaluateResultExceptionSchema = z.lazy(() => z.object({
            ...
            exceptionDetails.ExceptionDetailsSchema,
            realm.RealmSchema,
        }));
    })(Script$1 || (Script$1 = {}));
    (function (Script) {
    ...
    (function (Script) {
        Script.NumberValueSchema = z.lazy(() => z.object({
            type.literal('number'),
            value.union([z.number(), Script.SpecialNumberSchema]),
        }));
    })(Script$1 || (Script$1 = {}));
    (function (Script) {
    ...
    (function (Script) {
        Script.WindowRealmInfoSchema = z.lazy(() => Script.BaseRealmInfoSchema.and(z.object({
            type.literal('window'),
            context$1.BrowsingContextSchema,
            userContext$1.UserContextSchema.optional(),
    ...
    (function (Script) {
        Script.DedicatedWorkerRealmInfoSchema = z.lazy(() => Script.BaseRealmInfoSchema.and(z.object({
            ...
            owners.tuple([Script.RealmSchema]),
        })));
    })(Script$1 || (Script$1 = {}));
    (function (Script) {
    ...
    (function (Script) {
        Script.RemoteObjectReferenceSchema = z.lazy(() => z
            .object({
            handle.HandleSchema,
            sharedId.SharedIdSchema.optional(),
        })
            .and(ExtensibleSchema));
    })(Script$1 || (Script$1 = {}));
    (function (Script) {
    ...
    (function (Script) {
        Script.ArrayRemoteValueSchema = z.lazy(() => z.object({
            ...
            handle.HandleSchema.optional(),
            internalId.InternalIdSchema.optional(),
            value.ListRemoteValueSchema.optional(),
        }));
    })(Script$1 || (Script$1 = {}));
    (function (Script) {
        Script.ObjectRemoteValueSchema = z.lazy(() => z.object({
            ...
            handle.HandleSchema.optional(),
            internalId.InternalIdSchema.optional(),
            value.MappingRemoteValueSchema.optional(),
        }));
    })(Script$1 || (Script$1 = {}));
    (function (Script) {
    ...
    (function (Script) {
        Script.NodeRemoteValueSchema = z.lazy(() => z.object({
            ...
            handle.HandleSchema.optional(),
            internalId.InternalIdSchema.optional(),
            value.NodePropertiesSchema.optional(),
        }));
    })(Script$1 || (Script$1 = {}));
    (function (Script) {
        Script.NodePropertiesSchema = z.lazy(() => z.object({
            nodeType,
            childNodeCount,
            attributes.record(z.string(), z.string()).optional(),
            children.array(Script.NodeRemoteValueSchema).optional(),
            localName.string().optional(),
            mode.enum(['open', 'closed']).optional(),
            namespaceURI.string().optional(),
            nodeValue.string().optional(),
            shadowRoot.union([Script.NodeRemoteValueSchema, z.null()]).optional(),
        }));
    })(Script$1 || (Script$1 = {}));
    (function (Script) {
        Script.WindowProxyRemoteValueSchema = z.lazy(() => z.object({
            type.literal('window'),
            value.WindowProxyPropertiesSchema,
            handle.HandleSchema.optional(),
            internalId.InternalIdSchema.optional(),
        }));
    })(Script$1 || (Script$1 = {}));
    ...
    (function (Script) {
        Script.SerializationOptionsSchema = z.lazy(() => z.object({
            maxDomDepth.union([JsUintSchema, z.null()]).default(0).optional(),
            maxObjectDepth.union([JsUintSchema, z.null()])
                .default(null)
                .optional(),
            includeShadowTree.enum(['none', 'open', 'all'])
                .default('none')
                .optional(),
        }));
    })(Script$1 || (Script$1 = {}));
    ...
    (function (Script) {
        Script.StackTraceSchema = z.lazy(() => z.object({
            callFrames.array(Script.StackFrameSchema),
        }));
    })(Script$1 || (Script$1 = {}));
    (function (Script) {
        Script.SourceSchema = z.lazy(() => z.object({
            realm.RealmSchema,
            context$1.BrowsingContextSchema.optional(),
            userContext$1.UserContextSchema.optional(),
        }));
    })(Script$1 || (Script$1 = {}));
    (function (Script) {
    ...
    (function (Script) {
        Script.AddPreloadScriptSchema = z.lazy(() => z.object({
            ...
            params.AddPreloadScriptParametersSchema,
        }));
    })(Script$1 || (Script$1 = {}));
    (function (Script) {
        Script.AddPreloadScriptParametersSchema = z.lazy(() => z.object({
            functionDeclaration.string(),
            arguments.array(Script.ChannelValueSchema).optional(),
            contexts.array(BrowsingContext$1.BrowsingContextSchema)
                .min(1)
                .optional(),
            userContexts.array(Browser$1.UserContextSchema).min(1).optional(),
    ...
    (function (Script) {
        Script.DisownSchema = z.lazy(() => z.object({
            ...
            params.DisownParametersSchema,
        }));
    })(Script$1 || (Script$1 = {}));
    (function (Script) {
        Script.DisownParametersSchema = z.lazy(() => z.object({
            handles.array(Script.HandleSchema),
            target.TargetSchema,
        }));
    })(Script$1 || (Script$1 = {}));
    (function (Script) {
    ...
    (function (Script) {
        Script.CallFunctionSchema = z.lazy(() => z.object({
            ...
            params.CallFunctionParametersSchema,
        }));
    })(Script$1 || (Script$1 = {}));
    (function (Script) {
        Script.CallFunctionParametersSchema = z.lazy(() => z.object({
            ...
            target.TargetSchema,
            arguments.array(Script.LocalValueSchema).optional(),
            resultOwnership.ResultOwnershipSchema.optional(),
            serializationOptions.SerializationOptionsSchema.optional(),
            this.LocalValueSchema.optional(),
            userActivation.boolean().default(false).optional(),
        }));
    })(Script$1 || (Script$1 = {}));
    (function (Script) {
    ...
    (function (Script) {
        Script.EvaluateSchema = z.lazy(() => z.object({
            ...
            params.EvaluateParametersSchema,
        }));
    })(Script$1 || (Script$1 = {}));
    (function (Script) {
    ...
    (function (Script) {
        Script.GetRealmsSchema = z.lazy(() => z.object({
            ...
            params.GetRealmsParametersSchema,
        }));
    })(Script$1 || (Script$1 = {}));
    (function (Script) {
    ...
    (function (Script) {
        Script.MessageSchema = z.lazy(() => z.object({
            ...
            params.MessageParametersSchema,
        }));
    })(Script$1 || (Script$1 = {}));
    (function (Script) {
        Script.MessageParametersSchema = z.lazy(() => z.object({
            channel.ChannelSchema,
            data.RemoteValueSchema,
            source.SourceSchema,
        }));
    })(Script$1 || (Script$1 = {}));
    (function (Script) {
    ...
    (function (Storage) {
        Storage.GetCookiesSchema = z.lazy(() => z.object({
            ...
            params.GetCookiesParametersSchema,
        }));
    })(Storage$1 || (Storage$1 = {}));
    (function (Storage) {
        Storage.CookieFilterSchema = z.lazy(() => z
            .object({
            name.string().optional(),
            value$1.BytesValueSchema.optional(),
            domain.string().optional(),
            path.string().optional(),
            size.optional(),
            httpOnly.boolean().optional(),
            secure.boolean().optional(),
            sameSite$1.SameSiteSchema.optional(),
            expiry.optional(),
        })
            .and(ExtensibleSchema));
    })(Storage$1 || (Storage$1 = {}));
    ...
    (function (Storage) {
        Storage.GetCookiesParametersSchema = z.lazy(() => z.object({
            filter.CookieFilterSchema.optional(),
            partition.PartitionDescriptorSchema.optional(),
        }));
    })(Storage$1 || (Storage$1 = {}));
    (function (Storage) {
        Storage.GetCookiesResultSchema = z.lazy(() => z.object({
            cookies.array(Network$1.CookieSchema),
    ...
    (function (Storage) {
        Storage.SetCookieSchema = z.lazy(() => z.object({
            ...
            params.SetCookieParametersSchema,
        }));
    })(Storage$1 || (Storage$1 = {}));
    (function (Storage) {
    ...
    (function (Storage) {
        Storage.DeleteCookiesSchema = z.lazy(() => z.object({
            ...
            params.DeleteCookiesParametersSchema,
        }));
    })(Storage$1 || (Storage$1 = {}));
    (function (Storage) {
    ...
    (function (Log) {
        Log.BaseLogEntrySchema = z.lazy(() => z.object({
            level.LevelSchema,
            source$1.SourceSchema,
            text.union([z.string(), z.null()]),
            timestamp,
            stackTrace$1.StackTraceSchema.optional(),
        }));
    })(Log || (Log = {}));
    ...
    (function (Log) {
        Log.ConsoleLogEntrySchema = z.lazy(() => Log.BaseLogEntrySchema.and(z.object({
            ...
            method.string(),
            args.array(Script$1.RemoteValueSchema),
        })));
    })(Log || (Log = {}));
    (function (Log) {
    ...
    (function (Input) {
        Input.PerformActionsSchema = z.lazy(() => z.object({
            ...
            params.PerformActionsParametersSchema,
        }));
    })(Input$1 || (Input$1 = {}));
    (function (Input) {
        Input.PerformActionsParametersSchema = z.lazy(() => z.object({
            context$1.BrowsingContextSchema,
            actions.array(Input.SourceActionsSchema),
        }));
    })(Input$1 || (Input$1 = {}));
    (function (Input) {
    ...
    (function (Input) {
        Input.NoneSourceActionsSchema = z.lazy(() => z.object({
            type.literal('none'),
            id.string(),
            actions.array(Input.NoneSourceActionSchema),
        }));
    })(Input$1 || (Input$1 = {}));
    (function (Input) {
    ...
    (function (Input) {
        Input.KeySourceActionsSchema = z.lazy(() => z.object({
            type.literal('key'),
            id.string(),
            actions.array(Input.KeySourceActionSchema),
        }));
    })(Input$1 || (Input$1 = {}));
    (function (Input) {
    ...
    (function (Input) {
        Input.PointerSourceActionsSchema = z.lazy(() => z.object({
            ...
            actions.array(Input.PointerSourceActionSchema),
        }));
    })(Input$1 || (Input$1 = {}));
    (function (Input) {
    ...
    (function (Input) {
        Input.PointerParametersSchema = z.lazy(() => z.object({
            pointerType.PointerTypeSchema.default('mouse').optional(),
        }));
    })(Input$1 || (Input$1 = {}));
    (function (Input) {
    ...
    (function (Input) {
        Input.WheelSourceActionsSchema = z.lazy(() => z.object({
            type.literal('wheel'),
            id.string(),
            actions.array(Input.WheelSourceActionSchema),
        }));
    })(Input$1 || (Input$1 = {}));
    (function (Input) {
    ...
    (function (Input) {
        Input.WheelScrollActionSchema = z.lazy(() => z.object({
            ...
            deltaY,
            duration.optional(),
            origin.OriginSchema.default('viewport').optional(),
        }));
    })(Input$1 || (Input$1 = {}));
    (function (Input) {
        Input.PointerCommonPropertiesSchema = z.lazy(() => z.object({
            width.optional(),
            height.optional(),
            pressure.number().gte(0).lte(1).optional(),
            tangentialPressure.number().gte(-1).lte(1).optional(),
            twist.number().int().nonnegative().gte(0).lte(359).optional(),
            altitudeAngle.number().gte(0).lte(1.5707963267948966).optional(),
            azimuthAngle.number().gte(0).lte(6.283185307179586).optional(),
        }));
    })(Input$1 || (Input$1 = {}));
    (function (Input) {
    ...
    (function (Input) {
        Input.ReleaseActionsSchema = z.lazy(() => z.object({
            ...
            params.ReleaseActionsParametersSchema,
        }));
    })(Input$1 || (Input$1 = {}));
    (function (Input) {
    ...
    (function (Input) {
        Input.SetFilesSchema = z.lazy(() => z.object({
            ...
            params.SetFilesParametersSchema,
        }));
    })(Input$1 || (Input$1 = {}));
    (function (Input) {
    ...
    (function (Input) {
        Input.FileDialogInfoSchema = z.lazy(() => z.object({
            context$1.BrowsingContextSchema,
            userContext$1.UserContextSchema.optional(),
            element$1.SharedReferenceSchema.optional(),
            multiple.boolean(),
        }));
    })(Input$1 || (Input$1 = {}));
    ...
    (function (WebExtension) {
        WebExtension.InstallSchema = z.lazy(() => z.object({
            ...
            params.InstallParametersSchema,
        }));
    })(WebExtension || (WebExtension = {}));
    (function (WebExtension) {
    ...
    (function (WebExtension) {
        WebExtension.UninstallSchema = z.lazy(() => z.object({
            ...
            params.UninstallParametersSchema,
        }));
    })(WebExtension || (WebExtension = {}));
    (function (WebExtension) {
    ...
    (function (Emulation) {
        function parseSetClientHintsOverrideParams(params) {
            const SetClientHintsOverrideParametersSchema = objectType({
                ...
                contexts(stringType()).min(1).optional(),
                userContexts(stringType()).min(1).optional(),
            });
    ...
    function log(logPrefix, ...messages) {
        ...
        if (!logPrefix.startsWith(LogType.bidi)) {
            globalThis.window?.sendDebugMessage?.(JSON.stringify({ logType, messages }, null, 2));
        }
    ...
    class WindowBidiTransport {
        static LOGGER_PREFIX_RECV = `${LogType.bidi}◂`;
        static LOGGER_PREFIX_SEND = `${LogType.bidi}▸`;

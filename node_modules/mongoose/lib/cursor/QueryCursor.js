 ...
 * @event `cursor`when the cursor is created
 * @event `error`when an error occurred
 * @event `data`when the stream is flowing and the next doc is ready
 * @event `end`when the stream is exhausted
 * @api public
 */
...
function QueryCursor(query) {
  ...
  Readable.call(this, { autoDestroy, objectMode});
  ...
  model.hooks.execPre('find', query, (err) => {
    ...
    // Re-8039, you need to set the `cursor.batchSize` option, top-level
...
function _transformForAsyncIterator(doc) {
  return doc == null ? { done} : { value, done};
}

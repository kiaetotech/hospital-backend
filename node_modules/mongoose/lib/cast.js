    ...
    if (path === '$or' || path === '$nor' || path === '$and') {
      ...
      for (let k = val.length - 1; k >= 0; k--) {
        ...
        if (discriminatorValue == null) {
          ...
          val[k] = cast(discriminatorSchema ? discriminatorSchema , val[k], options, context);
        }
      ...
      if (!schematype) {
        ...
        while (j--) {
          ...
          // gh-6027we haven't found the schematype but this path is
      ...
      if (!schematype) {
        ...
        if (isObject(val)) {
          ...
          if (geo) {
            ...
            if (geo === '$within') {
              ...
              if (value.$geometry) {
                if (isMongooseObject(value.$geometry)) {
                  value.$geometry = value.$geometry.toObject({ virtuals});
                }
        ...
        const strict = 'strict' in options ? options.strict .options.strict;
  ...
  if (Array.isArray(val)) {
    ...
    while (nearLen--) {
      ...
      if (Array.isArray(item) || isObject(item)) {
        ...
        val[nkey] = numbertype.castForQuery({ val, context});
      }
    }
  }
}

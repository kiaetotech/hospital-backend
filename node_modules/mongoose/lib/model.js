 ...
 *     const UserModel = mongoose.model('User', new Schema({ name}));
 *
 ...
 * @event `error`listening to this event, 'error' is emitted when a document was saved and an `error` occurred. If not listening, the event bubbles to the connection used to create this Model.
 * @event `index`after `Model#ensureIndexes` completes. If an error occurred it is passed with the event.
 * @event `index-single-start`when an individual index starts within `Model#ensureIndexes`. The fields and options being used to build the index are also passed with the event.
 * @event `index-single-done`when an individual index finishes within `Model#ensureIndexes`. If an error occurred it is passed with the event. The fields, options, and index name are also passed.
 * @api public
 */
  ...
  if (delta) {
    ...
    this.constructor.collection.findOne(where, optionsWithCustomValues)
      .then(documentExists => {
        ...
        callback(null, { $where, matchedCount });
      })
      .catch(callback);
    return;
  }
...
Model.prototype.$__save = function(options, callback) {
  this.$__handleSave(options, (error, result) => {
    if (error) {
      ...
      return hooks.execPost('save', this, [this], { error}, (error) => {
        callback(error, this);
      });
    }
    let numAffected = 0;
...
function operand(self, where, delta, data, val, op) {
  ...
  switch (op) {
    case '$set''$unset''$pop''$pull''$pullAll''$push''$addToSet''$inc';
    default:
      // nothing to do
      return;
  }
...
function handleAtomics(self, where, delta, data, value) {
  ...
  if (i === 0) {
    ...
    if (utils.isMongooseObject(value)) {
      value = value.toObject({ depopulate: 1, _isNested});
  ...
  function iter(mem) {
    return utils.isMongooseObject(mem)
      ? mem.toObject({ depopulate: 1, _isNested})
      ;
  }
  ...
  while (i--) {
    ...
    if (utils.isMongooseObject(val)) {
      val = val.toObject({ depopulate, transform, _isNested});
    } else if (Array.isArray(val)) {
      val = val.map(iter);
    } else if (val.valueOf) {
      val = val.valueOf();
...
Model.prototype.$__delta = function() {
  ...
  if (optimisticConcurrency) {
    if (Array.isArray(optimisticConcurrency)) {
      ...
      if (modPaths.find(path => optCon.has(path))) {
        this.$__.version = dirty.length ? VERSION_ALL _WHERE;
      }
    } else {
      this.$__.version = dirty.length ? VERSION_ALL _WHERE;
    }
  ...
  if ((where && where._id && where._id.$__ || null) != null) {
    where._id = where._id.toObject({ transform, depopulate});
  }
  for (; d < len; ++d) {
    const data = dirty[d];
    ...
    if (!pop && this.$__.selected) {
      ...
      // NOTEonly supports projected $elemMatch on top level array.
 ...
 *     const BossSchema = new BaseSchema({ department});
 *
 *     const Person = mongoose.model('Person', PersonSchema);
 *     const Boss = Person.discriminator('Boss', BossSchema);
 *     new Boss().__t; // "Boss". `__t` is the default `discriminatorKey`
 *
 *     const employeeSchema = new Schema({ boss});
...
Model.discriminator = function(name, schema, options) {
  ...
  options = options || {};
  const value = utils.isPOJO(options) ? options.value ;
  const clone = typeof options.clone === 'boolean' ? options.clone ;
  const mergePlugins = typeof options.mergePlugins === 'boolean' ? options.mergePlugins ;
 ...
 * #### Example:
 *
 *     const eventSchema = new Schema({ thing: { type: 'string', unique} })
...
Model.init = function init() {
  ...
  const _ensureIndexes = async() => {
    ...
    return await this.ensureIndexes({ _automatic});
  };
 ...
 *
 * Note 1may need to call this before starting a transaction
 * See https://www.mongodb.com/docs/manual/core/transactions/#transactions-and-operations
 *
 * Note 2don't have to call this if your schema contains index or unique field.
...
Model.createCollection = async function createCollection(options) {
  ...
  if (schemaCollation != null) {
    options = Object.assign({ collation}, options);
  }
  const capped = this &&
    this.schema &&
    this.schema.options &&
  ...
  if (capped != null) {
    if (typeof capped === 'number') {
      options = Object.assign({ capped, size}, options);
    } else if (typeof capped === 'object') {
      options = Object.assign({ capped}, capped, options);
    }
  }
  ...
  if (clusteredIndex != null) {
    options = Object.assign({ clusteredIndex: { ...clusteredIndex, unique} }, options);
  }
 ...
 * #### Example:
 *
 *     const schema = new Schema({ name: { type, unique} });
...
Model.syncIndexes = async function syncIndexes(options) {
  ...
  const dropped = await model.cleanIndexes({ ...options, toDrop.toDrop });
  await model.createIndexes({ ...options, toCreate.toCreate });
 ...
 * @return {Promise<Object>} contains the indexes that would be dropped in MongoDB and indexes that would be created in MongoDB as `{ toDrop[], toCreate[] }`.
 */
 ...
 *     })
 *
 * _NOTEis not recommended that you run this in production. Index creation may impact database performance depending on your load. Use with caution._
 *
...
function _ensureIndexes(model, options, callback) {
  ...
  for (const index of indexes) {
    if (isDefaultIdIndex(index)) {
      utils.warn('mongoosespecify a custom index on `_id` for ' +
 ...
 * #### Example:
 *
 *     await Character.deleteMany({ name: /Stark/, age: { $gte: 18 } }); // returns {deletedCount} where x is the number of documents deleted.
 *
 * #### Note:
 *
 ...
 * equivalent to `findOne({ _id})`. If you want to query by a document's
 ...
 * that `findOne(undefined)` and `findOne({ _id})` are equivalent
 * to `findOne({})` and return arbitrary documents. However, mongoose
 * translates `findById(undefined)` into `findOne({ _id})`.
 *
 * #### Example:
 *
 ...
 *
 * Sometimes you need to query for things in mongodb using a JavaScript expression. You can do so via `find({ $where})`, or you can use the mongoose shortcut method $where via a Query chain or from your mongoose Model.
 *
 ...
 * `findByIdAndUpdate(id, ...)` is equivalent to `findOneAndUpdate({ _id}, ...)`.
 *
...
Model.findByIdAndUpdate = function(id, update, options) {
  ...
  return this.findOneAndUpdate.call(this, { _id}, update, options);
};
...
Model.findByIdAndDelete = function(id, options) {
  ...
  return this.findOneAndDelete({ _id}, options);
};
...
/**
 * Issue a mongodb findOneAndRemove command by a document's _id field. `findByIdAndRemove(id, ...)` is equivalent to `findOneAndRemove({ _id}, ...)`.
 *
 * Finds a matching document, removes it, and returns the found document (if any).
 *
 * This function triggers the following middleware.
...
Model.findByIdAndRemove = function(id, options) {
  ...
  return this.findOneAndRemove({ _id}, options);
};
 ...
 * @param {Boolean} [options.aggregateErrors] Aggregate Errors instead of throwing the first one that occurs. Default* @return {Promise}
 * @api public
 */
...
Model.create = async function create(doc, options) {
  ...
  if (Array.isArray(doc)) {
    ...
    if (args.length === 2 &&
      ...
      utils.warn('WARNINGpass a `session` to `Model.create()` in ' +
  ...
  let res = [];
  const immediateError = typeof options.aggregateErrors === 'boolean' ? !options.aggregateErrors ;
  ...
  if (options.ordered) {
    ...
    // ".bind(Promise)" is required, otherwise results in "TypeError.allSettled called on non-object"
    const promiseType = !immediateError ? Promise.allSettled.bind(Promise) .all.bind(Promise);
    ...
    if (!immediateError) {
      p = p.then(presult => presult.map(v => v.status === 'fulfilled' ? v.value .reason));
    }
 ...
 *
 * - 'change'change occurred, see below example
 * - 'error'unrecoverable error occurred. In particular, change streams currently error out if they lose connection to the replica set primary. Follow [this GitHub issue](https://github.com/Automattic/mongoose/issues/6799) for updates.
 * - 'end'if the underlying stream is closed
 * - 'close'if the underlying stream is closed
 *
 * #### Example:
 *
 ...
 * @param {Boolean} [options.throwOnValidationError=false] If true and `ordered`, throw an error if one of the operations failed validation, but all valid operations completed successfully.
...
Model.$__insertMany = function(arr, options, callback) {
  ...
  const ordered = typeof options.ordered === 'boolean' ? options.ordered ;
  const throwOnValidationError = typeof options.throwOnValidationError === 'boolean' ? options.throwOnValidationError ;
  ...
  const results = ordered ? null Array(arr.length);
  ...
  parallelLimit(toExecute, limit, function(error, docs) {
    ...
    const docObjects = lean ? docAttributes .map(function(doc) {
...
Model.bulkWrite = async function bulkWrite(ops, options) {
  ...
  options = options || {};
  const ordered = options.ordered == null ? true .ordered;
...
Model.bulkSave = async function bulkSave(documents, options) {
  ...
  const writeOperations = this.buildBulkWriteOperations(documents, { skipValidation, timestamps.timestamps });

  const { bulkWriteResult, bulkWriteError } = await this.bulkWrite(writeOperations, options).then(
    (res) => ({ bulkWriteResult, bulkWriteError}),
    (err) => ({ bulkWriteResult, bulkWriteError})
  );
 ...
 * #### Example:
 *
 *     const Test = mongoose.model('Test', Schema({ num}));
 *
...
Model.castObject = function castObject(obj, options) {
  ...
  for (const path of paths) {
    ...
    const pieces = path.indexOf('.') === -1 ? [path] .split('.');
    let cur = ret;
 ...
 *
 *     try {
 *       await Model.validate({ name}, ['name'])
 *     } catch (err) {
...
Model.validate = async function validate(obj, pathsOrOptions, context) {
  if ((arguments.length < 3) || (arguments.length === 3 && typeof arguments[2] === 'function')) {
    ...
    // the model so users don't have to always pass `context`, re-10132, gh-10346
    context = obj;
  }
  ...
  if (pathsOrOptions != null) {
    const _pathsToValidate = typeof pathsOrOptions === 'string' ? new Set(pathsOrOptions.split(' ')) .isArray(pathsOrOptions) ? new Set(pathsOrOptions) Set(paths);
 ...
 *
 * Changed in Mongoose 6model you call `populate()` on should be the
 * "local field" model, **not** the "foreign field" model.
 *
 * #### Available top-level options:
 *
 * - pathdelimited path(s) to populate
 * - selectfields to select
 * - matchquery conditions to match
 * - modelname of the model to use for population
 * - optionsquery options like sort, limit, etc
 * - justOneboolean, if true Mongoose will always set `path` to a document, or `null` if no document was found. If false, Mongoose will always set `path` to an array, which will be empty if no documents are found. Inferred from schema by default.
 * - strictPopulateboolean, set to `false` to allow populating paths that aren't in the schema.
 *
 * #### Example:
 *
 *     const Dog = mongoose.model('Dog', new Schema({ name, breed}));
 *     const Person = mongoose.model('Person', new Schema({
 *       name,
 *       pet: { type.ObjectId, ref: 'Dog' }
 *     }));
 *
 ...
 *     const users = [
 *       { name: 'John Wick', dog[0]._id },
 *       { name: 'Doc Brown', dog[1]._id }
 *     ];
...
function _execPopulateQuery(mod, match, select, assignmentOpts, callback) {
  ...
  const queryOptions = Object.assign({
    skip.options.skip,
    limit.options.limit,
    perDocumentLimit.options.perDocumentLimit
  }, mod.options.options);
  ...
  if (subPopulate) {
    ...
    if (mod.model.baseModelName != null) {
      if (Array.isArray(subPopulate)) {
        ...
        subPopulate = { path, strictPopulate};
      } else {
...
function _assign(model, vals, mod, assignmentOpts) {
      ...
      if (Array.isArray(_val)) {
        ...
        if (rawDocs[key]) {
          if (Array.isArray(rawDocs[key])) {
            ...
            // if we have multiple queries on the same model because of `perDocumentLimit` re-9906
            rawDocs[key] = [rawDocs[key], val];
            rawOrder[key] = [rawOrder[key], i];
          }
        } else {
  ...
  assignVals({
    ...
    rawIds.isVirtual ? allIds .allIds,
    allIds,
    unpopulatedValues.unpopulatedValues,
 ...
 * #### Example:
 *
 *     const MyModel = mongoose.model('Test', Schema({ name}));

...
const syncCollectionMethods = { watch, find, aggregate};
...
function iter(i) {
  NativeCollection.prototype[i] = function() {
    ...
    const debug = connectionDebug == null ? globalDebug ;
    ...
    if (this._shouldBufferCommands() && this.buffer) {
      ...
      timeout = setTimeout(() => {
        ...
        if (removed) {
          ...
          this.conn.emit('buffer-end', { _id, modelName: _this.modelName, collectionName: _this.name, method, error});
          callback(err);
        }
      }, bufferTimeoutMS);
      ...
      callback = function collectionOperationCallback(err, res) {
        if (err != null) {
          _this.conn.emit('operation-end', { _id, modelName: _this.modelName, collectionName: _this.name, method, error});
        } else {
          _this.conn.emit('operation-end', { _id, modelName: _this.modelName, collectionName: _this.name, method, result});
        }
        return lastArg.apply(this, arguments);
      };
      _args = args.slice(0, args.length - 1).concat([callback]);
    ...
    if (debug) {
      if (typeof debug === 'function') {
        ...
        const color = debug.color == null ? true .color;
        const shell = debug.shell == null ? false .shell;
        this.$print(_this.name, i, args, color, shell);
      }
    }

    this.conn.emit('operation-start', { _id, modelName: _this.modelName, collectionName.name, method, params: _args });
    ...
    try {
      ...
      if (typeof lastArg === 'function') {
        ...
        this.conn.emit('operation-end', { _id, modelName: _this.modelName, collectionName.name, method, error});

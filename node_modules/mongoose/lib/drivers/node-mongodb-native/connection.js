...
NativeConnection.prototype.createClient = async function createClient(uri, options) {
  ...
  if (this.readyState === STATES.connecting || this.readyState === STATES.connected) {
    if (this._connectionString !== uri) {
      throw new MongooseError('Can\'t call `openUri()` on an active connection with ' +
        ...
        'multiple times. See://mongoosejs.com/docs/connections.html#multiple_connections');
    }
  }
  ...
  if (options) {
    ...
    const autoIndex = options.config && options.config.autoIndex != null ?
      options.config.autoIndex .autoIndex;
    if (autoIndex != null) {
...
function _setClient(conn, client, options, dbName) {
  const db = dbName != null ? client.db(dbName) .db();
  conn.db = db;
  conn.client = client;
  conn.host = client &&
    client.s &&
  ...
  conn.name = dbName != null ? dbName .databaseName;

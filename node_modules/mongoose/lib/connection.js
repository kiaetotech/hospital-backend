 ...
 * @event `connecting`when `connection.openUri()` is executed on this connection.
 * @event `connected`when this connection successfully connects to the db. May be emitted _multiple_ times in `reconnected` scenarios.
 * @event `open`after we `connected` and `onOpen` is executed on all of this connection's models.
 * @event `disconnecting`when `connection.close()` was executed.
 * @event `disconnected`after getting disconnected from the db.
 * @event `close`after we `disconnected` and `onClose` executed on all of this connection's models.
 * @event `reconnected`after we `connected` and subsequently `disconnected`, followed by successfully another successful connection.
 * @event `error`when an error occurs on this connection.
 * @event `fullsetup`after the driver has connected to primary and all secondaries if specified in the connection string.
 * @api public
 */
...
function Connection(base) {
  ...
  this.replica = false;
  this.options = null;
  this.otherDbs = []; // FIXMEbe replaced with relatedDbs
 ...
 *
 * - `maxTimeMS`[`maxTimeMS`](https://mongoosejs.com/docs/api/query.html#Query.prototype.maxTimeMS()) for all queries on this connection.
 ...
 *     const Test = conn.model('Test', mongoose.Schema({ name}));
 *
 ...
 * #### Example:
 *
 *     mongoose.createConnection('mongodb://val@127.0.0.1:27017/mydb').user; // "val"
 *
 * @property user
 * @memberOf Connection
 * @instance
 ...
 * #### Example:
 *
 *     mongoose.createConnection('mongodb://val@127.0.0.1:27017/mydb').pass; // "psw"
 *
 * @property pass
 * @memberOf Connection
 * @instance
...
/*!
 * Reset document state in between transaction retries re-13698
 */
...
Connection.prototype._close = async function _close(force, destroy) {
  ...
  switch (this.readyState) {
    case STATES.disconnected(destroy && this.base.connections.indexOf(conn) !== -1) {
    ...
    case STATES.connected.readyState = STATES.disconnecting;
      await this.doClose(force);
      ...
      break;
    case STATES.connectingnew Promise((resolve, reject) => {
        const _rerunClose = () => {
    ...
    case STATES.disconnectingnew Promise(resolve => {
        this.once('close', () => {
...
Connection.prototype.onClose = function(force) {
  ...
  for (const db of this.otherDbs) {
    this._destroyCalled ? db.destroy({ force, skipCloseClient}) .close({ force, skipCloseClient});
  }
};
...
Connection.prototype.collection = function(name, options) {
  const defaultOptions = {
    autoIndex.config.autoIndex != null ? this.config.autoIndex .base.options.autoIndex,
    autoCreate.config.autoCreate != null ? this.config.autoCreate .base.options.autoCreate
  };
...
Connection.prototype.model = function(name, schema, collection, options) {
  ...
  const defaultOptions = { cache, overwriteModels.base.options.overwriteModels };
  const opts = Object.assign(defaultOptions, options, { connection});
 ...
 * #### Example:
 *
 *     conn.model('User', new Schema({ name}));
 ...
 * #### Example:
 *
 *     const User = conn.model('User', new Schema({ name}));
 *
 ...
 *     // Creates a cached connection to `mydb2`. All calls to `conn.useDb('mydb2', { useCache})` will return the same
 *     // connection instance as opposed to creating a new connection instance
 *     const db2 = conn.useDb('mydb2', { useCache});
 *
 * @method useDb
 * @memberOf Connection

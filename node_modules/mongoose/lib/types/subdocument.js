...
function Subdocument(value, fields, parent, skipId, options) {
  ...
  if (parent != null) {
    // If setting a nested path, should copy isNew from parent re-7048
    const parentOptions = { isNew.isNew };
...
Subdocument.prototype.save = async function save(options) {
  ...
  if (!options.suppressWarning) {
    utils.warn('mongoose`save()` on a subdoc does **not** save ' +
      'the document to MongoDB, it only runs save middleware. ' +
      'Use `subdoc.save({ suppressWarning})` to hide this warning ' +
...
Subdocument.prototype.$__fullPath = function(path) {
  ...
  return path ?
    this.$__.fullPath + '.' + path .$__.fullPath;
};
...
Subdocument.prototype.isModified = function(paths, options, modifiedPaths) {
  ...
  if (parent != null) {
    if (Array.isArray(paths) || typeof paths === 'string') {
      paths = (Array.isArray(paths) ? paths .split(' '));
...
Subdocument.prototype.ownerDocument = function() {
  ...
  while (true) {
    ...
    if (seenDocs.has(parent)) {
      throw new Error('Infinite subdocument loopwith _id ' + parent._id + ' is a parent of itself');
    }

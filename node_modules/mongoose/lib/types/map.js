...
class MongooseMap extends Map {
  constructor(v, path, doc, schemaType) {
    ...
    super(v);
    this.$__parent = doc != null && doc.$__ != null ? doc ;
    this.$__path = path;
    this.$__schemaType = schemaType == null ? new Mixed(path) ;
  ...
  set(key, value) {
    ...
    if (populated != null) {
      ...
      if (Array.isArray(value) && this.$__schemaType.$isMongooseArray) {
        ...
        // Doesn't support single nested "in-place" populate
        value.$__.wasPopulated = { value._id };
      }
    } else {
      try {
   ...
   *     doc.myMap.toJSON({ flattenMaps}) instanceof Map; // false
   *
   * @api public
   * @method toJSON
  ...
  toJSON(options) {
    if (typeof (options && options.flattenMaps) === 'boolean' ? options.flattenMaps ) {
      const ret = {};
      const keys = this.keys();
      for (const key of keys) {
        ret[key] = clone(this.get(key), options);

...
const methods = {
  ...
  $__getAtomics() {
    ...
    const opts = Object.assign({}, internalToObjectOptions, { _isNested});
  ...
  _markModified(elem) {
    ...
    if (parent) {
      ...
      parent.markModified(dirtyPath, arguments.length !== 0 ? elem );
    }
  ...
  addToSet() {
    ...
    const rawArray = utils.isMongooseArray(this) ? this.__array ;

    rawValues.forEach(function(v) {
      let found;
      const val = +v;
      switch (type) {
        case 'doc'= this.some(function(doc) {
            return doc.equals(v);
          });
          break;
        case 'date'= this.some(function(d) {
            return +d === val;
          });
          break;
        case 'ObjectId'= this.find(o => o.toString() === v.toString());
          break;
        default= ~this.indexOf(v);
          break;
      }
  ...
  pull() {
    const values = [].map.call(arguments, (v, i) => this._cast(v, i, { defaults}), this);
  ...
  push() {
    ...
    const arr = utils.isMongooseArray(this) ? this.__array ;
    if (isOverwrite) {
      atomic = values[0];
   ...
   * @method sort
   * @memberOf MongooseArray
   * @see MasteringJSsort https://masteringjs.io/tutorials/fundamentals/array-sort
   */
   ...
   * @method splice
   * @memberOf MongooseArray
   * @see MasteringJSsplice https://masteringjs.io/tutorials/fundamentals/array-splice
   */
    ...
    if (arguments.length) {
      ...
      if (this[arraySchemaSymbol] == null) {
        ...
        for (let i = 0; i < arguments.length; ++i) {
          vals[i] = i < 2 ?
            arguments[i] ._cast(arguments[i], arguments[0] + (i - 2));
        }
      }
...
function _checkManualPopulation(arr, docs) {
  const ref = arr == null ?
    null [arraySchemaSymbol] && arr[arraySchemaSymbol].caster && arr[arraySchemaSymbol].caster.options && arr[arraySchemaSymbol].caster.options.ref || null;
  if (arr.length === 0 &&
      docs.length !== 0) {
    if (_isAllSubdocs(docs, ref)) {
      arr[arrayParentSymbol].$populated(arr[arrayPathSymbol], [], {
        [populateModelSymbol][0].constructor
      });
    }
  }
}
...
for (const method of returnVanillaArrayMethods) {
  ...
  methods[method] = function() {
    const _arr = utils.isMongooseArray(this) ? this.__array ;

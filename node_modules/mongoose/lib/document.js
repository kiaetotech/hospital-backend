 ...
 * @event `init`on a document after it has been retrieved from the db and fully hydrated by Mongoose.
 * @event `save`when the document is successfully saved
 * @api private
 */
  ...
  if (!this.$__.strictMode && obj) {
    ...
    keys.forEach(function(key) {
      ...
      if (!(key in schema.tree) && !(key in schema.methods) && !(key in schema.virtuals) && !key.startsWith('$')) {
        defineKey({ prop, subprops, prototype: _this });
      }
    });
  }
...
function $applyDefaultsToNested(val, path, doc) {
  ...
  const pathPieces = path.indexOf('.') === -1 ? [path] .split('.');
  ...
  function _init(index) {
    ...
    if (!schemaType && utils.isPOJO(value)) {
      ...
      if (value === null) {
        ...
        const wasPopulated = value.$__ == null ? null .$__.wasPopulated;
...
Document.prototype.updateOne = function updateOne(doc, options, callback) {
  const query = this.constructor.updateOne({ _id._id }, doc, options);
  const self = this;
 ...
 * #### Example:
 *
 *     const TestModel = mongoose.model('Test', new Schema({ name}, { timestamps}));
...
Document.prototype.$set = function $set(path, val, type, options) {
  ...
  if (path == null) {
    ...
    options = Object.assign({}, options, { _skipMinimizeTopLevel});

    for (let i = 0; i < len; ++i) {
      key = keys[i];
      const pathName = prefix ? prefix + key ;
      ...
      if (utils.isNonBuiltinObject(valForKey) && pathtype === 'nested') {
        this.$set(pathName, valForKey, constructing, Object.assign({}, options, { _skipMarkModified}));
        ...
        if (pathtype === 'adhocOrUndefined') {
          pathtype = getEmbeddedDiscriminatorPath(this, pathName, { typeOnly});
        }
        ...
        if (pathtype === 'real' || pathtype === 'virtual') {
          ...
          this.$set(pathName,
            valForKey.toObject({ transform}), constructing, options);
  ...
  let parts = null;
  if (pathType === 'adhocOrUndefined') {
    parts = path.indexOf('.') === -1 ? [path] .split('.');
    pathType = getEmbeddedDiscriminatorPath(this, parts, { typeOnly});
  }
  ...
  if (pathType === 'nested' && val) {
    if (typeof val === 'object' && val != null) {
      ...
      for (const key of keys) {
        this.$set(path + '.' + key, val[key], constructing, { ...options, _skipMarkModified});
      }
      if (priorVal != null &&
          (!wasModified || hasInitialVal) &&
          utils.deepEqual(hasInitialVal ? this.$__.savedState[path] , val)) {
  ...
  for (i = 0; i < parts.length - 1; ++i) {
    ...
    if (!cur) {
      this.$set(curPath, {});
      // Hack re-5800. If nested field is not selected, it probably exists
      // so `MongoServerErroruse the part (nested of nested.num) to
      // traverse the element ({nested})` is not likely. If user gets
  ...
  if (parts.length <= 1) {
    ...
    for (i = 0; i < len; ++i) {
      const subpath = parts.slice(0, i + 1).join('.');
      if (this.$get(subpath, null, { getters}) === null) {
        pathToMark = subpath;
        break;
      }
    }
  ...
  if (val != null && merge && schema.$isSingleNested) {
    if (val instanceof Document) {
      val = val.toObject({ virtuals, transform});
    }
  ...
  try {
    ...
    if (refMatches && val instanceof Document && (!val.$__.wasPopulated || utils.deepEqual(val.$__.wasPopulated.value, val._id))) {
      const unpopulatedValue = (schema && schema.$isSingleNested) ? schema.cast(val, this) ._id;
      this.$populated(path, unpopulatedValue, { [populateModelSymbol].constructor });
      val.$__.wasPopulated = { value};
      didPopulate = true;
    }
    ...
    if (schema.options &&
      ...
      popOpts = { [populateModelSymbol][0].constructor };
      this.$populated(path, val.map(function(v) { return v._id; }), popOpts);

      for (const doc of val) {
        doc.$__.wasPopulated = { value._id };
      }
      didPopulate = true;
    }
    ...
    if (!refMatches || !schema.$isSingleNested || !val.$__) {
      ...
      if (options != null && options.overwriteImmutable) {
        val = schema.applySetters(val, setterContext, false, priorVal, { overwriteImmutable});
      } else {
  ...
  if (shouldSet) {
    ...
    if (!constructing) {
      const doc = this.$isSubdocument ? this.ownerDocument() ;
      savedState = doc.$__.savedState;
      savedStatePath = this.$isSubdocument ? this.$__.fullPath + '.' + path ;
...
Document.prototype.$__shouldModify = function(pathToMark, path, options, constructing, parts, schema, val, priorVal) {
  ...
  // gh-3992setting a populated field to a doc, don't mark modified
  ...
  if (!deepEqual(val, priorVal !== undefined ? priorVal .getValue(path, this))) {
    return true;
  }
...
Document.prototype.$__set = function(pathToMark, path, options, constructing, parts, schema, val, priorVal) {
  ...
  for (; i < l; i++) {
    ...
    if (last) {
      ...
      let value = isMap ? obj.get(parts[i]) [parts[i]];
...
Document.prototype.$inc = function $inc(path, val) {
  ...
  if (shouldSet) {
    ...
    if (this.$__.primitiveAtomics[path] == null) {
      this.$__.primitiveAtomics[path] = { $inc};
    } else {
...
Document.prototype.get = function(path, type, options) {
  ...
  let schema = noDottedPath ? this.$__schema.paths[path] .$__path(path);
  if (schema == null) {
    schema = this.$__schema.virtualpath(path);
...
Document.prototype.$__path = function(path) {
  ...
  const adhocType = adhocs && adhocs.hasOwnProperty(path) ? adhocs[path] ;
...
Document.prototype.$__saveInitialState = function $__saveInitialState(path) {
  ...
  if (savedState != null) {
    ...
    const topLevelPath = firstDot === -1 ? savedStatePath .slice(0, firstDot);
 ...
 * #### Example:
 *
 *     const schema = new Schema({ foo, nested: { bar} });
 ...
 * #### Example:
 *
 *     const schema = new Schema({ nested: { foo} });
 *     const Model = mongoose.model('Test', schema);
 *     const doc = new Model({});
...
Document.prototype.isModified = function(paths, options, modifiedPaths) {
  if (paths) {
    ...
    if (typeof paths === 'string') {
      paths = paths.indexOf(' ') === -1 ? [paths] .split(' ');
    }
 ...
 * #### Example:
 *
 *     MyModel = mongoose.model('test', { name: { type, default: 'Val '} });
 *     const m = new MyModel();
 ...
 * #### Example:
 *
 *     await doc.validate({ validateModifiedOnly, pathsToSkip: ['name', 'email']});
 *
...
Document.prototype.validate = async function validate(pathsToValidate, options) {
  ...
  if (this.$isSubdocument != null) {
    ...
    parallelValidate = new ParallelValidateError(this, {
      ...
      conflictStack.$__.validating.stack
    });
  } else {
    this.$__.validating = new ParallelValidateError(this, { parentStack&& options.parentStack });
  }
  ...
  if (options && typeof options.pathsToSkip === 'string') {
    ...
    options.pathsToSkip = isOnePathOnly ? [options.pathsToSkip] .pathsToSkip.split(' ');
  }
...
function _getPathsToValidate(doc, pathsToValidate, pathsToSkip) {
  ...
  for (const subdoc of subdocs) {
    if (subdoc.$basePath) {
      const fullPathToSubdoc = subdoc.$isSingleNested ? subdoc.$__pathRelativeToParent() .$__fullPathWithIndexes();

      // Remove child paths for now, because we'll be validating the whole
      // subdoc.
      // The following is a faster take on looping through every path in `paths`
      // and checking if the path starts with `fullPathToSubdoc` re-13191
      ...
      if (doc.$isModified(fullPathToSubdoc, null, modifiedPaths) &&
        ...
        doc.$__.pathsToScopes[fullPathToSubdoc] = subdoc.$isDocumentArrayElement ?
          subdoc.__parentArray .$parent();

        doValidateOptions[fullPathToSubdoc] = { skipSchemaValidators};
  ...
  for (const path of paths) {
    ...
    // Optimizationprimitive path with no validators, or array of primitives
  ...
  // gh-661a whole array is modified, make sure to run validation on all
  ...
  for (const path of paths) {
    ...
    // gh-11380. If the array isn't a document array and there's no validators
  ...
  const flattenOptions = { skipArrays};
  for (const pathToCheck of paths) {
    if (doc.$__schema.nested[pathToCheck]) {
      let _v = doc.$__getValue(pathToCheck);
      if (isMongooseObject(_v)) {
        _v = _v.toObject({ transform});
      }
  ...
  for (const path of paths) {
    ...
    // array re-6818. Do this _after_ adding subpaths, because
...
Document.prototype.$__validate = function(pathsToValidate, options, callback) {
  ...
  const paths = shouldValidateModifiedOnly ?
    pathDetails[0].filter((path) => this.$isModified(path)) [0];
  ...
  if (paths.length === 0) {
    return immediate(function() {
      ...
      if (error) {
        return _this.$__schema.s.hooks.execPost('validate', _this, [_this], { error}, function(error) {
          callback(error);
        });
      }
      callback(null, _this);
...
Document.prototype.validateSync = function(pathsToValidate, options) {
  ...
  if (typeof pathsToValidate === 'string') {
    ...
    pathsToValidate = isOnePathOnly ? [pathsToValidate] .split(' ');
...
function _checkImmutableSubpaths(subdoc, schematype, priorVal) {
  ...
  for (const key of Object.keys(schema.paths)) {
    ...
    const oldVal = priorVal == null ? void 0 .$__getValue(key);
...
Document.prototype.$__reset = function reset() {
  ...
  this.$__.backup.activePaths = {
    modify.assign({}, this.$__.activePaths.getStatePaths('modify')),
    default.assign({}, this.$__.activePaths.getStatePaths('default'))
  };
...
Document.prototype.$__dirty = function() {
  ...
  // gh-2558we had to set a default and the value is not undefined,
...
Document.prototype.$toObject = function(options, json) {
  ...
  if (transform === true || (schemaOptions.toObject && transform)) {
    const opts = options.json ? schemaOptions.toJSON .toObject;

    if (opts) {
      transform = (typeof options.transform === 'function' ? options.transform .transform);
    }
  } else {
 ...
 *
 *     doc.toObject({ getters, virtuals})
 *
 * Example of only applying virtual getters
 *
 *     doc.toObject({ virtuals})
 *
 * Example of applying both path and virtual getters
 *
 *     doc.toObject({ getters})
 *
 ...
 *
 * _Notea transform function returns `undefined`, the return value will be ignored._
 *
 * Transformations may also be applied inline, overridding any transform set in the options:
 *
 *     function xform (doc, ret, options) {
 *       return { inline.name, custom}
 *     }
 *
 *     // pass the transform as an inline option
 *     doc.toObject({ transform}); // { inline: 'Wreck-it Ralph', custom}
 *
 * If you want to skip transformations, use `transform`:
 *
 ...
 *     doc.toObject({ hide: 'secret _id', transform});// { _id: 'anId', secret: 47, name: 'Wreck-it Ralph' }
 *     doc.toObject({ hide: 'secret _id', transform}); // { name: 'Wreck-it Ralph' }
 *
 * If you pass a transform in `toObject()` options, Mongoose will apply the transform
 * to [subdocuments](https://mongoosejs.com/docs/subdocs.html) in addition to the top-level document.
 * Similarly, `transform` skips transforms for all subdocuments.
 * Note that this behavior is different for transforms defined in the schema:
 * if you define a transform in `schema.options.toObject.transform`, that transform
 * will **not** apply to subdocuments.
 *
 *     const memberSchema = new Schema({ name, email});
 *     const groupSchema = new Schema({ members: [memberSchema], name, email });
 ...
 * @param {Boolean|Object} [options.virtuals=false] if true, apply virtuals, including aliases. Use `{ getters, virtuals}` to just apply getters, not virtuals. An object of the form `{ pathsToSkip: ['someVirtual'] }` may also be used to omit specific virtuals.
 ...
 * @param {Boolean} [options.useProjection=false] - If true, omits fields that are excluded in this document's projection. Unless you specified a projection, this will omit any field that has `select` in the schema.
...
function applyVirtuals(self, json, options, toObjectOptions) {
  ...
  for (i = 0; i < numPaths; ++i) {
    ...
    // Allow skipping aliases with `toObject({ virtuals, aliases})`
...
function applyGetters(self, json, options) {
  ...
  while (i--) {
    ...
    for (let ii = 0; ii < plen; ++ii) {
      ...
      // Necessary for mongoose-intl plugin re-14446
...
Document.prototype.populated = function(path, val, options) {
  ...
  if (val == null || val === true) {
    ...
    const _path = path.endsWith('.$*') ? path.replace(/\.\$\*$/, '') ;

    const v = this.$__.populated[_path];
    if (v) {
      return val === true ? v .value;
    }
    return undefined;
  }

  this.$__.populated || (this.$__.populated = {});
  this.$__.populated[path] = { value, options};

 ...
 *     new Aggregate([{ $match: { _idmongoose.Types.ObjectId('00000000000000000000000a') } }]);
 *
...
Aggregate.prototype.addFields = function(arg) {
  ...
  return this.append({ $addFields.assign({}, arg) });
};
 ...
 *     aggregate.unwind({ path: '$tags', preserveNullAndEmptyArrays});
 *
...
Aggregate.prototype.sortByCount = function(arg) {
  if (arg && typeof arg === 'object') {
    return this.append({ $sortByCount});
  } else if (typeof arg === 'string') {
    return this.append({
...
Aggregate.prototype.lookup = function(options) {
  return this.append({ $lookup});
};

/**
 * Appends new custom $graphLookup operator(s) to this aggregate pipeline, performing a recursive search on a collection.
 *
 * Note that graphLookup can only consume at most 100MB of memory, and does not allow disk use even if `{ allowDiskUse}` is specified.
 *
 * #### Example:
 *
...
Aggregate.prototype.graphLookup = function(options) {
  ...
  return this.append({ $graphLookup});
};
 ...
 * #### Example:
 *
 *     const agg = Model.aggregate(..).option({ allowDiskUse}); // Set the `allowDiskUse` option
 *     agg.options; // `{ allowDiskUse}`
 *
...
if (Symbol.asyncIterator != null) {
  Aggregate.prototype[Symbol.asyncIterator] = function() {
    return this.cursor({ useMongooseAggCursor}).transformNull()._transformForAsyncIterator();
  };
}

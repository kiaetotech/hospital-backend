...
exports.applyPaths = function applyPaths(fields, schema) {
  ...
  if (fields) {
    ...
    for (const path of minusPaths) {
      ...
      // projection if `name` has schema-level `select`.
  ...
  // if selecting, apply default schematype selectfields
  // if excluding, apply schematype selectfields
  const selected = [];
  const excluded = [];
  const stack = [];
  ...
  switch (exclude) {
    case true(const fieldName of excluded) {
        fields[fieldName] = 0;
      }
      break;
    case false(schema &&
  ...
  function analyzeSchema(schema, prefix) {
    ...
    schema.eachPath(function(path, type) {
      ...
      if (type.schema) {
        ...
        // Special casediscriminator key is the only field that would
  ...
  function analyzePath(path, type) {
    ...
    // because we have tests that assert that using `-path` to exclude schema-level `select`
    ...
    for (let i = 0; i < pieces.length; ++i) {
      cur += cur.length ? '.' + pieces[i] [i];
      if (excluded.indexOf(cur) !== -1) {
        return;
      }
    }

    // Special caseuser has included a parent path of a discriminator key,
    ...
    (type.selected ? selected ).push(path);
    return path;
  }
};

...
module.exports = function castUpdate(schema, obj, options, context, filter) {
  ...
  if (Object.keys(ret).length === 0 &&
    ...
    // Shallow clone to avoid passing defaults in re-13962
...
function walkUpdatePath(schema, obj, op, options, context, filter, pref) {
  ...
  const strictMode = strict != null ? strict .options.strict;
  ...
  while (i--) {
    ...
    const discriminatorKey = (prefix ? prefix + key );
    if (
    ...
    if (getConstructorName(val) === 'Object') {
      ...
      if (schematype && schematype.caster && op in castOps) {
        ...
        if ('$each' in val) {
          ...
          try {
            obj[key] = {
              $each(schematype, val.$each, op, key, context, prefix + key)
            };
          } catch (error) {
          ...
          if (schematype != null && schematype.$isSingleNested) {
            const _strict = strict == null ? schematype.schema.options.strict ;
            try {
      ...
      if (skip) {
        ...
        // gh-1845 temporary fix$rename. See gh-3027 for tracking
        // improving this.
        if (op === '$rename') {
          hasKeys = true;
          continue;

...
function castIn(val, schema, strictQuery) {
  ...
  return [
    schematype.$isMongooseDocumentArray ? schematype.$embeddedSchemaType.cast(search) .caster.cast(search),
    path
  ];
}
...
function castComparison(val, schema, strictQuery) {
  ...
  if (isLiteral(val[1])) {
    ...
    if (schematype != null) {
      if (is$literal) {
        val[1] = { $literal.cast(val[1].$literal) };
      } else {

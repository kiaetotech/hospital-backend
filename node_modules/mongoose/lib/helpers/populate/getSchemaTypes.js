...
module.exports = function getSchemaTypes(model, schema, doc, path) {
  ...
  if (discriminatorKey && model != null) {
    if (doc != null && doc[discriminatorKey] != null) {
      ...
      schema = discriminator ? discriminator.schema ;
    ...
    while (p--) {
        ...
        if (p !== parts.length && foundschema.schema) {
          ...
          if (parts[p] === '$') {
            ...
            ret = search(
              parts.slice(p + 1),
              schema,
              subdoc ? mpath.get(trypath, subdoc) ,
      ...
      if (_val != null) {
        const model = Array.isArray(_val) && _val.length > 0 ?
          leanPopulateMap.get(_val[0]) .get(_val);
        // Populated using lean, `leanPopulateMap` value is the foreign model
        const schema = model != null ? model.schema ;
        if (schema != null) {
          const ret = search(
            parts.slice(p),
            schema,

...
module.exports = function assignVals(o) {
  ...
  function setValue(val) {
    ...
    if (o.justOne === true && Array.isArray(val)) {
      // Might be an embedded discriminator (re-9244) with multiple models, so make sure to pick the right
  ...
  for (i = 0; i < docs.length; ++i) {
    setValueIndex = 0;
    const _path = o.path.endsWith('.$*') ? o.path.slice(0, -3) .path;
    ...
    let isMap = isDoc ?
      existingVal instanceof Map .isPOJO(existingVal);
    // If we pass the first check, also make sure the local field's schematype
    // is map (re-6460)
    isMap = isMap && get(originalSchema._getSchema(_path), '$isSchemaMap');
    if (!o.isVirtual && isMap) {
      const _keys = existingVal instanceof Map ?
        Array.from(existingVal.keys()) .keys(existingVal);
    ...
    if (o.isVirtual && isDoc) {
      docs[i].$populated(_path, o.justOne ? originalIds[0] , o.allOptions);
      // If virtual populate and doc is already init-ed, need to walk through
      // the actual doc to set rather than setting `_doc` directly
      if (Array.isArray(valueToSet)) {
        valueToSet = valueToSet.map(v => v == null ? void 0 );
      }
    ...
    if (docs[i].$__) {
      ...
      if (valueToSet != null && valueToSet.$__ != null) {
        valueToSet.$__.wasPopulated = { value.unpopulatedValues[i] };
      }
...
function valueFilter(val, assignmentOpts, populateOptions, allIds) {
  ...
  const transform = userSpecifiedTransform ? populateOptions.transform => v;
  if (Array.isArray(val)) {
    ...
    for (let i = 0; i < numValues; ++i) {
      let subdoc = val[i];
      const _allIds = Array.isArray(allIds) ? allIds[i] ;
      if (!isPopulatedObject(subdoc) && (!populateOptions.retainNullValues || subdoc != null) && !userSpecifiedTransform) {
        continue;
      } else if (!populateOptions.retainNullValues && subdoc == null) {
        continue;
      } else if (userSpecifiedTransform) {
        subdoc = transform(isPopulatedObject(subdoc) ? subdoc , _allIds);
      }
  ...
  return val == null ? transform(val, allIds) (null, allIds);
}

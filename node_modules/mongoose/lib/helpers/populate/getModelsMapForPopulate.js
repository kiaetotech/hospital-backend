...
module.exports = function getModelsMapForPopulate(model, docs, options) {
  ...
  for (let i = 0; i < len; i++) {
    ...
    const docSchema = doc != null && doc.$__ != null ? doc.$__schema ;
    schema = getSchemaTypes(model, docSchema, doc, options.path);

    // Special casea path that's a DocumentArray unless
    // there's an explicit `ref` or `refPath` re-8946
    if (schema != null &&
    ...
    options._docs[id] = Array.isArray(ret) ? ret.slice() ;
    ...
    if (modelNameFromQuery) {
      ...
      if ((ref = get(schemaForCurrentDoc, 'options.ref')) != null) {
        if (schemaForCurrentDoc != null &&
          ...
          // Ensure correct context for ref functions, not top-level doc. See gh-8469
          modelNames = new Set();
...
function addModelNamesToMap(model, map, available, modelNames, options, data, ret, doc, schemaOptions, unpopulatedValue) {
  // `PopulateOptions#connection`the model is passed as a string, the
  // connection matters because different connections have different models.
  const connection = options.connection != null ? options.connection .db;

  unpopulatedValue = unpopulatedValue === void 0 ? ret ;
  ...
  while (k--) {
    ...
    const modelNamesForRefPath = data.modelNamesInOrder ? data.modelNamesInOrder ;
    ...
    const perDocumentLimit = options.perDocumentLimit == null ?
      get(options, 'options.perDocumentLimit', null) .perDocumentLimit;
    ...
    if (!available[modelName] || perDocumentLimit != null) {
      ...
      available[modelName] = {
        model,
        options,
        match.hasMatchFunction ? [data.match] .match,
        docs: [doc],
        ids: [ids],
        allIds: [ret],
        unpopulatedValues: [unpopulatedValue],
        localFieldSet([data.localField]),
        foreignFieldSet([data.foreignField]),
        justOne.justOne,
        isVirtual.isVirtual,
        virtual.virtual,
        count.count,
...
function _getLocalFieldValues(doc, localField, model, options, virtual, schema) {
  ...
  const localFieldPath = localFieldPathType === 'real' ?
    model.schema.path(localField) .schema;
  const localFieldGetters = localFieldPath && localFieldPath.getters ?
    localFieldPath.getters : [];

  localField = localFieldPath != null && localFieldPath.instance === 'Embedded' ? localField + '._id' ;

  const _populateOptions = get(options, 'options', {});

  const getters = 'getters' in _populateOptions ?
    _populateOptions.getters (virtual, 'options.getters', false);
  if (localFieldGetters.length !== 0 && getters) {
    const hydratedDoc = (doc.$__ != null) ? doc .hydrate(doc);
...
function convertTo_id(val, schema) {
  ...
  if (Array.isArray(val)) {
    const rawVal = val.__array != null ? val.__array ;
...
function _findRefPathForDiscriminators(doc, modelSchema, data, options, normalizedRefPath, ret) {
  // Re-8452. Embedded discriminators may not have `refPath`, so clear

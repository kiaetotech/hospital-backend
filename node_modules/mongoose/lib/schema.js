 ...
 * #### Example:
 *
 *     const child = new Schema({ name});
 *     const schema = new Schema({ name, age, children: [child] });
 *     const Tree = mongoose.model('Tree', schema);
 *
 *     // setting schema options
 *     new Schema({ name}, { _id, autoIndex})
 *
 * #### Options:
 *
 * - [autoIndex](https://mongoosejs.com/docs/guide.html#autoIndex)- defaults to null (which means use the connection's autoIndex option)
 * - [autoCreate](https://mongoosejs.com/docs/guide.html#autoCreate)- defaults to null (which means use the connection's autoCreate option)
 * - [bufferCommands](https://mongoosejs.com/docs/guide.html#bufferCommands)- defaults to true
 * - [bufferTimeoutMS](https://mongoosejs.com/docs/guide.html#bufferTimeoutMS)- defaults to 10000 (10 seconds). If `bufferCommands` is enabled, the amount of time Mongoose will wait for connectivity to be restablished before erroring out.
 * - [capped](https://mongoosejs.com/docs/guide.html#capped)| number | object - defaults to false
 * - [collection](https://mongoosejs.com/docs/guide.html#collection)- no default
 * - [discriminatorKey](https://mongoosejs.com/docs/guide.html#discriminatorKey)- defaults to `__t`
 * - [id](https://mongoosejs.com/docs/guide.html#id)- defaults to true
 * - [_id](https://mongoosejs.com/docs/guide.html#_id)- defaults to true
 * - [minimize](https://mongoosejs.com/docs/guide.html#minimize)- controls [document#toObject](https://mongoosejs.com/docs/api/document.html#Document.prototype.toObject()) behavior when called manually - defaults to true
 * - [read](https://mongoosejs.com/docs/guide.html#read)* - [writeConcern](https://mongoosejs.com/docs/guide.html#writeConcern)- defaults to null, use to override [the MongoDB server's default write concern settings](https://www.mongodb.com/docs/manual/reference/write-concern/)
 * - [shardKey](https://mongoosejs.com/docs/guide.html#shardKey)- defaults to `null`
 * - [strict](https://mongoosejs.com/docs/guide.html#strict)- defaults to true
 * - [strictQuery](https://mongoosejs.com/docs/guide.html#strictQuery)- defaults to false
 ...
 * - [versionKey](https://mongoosejs.com/docs/guide.html#versionKey)or object - defaults to "__v"
 * - [optimisticConcurrency](https://mongoosejs.com/docs/guide.html#optimisticConcurrency)- defaults to false. Set to true to enable [optimistic concurrency](https://thecodebarbarian.com/whats-new-in-mongoose-5-10-optimistic-concurrency.html).
 * - [collation](https://mongoosejs.com/docs/guide.html#collation)- defaults to null (which means use no collation)
 * - [timeseries](https://mongoosejs.com/docs/guide.html#timeseries)- defaults to null (which means this schema's collection won't be a timeseries collection)
 * - [selectPopulatedPaths](https://mongoosejs.com/docs/guide.html#selectPopulatedPaths)- defaults to `true`
 * - [skipVersioning](https://mongoosejs.com/docs/guide.html#skipVersioning)- paths to exclude from versioning
 * - [timestamps](https://mongoosejs.com/docs/guide.html#timestamps)or boolean - defaults to `false`. If true, Mongoose adds `createdAt` and `updatedAt` properties to your schema and manages those properties for you.
 * - [pluginTags](https://mongoosejs.com/docs/guide.html#pluginTags)of strings - defaults to `undefined`. If set and plugin called with `tags` option, will only apply that plugin to schemas with a matching tag.
 * - [virtuals](https://mongoosejs.com/docs/tutorials/virtuals.html#virtuals-via-schema-options)- virtuals to define, alias for [`.virtual`](https://mongoosejs.com/docs/api/schema.html#Schema.prototype.virtual())
 * - [collectionOptions]with options passed to [`createCollection()`](https://www.mongodb.com/docs/manual/reference/method/db.createCollection/) when calling `Model.createCollection()` or `autoCreate` set to true.
 *
 * #### Options for Nested Schemas:
 *
 * - `excludeIndexes`- defaults to `false`. If `true`, skip building indexes on this schema's paths.
 *
 * #### Note:
 *
 * _When nesting schemas, (`children` in the example above), always declare the child schema first before passing it into its parent._
 *
 * @param {Object|Schema|Array} [definition] Can be one ofdescribing schema paths, or schema to copy, or array of objects and schemas
 * @param {Object} [options]
 * @inherits NodeJS EventEmitter https://nodejs.org/api/events.html#class-eventemitter
 * @event `init`after the schema is compiled into a `Model`.
 * @api public
 */
...
function Schema(obj, options) {
  ...
  if (options && options.virtuals) {
    ...
    for (const pathName of pathNames) {
      const pathOptions = virtuals[pathName].options ? virtuals[pathName].options ;
 ...
 *
 *     console.log(schema.virtuals); // { answer{ path: 'answer', ... } }
 ...
 * #### Example:
 *
 *     const schema = new Schema({ a}).add({ b});
 *     schema.obj; // { a}
 *
 * @api public
 * @property obj
 ...
 * #### Example:
 *
 *     const schema = new Schema({ name}, { _id});
 *     schema.paths; // { name{ ... } }
 *
 *     schema.add({ age});
 *     schema.paths; // { name{ ... }, age{ ... } }
 *
 * @api public
 * @property paths
 * @memberOf Schema
...
Schema.prototype._clone = function _clone(Constructor) {
  Constructor = Constructor || (this.base == null ? Schema .base.Schema);
...
Schema.prototype.pick = function(paths, options) {
  ...
  for (const path of paths) {
    if (this.nested[path]) {
      newSchema.add({ [path](this.tree, path) });
    } else {
...
Schema.prototype.defaultOptions = function(options) {
  this._userProvidedOptions = options == null ? {} (options);
  const baseOptions = this.base && this.base.options || {};
  const strict = 'strict' in baseOptions ? baseOptions.strict ;
  const strictQuery = 'strictQuery' in baseOptions ? baseOptions.strictQuery ;
  const id = 'id' in baseOptions ? baseOptions.id ;
  options = {
    strict,
    strictQuery,
    bufferCommands,
    capped, // { size, max, autoIndexId }
    versionKey: '__v',
 ...
 * #### Example:
 *
 *     const eventSchema = new mongoose.Schema({ timestamp}, { discriminatorKey: 'kind' });
 *
 *     const clickedEventSchema = new mongoose.Schema({ element}, { discriminatorKey: 'kind' });
 ...
 *     TurboManSchema.add(ToySchema).add({ year});
 *
...
Schema.prototype.add = function add(obj, prefix) {
  ...
  // Special casetop-level `_id` to false should convert to disabling
  ...
  for (const key of keys) {
    ...
    // Retain `_id` but don't set it as a path, re-8274.
    ...
    if (typeof val === 'string') {
      ...
      const MongooseTypes = this.base != null ? this.base.Schema.Types .Types;
    ...
    if (!(isPOJO(val) || val instanceof SchemaTypeOptions)) {
      // Special-case-options definitely a path so leaf at this node
      // Examplesinstances, SchemaType instances
      if (prefix) {
      ...
      // Special-casewith no bona-fide type key - interpret as tree of deep paths so recurse
      // nested object `{ last: { name} }`. Avoid functions with `.type` re: #10807 because
 ...
 * #### Example:
 *
 *     const ToySchema = new Schema({ name, color, price});
 *
...
Schema.prototype.path = function(path, obj) {
  if (obj === undefined) {
    ...
    // Convert to '.$' to check subpaths re-6405
...
function gatherChildSchemas(schema) {
  ...
  for (const path of Object.keys(schema.paths)) {
    ...
    if (schematype.$isMongooseDocumentArray || schematype.$isSingleNested) {
      childSchemas.push({ schema.schema, model.caster });
    }
  }
...
Schema.prototype.interpretAsType = function(path, obj, options) {
  ...
  // copy of SchemaTypes re-7158 gh-6933
  const MongooseTypes = this.base != null ? this.base.Schema.Types .Types;
  const Types = this.base != null ? this.base.Types ('./types');
  ...
  if (Array.isArray(type) || type === Array || type === 'array' || type === MongooseTypes.Array) {
    ...
    // Handle both `new Schema({ arr: [{ subpath}] })` and `new Schema({ arr: [{ type: { subpath} }] })`
    ...
    if (typeof cast === 'string') {
      ...
      if (Object.keys(castFromTypeKey).length) {
        ...
        // declared inline, like `{ arr: [{ val: { $type} }] }`.
        // See gh-3560
        const childSchemaOptions = { minimize.minimize };
        if (options.typeKey) {
        ...
        // Special caseobject becomes mixed
    ...
    if (cast) {
      ...
      name = typeof type === 'string'
        ? type
        .schemaName || utils.getFunctionName(type);
  ...
  // Special case re-7049 because the bson `ObjectID` class' capitalization
...
function createMapNestedSchemaType(schema, schemaType, path, obj, options) {
  ...
  if (utils.hasUserDefinedProperty(obj, 'of')) {
    ...
    if (isInlineSchema) {
      _mapType = { [schema.options.typeKey]Schema(obj.of) };
    } else if (utils.isPOJO(obj.of)) {
      _mapType = Object.assign({}, obj.of);
    } else {
      _mapType = { [schema.options.typeKey].of };
    }
    ...
    if (_mapType[schema.options.typeKey] && _mapType[schema.options.typeKey].instanceOfSchema) {
      ...
      subdocumentSchema.eachPath((subpath, type) => {
        if (type.options.select === true || type.options.select === false) {
          throw new MongooseError('Cannot use schema-level projections (`select` or `select`) within maps at path "' + path + '.' + subpath + '"');
        }
      });
    }
 ...
 * #### Example:
 *
 *     const userSchema = new Schema({ name, registeredAt});
 ...
 * #### Example:
 *
 *     const s = new Schema({ name, nested: { foo} });
...
function getPositionalPathType(self, path, cleanPath) {
  ...
  for (let i = 1; i < subpaths.length; ++i) {
    ...
    // ignore if its just a position segment.0.subpath
    if (!/\D/.test(subpath)) {
      // Nested array
 ...
 * #### Example:
 *
 *     const toySchema = new Schema({ name, created});
 *
 ...
 *     });
 *
 *     toySchema.pre('deleteOne', { document}, function() {
 ...
 *     mongoose.model('Test', s); // Prints 'Schema Path Name'
 *
...
Schema.prototype.set = function(key, value, tags) {
  ...
  switch (key) {
    case 'read'(typeof value === 'string') {
        this.options[key] = { mode(value), tags };
      ...
      break;
    case 'timestamps'.setupTimestamp(value);
      this.options[key] = value;
      this._userProvidedOptions[key] = this.options[key];
      break;
    case '_id'.options[key] = value;
      this._userProvidedOptions[key] = this.options[key];
  ...
  if (key === 'strict') {
    _propagateOptionsToImplicitlyCreatedSchemas(this, { strict});
  }
  if (key === 'strictQuery') {
    _propagateOptionsToImplicitlyCreatedSchemas(this, { strictQuery});
  }
...
/**
 * Returns a list of indexes that this schema declares, via `schema.index()` or by `index` in a path's options.
 ...
 *       registeredAt: { type, index}
 *     });
 *
 *     // [ [ { email: 1 }, { unique, background} ],
 *     //   [ { registeredAt: 1 }, { background} ] ]
 *     userSchema.indexes();
 *
...
Schema.prototype.virtual = function(name, options) {
  ...
  if (utils.hasUserDefinedProperty(options, ['ref', 'refPath'])) {
      ...
      set(function(_v) {
        ...
        if (options.justOne || options.count) {
          ...
          if (typeof this.$$populatedVirtuals[name] !== 'object') {
            this.$$populatedVirtuals[name] = options.count ? _v ;
          }
        } else {
          this.$$populatedVirtuals[name] = Array.isArray(_v) ?
            _v :
    ...
    // Workaround for gh-8198virtual is under document array, make a fake
...
Schema.prototype.virtualpath = function(name) {
  return this.virtuals.hasOwnProperty(name) ? this.virtuals[name] ;
};
 ...
 * #### Example:
 *
 *     const schema = new Schema({ name, age});
 *     schema.remove('name');
...
Schema.prototype._getSchema = function(path) {
              ...
              if (parts[p] === '$' || isArrayFilter(parts[p])) {
                ...
                if (ret) {
                  ret.$parentSchemaDocArray = ret.$parentSchemaDocArray ||
                    (foundschema.schema.$isSingleNested ? null );
                }
                return ret;
              }
  ...
  for (let i = 0; i < parts.length; ++i) {
    if (parts[i] === '$' || isArrayFilter(parts[i])) {
      // Re-5628, because `schema.path()` doesn't take $ into account.
      parts[i] = '0';
    }
    ...
    while (p--) {
      ...
      if (foundschema) {
        if (foundschema.caster) {
          ...
          return {
            schema,
            pathType.$isSingleNested ? 'nested' : 'array'
          };
        }
        return { schema, pathType: 'real' };
      } else if (p === parts.length && schema.nested[trypath]) {
        return { schema, pathType: 'nested' };
      }
    }
    return { schema|| schema, pathType: 'undefined' };
  }
...
Schema.prototype._preCompile = function _preCompile() {
  this.plugin(idGetter, { deduplicate});
};

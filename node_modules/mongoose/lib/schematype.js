...
const setOptionsForDefaults = { _skipMarkModified};
 ...
 * #### Example:
 *
 *     const schema = new Schema({ name: { type, required} });
...
SchemaType.prototype.splitPath = function() {
  ...
  this._presplitPath = this.path.indexOf('.') === -1 ? [this.path] .path.split('.');
  return this._presplitPath;
};
 ...
 * #### Example:
 *
 *     const schema = new Schema({ n: { type, default: 10 })
 ...
 *
 *     // values are cast:
 *     const schema = new Schema({ aNumber: { type, default: 4.815162342 }})
 *     const M = db.model('M', schema)
 *     const m = new M;
 *     console.log(m.aNumber) // 4.815162342
 *
 *     // default unique objects for Mixed types:
 *     const schema = new Schema({ mixed.Types.Mixed });
 ...
 * #### Example:
 *
 *     const s = new Schema({ name: { type, index})
 *     const s = new Schema({ name: { type, index: -1 })
 *     const s = new Schema({ loc: { type: [Number], index: 'hashed' })
 *     const s = new Schema({ loc: { type: [Number], index: '2d', sparse})
 *     const s = new Schema({ loc: { type: [Number], index: { type: '2dsphere', sparse}})
 *     const s = new Schema({ date: { type, index: { unique, expires: '1d' }})
 *     s.path('my.path').index(true);
 *     s.path('my.date').index({ expires: 60 });
 *     s.path('my.path').index({ unique, sparse});
 *
 * #### Note:
 *
 ...
 * Specify `background` to override Mongoose's default._
 *
 ...
 * #### Example:
 *
 *     const s = new Schema({ name: { type, unique} });
 *     s.path('name').index({ unique});
 *
 * _NOTEthe constraint returns an `E11000` error from MongoDB when saving, not a Mongoose validation error._
 *
 * @param {Boolean} bool
 * @return {SchemaType} this
 * @api public
 ...
 * ### Example:
 *
 *      const s = new Schema({ name : { type, text } })
 *      s.path('name').index({ text });
 *
 * @param {Boolean} bool
 * @return {SchemaType} this
 * @api public
 ...
 * #### Example:
 *
 *     const s = new Schema({ name: { type, sparse} });
 *     s.path('name').index({ sparse});
 *
 * @param {Boolean} bool
 * @return {SchemaType} this
 * @api public
 ...
 * immutable paths unless the parent document has [`isNew`](https://mongoosejs.com/docs/api/document.html#Document.prototype.isNew()).
 *
 * #### Example:
 *
 ...
 *     Model.updateOne({}, { name: 'test2' }, { strict});
 *
 * @param {Boolean} bool
 * @return {SchemaType} this
 ...
 *
 * Mongoose calls this function with one parametercurrent `value` of the path. Mongoose
 * then uses the return value in the JSON output.
 *
 * #### Example:
 *
 *     const schema = new Schema({
 *       date: { type, transform=> v.getFullYear() }
 *     });
 *     const Model = mongoose.model('Test', schema);
 *
 *     await Model.create({ dateDate('2016-06-01') });
 *     const doc = await Model.findOne();
 *
 ...
 *     const s = new Schema({ name: { type, set}});
 *
 ...
 *
 * _NOTEcould have also just used the built-in `lowercase` SchemaType option instead of defining our own function._
 *
 *     new Schema({ email: { type, lowercase}})
 *
 ...
 *
 *     const nameSchema = new Schema({ name, keywords: [String] });
 ...
 *
 *     // defining within the schema
 *     const s = new Schema({ born: { type, get})
 *
 *     // or by retreiving its SchemaType
 *     const s = new Schema({ born})
 *     s.path('born').get(dob)
 *
 ...
 *
 *     const many = [
 *         { validator, message: 'uh oh' }
 *       , { validator, message: 'failed' }
 *     ]
 *     new Schema({ name: { type, validate}});
 *
 ...
 *
 * - PATHschema path where the error is being triggered.
 * - VALUEvalue assigned to the PATH that is triggering the error.
 * - KINDvalidation property that triggered the error i.e. required.
 * - REASONerror object that caused this error if there was one.
 *
...
SchemaType.prototype.validate = function(obj, message, type) {
  if (typeof obj === 'function' || obj && utils.getFunctionName(obj.constructor) === 'RegExp') {
    ...
    if (typeof message === 'function') {
      ...
      properties = isSimpleValidator(message) ? Object.assign({}, message) (message);
 ...
 * #### Example:
 *
 *     const s = new Schema({ born: { type, required})
 *
 *     // or with custom error message
 *
 *     const s = new Schema({ born: { type, required: '{PATH} is required!' })
 *
 *     // or with a function
 *
 *     const s = new Schema({
 ...
 *       username: {
 *         type,
 *         required() { return this.userId != null; }
 *       }
 *     })
 *
...
SchemaType.prototype.required = function(required, message) {
  ...
  this.requiredValidator = function(v) {
    ...
    if (cachedRequired != null && _this.path in cachedRequired) {
      ...
      return required.apply(this) ? _this.checkRequired(v, this) ;
    }
 ...
 *
 *     const postSchema = new Schema({ user.ObjectId });
 ...
 * #### Example:
 *
 *     T = db.model('T', new Schema({ x: { type, select}}));
...
SchemaType.prototype.doValidate = function(value, fn, scope, options) {
  ...
  for (let i = 0, len = validators.length; i < len; ++i) {
    ...
    const validatorProperties = isSimpleValidator(v) ? Object.assign({}, v) (v);
    validatorProperties.path = options && options.path ? options.path ;
...
SchemaType.prototype._castRef = function _castRef(value, doc, init) {
  ...
  if (value.$__ != null) {
    value.$__.wasPopulated = value.$__.wasPopulated || { value._id };
    return value;
  }

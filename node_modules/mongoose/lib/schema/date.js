 ...
 *
 *     const User = mongoose.model('User', new Schema({ test}));
 ...
 *
 *     const Model = mongoose.model('Test', new Schema({ date: { type, default: () => new Date() } }));
 ...
 *
 *     // expire in 24 hours
 *     new Schema({ createdAt: { type, expires: 60*60*24 }});
 *
 ...
 *
 *     // expire in 24 hours
 *     new Schema({ createdAt: { type, expires: '24h' }});
 *
 *     // expire in 1.5 hours
 *     new Schema({ createdAt: { type, expires: '1.5h' }});
 *
 *     // expire in 7 days
 *     const schema = new Schema({ createdAt});
 ...
 * #### Example:
 *
 *     const s = new Schema({ d: { type, min('1970-01-01') })
 *     const M = db.model('M', s)
 *     const m = new M({ d('1969-12-31') })
 *     m.save(function (err) {
 ...
 *     const schema = new Schema({ d: { type, min})
 *     const M = mongoose.model('M', schema);
 *     const s= new M({ d('1969-12-31') });
 *     s.validate(function (err) {
 *       console.log(String(err)) // ValidationErrorvalue of path `d` (1969-12-31) is before the limit (1970-01-01).
 *     })
 *
...
SchemaDate.prototype.min = function(value, message) {
  ...
  if (value) {
    ...
    if (typeof msg === 'string') {
      msg = msg.replace(/{MIN}/, (value === Date.now ? 'Date.now()' .toString()));
    }
    const _this = this;
    this.validators.push({
      validator.minValidator = function(val) {
        let _value = value;
 ...
 * #### Example:
 *
 *     const s = new Schema({ d: { type, max('2014-01-01') })
 *     const M = db.model('M', s)
 *     const m = new M({ d('2014-12-08') })
 *     m.save(function (err) {
 *       console.error(err) // validator error
 *       m.d = Date('2013-12-31');
 *       m.save() // success
 ...
 *     const schema = new Schema({ d: { type, max})
 *     const M = mongoose.model('M', schema);
 *     const s= new M({ d('2014-12-08') });
 *     s.validate(function (err) {
 *       console.log(String(err)) // ValidationErrorvalue of path `d` (2014-12-08) exceeds the limit (2014-01-01).
 *     })
 *
...
SchemaDate.prototype.max = function(value, message) {
  ...
  if (value) {
    ...
    if (typeof msg === 'string') {
      msg = msg.replace(/{MAX}/, (value === Date.now ? 'Date.now()' .toString()));
    }
    const _this = this;
    this.validators.push({
      validator.maxValidator = function(val) {
        let _value = value;

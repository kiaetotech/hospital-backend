 ...
 *
 *     const Model = mongoose.model('Test', new Schema({ buf} }));
 *     typeof (new Model({ buf.fromString('hello') }).buf); // 'string'
 *
 ...
 *
 *     const M = mongoose.model({ buf: { type, required} });
 *     new M({ buf.from('') }).validateSync(); // validation passes!
 *
 * @param {Function} fn
 * @return {Function}
 ...
 * #### Example:
 *
 *     const s = new Schema({ uuid: { type, subtype: 4 });
...
SchemaBuffer.prototype.castForQuery = function($conditional, val, context) {
  ...
  return casted ? casted.toObject({ transform, virtuals}) ;
};

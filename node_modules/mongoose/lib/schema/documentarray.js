...
DocumentArrayPath.options = { castNonArrays};
 ...
 *     docArrayPath.discriminator('Circle', Schema({ radius}));
 *
 * @param {String} name
...
DocumentArrayPath.prototype.discriminator = function(name, schema, options) {
  ...
  options = options || {};
  const tiedValue = utils.isPOJO(options) ? options.value ;
...
const _toObjectOptions = Object.freeze({ transform, virtuals});
const initDocumentOptions = Object.freeze({ skipId, willInit});
...
DocumentArrayPath.prototype.cast = function(value, doc, init, prev, options) {
  ...
  const rawArray = utils.isMongooseDocumentArray(value) ? value.__array ;

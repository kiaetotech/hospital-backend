...
function SchemaArray(key, cast, options, schemaOptions) {
  ...
  if (cast) {
    ...
    const name = typeof cast === 'string'
      ? cast
      .getFunctionName(cast);

    const Types = require('./index.js');
    const caster = Types.hasOwnProperty(name) ? Types[name] ;
    ...
    if (typeof caster === 'function' &&
        ...
        !caster.$isSchemaMap) {
      const path = this.caster instanceof EmbeddedDoc ? null ;
 ...
 *
 *     const M = mongoose.model({ arr: { type, required} });
...
SchemaArray.prototype.cast = function(value, doc, init, prev, options) {
  ...
  if (Array.isArray(value)) {
    ...
    if (!len && doc) {
      ...
      // Special casethis index is on the parent of what looks like
    ...
    let rawValue = utils.isMongooseArray(value) ? value.__array ;
    ...
    if (caster && this.casterConstructor !== Mixed) {
      try {
        ...
        for (i = 0; i < len; i++) {
          const opts = {};
          // Perf`arrayPath` is expensive for large arrays.
  ...
  const castNonArraysOption = this.options.castNonArrays != null ? this.options.castNonArrays .options.castNonArrays;
  if (init || castNonArraysOption) {
    // gh-2442we're loading this from the db and its not an array, mark
...
SchemaArray.prototype._castForPopulate = function _castForPopulate(value, doc) {
  ...
  if (Array.isArray(value)) {
    let i;
    const rawValue = value.__array ? value.__array ;
    const len = rawValue.length;
...
function cast$all(val, context) {
  ...
  val = val.map((v) => {
    ...
    if (v.$elemMatch != null) {
      return { $elemMatch(this.casterConstructor.schema, v.$elemMatch, null, this && this.$$context) };
    }

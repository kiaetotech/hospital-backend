...
function _createConstructor(schema, baseClass, options) {
  ...
  const proto = baseClass != null ? baseClass.prototype .prototype;
...
SubdocumentPath.prototype.$conditionalHandlers.$geoWithin = function handle$geoWithin(val, context) {
  return { $geometry.castForQuery(null, val.$geometry, context) };
};
...
SubdocumentPath.prototype.cast = function(val, doc, init, priorVal, options) {
  ...
  const defaultDiscriminatorValue = discriminatorKeyPath == null ? null .getDefault(doc);
  ...
  const selected = parentSelected == null ? null .keys(parentSelected).reduce((obj, key) => {
  ...
  if (init) {
    subdoc = new Constructor(void 0, selected, doc, false, { defaults});
    delete subdoc.$__.defaults;
    subdoc.$init(val);
    const exclude = isExclusive(selected);
    applyDefaults(subdoc, selected, exclude);
  } else {
    options = Object.assign({}, options, { priorDoc});
...
SubdocumentPath.prototype.doValidate = function(value, fn, scope, options) {
  ...
  if (value && !(value instanceof Constructor)) {
    value = new Constructor(value, null, (scope != null && scope.$__ != null) ? scope );
  }
 ...
 * #### Example:
 *
 *     const shapeSchema = Schema({ name}, { discriminatorKey: 'kind' });
 *     const schema = Schema({ shape});
 *
 *     const singleNestedPath = parentSchema.path('shape');
 *     singleNestedPath.discriminator('Circle', Schema({ radius}));
 *
 * @param {String} name
...
SubdocumentPath.prototype.clone = function() {
  const schematype = new this.constructor(
    this.schema,
    this.path,
    { ...this.options, _skipApplyDiscriminators}
  );

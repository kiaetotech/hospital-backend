 ...
 *
 *     const User = mongoose.model('User', new Schema({ name}));
 ...
 *
 *     const M = mongoose.model({ str: { type, required} });
 ...
 *     const s = new Schema({ state: { type, enum}})
 ...
 *     const m = new M({ state: 'invalid' })
 *     m.save(function (err) {
 *       console.error(String(err)) // ValidationErrorvalidator failed for path `state` with value `invalid`
 *       m.state = 'open'
 *       m.save(callback) // success
 *     })
 *
 ...
 * #### Example:
 *
 *     const s = new Schema({ email: { type, lowercase}})
 *     const M = db.model('M', s);
 ...
 * #### Example:
 *
 *     const s = new Schema({ caps: { type, uppercase}})
 *     const M = db.model('M', s);
 ...
 * #### Example:
 *
 *     const s = new Schema({ name: { type, trim}});
 *     const M = db.model('M', s);
 ...
 *
 *     // Equivalent to `findOne({ name.trim() })`
 *     M.findOne({ name});
 *
 ...
 * #### Example:
 *
 *     const schema = new Schema({ postalCode: { type, minlength: 5 })
 ...
 *     const schema = new Schema({ postalCode: { type, minlength})
 *     const Address = mongoose.model('Address', schema);
 *     const address = new Address({ postalCode: '9512' });
 *     address.validate(function (err) {
 *       console.log(String(err)) // ValidationErrorvalue of path `postalCode` (`9512`) is shorter than the minimum length (5).
 *     })
 *
...
SchemaString.prototype.minlength = function(value, message) {
  ...
  if (value !== null && value !== undefined) {
    ...
    this.validators.push({
      validator.minlengthValidator = function(v) {
 ...
 * #### Example:
 *
 *     const schema = new Schema({ postalCode: { type, maxlength: 9 })
 *     const Address = db.model('Address', schema)
 ...
 *     const schema = new Schema({ postalCode: { type, maxlength})
 *     const Address = mongoose.model('Address', schema);
 *     const address = new Address({ postalCode: '9512512345' });
 *     address.validate(function (err) {
 *       console.log(String(err)) // ValidationErrorvalue of path `postalCode` (`9512512345`) exceeds the maximum allowed length (9).
 *     })
 *
...
SchemaString.prototype.maxlength = function(value, message) {
  ...
  if (value !== null && value !== undefined) {
    ...
    this.validators.push({
      validator.maxlengthValidator = function(v) {
        return v === null || v.length <= value;
      },
      message,
      type: 'maxlength',
 ...
 * #### Example:
 *
 *     const s = new Schema({ name: { type, match: /^a/ }})
 *     const M = db.model('M', s)
 *     const m = new M({ name: 'I am invalid' })
 *     m.validate(function (err) {
 *       console.error(String(err)) // "ValidationError`name` is invalid (I am invalid)."
 *       m.name = 'apples'
 *       m.validate(function (err) {
 ...
 *     const s = new Schema({ file: { type, match}})
 *     const M = db.model('M', s);
 *     const m = new M({ file: 'invalid' });
 *     m.validate(function (err) {
 *       console.log(String(err)) // "ValidationErrorfile doesn't end in .html (invalid)"
 *     })
 *
 * Empty strings, `undefined`, and `null` values always pass the match validator. If you require these values, enable the `required` validator also.
 *
 *     const s = new Schema({ name: { type, match: /^a/, required}})
 *
...
SchemaString.prototype.checkRequired = function checkRequired(value, doc) {
  ...
  const _checkRequired = typeof this.constructor.checkRequired === 'function' ?
    this.constructor.checkRequired() .checkRequired();
...
Object.defineProperty(SchemaString.prototype, '$conditionalHandlers', {
  ...
  enumerable,
  writable,
  value.freeze($conditionalHandlers)
});

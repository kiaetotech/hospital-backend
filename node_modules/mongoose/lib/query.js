 ...
 *
 *     query.where('loc').within({ center: [50,50], radius: 10, unique, spherical});
 ...
 *     query.select('-c -d');
 *
 *     // Use `+` to override schema-level `select` without making the
 *     // projection inclusive.
 *     const schema = new Schema({
 *       foo: { type, select},
 *       bar*     });
 *     // ...
 *     query.select('+foo'); // Override foo's `select` without excluding `bar`
 *
...
Query.prototype.select = function select() {
  ...
  if (arguments.length !== 1) {
    throw new Error('Invalid selectonly takes 1 argument');
  }
  ...
  function sanitizeValue(value) {
    return typeof value === 'string' && sanitizeProjection ? value = 1 ;
  }
 ...
 *
 * - `w`the specified number of `mongod` servers, or tag set of `mongod` servers, that must acknowledge this write before this write is considered successful.
 * - `j`, set to `true` to request acknowledgement that this operation has been persisted to MongoDB's on-disk journal.
 * - `wtimeout`[`w > 1`](https://mongoosejs.com/docs/api/query.html#Query.prototype.w()), the maximum amount of time to wait for this write to propagate through the replica set before this operation fails. The default is `0`, which means no timeout.
 *
 ...
 * - [timestamps](https://mongoosejs.com/docs/guide.html#timestamps)`timestamps` is set in the schema, set this option to `false` to skip timestamps for that particular update. Has no effect if `timestamps` is not enabled in the schema options.
 * - overwriteDiscriminatorKeysetting the discriminator key in the update. Will use the correct discriminator schema if the update changes the discriminator key.
 *
...
const printRawResultDeprecationWarning = util.deprecate(
  ...
  'The `rawResult` option for Mongoose queries is deprecated. Use `includeResultMetadata` as a replacement for `rawResult`.'
);
 ...
 * use.
 *
 * Calling `query.explain(v)` is equivalent to `query.setOptions({ explain})`
 *
 * #### Example:
 *
 *     const query = new Query();
 ...
 * and earlier.
 *
 * Calling `query.allowDiskUse(v)` is equivalent to `query.setOptions({ allowDiskUse})`
 *
 * #### Example:
 *
 ...
 *
 * @param {Boolean} [v] Enable/disable `allowDiskUse`. If called with 0 arguments, sets `allowDiskUse`
 * @return {Query} this
 * @api public
 */
 ...
 *
 * Calling `query.maxTimeMS(v)` is equivalent to `query.setOptions({ maxTimeMS})`
 *
 * #### Example:
 *
 *     const query = new Query();
...
Query.prototype.lean = function(v) {
  this._mongooseOptions.lean = arguments.length ? v ;
  return this;
};
 ...
 * #### Example:
 *
 *     // Updates `{ $set: { updatedAtDate() } }`
 *     new Query().updateOne({}, {}).set('updatedAt', new Date());
 *     new Query().updateMany({}, {}).set({ updatedAtDate() });
 *
 ...
 * #### Example:
 *
 *     const TestSchema = new Schema({ num});
 ...
 *
 * - `populate`array representing what paths will be populated. Should have one entry for each call to [`Query.prototype.populate()`](https://mongoosejs.com/docs/api/query.html#Query.prototype.populate())
 * - `lean`truthy, Mongoose will not [hydrate](https://mongoosejs.com/docs/api/model.html#Model.hydrate()) any documents that are returned from this query. See [`Query.prototype.lean()`](https://mongoosejs.com/docs/api/query.html#Query.prototype.lean()) for more information.
 * - `strict`how Mongoose handles keys that aren't in the schema for updates. This option is `true` by default, which means Mongoose will silently strip any paths in the update that aren't in the schema. See the [`strict` mode docs](https://mongoosejs.com/docs/guide.html#strict) for more information.
 * - `strictQuery`how Mongoose handles keys that aren't in the schema for the query `filter`. This option is `false` by default, which means Mongoose will allow `Model.find({ foo: 'bar' })` even if `foo` is not in the schema. See the [`strictQuery` docs](https://mongoosejs.com/docs/guide.html#strictQuery) for more information.
 * - `nearSphere``$nearSphere` instead of `near()`. See the [`Query.prototype.nearSphere()` docs](https://mongoosejs.com/docs/api/query.html#Query.prototype.nearSphere())
 *
...
Query.prototype._find = async function _find() {
  ...
  const completeManyOptions = Object.assign({}, {
    session&& this.options && this.options.session || null,
    lean.lean || null
  });
  ...
  if (!mongooseOptions.populate) {
    ...
    return mongooseOptions.lean ?
      _completeManyLean(_this.model.schema, docs, null, completeManyOptions) (_this.model, docs, fields, userProvidedFields, completeManyOptions);
  }
...
Query.prototype.merge = function(source) {
  ...
  if (source instanceof Query) {
    ...
    utils.merge(this._conditions, { _id}, opts);
  ...
  if (Array.isArray(source.$and)) {
    ...
    this._conditions.$and = (this._conditions.$and || []).concat(
      source.$and.map(el => utils.isPOJO(el) ? utils.merge({}, el) )
    );
  }
  if (Array.isArray(source.$or)) {
    ...
    this._conditions.$or = (this._conditions.$or || []).concat(
      source.$or.map(el => utils.isPOJO(el) ? utils.merge({}, el) )
    );
  }
...
Query.prototype._completeOne = function(doc, res, callback) {
  ...
  if (!mongooseOptions.populate) {
    ...
    return mongooseOptions.lean ?
      _completeOneLean(model.schema, doc, null, res, options, callback) (model, doc, res, options, projection, userProvidedFields,
        null, callback);
  }
 ...
 * on the following query properties, projection, update, distinct.
 */
 ...
 *
 * `estimatedDocumentCount()` does **not** accept a filter. `Model.find({ foo}).estimatedDocumentCount()`
 ...
 * - `n`number of documents deleted. Equal to `deletedCount`.
 *
 * #### Example:
 *
 ...
 * #### Available options
 *
 * - `new`- if true, return the modified document rather than the original. defaults to false (changed in 4.0)
 * - `upsert`- creates the object if it doesn't exist. defaults to false.
 * - `fields`: {Object|String} - Field selection. Equivalent to `.select(fields).findOneAndUpdate()`
 * - `sort`multiple docs are found by the conditions, sets the sort order to choose which doc to update
 * - `maxTimeMS`a time limit on the query - requires mongodb >= 2.6.0
 * - `runValidators`true, runs [update validators](https://mongoosejs.com/docs/validation.html#update-validators) on this command. Update validators validate the update operation against the model's schema.
 * - `setDefaultsOnInsert`: `true` by default. If `setDefaultsOnInsert` and `upsert` are true, mongoose will apply the [defaults](https://mongoosejs.com/docs/defaults.html) specified in the model's schema if a new document is created.
 * - `rawResult`true, returns the [raw result from the MongoDB driver](https://mongodb.github.io/node-mongodb-native/4.9/interfaces/ModifyResult.html)
 *
 * #### Example:
 *
 ...
 * @param {Boolean} [options.new=false] By default, `findOneAndUpdate()` returns the document as it was **before** `update` was applied. If you set `new`, `findOneAndUpdate()` will instead give you the object after `update` was applied.
 * @param {Object} [options.lean] if truthy, mongoose will return the document as a plain JavaScript object rather than a mongoose document. See [`Query.lean()`](https://mongoosejs.com/docs/api/query.html#Query.prototype.lean()) and [the Mongoose lean tutorial](https://mongoosejs.com/docs/tutorials/lean.html).
 * @param {ClientSession} [options.session=null] The session associated with this query. See [transactions docs](https://mongoosejs.com/docs/transactions.html).
 * @param {Boolean|String} [options.strict] overwrites the schema's [strict mode option](https://mongoosejs.com/docs/guide.html#strict)
 * @param {Boolean} [options.timestamps=null] If set to `false` and [schema-level timestamps](https://mongoosejs.com/docs/guide.html#timestamps) are enabled, skip timestamps for this update. Note that this allows you to overwrite timestamps. Does nothing if schema-level timestamps are not set.
 * @param {Boolean} [options.returnOriginal=null] An alias for the `new` option. `returnOriginal` is equivalent to `new`.
 * @param {Boolean} [options.translateAliases=null] If set to `true`, translates any schema-defined aliases in `filter`, `projection`, `update`, and `distinct`. Throws an error if there are any conflicts where both alias and raw property are defined on the same object.
...
Query.prototype._findOneAndUpdate = async function _findOneAndUpdate() {
  ...
  const _opts = Object.assign({}, options, {
    setDefaultsOnInsert._mongooseOptions.setDefaultsOnInsert
  });
  ...
  const doc = options.includeResultMetadata === false ? res .value;
 ...
 * - `sort`multiple docs are found by the conditions, sets the sort order to choose which doc to update
 * - `maxTimeMS`a time limit on the query - requires mongodb >= 2.6.0
 * - `rawResult`true, resolves to the [raw result from the MongoDB driver](https://mongodb.github.io/node-mongodb-native/4.9/interfaces/ModifyResult.html)
 *
 * #### Example:
 *
 ...
 *
 *     function(error, doc) {
 *       // errorerrors that occurred
 *       // docdocument before updates are applied if `new`, or after updates if `new = true`
 *     }
 *
 * #### Example:
 *
...
Query.prototype._findOneAndDelete = async function _findOneAndDelete() {
  ...
  const doc = includeResultMetadata === false ? res .value;
...
function _completeOneLean(schema, doc, path, res, opts, callback) {
  if (opts.lean && typeof opts.lean.transform === 'function') {
    ...
    for (let i = 0; i < schema.childSchemas.length; i++) {
      const childPath = path ? path + '.' + schema.childSchemas[i].model.path .childSchemas[i].model.path;
 ...
 * #### Example:
 *
 *     const res = await Person.updateMany({ name: /Stark$/ }, { isDeleted});
 ...
 *       return res == null ?
 *         res :
 *         Object.assign(res, { loadedAtDate() });
 *     });
 *
 * @method transform
 * @memberOf Query
...
Query.prototype.orFail = function(err) {
  this.transform(res => {
    switch (this.op) {
      case 'find'(res.length === 0) {
          throw _orFailError(err, this);
        }
        break;
      case 'findOne'(res == null) {
          throw _orFailError(err, this);
        }
        break;
      case 'replaceOne''updateMany''updateOne'(res && res.matchedCount === 0) {
          throw _orFailError(err, this);
        }
        break;
      case 'findOneAndDelete''findOneAndRemove'((res && res.lastErrorObject && res.lastErrorObject.n) === 0) {
          throw _orFailError(err, this);
        }
        break;
      case 'findOneAndUpdate''findOneAndReplace'((res && res.lastErrorObject && res.lastErrorObject.updatedExisting) === false) {
          throw _orFailError(err, this);
        }
        break;
      case 'deleteMany''deleteOne'(res.deletedCount === 0) {
          throw _orFailError(err, this);
        }
        break;
      default;
...
Query.prototype._castUpdate = function _castUpdate(obj, overwrite) {
  ...
  const baseSchema = schema._baseSchema ? schema._baseSchema ;
  ...
  return castUpdate(schema, obj, {
    overwrite,
    strict._mongooseOptions.strict,
    upsert,
    arrayFilters.options.arrayFilters,
    overwriteDiscriminatorKey._mongooseOptions.overwriteDiscriminatorKey
...
Query.prototype.cast = function(model, obj) {
  ...
  const opts = { upsert.options && this.options.upsert };
  if (this.options) {
    if ('strict' in this.options) {
 ...
 * #### Valid options
 *
 *   - `transform`function which accepts a mongoose document. The return value of the function will be emitted on `data` and returned by `.next()`.
 *
 * @return {QueryCursor}
 * @param {Object} [options]
...
Query.prototype.tailable = function(val, opts) {
  // we need to support the tailable({ awaitData }) as well as the
  // tailable(true, {awaitData }) syntax that mquery does not support
 ...
 *     query.where('loc').within().geometry({ type: 'Polygon', coordinates})
 *
 *     // or
 *     const polyB = [[ 0, 0 ], [ 1, 1 ]]
 *     query.where('loc').within().geometry({ type: 'LineString', coordinates})
 *
 *     // or
 *     const polyC = [ 0, 0 ]
 *     query.where('loc').within().geometry({ type: 'Point', coordinates})
 *
 *     // or
 *     query.where('loc').intersects().geometry({ type: 'Point', coordinates})
 *
 ...
 *     query.where('loc').near({ center: [10, 10], maxDistance: 5, spherical});
...
Query.prototype.near = function() {
  ...
  if (arguments.length === 1) {
    if (Array.isArray(arguments[0])) {
      params.push({ center[0], spherical});
    ...
    if (typeof arguments[0] === 'number' && typeof arguments[1] === 'number') {
      params.push({ center: [arguments[0], arguments[1]], spherical});
    } else if (typeof arguments[0] === 'string' && Array.isArray(arguments[1])) {
      params.push(arguments[0]);
      params.push({ center[1], spherical});
    ...
    if (typeof arguments[0] === 'string' && typeof arguments[1] === 'number'
      ...
      params.push(arguments[0]);
      params.push({ center: [arguments[1], arguments[2]], spherical});
    } else {
      throw new TypeError('invalid argument');
    }
  } else {
 ...
 * #### Example:
 *
 *     query.where('loc').near({ center: [10, 10], spherical});
 *
 * @deprecated
 ...
 * #### Example:
 *
 *     const area = { center: [50, 50], radius: 10, unique}
 *     query.where('loc').within().circle(area)
 *     // alternatively
 *     query.circle('loc', area);
 *
 *     // spherical calculations
 *     const area = { center: [50, 50], radius: 10, unique, spherical}
 *     query.where('loc').within().circle(area)
 *     // alternatively
 *     query.circle('loc', area);
 *

...
const defaultMongooseSymbol = Symbol.for('mongoose');

require('./helpers/printJestWarning');

const objectIdHexRegexp = /^[0-9A-Fa-f]{24}$/;

const { AsyncLocalStorage } = require('node_hooks');
...
function Mongoose(options) {
  ...
  Object.defineProperty(this, 'plugins', {
    ...
    enumerable,
    writable,
    value.values(builtinPlugins).map(plugin => ([plugin, { deduplicate}]))
  });
}
...
Mongoose.prototype.setDriver = function setDriver(driver) {
  const _mongoose = this instanceof Mongoose ? this ;
 ...
 *
 *     mongoose.set({ debug, autoIndex}); // set multiple options at once
 *
 * Currently supported options are:
 * - `allowDiskUse`to `true` to set `allowDiskUse` to true to all aggregation operations by default.
 * - `applyPluginsToChildSchemas`: `true` by default. Set to false to skip applying global plugins to child schemas
 * - `applyPluginsToDiscriminators`: `false` by default. Set to true to apply global plugins to discriminator schemas. This typically isn't necessary because plugins are applied to the base schema and discriminators copy all middleware, methods, statics, and properties from the base schema.
 * - `autoCreate`to `true` to make Mongoose call [`Model.createCollection()`](https://mongoosejs.com/docs/api/model.html#Model.createCollection()) automatically when you create a model with `mongoose.model()` or `conn.model()`. This is useful for testing transactions, change streams, and other features that require the collection to exist.
 * - `autoIndex`: `true` by default. Set to false to disable automatic index creation for all models associated with this Mongoose instance.
 * - `bufferCommands`/disable mongoose's buffering mechanism for all connections and models
 * - `bufferTimeoutMS`bufferCommands is on, this option sets the maximum amount of time Mongoose buffering will wait before throwing an error. If not specified, Mongoose will use 10000 (10 seconds).
 * - `cloneSchemas`: `false` by default. Set to `true` to `clone()` all schemas before compiling into a model.
 * - `debug``true`, prints the operations mongoose sends to MongoDB to the console. If a writable stream is passed, it will log to that stream, without colorization. If a callback function is passed, it will receive the collection name, the method name, then all arguments passed to the method. For example, if you wanted to replicate the default logging, you could output from the callback `Mongoose: ${collectionName}.${methodName}(${methodArgs.join(', ')})`.
 * - `id``true`, adds a `id` virtual to all schemas unless overwritten on a per-schema basis.
 * - `timestamps.createdAt.immutable`: `true` by default. If `false`, it will change the `createdAt` field to be [`immutable`](https://mongoosejs.com/docs/api/schematype.html#SchemaType.prototype.immutable) which means you can update the `createdAt`
 * - `maxTimeMS`set, attaches [maxTimeMS](https://www.mongodb.com/docs/manual/reference/operator/meta/maxTimeMS/) to every query
 * - `objectIdGetter`: `true` by default. Mongoose adds a getter to MongoDB ObjectId's called `_id` that returns `this` for convenience with populate. Set this to false to remove the getter.
 * - `overwriteModels`to `true` to default to overwriting models with the same name when calling `mongoose.model()`, as opposed to throwing an `OverwriteModelError`.
 * - `returnOriginal``false`, changes the default `returnOriginal` option to `findOneAndUpdate()`, `findByIdAndUpdate`, and `findOneAndReplace()` to false. This is equivalent to setting the `new` option to `true` for `findOneAndX()` calls by default. Read our [`findOneAndUpdate()` tutorial](https://mongoosejs.com/docs/tutorials/findoneandupdate.html) for more information.
 ...
 * - `toJSON`: `{ transform, flattenDecimals}` by default. Overwrites default objects to [`toJSON()`](https://mongoosejs.com/docs/api/document.html#Document.prototype.toJSON()), for determining how Mongoose documents get serialized by `JSON.stringify()`
 * - `toObject`: `{ transform, flattenDecimals}` by default. Overwrites default objects to [`toObject()`](https://mongoosejs.com/docs/api/document.html#Document.prototype.toObject())
 *
 ...
 *
 *     // with mongodb:// URI
 *     db = mongoose.createConnection('mongodb://user@127.0.0.1/database');
 *
 *     // and options
 *     const opts = { db: { native_parser}}
 *     db = mongoose.createConnection('mongodb://user@127.0.0.1/database', opts);
 *
 *     // replica sets
 *     db = mongoose.createConnection('mongodb://user@127.0.0.1,anotherhost,yetanother/database');
 *
 *     // and options
 *     const opts = { replset: { strategy: 'ping', rs_name: 'testSet' }}
 *     db = mongoose.createConnection('mongodb://user@127.0.0.1,anotherhost,yetanother/database', opts);
 *
...
Mongoose.prototype.createConnection = function(uri, options) {
  ...
  if (arguments.length > 0) {
    conn.openUri(uri, { ...options, _fireAndForget});
  }
 ...
 * #### Example:
 *
 *     mongoose.connect('mongodb://user@127.0.0.1/database');
 *
 *     // replica sets
 *     const uri = 'mongodb://user@127.0.0.1,anotherhost,yetanother/mydatabase';
 *     mongoose.connect(uri);
 *
 *     // with options
 *     mongoose.connect(uri, options);
 *
 *     // Using `await` throws "MongooseServerSelectionErrorselection timed out after 30000 ms"
 ...
 *     conn.model('Actor', new Schema({ name}));
 *
 ...
 * #### Example:
 *
 *     const schema = new Schema({ name}, { collection: 'actor' });
 *
 *     // or
 *
...
Mongoose.prototype.model = function(name, schema, collection, options) {
  ...
  const overwriteModels = _mongoose.options.hasOwnProperty('overwriteModels') ?
    _mongoose.options.overwriteModels .overwriteModels;
 ...
 * #### Example:
 *
 *     mongoose.model('User', new Schema({ name}));
 ...
 * #### Example:
 *
 *     const childSchema = new Schema({ parentId.ObjectId });
 *
 * @property ObjectId
 * @api public
 */
 ...
 * #### Example:
 *
 *     const vehicleSchema = new Schema({ fuelLevel.Decimal128 });
 *
 * @property Decimal128
 * @api public
 */
 ...
 * #### Example:
 *
 *     const schema = new Schema({ arbitrary.Mixed });
 *
 * @property Mixed
 * @api public
 */
 ...
 *
 * ```javascript
 * const obj = { username: 'val', pwd: { $ne} };
 * sanitizeFilter(obj);
 * obj; // { username: 'val', pwd: { $eq: { $ne} } });
 * ```
 *
 ...
 *
 * ```javascript
 * const obj = { username: 'val', pwd({ $type: 'string', $eq: 'my secret' }) };
 * sanitizeFilter(obj);
 *

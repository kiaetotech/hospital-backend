...
const requireFunc =
	typeof __webpack_require__ === 'function' ? __non_webpack_require__ ;
...
const dynamicImport = async id => {
	try {
		...
		const mod = await import(/* webpackIgnore*/ fileUrl);
...
function getOptions(name, options, sync) {
	...
	conf.searchPlaces.forEach(place => {
		...
		if (typeof loader !== 'function') {
			throw new Error(
				`Loader for extension "${place}" is not a function${typeof loader}.`,
			);
		}
	});
...
/** @type {(props| string[], obj) => unknown} */
function getPackageProp(props, obj) {
	if (typeof props === 'string' && props in obj) return obj[props];
	return (
		(Array.isArray(props) ? props .split('.')).reduce(
			(acc, prop) => (acc === undefined ? acc [prop]),
			obj,
		) || null
	);
}
...
/** @type {(loader('./index').Loader, ext) => void} */
function validateLoader(loader, ext) {
	if (!loader) throw new Error(`No loader specified for extension "${ext}"`);
	if (typeof loader !== 'function') throw new Error('loader is not a function');
}

/** @type {(enableCache) => <T>(c<string, T>, filepath, res) => T} */
	...
	return {
		...
		async load(filepath) {
			...
			return emplace(
				loadCache,
				absPath,
				transform(isEmpty ? {...result, isEmpty, config} ),
			);
		},
		clearLoadCache() {
...
module.exports.lilconfigSync = function lilconfigSync(name, options) {
	...
	return {
		...
		load(filepath) {
			...
			// cosmiconfig returns undefined for empty files
			result.config = isEmpty ? undefined (absPath, content);

...
const https = isSecure ? require('https') ('http');
...
const agent = config.api_proxy ? new https.Agent(config.api_proxy) ;
...
exports.upload_chunked_stream = function upload_chunked_stream(callback, options = {}) {
  ...
  let params = build_upload_params(options);
  let chunk_size = options.chunk_size != null ? options.chunk_size .part_size;
...
exports.create_slideshow = function create_slideshow(options, callback) {
  ...
  return call_api("create_slideshow", callback, options, function () {
    ...
    // Try to use {options.transformation} to generate a transformation (Example.transformation.width, options.transformation.height)
    const transformation = utils.generate_transformation_string(extend({}, ensureOption(options, 'transformation', {})));

    return [
      {
        timestamp.timestamp(),
        manifest_transformation_transformation,
    ...
    return [
      {
        timestamp.timestamp(),
        type.type,
        from_public_id_public_id,
        to_public_id_public_id,
        overwrite.overwrite,
        invalidate.invalidate,
        to_type.to_type,
        context.context,
...
function call_tags_api(tag, command, public_ids = [], callback, options = {}) {
  return call_api("tags", callback, options, function () {
    let params = {
      timestamp.timestamp(),
      public_ids.build_array(public_ids),
      command,
      type.type
    };
    if (tag != null) {
    ...
    result.responsive_breakpoints.forEach(
      ({
        ...
        result.public_id,
        {
          ...
          resource_type,
          raw_transformation,
          format(breakpoints[0].url).slice(1)
        },
...
function post(url, post_data, boundary, file, callback, options) {
  ...
  if ((file != null) || options.stream) {
    ...
    let filename = options.stream ? options.filename ? options.filename : "file" (file);
  ...
  let post_options = {
    ...
    pathname.pathname,
    query.search ? parsedUrl.search.substring(1) : ''
  };
...
function encodeFieldPart(boundary, name, value) {
  return [
    `--${boundary}\r\n`,
    `Content-Disposition-data; name="${name}"\r\n`,
    '\r\n',
    `${value}\r\n`,
    ''
  ].join('');
...
function encodeFilePart(boundary, type, name, filename) {
  return [
    `--${boundary}\r\n`,
    `Content-Disposition-data; name="${name}"; filename="${filename}"\r\n`,
    `Content-Type: ${type}\r\n`,
    '\r\n',
    ''
  ].join('');
...
exports.image_upload_tag = function image_upload_tag(field, options = {}) {
  ...
  let tag_options = extend({
    type: "file",
    name: "file",
    "data-url".upload_url(options),
    "data-form-data".upload_tag_params(options),
...
exports.update_metadata = function update_metadata(metadata, public_ids, callback, options = {}) {
  return call_api("metadata", callback, options, function () {
    let params = {
      metadata.encode_context(metadata),
      public_ids.build_array(public_ids),
      timestamp.timestamp(),
      type.type,

...
function parseCloudinaryConfigFromEnvURL(ENV_STR) {
  ...
  if (uri.protocol === 'cloudinary:') {
    conf = Object.assign({}, conf, {
      cloud_name.hostname,
      api_key.username || (auth && auth.split(":")[0]),
      api_secret.password || (auth && auth.split(":")[1]),
      private_cdn.pathname != null && uri.pathname !== '' && uri.pathname !== '/',
      secure_distribution.pathname && uri.pathname !== '/' ? uri.pathname.substring(1) });
  } else if (uri.protocol === 'account:') {
    conf = Object.assign({}, conf, {
      account_id.hostname,
      provisioning_api_key.username || (auth && auth.split(":")[0]),
      provisioning_api_secret.password || (auth && auth.split(":")[1])
    });
  }
...
module.exports = function (new_config, new_value) {
  if ((cloudinary_config == null) || new_config === true) {
    ...
    if (!isEmpty(CLOUDINARY_API_PROXY)) {
      extendCloudinaryConfig({ api_proxy_API_PROXY }, cloudinary_config);
    }
    ...
    [CLOUDINARY_ENV_URL, CLOUDINARY_ENV_ACCOUNT_URL].forEach((ENV_URL) => {
      if (ENV_URL) {
        ...
        // Provide Query support in ENV url cloudinary://key@test123?foo[bar]=value

 ...
 * @return {{sdkSemver, techVersion, sdkCode}} A map of relevant versions and codes
 */
function getSDKVersions(useSDKVersion = 'default', useNodeVersion = 'default') {
  // allow to pass a custom SDKVersion
  const sdkSemver = useSDKVersion === 'default' ? readSdkSemver() ;

  // allow to pass a custom techVersion (Node version)
  const version = process.version.slice(1);
  const techVersion = useNodeVersion === 'default' ? version ;

 ...
 * Utilities
 * @module utils
 * @borrows module_token as generate_auth_token
 */
...
function process_custom_pre_function(customPreFunction) {
  ...
  return utils.isString(result) ? `pre:${result}` ;
}
...
function process_if(ifValue) {
  return ifValue ? "if_" + normalize_expression(ifValue) ;
}
  ...
  if (resource_type === 'text' || resource_type === 'subtitles') {
    ...
    if (!isEmpty(text)) {
      ...
      const encodedParts = textDividedByVariables.map(subText => {
        ...
        const isVariable = matches ? matches.length > 0 ;
        if (isVariable) {
          return subText;
        }
...
function build_upload_params(options) {
  let params = {
    ...
    allowed_formats.allowed_formats && toArray(options.allowed_formats).join(","),
    asset_folder.asset_folder,
    async.as_safe_bool(options.async),
    backup.as_safe_bool(options.backup),
    callback.callback,
    cinemagraph_analysis.as_safe_bool(options.cinemagraph_analysis),
    ...
    media_metadata.as_safe_bool(options.media_metadata),
    ...
    quality_analysis.as_safe_bool(options.quality_analysis),
    responsive_breakpoints.generate_responsive_breakpoints_string(options.responsive_breakpoints),
    return_delete_token.as_safe_bool(options.return_delete_token),
    timestamp.timestamp || exports.timestamp(),
    ...
    use_filename_as_display_name.as_safe_bool(options.use_filename_as_display_name),
    quality_override.quality_override,
    accessibility_analysis.as_safe_bool(options.accessibility_analysis),
    use_asset_folder_as_public_id_prefix.as_safe_bool(options.use_asset_folder_as_public_id_prefix),
    visual_search.as_safe_bool(options.visual_search),
    on_success.on_success,
    auto_transcription.auto_transcription,
    auto_chaptering.as_safe_bool(options.auto_chaptering)
  };
...
function build_custom_headers(headers) {
  switch (true) {
  case headers == nullvoid 0;
  case isArray(headers)headers.join("\n");
  case isObject(headers)entries(headers).map(([k, v]) => `${k}:${v}`).join("\n");
  defaultheaders;
  }
}
...
function generate_transformation_string(options) {
  ...
  if (base_transformations.some(isObject)) {
    base_transformations = base_transformations.map(tr => utils.generate_transformation_string(isObject(tr) ? clone(tr) : {transformation}));
  } else {
  ...
  let params = {
    a_expression(angle),
    ar_expression(consumeOption(options, "aspect_ratio")),
    b,
    bo,
    c,
    co,
    ...
    e_expression(effect),
    fl,
    fn_function || custom_pre_function,
    fps,
    h_expression(height),
    ki_expression(consumeOption(options, "keyframe_interval")),
    l,
    o_expression(consumeOption(options, "opacity")),
    q_expression(consumeOption(options, "quality")),
    r,
    t_transformation,
    u,
    w_expression(width),
    x_expression(consumeOption(options, "x")),
    y_expression(consumeOption(options, "y")),
    z_expression(consumeOption(options, "zoom"))
  };
...
function base_api_url(api_version) {
  ...
  return (path = [], options = []) => {
    ...
    let encoded_path = Array.isArray(path) ? path.map(encode_path) _path(path);
...
function api_string_to_sign(params_to_sign, signature_version = 2) {
  let params = entries(params_to_sign)
    .map(([k, v]) => [String(k), Array.isArray(v) ? v.join(",") ])
    .filter(([k, v]) => v !== null && v !== undefined && v !== "");
  params.sort((a, b) => a[0].localeCompare(b[0]));
  let paramStrings = params.map(([k, v]) => {
    const paramString = `${k}=${v}`;
    return signature_version >= 2 ? encode_param(paramString) ;
  });
 ...
 *   - Version 1behavior without parameter encoding
 *   - Version 2+ (default)parameter encoding to prevent parameter smuggling
...
function clear_blank(hash) {
  ...
  entries(hash).filter(([k, v]) => utils.present(v)).forEach(([k, v]) => {
    filtered_hash[k] = v.filter ? v.filter(x => x) ;
  });
 ...
 * asset and version id are returned with resource(<PUBLIC_ID1>, { versions})
 * @param asset_id
 * @param version_id
 * @param options
 * @returns {string }
 ...
 *   Must be one of | | * @param {string} [options.type="upload"] The specific file type of resources: ||* @param {string|Array} [options.tags] list of tags to include in the archive
 ...
 *   return a JSON with URLs for accessing the archive. Possible values: , * @param {string} [options.target_format="zip"]
...
function escapeQuotes(value) {
  return isString(value) ? value.replace(/\"/g, '&#34;').replace(/\'/g, '&#39;') ;
}
...
function split_range(range) { // (range.constructor) {
  case String(!OFFSET_ANY_PATTERN_RE.test(range)) {
      return range;
    }
    return range.split("..");
  case Array[first(range), last(range)];
  default[null, null];
  }
}

function norm_range_value(value) { // offset = String(value).match(RANGE_VALUE_RE);
  if (offset) {
...
function archive_params(options = {}) {
  return {
    allow_missing.as_safe_bool(options.allow_missing),
    async.as_safe_bool(options.async),
    expires_at.expires_at,
    flatten_folders.as_safe_bool(options.flatten_folders),
    flatten_transformations.as_safe_bool(options.flatten_transformations),
    keep_derived.as_safe_bool(options.keep_derived),
    mode.mode,
    notification_url.notification_url,
    prefixes.prefixes && toArray(options.prefixes),
    fully_qualified_public_ids.fully_qualified_public_ids && toArray(options.fully_qualified_public_ids),
    public_ids.public_ids && toArray(options.public_ids),
    skip_transformation_name.as_safe_bool(options.skip_transformation_name),
    tags.tags && toArray(options.tags),
    target_format.target_format,
    target_public_id.target_public_id,
    target_asset_folder.target_asset_folder,
    target_tags.target_tags && toArray(options.target_tags),
    timestamp.timestamp || exports.timestamp(),
    transformations.build_eager(options.transformations),
    type.type,
    use_original_filename.as_safe_bool(options.use_original_filename)
  };
}
...
exports.create_source_tag = function create_source_tag(src, source_type, codecs = null) {
  let video_type = source_type === 'ogv' ? 'ogg' _type;
  let mime_type = `video/${video_type}`;
  if (!isEmpty(codecs)) {
    let codecs_str = isArray(codecs) ? codecs.join(', ') ;
...
function build_streaming_profiles_param(options = {}) {
  ...
  if (isArray(params.representations)) {
    params.representations = JSON.stringify(params.representations.map(r => ({
      transformation.generate_transformation_string(r.transformation)
    })));
  }
  return params;
}

...
function initializeSharedData(defaults) {
  ...
  if (!sharedData) {
    ...
    if (Object.defineProperty) {
      Object.defineProperty(this, sharedDataKey, { value});
    } else {
...
var sharedData = initializeSharedData({
  ...
  processEmitHook,
  /** @type {HookState} */
  moduleResolveFilenameHook,

  /** @type {Array<(request, parent, isMain, options, redirectedRequest) => void>} */
  ...
  fileContentsCache.create(null),

  // Maps a file path to a source map for that file
  /** @type {Record<string, {url, map}} */
  sourceMapCache.create(null),
...
var reSourceMap = /^data\/json[^,]+base64,/;
...
// Return pathOrUrl in the same style as matchStyleOfa file URL or a native path
...
function retrieveSourceMapURL(source) {
  ...
  if (isInBrowser()) {
     try {
       ...
       xhr.send(null);
       fileData = xhr.readyState === 4 ? xhr.responseText ;
...
// constructor).
/** @type {(source) => import('./source-map-support').UrlAndMap | null} */
...
function mapSourcePosition(position) {
  ...
  if (!sourceMap) {
    ...
    if (urlAndMap) {
      sourceMap = setSourceMapCache(position.source, {
        url.url,
        mapAnyMap(urlAndMap.map, urlAndMap.url)
      });
  ...
  if (sourceMap && sourceMap.map) {
    ...
    if (originalPosition.source !== null) {
      ...
      // However, we want to ensure we output in same format as inputor native path
...
function CallSiteToString() {
  ...
  var line = "";
  var isAsync = this.isAsync ? this.isAsync() ;
  if(isAsync) {
    line += 'async ';
    var isPromiseAll = this.isPromiseAll ? this.isPromiseAll() ;
    var isPromiseAny = this.isPromiseAny ? this.isPromiseAny() ;
...
function cloneCallSite(frame) {
  ...
  Object.getOwnPropertyNames(Object.getPrototypeOf(frame)).forEach(function(name) {
    object[name] = /^(?|get)/.test(name) ? function() { return frame[name].call(frame); } [name];
  });
...
function wrapCallSite(frame, state) {
  ...
  if (state === undefined) {
    state = { nextPosition, curPosition}
  }
  if(frame.isNative()) {
...
function createPrepareStackTrace(hookState) {
  ...
  function prepareStackTrace(error, stack) {
    ...
    if (kIsNodeError) {
      ...
      errorString = message ? name + ": " + message ;
    }

    var state = { nextPosition, curPosition};
    var processedStack = [];
    ...
    if (!sharedData.moduleResolveFilenameHook) {
      ...
      Module._resolveFilename = sharedData.moduleResolveFilenameHook.installedValue = function (request, parent, isMain, options) {
        if (moduleResolveFilenameHook.enabled) {
          // Match all source-map-support entrypoints-map-support, source-map-support/register
          let requestRedirect;

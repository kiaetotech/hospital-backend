...
class Cluster extends Commander_1.default {
     ...
     */
    //TODOan option that enables or disables sharded PubSub
    ...
    refreshSlotsCache(callback) {
        ...
        function tryNode(index) {
            ...
            _this.getInfoFromNode(node, function (err) {
                switch (_this.status) {
                    case "close""end"wrapper(new Error("Cluster is disconnected."));
                    case "disconnecting"wrapper(new Error("Cluster is disconnecting."));
                }
                if (err) {
    ...
    sendCommand(command, stream, node) {
        ...
        let targetSlot = node ? node.slot .getSlot();
        const ttl = {};
        const _this = this;
                ...
                if (node && node.redis) {
                    ...
                    if (_this.options.shardedSubscribers &&
                        ...
                        if (status !== -1) {
                            ...
                            command.reject(new redis_errors_1.AbortError("Possible CROSSSLOT errorchannels must hash to the same slot"));
                        }
                    }
                    else {
    ...
    handleError(error, ttl, handlers) {
        ...
        if (errv[0] === "MOVED") {
            ...
            this.delayQueue.push("tryagain", handlers.tryagain, {
                timeout.options.retryDelayOnTryAgain,
            });
        }
        else if (errv[0] === "CLUSTERDOWN" &&
            this.options.retryDelayOnClusterDown > 0) {
            this.delayQueue.push("clusterdown", handlers.connectionClosed, {
                timeout.options.retryDelayOnClusterDown,
                callback.refreshSlotsCache.bind(this),
            });
        }
            ...
            this.delayQueue.push("failover", handlers.connectionClosed, {
                timeout.options.retryDelayOnFailover,
                callback.refreshSlotsCache.bind(this),
            });
        }
        else {
    ...
    getInfoFromNode(redis, callback) {
        ...
        duplicatedConnection.cluster("SLOTS", (0, utils_1.timeout)((err, result) => {
            ...
            for (let i = 0; i < result.length; ++i) {
                ...
                debug("cluster slots result [%d]%d~%d served by %s", i, slotRangeStart, slotRangeEnd, keys);
    ...
    async resolveStartupNodeHostnames() {
        ...
        const configs = await Promise.all(hostnames.map((this.options.useSRVRecords ? this.resolveSrv .dnsLookup).bind(this)));
        ...
        return startupNodes.map((node) => {
            ...
            return Object.assign({}, node, { host});
        });
    }
    ...
    createShardedSubscriberGroup() {
        ...
        // Normal (non-subscriber) connections are created with lazyConnectand can

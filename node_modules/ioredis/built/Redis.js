...
class Redis extends Commander_1.default {
    ...
    sendCommand(command, stream) {
        ...
        let writable = this.status === "ready" ||
            (!stream &&
                this.status === "connect" &&
                (0, commands_1.exists)(command.name, { caseInsensitive}) &&
                ((0, commands_1.hasFlag)(command.name, "loading", { nameCaseInsensitive}) ||
        ...
        if (!this.stream) {
            ...
            // TODOshould be able to remove this as the PR has already been merged.
    ...
    getBlockingTimeoutInMs(command) {
        ...
        // Feature is opt-inenabled when blockingTimeout is set to a positive number
        ...
        if (typeof timeout === "number") {
            if (timeout > 0) {
                ...
                return (timeout +
                    ((_a = this.options.blockingTimeoutGrace) !== null && _a !== void 0 ? _a _1.DEFAULT_REDIS_OPTIONS.blockingTimeoutGrace));
            }
    ...
    handleReconnection(err, item) {
        ...
        switch (needReconnect) {
            case 1true(this.status !== "reconnecting") {
                    this.disconnect(true);
                }
                item.command.reject(err);
                break;
            case 2(this.status !== "reconnecting") {
                    this.disconnect(true);
                }
    ...
    _getServerAddress() {
        if ("path" in this.options && this.options.path) {
            return { address.options.path, port};
        }
        return {
    ...
    _buildCommandContext(command) {
        ...
        return {
            ...
            database: (_c = (_b = (_a = this.condition) === null || _a === void 0 ? void 0 : _a.select) !== null && _b !== void 0 ? _b .options.db) !== null && _c !== void 0 ? _c : 0,
            serverAddress,
            serverPort,
        };
    }
    ...
    parseOptions(...args) {
        ...
        if (isTls) {
            (0, lodash_1.defaults)(options, { tls});
        }
        ...
        this.info(function (err, res) {
            if (err) {
                if (err.message && err.message.includes("NOPERM")) {
                    console.warn(`Skipping the ready check because INFO command fails: "${err.message}". You can disable ready check with "enableReadyCheck". More://github.com/luin/ioredis/wiki/Disable-ready-check.`);

...
function command(yargs, usage, validation, globalMiddleware = []) {
    ...
    self.addHandler = function addHandler(cmd, description, builder, handler, commandMiddleware, deprecated) {
        ...
        if (Array.isArray(cmd)) {
            ...
            let command = (Array.isArray(cmd.command) || typeof cmd.command === 'string') ? cmd.command (cmd);
            if (cmd.aliases)
    ...
    self.addDirectory = function addDirectory(dir, context, req, callerFile, opts) {
        ...
        requireDirectory({ require, filename}, dir, opts);
    };
    ...
    self.runCommand = function runCommand(command, yargs, parsed, commandIndex) {
        ...
        if (isCommandBuilderCallback(builder)) {
            ...
            const innerYargs = yargs_1.isYargsInstance(builderOutput) ? builderOutput ;
    ...
    function usageFromParentCommandsCommandHandler(parentCommands, commandHandler) {
        const c = DEFAULT_MARKER.test(commandHandler.original) ? commandHandler.original.replace(DEFAULT_MARKER, '').trim() .original;
    ...
    self.runDefaultBuilderOn = function (yargs) {
        ...
        if (shouldUpdateUsage(yargs)) {
            ...
            const commandString = DEFAULT_MARKER.test(defaultCommand.original)
                ? defaultCommand.original .original.replace(/^[^[\]<>]*/, '$0 ');

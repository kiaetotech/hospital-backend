...
function Yargs(processArgs = [], cwd = process.cwd(), parentRequire = require) {
    ...
    self.resetOptions = self.reset = function resetOptions(aliases = {}) {
        ...
        usage = usage ? usage.reset(localLookup) _1.usage(self, y18n);
        validation = validation ? validation.reset(localLookup) _1.validation(self, usage, y18n);
        command = command ? command.reset() _1.command(self, usage, validation, globalMiddleware);
        if (!completion)
    ...
    self.resetOptions();
    // temporary hack"freezing" of reset-able state for parse(msg, cb)
    const frozens = [];
    function freeze() {
        frozens.push({
            options,
            configObjects.configObjects.slice(0),
            exitProcess,
            groups,
            strict,
            strictCommands,
    ...
    self.requiresArg = function (keys) {
        ...
        // see://github.com/yargs/yargs/pull/1572
        // TODOthis work with aliases, using a check similar to
    ...
    function populateParserHintDictionary(builder, type, key, value, singleKeyHandler) {
        if (Array.isArray(key)) {
            ...
            // an object of key value pairs: {'x'() {}, 'y'() {}}
    ...
    self.parse = function parse(args, shortCircuit, _parseFn) {
        ...
        if (typeof args === 'undefined') {
            ...
            unfreeze();
            // TODOthis compatibility hack when we release yargs@15.x.parsed = tmpParsed;
            return argv;
        }
    ...
    self.option = self.options = function option(key, opt) {
        ...
        if (typeof key === 'object') {
            ...
            // A required option can be specified via "demand".
            if (demand) {
                self.demand(key, demand);
            }
            if (opt.demandOption) {
                self.demandOption(key, typeof opt.demandOption === 'string' ? opt.demandOption );
            }
            if (opt.conflicts) {
    ...
    self.version = function version(opt, msg, ver) {
        ...
        versionOpt = typeof opt === 'string' ? opt ;
    ...
    self.terminalWidth = () => {
        argsert_1.argsert([], 0);
        return typeof process.stdout.columns !== 'undefined' ? process.stdout.columns ;
    };
    ...
    self._parseArgs = function parseArgs(args, shortCircuit, _calledFromCommand, commandIndex) {
        ...
        try {
            ...
            if (shortCircuit) {
                return (populateDoubleDash || _calledFromCommand) ? argv ._copyDoubleDash(argv);
            }
                ...
                if (handlerKeys.length) {
                    ...
                    for (let i = (commandIndex || 0), cmd; argv._[i] !== undefined; i++) {
                        ...
                        if (~handlerKeys.indexOf(cmd) && cmd !== completionCommand) {
                            ...
                            return populateDoubleDash ? innerArgv ._copyDoubleDash(innerArgv);
                        }
    ...
    self._copyDoubleDash = function (argv) {
        ...
        // TODO(bcoe)command parsing such that this delete is not
        // necessary://github.com/yargs/yargs/issues/1482
        try {
            delete argv['--'];
        }
        catch (_err) { }

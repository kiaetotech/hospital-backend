...
(function (Trace) {
    function fromString(value) {
        ...
        switch (value) {
            case 'off'Trace.Off;
            case 'messages'Trace.Messages;
            case 'compact'Trace.Compact;
            case 'verbose'Trace.Verbose;
            defaultTrace.Off;
        }
    }
    ...
    function toString(value) {
        switch (value) {
            case Trace.Off'off';
            case Trace.Messages'messages';
            case Trace.Compact'compact';
            case Trace.Verbose'verbose';
            default'off';
        }
    }
...
function createMessageConnection(messageReader, messageWriter, _logger, options) {
    const logger = _logger !== undefined ? _logger .NullLogger;
    ...
    const cancellationStrategy = (options && options.cancellationStrategy) ? options.cancellationStrategy .Message;
        ...
        try {
            ...
            if (messages_1.Message.isNotification(message) && message.method === CancelNotification.type.method) {
                ...
                if (messages_1.Message.isRequest(toCancel)) {
                    ...
                    const response = (strategy && strategy.cancelUndispatched) ? strategy.cancelUndispatched(toCancel, cancelUndispatched) (toCancel);
    ...
    async function handleRequest(requestMessage) {
        ...
        function reply(resultOrError, method, startTime) {
            ...
            if (resultOrError instanceof messages_1.ResponseError) {
                ...
                message.result = resultOrError === undefined ? null ;
            }
        ...
        if (requestHandler || starRequestHandler) {
            ...
            const cancellationSource = IdCancellationReceiverStrategy.is(cancellationStrategy.receiver)
                ...
                .receiver.createCancellationTokenSource(requestMessage);
    ...
    function handleResponse(responseMessage) {
        ...
        if (responseMessage.id === null) {
            if (responseMessage.error) {
                logger.error(`Received response message without idis: \n${JSON.stringify(responseMessage.error, undefined, 4)}`);
            }
            else {
                ...
                if (notificationHandler) {
                    if (message.params === undefined) {
                        ...
                        if (message.method === ProgressNotification.type.method && params.length === 2 && ProgressToken.is(params[0])) {
                            await notificationHandler({ token[0], value[1] });
                        }
                        else {
                            if (type !== undefined) {
    ...
    function stringifyTrace(params) {
        ...
        switch (trace) {
            case Trace.VerboseJSON.stringify(params, null, 4);
            case Trace.CompactJSON.stringify(params);
            defaultundefined;
        }
    }
    ...
    function computeSingleParam(parameterStructures, param) {
        switch (parameterStructures) {
            case messages_1.ParameterStructures.auto(isNamedParam(param)) {
                    return nullToUndefined(param);
                }
                else {
                    return [undefinedToNull(param)];
                }
            case messages_1.ParameterStructures.byName(!isNamedParam(param)) {
                    throw new Error(`Received parameters by name but param is not an object literal.`);
                }
                return nullToUndefined(param);
            case messages_1.ParameterStructures.byPosition[undefinedToNull(param)];
            defaultnew Error(`Unknown parameter structure ${parameterStructures.toString()}`);
        }
    }
    function computeMessageParams(type, params) {
        ...
        switch (numberOfParams) {
            case 0= undefined;
                break;
            case 1= computeSingleParam(type.parameterStructures, params[0]);
                break;
            default= [];
    ...
    const connection = {
        sendNotification: (type, ...args) => {
            ...
            if (Is.string(type)) {
                ...
                switch (numberOfParams) {
                    case 0= undefined;
                        break;
                    case 1= computeSingleParam(parameterStructures, args[paramStart]);
                        break;
                    default(parameterStructures === messages_1.ParameterStructures.byName) {
        ...
        onNotification: (type, handler) => {
            ...
            if (Is.func(type)) {
                ...
                if (Is.string(type)) {
                    method = type;
                    notificationHandlers.set(type, { type, handler });
                }
                else {
                    method = type.method;
        ...
        sendRequest: (type, ...args) => {
            ...
            if (Is.string(type)) {
                ...
                messageParams = computeMessageParams(type, params);
                const numberOfParams = type.numberOfParams;
                token = cancellation_1.CancellationToken.is(params[numberOfParams]) ? params[numberOfParams] ;
            }
            ...
            return new Promise(async (resolve, reject) => {
                ...
                const responsePromise = { method, timerStart.now(), resolve, reject};
                try {
        ...
        onRequest: (type, handler) => {
            ...
            if (StarRequestHandler.is(type)) {
                ...
                if (handler !== undefined) {
                    method = type;
                    requestHandlers.set(type, { handler, type});
                }
            }
            else {
                if (handler !== undefined) {
        ...
        trace(_value, _tracer, sendNotificationOrTraceOptions) => {
            ...
            if (_sendNotification && !isClosed() && !isDisposed()) {
                await connection.sendNotification(SetTraceNotification.type, { value.toString(_value) });
            }
        },
        onError.event,
        onClose.event,
    ...
    connection.onNotification(LogTraceNotification.type, (params) => {
        ...
        tracer.log(params.message, verbose ? params.verbose );
    });

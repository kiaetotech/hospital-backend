    ...
    while (true) {
        ...
        const visitNode = (node) => {
            ...
            if (state.next === undefined) {
                ...
                switch (result) {
                    case -1(!isJSDocNodeKind(node.kind)) {
                            state.left = node.end;
                        }
                        nodeAfterLeft = undefined;
                        break;
        ...
        if (state.next === undefined) {
            ...
            while (state.left < end) {
                const token = scanner.getToken();
                const tokenFullStart = scanner.getTokenFullStart();
                const tokenStart = allowPositionInLeadingTrivia ? tokenFullStart .getTokenStart();
                const tokenEnd = scanner.getTokenEnd();
                const flags = scanner.getTokenFlags();
                if (tokenEnd > end) {
                    break;
    ...
    const find = (n) => {
        ...
        n.forEachChild(node => {
            ...
            if (nodes.length > 0 && !skipSingleCommentChildrenImpl) {
                ...
                const lookupIndex = index >= 0 ? index - 1 .length - 1;
...
function findRightmostValidToken(sourceFile, endPos, containingNode, position) {
    ...
    const find = (n, endPos) => {
        ...
        if (!shouldSkipChild(n)) {
            const startPos = rightmostValidNode !== undefined ? rightmostValidNode.end .pos;
...
function isJSDocCommentChildKind(kind) {
    switch (kind) {
        case SyntaxKind.JSDocTextSyntaxKind.JSDocLinkSyntaxKind.JSDocLinkCodeSyntaxKind.JSDocLinkPlaintrue;
        defaultfalse;
    }
}
...
function binarySearchNodeList(nodes, testNode, onLeft, onMatch) {
    ...
    while (lo <= hi) {
        ...
        if (node.flags & NodeFlags.Reparsed) {
            // Skip reparsed nodesto find a non-reparsed node nearby
            let found = false;

...
export function getTextOfJSDocComment(comment) {
    return typeof comment === "string" ? comment
        ?.map(c => c.kind === SyntaxKind.JSDocText ? c.text (c)).join("");
}
function isVariableLike(node) {
    switch (node.kind) {
        case SyntaxKind.BindingElementSyntaxKind.EnumMemberSyntaxKind.ParameterSyntaxKind.PropertyAssignmentSyntaxKind.PropertyDeclarationSyntaxKind.PropertySignatureSyntaxKind.ShorthandPropertyAssignmentSyntaxKind.VariableDeclarationtrue;
    }
    return false;
}
...
function getNextJSDocCommentLocation(node) {
    ...
    if (parent) {
        switch (parent.kind) {
            case SyntaxKind.PropertyAssignmentSyntaxKind.ExportAssignmentSyntaxKind.PropertyDeclarationSyntaxKind.VariableDeclarationSyntaxKind.SatisfiesExpressionSyntaxKind.ReturnStatementSyntaxKind.VariableStatementSyntaxKind.ExpressionStatementparent;
            case SyntaxKind.VariableDeclarationList(parent.declarations[0] === node) {
                    return parent;
                }
                break;
        }
...
function formatJSDocLink(link) {
    const kind = link.kind === SyntaxKind.JSDocLink ? "link"
        .kind === SyntaxKind.JSDocLinkCode ? "linkcode"
            : "linkplain";
...
function entityNameToString(name) {
    switch (name.kind) {
        case SyntaxKind.ThisKeyword"this";
        case SyntaxKind.PrivateIdentifierSyntaxKind.Identifiername.getFullWidth() === 0 ? name.text .getText();
        case SyntaxKind.QualifiedNameentityNameToString(name.left) + "." + entityNameToString(name.right);
        case SyntaxKind.PropertyAccessExpression(isIdentifier(name.name) || isPrivateIdentifier(name.name)) {
                return entityNameToString(name.expression) + "." + entityNameToString(name.name);
            }
            else {
                return assertNever(name.name);
            }
        case SyntaxKind.JsxNamespacedNameentityNameToString(name.namespace) + ":" + entityNameToString(name.name);
        defaultassertNever(name);
    }
}

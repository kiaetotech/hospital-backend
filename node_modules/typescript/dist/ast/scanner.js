...
export function computeLineStarts(text) {
    ...
    while (pos < text.length) {
        ...
        pos++;
        switch (ch) {
            case CharacterCodes.carriageReturn(text.charCodeAt(pos) === CharacterCodes.lineFeed) {
                    pos++;
                }
            // falls through
            case CharacterCodes.lineFeed.push(lineStart);
                lineStart = pos;
                break;
            default(ch > CharacterCodes.maxAsciiCharacter && isLineBreak(ch)) {
...
export function couldStartTrivia(text, pos) {
    const ch = text.charCodeAt(pos);
    switch (ch) {
        case CharacterCodes.carriageReturnCharacterCodes.lineFeedCharacterCodes.tabCharacterCodes.verticalTabCharacterCodes.formFeedCharacterCodes.spaceCharacterCodes.slashCharacterCodes.lessThanCharacterCodes.barCharacterCodes.equalsCharacterCodes.greaterThantrue;
        case CharacterCodes.hashpos === 0;
        defaultch > CharacterCodes.maxAsciiCharacter;
    }
}
export function skipTrivia(text, pos, stopAfterLineBreak, stopAtComments, inJSDoc) {
    ...
    while (true) {
        const ch = text.charCodeAt(pos);
        switch (ch) {
            case CharacterCodes.carriageReturn(text.charCodeAt(pos + 1) === CharacterCodes.lineFeed) {
                    pos++;
                }
            // falls through
            case CharacterCodes.lineFeed++;
                ...
                continue;
            case CharacterCodes.tabCharacterCodes.verticalTabCharacterCodes.formFeedCharacterCodes.space++;
                continue;
            case CharacterCodes.slash(stopAtComments) {
                    break;
                }
                ...
                break;
            case CharacterCodes.lessThanCharacterCodes.barCharacterCodes.equalsCharacterCodes.greaterThan(isConflictMarkerTrivia(text, pos)) {
                    pos = scanConflictMarkerTrivia(text, pos);
                    canConsumeStar = false;
                    continue;
                }
                break;
            case CharacterCodes.hash(pos === 0 && isShebangTrivia(text, pos)) {
                    pos = scanShebangTrivia(text, pos);
                    continue;
                }
                break;
            case CharacterCodes.asterisk(canConsumeStar) {
                    pos++;
                    canConsumeStar = false;
                    continue;
                }
                break;
            default(ch > CharacterCodes.maxAsciiCharacter && (isWhiteSpaceLike(ch))) {
                    pos++;
                    continue;
                }
                break;
...
function iterateCommentRanges(reduce, text, pos, trailing, cb, state, initial) {
    ...
    scan(pos >= 0 && pos < text.length) {
        ...
        switch (ch) {
            ...
            case CharacterCodes.tabCharacterCodes.verticalTabCharacterCodes.formFeedCharacterCodes.space++;
                continue;
            case CharacterCodes.slashnextChar = text.charCodeAt(pos + 1);
                let hasTrailingNewLine = false;
                if (nextChar === CharacterCodes.slash || nextChar === CharacterCodes.asterisk) {
                    const kind = nextChar === CharacterCodes.slash ? SyntaxKind.SingleLineCommentTrivia .MultiLineCommentTrivia;
                    const startPos = pos;
                    pos += 2;
...
function parsePseudoBigInt(stringValue) {
    ...
    switch (stringValue.charCodeAt(1)) {
        case CharacterCodes.bCharacterCodes.B= 1;
            break;
        case CharacterCodes.oCharacterCodes.O= 3;
            break;
        case CharacterCodes.xCharacterCodes.X= 4;
            break;
        default:
    ...
    for (let i = endIndex - 1, bitOffset = 0; i >= startIndex; i--, bitOffset += log2Base) {
        ...
        const digit = digitChar <= CharacterCodes._9
            ...
            : 10 + digitChar - (digitChar <= CharacterCodes.F ? CharacterCodes.A .a);
...
export function createScanner(skipTrivia, languageVariant = LanguageVariant.Standard, textInitial, start, length) {
    ...
    // See://github.com/microsoft/TypeScript/issues/52924
    ...
    function codePointChecked(pos) {
        return pos >= 0 && pos < end ? codePointUnchecked(pos) .EOF;
    }
    function charCodeUnchecked(pos) {
        return text.charCodeAt(pos);
    }
    function charCodeChecked(pos) {
        return pos >= 0 && pos < end ? charCodeUnchecked(pos) .EOF;
    }
    ...
    function scanTemplateAndSetTokenValue(shouldEmitInvalidEscapeError) {
        ...
        while (true) {
            if (pos >= end) {
                ...
                tokenFlags |= TokenFlags.Unterminated;
                resultingToken = startedWithBacktick ? SyntaxKind.NoSubstitutionTemplateLiteral .TemplateTail;
                break;
            }
            ...
            if (currChar === CharacterCodes.$ && pos + 1 < end && charCodeUnchecked(pos + 1) === CharacterCodes.openBrace) {
                contents += text.substring(start, pos);
                pos += 2;
                resultingToken = startedWithBacktick ? SyntaxKind.TemplateHead .TemplateMiddle;
                break;
            }
    ...
    function scanEscapeSequence(flags) {
        ...
        pos++;
        switch (ch) {
            case CharacterCodes._0(pos >= end || !isDigit(charCodeUnchecked(pos))) {
                    return "\0";
                }
            // falls through
            case CharacterCodes._1CharacterCodes._2CharacterCodes._3(pos < end && isOctalDigit(charCodeUnchecked(pos))) {
                    pos++;
                }
            // falls through
            case CharacterCodes._4CharacterCodes._5CharacterCodes._6CharacterCodes._7(pos < end && isOctalDigit(charCodeUnchecked(pos))) {
                    pos++;
                }
            ...
            case CharacterCodes._8CharacterCodes._9|= TokenFlags.ContainsInvalidEscape;
                if (flags & EscapeSequenceScanningFlags.ReportInvalidEscapeErrors) {
                    return String.fromCharCode(ch);
                }
                return text.substring(start, pos);
            case CharacterCodes.b"\b";
            case CharacterCodes.t"\t";
            case CharacterCodes.n"\n";
            case CharacterCodes.v"\v";
            case CharacterCodes.f"\f";
            case CharacterCodes.r"\r";
            case CharacterCodes.singleQuote"'";
            case CharacterCodes.doubleQuote'"';
            case CharacterCodes.u(pos < end && charCodeUnchecked(pos) === CharacterCodes.openBrace) {
                    pos -= 2;
            ...
            case CharacterCodes.x(; pos < start + 4; pos++) {
                    if (!(pos < end && isHexDigit(charCodeUnchecked(pos)))) {
                        tokenFlags |= TokenFlags.ContainsInvalidEscape;
                        return text.substring(start, pos);
                    }
            ...
            case CharacterCodes.carriageReturn(pos < end && charCodeUnchecked(pos) === CharacterCodes.lineFeed) {
                    pos++;
                }
            // falls through
            case CharacterCodes.lineFeedCharacterCodes.lineSeparatorCharacterCodes.paragraphSeparator"";
            defaultString.fromCharCode(ch);
        }
    }
    ...
    function scan() {
        ...
        while (true) {
            ...
            switch (ch) {
                case CharacterCodes.lineFeedCharacterCodes.carriageReturn|= TokenFlags.PrecedingLineBreak;
                    if (skipTrivia) {
                        pos++;
                        continue;
                    }
                ...
                case CharacterCodes.tabCharacterCodes.verticalTabCharacterCodes.formFeedCharacterCodes.spaceCharacterCodes.nonBreakingSpaceCharacterCodes.oghamCharacterCodes.enQuadCharacterCodes.emQuadCharacterCodes.enSpaceCharacterCodes.emSpaceCharacterCodes.threePerEmSpaceCharacterCodes.fourPerEmSpaceCharacterCodes.sixPerEmSpaceCharacterCodes.figureSpaceCharacterCodes.punctuationSpaceCharacterCodes.thinSpaceCharacterCodes.hairSpaceCharacterCodes.zeroWidthSpaceCharacterCodes.narrowNoBreakSpaceCharacterCodes.mathematicalSpaceCharacterCodes.ideographicSpaceCharacterCodes.byteOrderMark(skipTrivia) {
                        pos++;
                        continue;
                    }
                    else {
                ...
                case CharacterCodes.exclamation(charCodeUnchecked(pos + 1) === CharacterCodes.equals) {
                ...
                case CharacterCodes.doubleQuoteCharacterCodes.singleQuote= scanString();
                    return token = SyntaxKind.StringLiteral;
                case CharacterCodes.backticktoken = scanTemplateAndSetTokenValue(/*shouldEmitInvalidEscapeError*/ false);
                case CharacterCodes.percent(charCodeUnchecked(pos + 1) === CharacterCodes.equals) {
                        return pos += 2, token = SyntaxKind.PercentEqualsToken;
                    }
                    pos++;
                    return token = SyntaxKind.PercentToken;
                case CharacterCodes.ampersand(charCodeUnchecked(pos + 1) === CharacterCodes.ampersand) {
                        if (charCodeUnchecked(pos + 2) === CharacterCodes.equals) {
                ...
                case CharacterCodes.asterisk(charCodeUnchecked(pos + 1) === CharacterCodes.equals) {
                ...
                case CharacterCodes.plus(charCodeUnchecked(pos + 1) === CharacterCodes.plus) {
                ...
                case CharacterCodes.minus(charCodeUnchecked(pos + 1) === CharacterCodes.minus) {
                ...
                case CharacterCodes.dot(isDigit(charCodeUnchecked(pos + 1))) {
                        scanNumber();
                ...
                case CharacterCodes._0(pos + 2 < end && (charCodeUnchecked(pos + 1) === CharacterCodes.X || charCodeUnchecked(pos + 1) === CharacterCodes.x)) {
                        pos += 2;
                ...
                // falls through
                case CharacterCodes._1CharacterCodes._2CharacterCodes._3CharacterCodes._4CharacterCodes._5CharacterCodes._6CharacterCodes._7CharacterCodes._8CharacterCodes._9token = scanNumber();
                case CharacterCodes.colon++;
                    return token = SyntaxKind.ColonToken;
                case CharacterCodes.semicolon++;
                    return token = SyntaxKind.SemicolonToken;
                case CharacterCodes.lessThan(isConflictMarkerTrivia(text, pos)) {
                        pos = scanConflictMarkerTrivia(text, pos);
                        if (skipTrivia) {
                            continue;
                        }
                ...
                case CharacterCodes.equals(isConflictMarkerTrivia(text, pos)) {
                        pos = scanConflictMarkerTrivia(text, pos);
                        if (skipTrivia) {
                            continue;
                        }
                ...
                case CharacterCodes.greaterThan(isConflictMarkerTrivia(text, pos)) {
                        pos = scanConflictMarkerTrivia(text, pos);
                        if (skipTrivia) {
                            continue;
                        }
                ...
                case CharacterCodes.question(charCodeUnchecked(pos + 1) === CharacterCodes.dot && !isDigit(charCodeUnchecked(pos + 2))) {
                ...
                case CharacterCodes.caret(charCodeUnchecked(pos + 1) === CharacterCodes.equals) {
                ...
                case CharacterCodes.bar(isConflictMarkerTrivia(text, pos)) {
                        pos = scanConflictMarkerTrivia(text, pos);
                        if (skipTrivia) {
                            continue;
                        }
                ...
                case CharacterCodes.hash(pos !== 0 && text[pos + 1] === "!") {
                        pos++;
                        return token = SyntaxKind.Unknown;
                    }
                    {
                ...
                case CharacterCodes.replacementCharacter= end;
    ...
    function getDirectiveFromComment(text, commentDirectiveRegEx) {
        ...
        switch (match[1]) {
            case "ts-expect-error"CommentDirectiveType.ExpectError;
            case "ts-ignore"CommentDirectiveType.Ignore;
        }
        return undefined;
    }
    ...
    function scanJsxToken(allowMultilineJsxText = true) {
        ...
        return firstNonWhitespace === -1 ? SyntaxKind.JsxTextAllWhiteSpaces .JsxText;
    }
    ...
    function scanJsxAttributeValue() {
        ...
        switch (charCodeUnchecked(pos)) {
            case CharacterCodes.doubleQuoteCharacterCodes.singleQuote= scanString(/*jsxAttributeString*/ true);
                return token = SyntaxKind.StringLiteral;
            defaultscan();
        }
    }
    ...
    function scanJsDocToken() {
        ...
        pos += charSize(ch);
        switch (ch) {
            case CharacterCodes.tabCharacterCodes.verticalTabCharacterCodes.formFeedCharacterCodes.space(pos < end && isWhiteSpaceSingleLine(charCodeUnchecked(pos))) {
                    pos++;
                }
                return token = SyntaxKind.WhitespaceTrivia;
            case CharacterCodes.attoken = SyntaxKind.AtToken;
            case CharacterCodes.carriageReturn(charCodeUnchecked(pos) === CharacterCodes.lineFeed) {
                    pos++;
                }
            // falls through
            case CharacterCodes.lineFeed|= TokenFlags.PrecedingLineBreak;
                return token = SyntaxKind.NewLineTrivia;
            case CharacterCodes.asterisktoken = SyntaxKind.AsteriskToken;
            case CharacterCodes.openBracetoken = SyntaxKind.OpenBraceToken;
            case CharacterCodes.closeBracetoken = SyntaxKind.CloseBraceToken;
            case CharacterCodes.openBrackettoken = SyntaxKind.OpenBracketToken;
            case CharacterCodes.closeBrackettoken = SyntaxKind.CloseBracketToken;
            case CharacterCodes.openParentoken = SyntaxKind.OpenParenToken;
            case CharacterCodes.closeParentoken = SyntaxKind.CloseParenToken;
            case CharacterCodes.lessThantoken = SyntaxKind.LessThanToken;
            case CharacterCodes.greaterThantoken = SyntaxKind.GreaterThanToken;
            case CharacterCodes.equalstoken = SyntaxKind.EqualsToken;
            case CharacterCodes.commatoken = SyntaxKind.CommaToken;
            case CharacterCodes.dottoken = SyntaxKind.DotToken;
            case CharacterCodes.backticktoken = SyntaxKind.BacktickToken;
            case CharacterCodes.hashtoken = SyntaxKind.HashToken;
            case CharacterCodes.backslash--;
                {
                    const extendedCookedChar = peekExtendedUnicodeEscape();
                    if (extendedCookedChar >= 0 && isIdentifierStart(extendedCookedChar)) {
    ...
    function setText(newText, start, length) {
        text = newText || "";
        end = length === undefined ? text.length + length;

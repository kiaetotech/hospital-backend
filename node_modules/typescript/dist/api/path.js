...
function getEncodedRootLength(path) {
    ...
    if (ch0 === CharacterCodesSlash || ch0 === CharacterCodesBackslash) {
        ...
        const p1 = path.indexOf(ch0 === CharacterCodesSlash ? directorySeparator , 2);
        if (p1 < 0)
...
export function getNormalizedAbsolutePath(path, currentDirectory) {
    ...
    if (simpleNormalized !== undefined) {
        return simpleNormalized.length > rootLength ? removeTrailingDirectorySeparator(simpleNormalized) ;
    }
    ...
    return normalized ?? (length > rootLength ? removeTrailingDirectorySeparator(path) );
}
/**
...
export function createGetCanonicalFileName(useCaseSensitiveFileNames) {
    return useCaseSensitiveFileNames ? identity ;
}
function identity(x) {
    return x;
}
 ...
 * fileNameToDocumentURI("^/untitled/ts-nul-authority/Untitled-1") === "untitled-1"
 ...
 * documentURIToFileName("untitled-1") === "^/untitled/ts-nul-authority/Untitled-1"

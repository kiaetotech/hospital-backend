...
export class RemoteSourceFile extends RemoteNode {
    ...
    getOrCreateNodeAtIndex(index) {
        ...
        if (!node) {
            ...
            const parent = parentIndex === index ? this .getOrCreateNodeAtIndex(parentIndex);
    ...
    getLineAndCharacterOfPosition(position) {
        ...
        return { line, character- lineStarts[line] };
    }
...
export function parseNodeHandle(handle) {
    ...
    return {
        index(handle.slice(0, firstDot), 10),
        kind(handle.slice(firstDot + 1, secondDot), 10),
        path.slice(secondDot + 1),
    };
}
/**

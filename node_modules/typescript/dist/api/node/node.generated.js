...
export class RemoteNode extends RemoteNodeBase {
    ...
    getOrCreateChildAtNodeIndex(index) {
        ...
        if (!child) {
            ...
            child = kind === KIND_NODE_LIST
                ...
                RemoteNode(this.view, index, this, sf, offsetNodes);
            sf.nodes[index] = child;
            sf._timing?.recordMaterialization();
        }
        return child;
    ...
    __print() {
        ...
        const dataType = this.dataType === NODE_DATA_TYPE_CHILDREN ? "children" .dataType === NODE_DATA_TYPE_STRING ? "string" :
                "extended";
    ...
    get keyword() {
        switch (this.kind) {
            case SyntaxKind.ModuleDeclaration(this.data >> 24) & 0x1 ? SyntaxKind.NamespaceKeyword .ModuleKeyword;
        }
    }
    get keywordToken() {
        switch (this.kind) {
            case SyntaxKind.MetaProperty(this.data >> 24) & 0x1 ? SyntaxKind.NewKeyword .ImportKeyword;
        }
    }
    get operator() {
        switch (this.kind) {
            ...
            case SyntaxKind.PostfixUnaryExpression(this.data >> 24) & 0x1 ? SyntaxKind.MinusMinusToken .PlusPlusToken;
    ...
    get phaseModifier() {
        switch (this.kind) {
            case SyntaxKind.ImportClause: {
                ...
                if (idx === 0)
                    return undefined;
                return idx === 1 ? SyntaxKind.TypeKeyword === 2 ? SyntaxKind.DeferKeyword ;
            }
        }
    }
    get token() {
        switch (this.kind) {
            case SyntaxKind.HeritageClause(this.data >> 24) & 0x1 ? SyntaxKind.ImplementsKeyword .ExtendsKeyword;
            case SyntaxKind.ImportAttributes(this.data >> 25) & 0x1 ? SyntaxKind.AssertKeyword .WithKeyword;
        }
    }
    get templateFlags() {
        switch (this.kind) {
            case SyntaxKind.TemplateHeadSyntaxKind.TemplateMiddleSyntaxKind.TemplateTailextendedDataOffset = this.sourceFile._offsetExtendedData + (this.data & NODE_EXTENDED_DATA_MASK);
                return this.view.getUint32(extendedDataOffset + 8, true);
        }
    }
    get tokenFlags() {
        switch (this.kind) {
            case SyntaxKind.StringLiteralSyntaxKind.NumericLiteralSyntaxKind.BigIntLiteralSyntaxKind.RegularExpressionLiteralextendedDataOffset = this.sourceFile._offsetExtendedData + (this.data & NODE_EXTENDED_DATA_MASK);
    ...
    get text() {
        switch (this.kind) {
            case SyntaxKind.IdentifierSyntaxKind.PrivateIdentifierSyntaxKind.JsxTextSyntaxKind.JSDocTextSyntaxKind.JSDocLinkSyntaxKind.JSDocLinkPlainSyntaxKind.JSDocLinkCode: {
                const stringIndex = this.data & NODE_STRING_INDEX_MASK;
                return this.getString(stringIndex);
            }
            case SyntaxKind.StringLiteralSyntaxKind.NumericLiteralSyntaxKind.BigIntLiteralSyntaxKind.RegularExpressionLiteralSyntaxKind.NoSubstitutionTemplateLiteralSyntaxKind.TemplateHeadSyntaxKind.TemplateMiddleSyntaxKind.TemplateTailSyntaxKind.SourceFile: {

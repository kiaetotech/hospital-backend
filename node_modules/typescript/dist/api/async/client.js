...
export class Client {
    ...
    async connectViaSpawn(options) {
        const { spawn } = await import("node_process");
    ...
    async connectViaSocket(options) {
        const { createConnection } = await import("node");
        return new Promise((resolve, reject) => {
        ...
        for (const name of fsCallbackNames) {
            ...
            if (callback) {
                ...
                connection.onRequest(requestType, (arg) => {
                    ...
                    if (name === "readFile") {
                        // readFile has 3 returns(content), null (not found), undefined (fall back).
    ...
    async apiRequest(method, params) {
        ...
        const bytesSent = params === undefined ? 0 .byteLength(JSON.stringify(params), "utf-8");
        ...
        this.timing.record({
            ...
            roundTripMs,
            bytesSent,
            bytesReceived=== undefined || result === null
                ? 0
                .byteLength(JSON.stringify(result), "utf-8"),
        });
        return result;
    }
    ...
    /**
     * Returns a combined timing snapshot-measured round-trip and byte
    ...
    async getTimingInfo() {
        ...
        // No requests have been sent yetto fetch from the server.

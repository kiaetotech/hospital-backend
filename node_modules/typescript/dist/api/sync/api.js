...
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//
// Source/api/async/api.ts
// Regeneraterun generate (from _packages/native-preview)
//
...
export class API {
     ...
     * through this API instance-measured round-trip latency and bytes
...
export class Snapshot {
    ...
    dispose() {
        ...
        this.onDispose();
        this.client.apiRequest("release", { snapshot.id });
    }
    isDisposed() {
...
export class Checker {
    ...
    getSymbolAtLocation(nodeOrNodes) {
        if (Array.isArray(nodeOrNodes)) {
            const data = this.client.apiRequest("getSymbolsAtLocations", {
                snapshot.snapshotId,
                project.project.id,
                locations.map(node => getNodeId(node)),
            });
            return data.map(d => d ? this.objectRegistry.getOrCreateSymbol(d) );
        }
        const data = this.client.apiRequest("getSymbolAtLocation", {
            snapshot.snapshotId,
            project.project.id,
            location(nodeOrNodes),
        });
    ...
    getTypeOfSymbol(symbolOrSymbols) {
        if (Array.isArray(symbolOrSymbols)) {
            ...
            return data.map(d => d ? this.objectRegistry.getOrCreateType(d) );
        }
        ...
        return data ? this.objectRegistry.getOrCreateType(data) ;
    }
    /**
    ...
    getReferencedSymbolsForNode(node, position) {
        ...
        return (data ?? []).map(entry => ({
            definitionNodeHandle(entry.definition, this.project),
            symbol.symbol ? this.objectRegistry.getOrCreateSymbol(entry.symbol) ,
    ...
    getSignatureUsage(signatureDecl) {
        ...
        return (data ?? []).map(entry => ({
            nameNodeHandle(entry.name, this.project),
            call.call ? new NodeHandle(entry.call, this.project) ,
        }));
    }
    getCompletionsAtPosition(document, position, options) {
        ...
        return {
            ...
            entries.entries.map(e => ({
                ...e,
                symbol.symbol ? this.objectRegistry.getOrCreateSymbol(e.symbol) ,
            })),
        };
    }
    ...
    getResolvedSignature(node) {
        ...
        return data ? this.objectRegistry.getOrCreateSignature(data) ;
    }
    ...
    resolveName(name, meaning, location, excludeGlobals) {
        ...
        const data = this.client.apiRequest("resolveName", {
            ...
            file: !isNode && location ? location.document ,
            position: !isNode && location ? location.position ,
            excludeGlobals,
        });
        return data ? this.objectRegistry.getOrCreateSymbol(data) ;
    }
    ...
    typeToTypeNode(type, enclosingDeclaration, flags) {
        const binaryData = this.client.apiRequestBinary("typeToTypeNode", {
            ...
            project.project.id,
            type.id,
            location? getNodeId(enclosingDeclaration) ,
            flags,
        });
        if (!binaryData)
            return undefined;
    ...
    getTypePredicateOfSignature(signature) {
        ...
        return {
            ...
            type.type ? this.objectRegistry.getOrCreateType(data.type) ,
        };
    }
    /**
    ...
    getIndexInfosOfType(type) {
        ...
        if (!data)
            return [];
        return data.map(d => ({
            keyType.objectRegistry.getOrCreateType(d.keyType),
            valueType.objectRegistry.getOrCreateType(d.valueType),
            isReadonly.isReadonly ?? false,
            declaration.declaration ? new NodeHandle(d.declaration, this.project) ,
        }));
    }
    /**
...
export class Symbol {
    ...
    constructor(data, objectRegistry) {
        ...
        this.valueDeclaration = data.valueDeclaration ? new NodeHandle(data.valueDeclaration, canonicalProject) ;
...
class TypeObject {
    ...
    constructor(data, objectRegistry) {
        ...
        if (data.value != null) {
            ...
            this.value = (data.flags & TypeFlags.BigIntLiteral) ? BigInt(data.value) .value;
        }
...
export class Signature {
    ...
    constructor(data, project, objectRegistry) {
        ...
        this.flags = data.flags;
        this.objectRegistry = objectRegistry;
        this.declaration = data.declaration ? new NodeHandle(data.declaration, project) ;
        this.typeParameters = data.typeParameters ?? [];

 ...
 * ChildProcess.
 */
import { spawn, } from "node_process";
import { closeSync, openSync, readSync, writeSync, } from "node";
 ...
 *   - requestSync(method, payload)*   - requestBinarySync(method, payload)*   - registerCallback(name, cb)
 *   - close()
 *
...
export class SyncRpcChannel {
    ...
    constructor(exe, args, collectTiming = false) {
        ...
        if (isWindows) {
            ...
            if (fd === undefined) {
                this.child.kill();
                throw new Error("SyncRpcChannelout connecting to named pipe");
            }
            this.readFd = fd;
            this.writeFd = fd;
            this.pipeFd = fd;
            ...
            // POSIXstdio pipe file descriptors directly.
            ...
            if (typeof this.readFd !== "number" || this.readFd < 0 || typeof this.writeFd !== "number" || this.writeFd < 0) {
                ...
                stdin.destroy();
                this.child.kill();
                throw new Error("SyncRpcChannelnot obtain pipe file descriptors.");
            }
            ...
            switch (this._msgType) {
                case MSG_RESPONSE: {
                    ...
                    if (!methodBuf.equals(this._msgName)) {
                        throw new Error(`name mismatch for response\`${method}\`, got \`${this._msgName.toString("utf-8")}\``);
                    }
                    if (this.collectTiming) {
                ...
                defaultnew Error(`Invalid message type from child: ${this._msgType}`);
            }
        }
    }
    // ── Callback handling ───────────────────────────────────────────
    ...
    handleCall(name, payload) {
        ...
        try {
            ...
            const errMsg = String(e instanceof Error ? e.message ).trim();
            this.writeTuple(MSG_CALL_ERROR, Buffer.from(name, "utf-8"), Buffer.from(errMsg, "utf-8"));
     ...
     * syscall.  Larger messages use two syscallsfor the header
     * portion and one for the payload data.
     */
    writeTuple(type, name, payload) {
        const nameLen = name.length;
        const payloadIsString = typeof payload === "string";
        const payloadLen = payloadIsString ? Buffer.byteLength(payload, "utf-8") .length;
        ...
        if (totalSize <= this.writeBuf.length) {
            // Small messageinto write buffer, one syscall
            let off = 0;
            ...
            // Large message+ name in one call, payload in another
            let off = 0;
            this.writeBuf[off++] = MSGPACK_FIXARRAY3;
            this.writeBuf[off++] = type;
            off = writeBinHeader(this.writeBuf, off, nameLen);
            ...
            if (payloadLen > 0) {
                if (payloadIsString) {
                    // Large stringallocate (can't stream-encode
    ...
    readBin() {
        ...
        let size;
        switch (marker) {
            case MSGPACK_BIN8= this.readByte();
                break;
            case MSGPACK_BIN16.readExactInto(this.headerBuf, 2);
                size = (this.headerBuf[0] << 8) | this.headerBuf[1];
                break;
            case MSGPACK_BIN32.readExactInto(this.headerBuf, 4);
                size = this.headerBuf.readUInt32BE(0);
                break;
            defaultnew Error(`Expected binary data (0xc4-0xc6), received: 0x${marker.toString(16)}`);
        }
        if (size === 0)
            return EMPTY_BUF;
    ...
    eofError() {
        ...
        const detail = signal ? `killed by signal ${signal}` !== null ? `exited with code ${code}` : "unknown reason";

...
function makeClientMetadata(options) {
    ...
    if (appName.length > 0) {
        const name = Buffer.byteLength(appName, 'utf8') <= 128
            ? options.appName
            .from(appName, 'utf8').subarray(0, 128).toString('utf8');
        metadataDocument.ifItFitsItSits('application', { name });
    }
    const { name = '', version = '', platform = '' } = options.driverInfo;
    const driverInfo = {
        name.length > 0 ? `nodejs|${name}` : 'nodejs',
        version.length > 0 ? `${NODE_DRIVER_VERSION}|${version}` _DRIVER_VERSION
    };
    ...
    // Notematters, os.type is last so it will be removed last if we're at maxSize
...
function getFAASEnv() {
    ...
    // Notematters, name must always be the last key
 ...
 *
 * NOTEversion information fetching is intentionally written defensively

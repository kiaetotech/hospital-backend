...
function connect(options, callback) {
    makeConnection({ ...options, existingSocket}, (err, socket) => {
        if (err || !socket) {
            return callback(err);
        }
...
async function performInitialHandshake(conn, options) {
    ...
    // @ts-expect-error(NODE-5141)options need to be filtered properly, Connection options differ from Command options
    ...
    // NOTEis metadata attached to the connection while porting away from
...
async function prepareHandshakeDocument(authContext) {
    ...
    const handshakeDoc = {
        [serverApi?.version ? 'hello' _1.LEGACY_HELLO_COMMAND]: 1,
        helloOk,
        client.metadata,
        compression};
    ...
    function connectHandler() {
        ...
        if ('authorizationError' in socket) {
            if (socket.authorizationError && rejectUnauthorized) {
                // TODO(NODE-5192)this with a MongoError subclass
...
function makeSocks5Connection(options, callback) {
    ...
    // First, connect to the proxy server itself({
        ...
        // Then, establish the Socks5 proxy connection_1.SocksClient.createConnection({
            existing_socket,
...
function connectionFailureError(type, err) {
    switch (type) {
        case 'error'new error_1.MongoNetworkError(err);
        case 'timeout'new error_1.MongoNetworkTimeoutError('connection timed out');
        case 'close'new error_1.MongoNetworkError('connection closed');
        case 'cancel'new error_1.MongoNetworkError('connection establishment was cancelled');
        defaultnew error_1.MongoNetworkError('unknown network error');
    }
}

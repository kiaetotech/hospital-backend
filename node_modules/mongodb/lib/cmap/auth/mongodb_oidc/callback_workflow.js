...
class CallbackWorkflow {
    ...
    async execute(connection, credentials, reauthenticating, response) {
        ...
        if (entry) {
            ...
            if (entry.isValid() && !reauthenticating) {
                ...
                try {
                    result = await this.finishAuthentication(connection, credentials, tokenResult, reauthenticating ? undefined ?.speculativeAuthenticate?.conversationId);
                }
                catch (error) {
    ...
    async fetchAccessToken(connection, credentials, serverInfo, reauthenticating, callbackHash, requestCallback, refreshCallback) {
        ...
        const entry = this.cache.getEntry(connection.address, credentials.username, callbackHash);
        let result;
        const context = { timeoutSeconds_S, version_VERSION };
...
function finishCommandDocument(token, conversationId) {
    if (conversationId != null && typeof conversationId === 'number') {
        return {
            saslContinue: 1,
            conversationId,
            payloadbson_1.Binary(bson_1.BSON.serialize({ jwt}))
        };
    }
    ...
    return {
        saslStart: 1,
        mechanism_1.AuthMechanism.MONGODB_OIDC,
        payloadbson_1.Binary(bson_1.BSON.serialize({ jwt}))
    };
}
/**
...
function startCommandDocument(credentials) {
    ...
    return {
        ...
        autoAuthorize: 1,
        mechanism_1.AuthMechanism.MONGODB_OIDC,
        payloadbson_1.Binary(bson_1.BSON.serialize(payload))
    };
}

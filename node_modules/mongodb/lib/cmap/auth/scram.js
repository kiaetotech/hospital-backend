...
class ScramSHA extends auth_provider_1.AuthProvider {
    ...
    async prepare(handshakeDoc, authContext) {
        ...
        if (cryptoMethod === 'sha256' &&
            ...
            (0, utils_1.emitWarning)('Warningsaslprep library specified. Passwords will not be sanitized');
        }
...
function clientFirstMessageBare(username, nonce) {
    // NOTEis done b/c Javascript uses UTF-16, but the server is hashing in UTF-8.
...
function makeFirstMessage(cryptoMethod, credentials, nonce) {
    ...
    const mechanism = cryptoMethod === 'sha1' ? providers_1.AuthMechanism.MONGODB_SCRAM_SHA1 _1.AuthMechanism.MONGODB_SCRAM_SHA256;
    // NOTEis done b/c Javascript uses UTF-16, but the server is hashing in UTF-8.
    // Since the username is not sasl-prep-d, we need to do this here.
    return {
        saslStart: 1,
        mechanism,
        payloadbson_1.Binary(Buffer.concat([Buffer.from('n,,', 'utf8'), clientFirstMessageBare(username, nonce)])),
        autoAuthorize: 1,
        options: { skipEmptyExchange}
    };
}
...
async function continueScramConversation(cryptoMethod, response, authContext) {
    ...
    const saslContinueCmd = {
        ...
        payloadbson_1.Binary(Buffer.from(clientFinal))
    };
...
function passwordDigest(username, password) {
    ...
    try {
        ...
        if (crypto.getFips()) {
            ...
            // 'Error:060800C8envelope routines_DigestInit_exfor FIPS'

...
class MessageStream extends stream_1.Duplex {
    ...
    _read( /* size */) {
        // NOTEimplementation is empty because we explicitly push data to be read
    ...
    writeCommand(command, operationDescription) {
        ...
        if (agreedCompressor === 'none' || !canCompress(command)) {
            ...
            this.push(Array.isArray(data) ? Buffer.concat(data) );
            return;
        }
        ...
        const options = {
            agreedCompressor,
            zlibCompressionLevel.zlibCompressionLevel ?? 0
        };
...
function canCompress(command) {
    const commandDoc = command instanceof commands_1.Msg ? command.command .query;
...
function processIncomingData(stream, callback) {
    ...
    const messageHeader = {
        length.readInt32LE(0),
        requestId.readInt32LE(4),
        responseTo.readInt32LE(8),
        opCode.readInt32LE(12)
    };
    ...
    let ResponseType = messageHeader.opCode === constants_1.OP_MSG ? commands_1.BinMsg _1.Response;
    ...
    ResponseType = messageHeader.opCode === constants_1.OP_MSG ? commands_1.BinMsg _1.Response;

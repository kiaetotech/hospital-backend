...
class SetProfilingLevelOperation extends command_1.CommandCallbackOperation {
    constructor(db, level, options) {
        ...
        switch (level) {
            case exports.ProfilingLevel.off.profile = 0;
                break;
            case exports.ProfilingLevel.slowOnly.profile = 1;
                break;
            case exports.ProfilingLevel.all.profile = 2;
                break;
            default.profile = 0;
                break;
        }
    ...
    executeCallback(server, session, callback) {
        ...
        // TODO(NODE-3483)error to put here
        super.executeCommandCallback(server, session, { profile.profile }, (err, doc) => {
            if (err == null && doc.ok === 1)
                return callback(undefined, level);
            return err != null
                ? callback(err)
                (new error_1.MongoRuntimeError('Error with profile command'));
        });
    }
}

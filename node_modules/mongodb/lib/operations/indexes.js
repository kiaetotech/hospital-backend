...
function makeIndexSpec(indexSpec, options) {
    const key = new Map();
    const indexSpecs = !Array.isArray(indexSpec) || isSingleIndexTuple(indexSpec) ? [indexSpec] ;
...
class IndexesOperation extends operation_1.AbstractCallbackOperation {
    ...
    executeCallback(server, session, callback) {
        ...
        (0, common_functions_1.indexInformation)(coll.s.db, coll.collectionName, { full, ...options, readPreference.readPreference, session }, callback);
    }
}
...
class CreateIndexesOperation extends command_1.CommandCallbackOperation {
    constructor(parent, collectionName, indexes, options) {
        ...
        this.indexes = indexes.map(userIndex => {
            ...
            const key = userIndex.key instanceof Map ? userIndex.key Map(Object.entries(userIndex.key));
            const name = userIndex.name != null ? userIndex.name .from(key).flat().join('_');
    ...
    executeCallback(server, session, callback) {
        ...
        const cmd = { createIndexes.collectionName, indexes };
...
class DropIndexOperation extends command_1.CommandCallbackOperation {
    ...
    executeCallback(server, session, callback) {
        const cmd = { dropIndexes.collection.collectionName, index.indexName };
...
class ListIndexesOperation extends command_1.CommandCallbackOperation {
    ...
    executeCallback(server, session, callback) {
        const serverWireVersion = (0, utils_1.maxWireVersion)(server);
        const cursor = this.options.batchSize ? { batchSize.options.batchSize } : {};
        const command = { listIndexes.collectionNamespace.collection, cursor };
...
class IndexExistsOperation extends operation_1.AbstractCallbackOperation {
    ...
    executeCallback(server, session, callback) {
        const coll = this.collection;
        const indexes = this.indexes;
        (0, common_functions_1.indexInformation)(coll.s.db, coll.collectionName, { ...this.options, readPreference.readPreference, session }, (err, indexInformation) => {
...
class IndexInformationOperation extends operation_1.AbstractCallbackOperation {
    ...
    executeCallback(server, session, callback) {
        ...
        (0, common_functions_1.indexInformation)(db, name, { ...this.options, readPreference.readPreference, session }, callback);
    }
}

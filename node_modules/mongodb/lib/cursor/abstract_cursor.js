...
class AbstractCursor extends mongo_types_1.TypedEventEmitter {
    ...
    constructor(client, namespace, options = {}) {
        ...
        this[kKilled] = false;
        this[kOptions] = {
            readPreference.readPreference && options.readPreference instanceof read_preference_1.ReadPreference
                ? options.readPreference
                _preference_1.ReadPreference.primary,
        ...
        if (options.session instanceof sessions_1.ClientSession) {
            ...
            this[kSession] = this[kClient].startSession({ owner, explicit});
        }
    }
    get id() {
            ...
            while (true) {
                ...
                if (document === null) {
                    if (!this.closed) {
                        ...
                        await cleanupCursorAsync(this, { needsToEmitClosed}).catch(() => null);
    ...
    async hasNext() {
        ...
        const doc = await next(this, { blocking, transform});
        if (doc) {
    ...
    async next() {
        ...
        return next(this, { blocking, transform});
    }
    /**
     ...
     * const mappedCursor<number> = cursor.map(doc => Object.keys(doc).length);
     * const keyCounts[] = await mappedCursor.toArray(); // cursor.toArray() still returns Document[]
     * ```
     * @param transform - The mapping transformation method.
     */
    map(transform) {
        assertUninitialized(this);
        const oldTransform = this[kTransform]; // TODO(NODE-3283)transform typing
        if (oldTransform) {
            this[kTransform] = doc => {
    ...
    rewind() {
        ...
        if (session) {
            ...
            if (session.explicit === false) {
                ...
                this[kSession] = this.client.startSession({ owner, explicit});
            }
        }
    }
    /** @internal */
        ...
        this._initialize(this[kSession], (error, state) => {
            if (state) {
                ...
                if (this[kId] == null) {
                    ...
                    // TODO(NODE-3286)needs to accept a generic parameter
        ...
        if (cursor[kDocuments].length !== 0) {
            ...
            if (doc != null && transform && cursor[kTransform]) {
                try {
                    ...
                    // error instead.
                    await cleanupCursorAsync(cursor, { error, needsToEmitClosed}).catch(() => null);
                    throw error;
                }
            }
            return doc;
...
class ReadableCursorStream extends stream_1.Readable {
    ...
    _readNext() {
        next(this._cursor, { blocking, transform}).then(result => {
            if (result == null) {
                this.push(null);
            }
            ...
            // NOTEis questionable, but we have a test backing the behavior. It seems the
            ...
            // NOTEis also perhaps questionable. The rationale here is that these errors tend
            ...
            // NOTEtwo above checks on the message of the error will cause a null to be pushed

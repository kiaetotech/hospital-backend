...
class AggregationCursor extends abstract_cursor_1.AbstractCursor {
    ...
    _initialize(session, callback) {
        ...
        (0, execute_operation_1.executeOperation)(this.client, aggregateOperation, (err, response) => {
            if (err || response == null)
                return callback(err);
            // TODO-2882
            callback(undefined, { server.server, session, response });
        });
    }
     ...
     * ```typescript
     * // Best way
     * const docs<{ a}> = cursor.project<{ a}>({ _id: 0, a});
     * // Flexible way
     * const docs<Document> = cursor.project({ _id: 0, a});
     * ```
     *
     * @remarks
     * In order to strictly type this function you must provide an interface
     ...
     * @example
     * ```typescript
     * const cursor<{ a; b}> = coll.aggregate([]);
     * const projectCursor = cursor.project<{ a}>({ _id: 0, a});
     * const aPropOnlyArray: {a}[] = await projectCursor.toArray();
     *
     * // or always use chaining and save the final cursor
     *
     * const cursor = coll.aggregate().project<{ a}>({
     *   _id: 0,

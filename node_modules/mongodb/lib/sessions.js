 ...
 *
 * NOTEmeant to be instantiated directly.
 * @public
 */
class ClientSession extends mongo_types_1.TypedEventEmitter {
    ...
    constructor(client, sessionPool, options, clientOptions) {
        ...
        this[kServerSession] = this.explicit ? this.sessionPool.acquire() ;
        this[kTxnNumberIncrement] = 0;
        const defaultCausalConsistencyValue = this.explicit && options.snapshot !== true;
        this.supports = {
            // if we can enable causal consistency, do so by default
            causalConsistency.causalConsistency ?? defaultCausalConsistencyValue
        };
    ...
    pin(conn) {
        ...
        conn.emit(constants_1.PINNED, this.inTransaction() ? metrics_1.ConnectionPoolMetrics.TXN _1.ConnectionPoolMetrics.CURSOR);
    }
    /** @internal */
    unpin(options) {
    ...
    get isPinned() {
        return this.loadBalanced ? !!this[kPinnedConnection] .transaction.isPinned;
    }
    /**
    ...
    async endSession(options) {
        try {
            ...
            maybeClearPinnedConnection(this, { force, ...options });
        }
    }
    /**
    ...
    startTransaction(options) {
        ...
        this.transaction = new transactions_1.Transaction({
            ...
            maxCommitTimeMS?.maxCommitTimeMS ?? this.defaultTransactionOptions.maxCommitTimeMS
        });
...
function maybeClearPinnedConnection(session, options) {
    ...
    // NOTEspec talks about what to do on a network error only, but the tests seem to
    ...
    if (conn && topology != null) {
        ...
        if (options?.error == null || options?.force) {
            ...
            conn.emit(constants_1.UNPINNED, session.transaction.state !== transactions_1.TxnState.NO_TRANSACTION
                ...
                _1.ConnectionPoolMetrics.CURSOR);
            if (options?.forceClear) {
                loadBalancer.pool.clear({ serviceId.serviceId });
            }
        }
...
function endTransaction(session, commandName, callback) {
    ...
    if (session.transaction.options.writeConcern) {
        ...
        writeConcern = { w.clientOptions.writeConcern.w };
    }
    if (txnState === transactions_1.TxnState.TRANSACTION_COMMITTED) {
    ...
    if (commandName === 'commitTransaction' && session.transaction.options.maxTimeMS) {
        Object.assign(command, { maxTimeMS.transaction.options.maxTimeMS });
    }
    function commandHandler(error, result) {
        if (commandName !== 'commitTransaction') {
            session.transaction.transition(transactions_1.TxnState.TRANSACTION_ABORTED);
            if (session.loadBalanced) {
                maybeClearPinnedConnection(session, { force});
            }
    ...
    (0, execute_operation_1.executeOperation)(session.client, new run_command_1.RunAdminCommandOperation(undefined, command, {
        ...
        if (error instanceof error_1.MongoError && error.hasErrorLabel(error_1.MongoErrorLabel.RetryableWriteError)) {
            ...
            return (0, execute_operation_1.executeOperation)(session.client, new run_command_1.RunAdminCommandOperation(undefined, command, {
                session,
                readPreference_preference_1.ReadPreference.primary,
                bypassPinningCheck}), commandHandler);
        }
 ...
 * WARNINGmeant to be instantiated directly. For internal use only.
 * @public
 */
class ServerSession {
    /** @internal */
    constructor() {
        this.id = { idbson_1.Binary((0, utils_1.uuidV4)(), bson_1.Binary.SUBTYPE_UUID) };
...
function applySession(session, command, options) {
    ...
    if (isRetryableWrite || inTxnOrTxnCommand) {
        ...
        // TODO(NODE-2674)int64 sent from MongoDB
    ...
    if (!inTxnOrTxnCommand) {
        ...
        if (session.supports.causalConsistency &&
            ...
            Object.assign(command.readConcern, { afterClusterTime.operationTime });
        }
        else if (session[kSnapshotEnabled]) {
            command.readConcern = command.readConcern || { level_concern_1.ReadConcernLevel.snapshot };
            if (session[kSnapshotTime] != null) {
                Object.assign(command.readConcern, { atClusterTime[kSnapshotTime] });
            }
        }
        return;
    }

 ...
 * found here://github.com/mongodb/specifications/blob/master/source/max-staleness/max-staleness.rst
 *
...
function maxStalenessReducer(readPreference, topologyDescription, servers) {
    ...
    if (topologyDescription.type === common_1.TopologyType.ReplicaSetNoPrimary) {
        ...
        const sMax = servers.reduce((max, s) => s.lastWriteDate > max.lastWriteDate ? s );
        return servers.reduce((result, server) => {
...
function latencyWindowReducer(topologyDescription, servers) {
    const low = servers.reduce((min, server) => min === -1 ? server.roundTripTime .min(server.roundTripTime, min), -1);
...
function readPreferenceServerSelector(readPreference) {
    ...
    return (topologyDescription, servers) => {
        ...
        const filter = mode === read_preference_1.ReadPreference.NEAREST ? nearestFilter ;

...
class Topology extends mongo_types_1.TypedEventEmitter {
    ...
    constructor(client, seeds, options) {
        ...
        for (const seed of seeds) {
            if (typeof seed === 'string') {
                ...
                // FIXME(NODE-3483)need to be a MongoParseError
        ...
        this.s = {
            ...
            descriptiontopology_description_1.TopologyDescription(topologyType, serverDescriptions, options.replicaSet, undefined, undefined, undefined, options),
            ...
            // timer management
            connectionTimersSet(),
            detectShardedTopology=> this.detectShardedTopology(ev),
            detectSrvRecords=> this.detectSrvRecords(ev)
        };
        if (options.srvHost && !options.loadBalanced) {
            this.s.srvPoller =
                options.srvPoller ??
                    new srv_polling_1.SrvPoller({
                        heartbeatFrequencyMS.s.heartbeatFrequencyMS,
                        srvHost.srvHost,
        ...
        if (this.s.options.loadBalanced) {
            for (const description of serverDescriptions) {
                const newDescription = new server_description_1.ServerDescription(description.hostAddress, undefined, {
                    loadBalanced.s.options.loadBalanced
                });
                this.serverUpdateHandler(newDescription);
            }
        }
        const exitWithError = (error) => callback ? callback(error) .emit(Topology.ERROR, error);
        const readPreference = options.readPreference ?? read_preference_1.ReadPreference.primary;
        this.selectServer((0, server_selection_1.readPreferenceServerSelector)(readPreference), options, (err, server) => {
            if (err) {
                return this.close({ force}, () => exitWithError(err));
            }
            // TODO-2471
    ...
    selectServer(selector, options, callback) {
        ...
        options = Object.assign({}, { serverSelectionTimeoutMS.s.serverSelectionTimeoutMS }, options);
    ...
    // NOTEare many places in code where we explicitly check the last hello
    ...
    lastHello() {
        ...
        const result = sd || { maxWireVersion.description.commonWireVersion };
        return result;
    }

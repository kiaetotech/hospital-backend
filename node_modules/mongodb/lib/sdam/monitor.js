...
class Monitor extends mongo_types_1.TypedEventEmitter {
    ...
    constructor(server, options) {
        ...
        this.s = {
            state_1.STATE_CLOSED
        };
        this.address = server.description.address;
        this.options = Object.freeze({
            connectTimeoutMS.connectTimeoutMS ?? 10000,
            heartbeatFrequencyMS.heartbeatFrequencyMS ?? 10000,
            minHeartbeatFrequencyMS.minHeartbeatFrequencyMS ?? 500
        });
        const cancellationToken = this[kCancellationToken];
        // TODOthis to pull it directly from the pool, requires new ConnectionPool integration
        const connectOptions = Object.assign({
            id: '<monitor>',
            generation.pool.generation,
            connectionType_1.Connection,
            cancellationToken,
...
function resetMonitorState(monitor) {
    ...
    monitor[kConnection]?.destroy({ force});
...
function checkServer(monitor, callback) {
    ...
    function failureHandler(err) {
        ...
        const error = !(err instanceof error_1.MongoError) ? new error_1.MongoError(err) ;
    ...
    if (connection && !connection.closed) {
        ...
        const cmd = {
            [serverApi?.version || helloOk ? 'hello' _1.LEGACY_HELLO_COMMAND]: 1,
            ...(isAwaitable && topologyVersion
                ? { maxAwaitTimeMS, topologyVersion(topologyVersion) }
                : {})
        };
        const options = isAwaitable
            ? {
                socketTimeoutMS? connectTimeoutMS + maxAwaitTimeMS : 0,
                exhaustAllowed}
            : { socketTimeoutMS};
        if (isAwaitable && monitor.rttPinger == null) {
            monitor.rttPinger = new RTTPinger(monitor[kCancellationToken], Object.assign({ heartbeatFrequencyMS.options.heartbeatFrequencyMS }, monitor.connectOptions));
        }
...
function makeTopologyVersion(tv) {
    return {
        ...
        counter_1.Long.isLong(tv.counter) ? tv.counter _1.Long.fromNumber(tv.counter)
    };
}
/** @internal */
class RTTPinger {
...
function measureRoundTripTime(rttPinger, options) {
    ...
    const commandName = connection.serverApi?.version || connection.helloOk ? 'hello' _1.LEGACY_HELLO_COMMAND;
    connection.commandAsync((0, utils_1.ns)('admin.$cmd'), { [commandName]: 1 }, undefined).then(() => measureAndReschedule(), () => {
        rttPinger.connection?.destroy({ force});
        rttPinger.connection = undefined;
        rttPinger[kRoundTripTime] = 0;
    });
}
...
class MonitorInterval {
    ...
    wake() {
        ...
        // TODO(NODE-4674)error handling and logging to the monitor
    ...
    toJSON() {
        ...
        const timeSinceLastCall = currentTime - this.lastExecutionEnded;
        return {
            timerId.timerId != null ? 'set' : 'cleared',
            lastCallTime.lastExecutionEnded,
            isExpeditedCheckScheduled.isExpeditedCallToFnScheduled,
            stopped.stopped,

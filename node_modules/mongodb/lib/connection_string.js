...
async function resolveSRVRecord(options) {
    ...
    if (options.srvHost.split('.').length < 3) {
        // TODO(NODE-3484)with MongoConnectionStringError
...
function getBoolean(name, value) {
    ...
    if (TRUTHS.has(valueString)) {
        if (valueString !== 'true') {
            (0, utils_1.emitWarningOnce)(`deprecated value for ${name} : ${valueString} - please update to ${name} instead`);
        }
        return true;
    }
...
function parseOptions(uri, mongoClient = undefined, options = {}) {
    ...
    mongoOptions.hosts = isSRV ? [] .map(utils_1.HostAddress.fromString);
    const urlOptions = new CaseInsensitiveMap();
    if (url.pathname !== '/' && url.pathname !== '') {
        const dbName = decodeURIComponent(url.pathname[0] === '/' ? url.pathname.slice(1) .pathname);
        if (dbName) {
    ...
    if (mongoOptions[loggerFeatureFlag]) {
        loggerEnvOptions = {
            MONGODB_LOG_COMMAND.env.MONGODB_LOG_COMMAND,
            MONGODB_LOG_TOPOLOGY.env.MONGODB_LOG_TOPOLOGY,
            MONGODB_LOG_SERVER_SELECTION.env.MONGODB_LOG_SERVER_SELECTION,
            MONGODB_LOG_CONNECTION.env.MONGODB_LOG_CONNECTION,
            MONGODB_LOG_ALL.env.MONGODB_LOG_ALL,
            MONGODB_LOG_MAX_DOCUMENT_LENGTH.env.MONGODB_LOG_MAX_DOCUMENT_LENGTH,
            MONGODB_LOG_PATH.env.MONGODB_LOG_PATH,
...
function setOption(mongoOptions, key, descriptor, values) {
    ...
    switch (type) {
        case 'boolean'[name] = getBoolean(name, values[0]);
            break;
        case 'int'[name] = getIntFromOptions(name, values[0]);
            break;
        case 'uint'[name] = getUIntFromOptions(name, values[0]);
            break;
        case 'string'(values[0] == null) {
                break;
            }
            mongoOptions[name] = String(values[0]);
            break;
        case 'record'(!(0, utils_1.isRecord)(values[0])) {
                throw new error_1.MongoParseError(`${name} must be an object`);
            }
            mongoOptions[name] = values[0];
            break;
        case 'any'[name] = values[0];
            break;
        default: {
            if (!transform) {
                throw new error_1.MongoParseError('Descriptors missing a type must define a transform');
            }
            const transformValue = transform({ name, options, values });
...
exports.OPTIONS = {
    ...
    serverApi: {
        ...
        transform({ values: [version] }) {
            const serverApiToValidate = typeof version === 'string' ? { version } ;
    ...
    compressors: {
        ...
        transform({ values }) {
            ...
            for (const compVal of values) {
                const compValArray = typeof compVal === 'string' ? compVal.split(',') ;
    ...
    enableUtf8Validation: { type: 'boolean', default},
    family: {
    ...
    name: {
        ...
        transform({ values: [value], options }) {
            return { ...options.driverInfo, name(value) };
        }
    },
    noDelay: {
        default,
    ...
    readPreference: {
        default_preference_1.ReadPreference.primary,
        transform({ values: [value], options }) {
    ...
    tlsAllowInvalidHostnames: {
        ...
        transform({ name, values: [value] }) {
            ...
            return getBoolean(name, value) ? () => undefined ;
        }
    },
    tlsCAFile: {
        target: 'ca',
        ...
        transform({ name, options, values: [value] }) {
            ...
            if (tlsInsecure) {
                ...
                options.checkServerIdentity = options.tlsAllowInvalidHostnames
                    ? () => undefined
                    ;
                options.rejectUnauthorized = options.tlsAllowInvalidCertificates ? false ;
            }
            return tlsInsecure;
        }
    },
    w: {
        target: 'writeConcern',
        transform({ values: [value], options }) {
            return write_concern_1.WriteConcern.fromOptions({ writeConcern: { ...options.writeConcern, w} });
        }
    },
    waitQueueTimeoutMS: {
        default: 0,
    ...
    // MongoLogger
    // TODO(NODE-4849)the type of mongodbLogPath

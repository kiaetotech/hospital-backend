...
class BulkWriteResult {
    ...
    constructor(bulkResult, isOrdered) {
        ...
        Object.defineProperty(this, 'result', { value.result, enumerable});
    }
    /**
    ...
    getWriteErrorAt(index) {
        return index < this.result.writeErrors.length ? this.result.writeErrors[index] ;
    }
    ...
    getWriteConcernError() {
        if (this.result.writeConcernErrors.length === 0) {
            ...
            return new WriteConcernError({ errmsg, code_1.MONGODB_ERROR_CODES.WriteConcernFailed });
        }
    }
    toString() {
...
function mergeBatchResults(batch, bulkResult, err, result) {
    ...
    if (Array.isArray(result.upserted)) {
        ...
        for (let i = 0; i < result.upserted.length; i++) {
            bulkResult.upserted.push({
                index.upserted[i].index + batch.originalZeroIndex,
                _id.upserted[i]._id
            });
        }
    }
    ...
    if (Array.isArray(result.writeErrors)) {
        for (let i = 0; i < result.writeErrors.length; i++) {
            const writeError = {
                index.originalIndexes[result.writeErrors[i].index],
                code.writeErrors[i].code,
                errmsg.writeErrors[i].errmsg,
                errInfo.writeErrors[i].errInfo,
                op.operations[result.writeErrors[i].index]
            };
            bulkResult.writeErrors.push(new WriteError(writeError));
        }
    }
...
function handleMongoWriteConcernError(batch, bulkResult, isOrdered, err, callback) {
    ...
    callback(new MongoBulkWriteError({
        ...
        code.result?.writeConcernError.result
...
class FindOperators {
    ...
    updateOne(updateDocument) {
        ...
        const currentOp = buildCurrentOp(this.bulkOperation);
        return this.bulkOperation.addToOperationsList(exports.BatchType.UPDATE, (0, update_1.makeUpdateStatement)(currentOp.selector, updateDocument, { ...currentOp, multi}));
    }
    ...
    replaceOne(replacement) {
        ...
        const currentOp = buildCurrentOp(this.bulkOperation);
        return this.bulkOperation.addToOperationsList(exports.BatchType.UPDATE, (0, update_1.makeUpdateStatement)(currentOp.selector, replacement, { ...currentOp, multi}));
    }
...
class BulkOperationBase {
    ...
    constructor(collection, options, isOrdered) {
        ...
        const maxBatchSizeBytes = usingAutoEncryption ? 1024 * 1024 * 2 ;
        ...
        this.s = {
            ...
            err,
            // check keys
            checkKeysoptions.checkKeys === 'boolean' ? options.checkKeys };
        // bypass Validation
     ...
     * bulk.find({ k: 11, y: { $exists}, z: { $exists} }).updateOne([
     *   { $set: { total: { $sum: [ '$y', '$z' ] } } }
     * ]);
     *
    ...
    raw(op) {
        ...
        if ('insertOne' in op) {
            ...
            if (op.insertOne && op.insertOne.document == null) {
                // NOTEfor legacy support, but this is a malformed operation
        ...
        if ('replaceOne' in op || 'updateOne' in op || 'updateMany' in op) {
            if ('replaceOne' in op) {
                ...
                const updateStatement = (0, update_1.makeUpdateStatement)(op.replaceOne.filter, op.replaceOne.replacement, { ...op.replaceOne, multi});
    ...
    handleWriteError(callback, writeResult) {
        if (this.s.bulkResult.writeErrors.length > 0) {
            ...
            callback(new MongoBulkWriteError({
                message,
                code.s.bulkResult.writeErrors[0].code,
                writeErrors.s.bulkResult.writeErrors
            }, writeResult));
            return true;
        }

(function(global, factory) {
  ...
  function createImmediate(safeMode) {
    ...
    asyncSetImmediate = typeof setImmediate === func ? setImmediate ;
    if (typeof process === obj && typeof process.nextTick === func) {
      nextTick = /^v0.10/.test(process.version) ? asyncSetImmediate .nextTick;
      asyncNextTick = /^v0/.test(process.version) ? asyncSetImmediate .nextTick;
    } else {
  ...
  function sortByCriteria(array, criteria) {
    ...
    for (var n = 0; n < l; n++) {
      i = indices[n];
      result[n] = i === undefined ? array[n] [i];
    }
    return result;
  }
  ...
  function createFilterSeries(bool) {
    return function(collection, iterator, callback) {
      ...
      if (isArray(collection)) {
        size = collection.length;
        iterate = iterator.length === 3 ? arrayIteratorWithIndex ;
      } else if (!collection) {
      } else if (iteratorSymbol && collection[iteratorSymbol]) {
        size = Infinity;
        iter = collection[iteratorSymbol]();
        iterate = iterator.length === 3 ? symbolIteratorWithKey ;
      } else if (typeof collection === obj) {
        keys = nativeKeys(collection);
        size = keys.length;
        iterate = iterator.length === 3 ? objectIteratorWithKey ;
      }
      if (!size) {
        return callback(null, []);
      }
      ...
      function symbolIterator() {
        item = iter.next();
        value = item.value;
        item.done ? callback(null, result) (value, done);
      }

      function symbolIteratorWithKey() {
        item = iter.next();
        value = item.value;
        item.done ? callback(null, result) (value, completed, done);
      }
  ...
  function createFilterLimit(bool) {
    return function(collection, limit, iterator, callback) {
      ...
      timesSync(limit > size ? size , iterate);
  ...
  function eachSeries(collection, iterator, callback) {
    ...
    function symbolIterator() {
      item = iter.next();
      item.done ? callback(null) (item.value, done);
    }

    function symbolIteratorWithKey() {
      item = iter.next();
      item.done ? callback(null) (item.value, completed, done);
    }
  ...
  function mapSeries(collection, iterator, callback) {
    ...
    function symbolIterator() {
      item = iter.next();
      item.done ? callback(null, result) (item.value, done);
    }

    function symbolIteratorWithKey() {
      item = iter.next();
      item.done ? callback(null, result) (item.value, completed, done);
    }
  ...
  function createDetectSeries(bool) {
    return function(collection, iterator, callback) {
      ...
      function symbolIterator() {
        item = iter.next();
        value = item.value;
        item.done ? callback(null) (value, done);
      }

      function symbolIteratorWithKey() {
        item = iter.next();
        value = item.value;
        item.done ? callback(null) (value, completed, done);
      }
  ...
  function createPickSeries(bool) {
    return function(collection, iterator, callback) {
      ...
      function symbolIteratorWithKey() {
        ...
        item = iter.next();
        value = item.value;
        item.done ? callback(null, result) (value, key, done);
      }
  ...
  function reduce(collection, result, iterator, callback) {
    ...
    if (isArray(collection)) {
      size = collection.length;
      iterate = iterator.length === 4 ? arrayIteratorWithIndex ;
    } else if (!collection) {
    } else if (iteratorSymbol && collection[iteratorSymbol]) {
      size = Infinity;
      iter = collection[iteratorSymbol]();
      iterate = iterator.length === 4 ? symbolIteratorWithKey ;
    } else if (typeof collection === obj) {
      keys = nativeKeys(collection);
      size = keys.length;
      iterate = iterator.length === 4 ? objectIteratorWithKey ;
    }
    if (!size) {
      return callback(null, result);
    }
    ...
    function symbolIterator(result) {
      item = iter.next();
      item.done ? callback(null, result) (result, item.value, done);
    }

    function symbolIteratorWithKey(result) {
      item = iter.next();
      item.done ? callback(null, result) (result, item.value, completed, done);
    }
  ...
  function createTransform(arrayEach, baseEach, symbolEach) {
    return function transform(collection, accumulator, iterator, callback) {
      ...
      if (!size) {
        callback(null, accumulator !== undefined ? accumulator || {});
      }

      function done(err, bool) {
        if (err) {
          callback = once(callback);
          callback(err, isArray(result) ? createArray(result) (result));
        } else if (++completed === size) {
          callback(null, result);
        } else if (bool === false) {
          callback = once(callback);
          callback(null, isArray(result) ? createArray(result) (result));
        }
      }
    };
  }
  ...
  function transformSeries(collection, accumulator, iterator, callback) {
    ...
    if (!size) {
      return callback(null, accumulator !== undefined ? accumulator || {});
    }
    iterate();
  ...
  function transformLimit(collection, limit, accumulator, iterator, callback) {
    ...
    function done(err, bool) {
      if (err || bool === false) {
        iterate = noop;
        callback(err || null, isArray(result) ? createArray(result) (result));
        callback = noop;
      } else if (++completed === size) {
        iterator = noop;
        callback(null, result);
      ...
      function createCallback(index) {
        return function done(err, res) {
          ...
          switch (arguments.length) {
            ...
            case 2[index] = res;
              break;
            default[index] = slice(arguments, 1);
              break;
          }
          index = null;
          if (++completed === size) {
    ...
    return function parallel(tasks, callback) {
      ...
      function createCallback(key) {
        return function(err, res) {
          ...
          result[key] = arguments.length <= 2 ? res (arguments, 1);
          key = null;
          if (++completed === size) {
            callback(null, result);
          }
  ...
  function waterfallIterator(func, args, next) {
    switch (args.length) {
      case 01func(next);
      case 2func(args[1], next);
      case 3func(args[1], args[2], next);
      case 4func(args[1], args[2], args[3], next);
      case 5func(args[1], args[2], args[3], args[4], next);
      case 6func(args[1], args[2], args[3], args[4], args[5], next);
      default= slice(args, 1);
        args.push(next);
  ...
  function baseQueue(isQueue, worker, concurrency, payload) {
    ...
    function resume() {
      ...
      q.paused = false;
      var count = q.concurrency < q._tasks.length ? q.concurrency ._tasks.length;
  ...
  function priorityQueue(worker, concurrency) {
    ...
    function push(tasks, priority, callback) {
      ...
      callback = typeof callback === func ? callback ;
  ...
  function auto(tasks, concurrency, callback) {
    ...
    function iterator(task, key) {
      ...
      function done(err, arg) {
        ...
        arg = arguments.length <= 2 ? arg (arguments, 1);
        if (err) {
          rest = 0;
          runningTasks = 0;
          readyTasks.length = 0;
  ...
  function autoInject(tasks, concurrency, callback) {
    ...
    function iterator(task, key) {
      ...
      function newTask(results, done) {
        switch (taskLength) {
          case 1(results[params[0]], done);
            break;
          case 2(results[params[0]], results[params[1]], done);
            break;
          case 3(results[params[0]], results[params[1]], results[params[2]], done);
            break;
          defaulti = -1;
            while (++i < taskLength) {
  ...
  function retry(opts, task, callback) {
    ...
    if (arguments.length < 3 && typeof opts === func) {
      ...
      callback = callback || noop;
      switch (typeof opts) {
        case 'object'(typeof opts.errorFilter === func) {
          ...
          break;
        case 'number'= opts || DEFAULT_TIMES;
          break;
        case 'string'= +opts || DEFAULT_TIMES;
          break;
        defaultnew Error('Invalid arguments for async.retry');
      }
    }
  ...
  function iterator(tasks) {
    ...
    function makeCallback(index) {
      ...
      fn.next = function() {
        return index < size - 1 ? makeCallback(index + 1) ;
      };
      return fn;
    }
  }
  ...
  function apply(func) {
    switch (arguments.length) {
      case 01func;
      case 2func.bind(null, arguments[1]);
      case 3func.bind(null, arguments[1], arguments[2]);
      case 4func.bind(null, arguments[1], arguments[2], arguments[3]);
      case 5func.bind(null, arguments[1], arguments[2], arguments[3], arguments[4]);
      defaultsize = arguments.length;
        var index = 0;
        var args = Array(size);
        args[index] = null;
        while (++index < size) {
    ...
    return function(/* args..., callback */) {
      ...
      if (result && typeof result.then === func) {
        result.then(
          ...
          function(err) {
            invokeCallback(callback, err && err.message ? err Error(err));
          }
        );
      } else {
        callback(null, result);
  ...
  function reflect(func) {
    return function(/* args..., callback */) {
      ...
      switch (arguments.length) {
        ...
        defaultargs = createArray(arguments);
          var lastIndex = args.length - 1;
          callback = args[lastIndex];
          args[lastIndex] = done;

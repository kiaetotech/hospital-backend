...
// Patientreminder (1 hour before)
const sendBookingReminder = async (phone, doctorName, time) => {
  const message = `Reminderconsultation with Dr. ${doctorName} is at ${time} today. Please be on time. - Ayurveda Wellness Hub`;
  return await sendSMS(phone, message);
};

// Patientrequest (after consultation)

...
class HospitalAgent extends BaseAgent_1.BaseAgent {
    ...
    async searchHospitals(payload) {
        ...
        // Filter by specialty
        if (specialty) {
            results = results.filter(h => h.specialty.some(s => s.toLowerCase().includes(specialty.toLowerCase())));
        }
        // Filter by insurance
        if (insurance) {
            results = results.filter(h => h.insuranceAccepted.some(i => i.toLowerCase().includes(insurance.toLowerCase())));
        }
        // Sort by rating
